...
```{r chunk-name-1, exercise = TRUE}`r ''`

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r chunk-name-1-hint, eval = FALSE}`r ''`
...
The Tutorial Written Exercise (with answer) addin adds this code to your R Markdown file and creates a prompt box with 6 rows. You can change the number of rows by changing the `rows` argument.
...
The Tutorial Written Exercise (no answer) is the same thing, it just doesn't have an `answer` field.
...
However, they look the same once they're knitted.

```{r}
knitr::include_graphics("images/written-exercise.png")
```

Finally, the Format Tutorial Chunk Labels add in formats the tutorial so that it automatically has the correct numbering. That means that you can add in a new exercise at the beginning of the tutorial and reformat it without having to manually change every single chunk label or exercise header. It's recommended to place this as a keyboard shortcut due to its usefulness.
