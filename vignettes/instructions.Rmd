...
<!-- Explain that templates need to go after setup code chun.. -->
...
The 3 mandatory sections (copy-code-chunk, info-section, download-answers) are "imported" into each tutorial Rmd through the use of child documents, which was suggested by [mine on RStudio Community](https://community.rstudio.com/t/include-custom-code-chunks-from-separate-files/109216).

Therefore, the actual Rmd of these sections are saved in the `inst/child_documents` folder.
...
To add images to a tutorial, first make a directory called `images` in the folder that contains the .Rmd. Store all images for that tutorial there. Then, use the function `include_graphics()` to add the image into the document. Include this code in its own setup chunk, in the place where you want the image to appear in the tutorial. `include_graphics()` is part of the `knitr` package, so you should use `library(knitr)` in the setup chunk to load it. Below is an example:

```{r example}
include_graphics("images/example.png")
```

If the directory is not named `images` and is not within the same directory as the Rmd, the document will knit but the image will not appear. The reasons for this are explained in a little bit more detail here: https://rstudio.github.io/learnr/#external_resources. 
