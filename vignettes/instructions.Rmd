...
* Be wary about the .Rbuildignore file. It is important, but dangerous. In particular, it ensures that only specified files are copied over to the R package. Specifically, within the `/inst/tutorials/` directories, only *Rmd files and all the files in `images` and `data` are installed.

* R CMD check will fail if you have added (and used) a file to a `data/` directory, unless/until you re-install the package in your main library. (See the "[Technical Details](details.html)" vignette for a discussion.) This must be done outside of your `primer.tutorials` project. Start another RStudio session, one which does not use **renv**. Execute:

```{r, echo = FALSE}
remotes::install_local("path-to-your-projects/primer.tutorials")
```

For me, this is:

```{r, echo = FALSE}
remotes::install_local("~/Desktop/projects/primer.tutorials/")
```

But the location where you place your projects might be different.

The key is that this installs all the data 
