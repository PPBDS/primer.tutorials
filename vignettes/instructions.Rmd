...
* Begin with an *Introduction* which is a sentence or two of knowledge and/or the question itself. If the length of the introductory text is longer than one or two lines on the knitted Rmd, then do not place the question code chunk in the same part. Instead, the Introduction is followed by the triple hashes, thereby creating the Continue button. If the length of the text is short enough that students are willing to read it (at most 2 lines), you can include the answer code chunk in the same part.

* Then follows the answer code chunk(s). See below for more discussion. This grouping is also followed by the triple hash. We want students to pause after they have submitted their answers and then considered the results of their submission.
...
`r ''````{r wrangling-b-ex-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

`r ''````{r wrangling-b-ex-7-hint-1, eval = FALSE}
