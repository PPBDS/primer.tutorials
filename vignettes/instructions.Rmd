...
Each Section begins with a sentence or two about what this group of Exercises is trying to accomplish. Then add a triple hash `###` before adding the first Exercise. (It is also OK to show a plot or some other target object we will create and then other triple hash.) Example from the Data-API tutorial:
...
The link will be formatted correctly once the tutorial is knitted.
...
`r ''````{r the-terminal-ex-3, exercise = TRUE}

```
````

The code chunk name should begin with the name of the section followed by the appropriate Exercise number. (Note that we are working on a way to automatically create/update code chunk names for the entire tutorial.) You must set `exercise = TRUE`. You must include at least one empty line in the code chunk, otherwise you will get a hard-to-diagnose error.
...
Write about four sentences explaining, in your own words, the meaning of the motto "No 
causation without manipulation."

`r ''````{r ex-written, eval = FALSE}
question_text(NULL,
...
````markdown
`r ''````{r ex-written-no-answer, eval = FALSE}
  question_text(NULL,
