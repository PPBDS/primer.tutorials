...
# gt needed because we show a Preceptor Table to the students, which is
...
# check? Should we make students load it anyway?
...
<!-- Looks like we need easystats for posterior predictive check. -->
...
<!-- How we are testing now that we don't have pp_check? Something from performance package? -->
...
<!-- The first (of three) versions of the model is a mathematical formula which includes parameters like \beta_1 as well as the error term. This model does not, yet, specify exactly which variables are included on the left-hand side, much less any transformations which are required.  DK: What is the terminology which we use to describe this basic model, before any parameters are estimated? -->
...
Maybe we should call each model twice: first with no tidy, and then with tidy, thereby providing plenty of opportunity for knowledge drop discussions.
-->
...
<!-- XX: Fit the model to a different categorical variable, ideally one with just multiple levels, like race. If that is not possible, use your best judgment as to what to do instead. -->
...
<!-- XX: We need to fit a couple of models. Then, we need to decide which of them to use. Then, we create the LaTeX code for the DGM. -->
