...
<!-- XX: linear_reg() is most common.  -->

Because our outcome variable is [XX: normal or binary or multinomial or ordinal], start to create the model by using `XX`.
...
Continue the pipe with `set_engine("XX")`.
...
<!-- XX: A knowledge drop something like: -->

<!-- XX: There is a direct connection between the mathematical form of the model created under Justice and the code we use to fit the model under Courage. `height ~ 1` is the code equivalent of $y_i =  \mu$.  -->

### Exercise 8

Behind the scenes of this tutorial, an object called `fit_XX` has been created which has the result of the code above. Type `fit_XX` and hit "Run Code." This generates the same results as using `print(fit_XX)`.
...
<!-- DK: How do we make the regression table nice? gtsummary()? Just use tidy?-->
...
<!-- DK: Below needs to be re-written to use marginaleffects instead of tidybayes. -->
