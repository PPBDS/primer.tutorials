...
<!-- Always create any modeling objects and then save them in data/ by hand. Then, when building the tutorial, just read in the object. See the below example. Note that the commented code is run once, by hand, but saved so that it could be modified in the future. The fit_gauss assigment is run every time one hits Run Tutorial. But it is fast, so there is no cost. -->
...
<!-- DK: Discuss newdata objects slowly. "A newdata tibble must include columns for all the right-hand side variables --- all the "covariates" --- in the model, and with all the same variable names and, mostly, variable types." 

First ask a question about which columns are needed in the newdata object. Second, ask a question about which values you want those variables to have. -->
