...
<!-- By the way, I have a new idea for the Temperance section: Perhaps we have students create a quarto document and then put in all the key stuff there. Recall the quarto documents you created in the RStudio and Github and Quarto Websites tutorials. It is easy to ensure that students are doing what we tell them when by using show_file(). And, this is good for students since they will need to do exactly this anyway when they build their Final Project web pages. -->

<!-- In my mind, each tutorial of 4 through 10 does this. The webpage would contain 4 main parts: First, the nice plot showing the answer to the question. Second, the paragraph which summarizes what they have concluded. Third, the mathematical formula for the model. (They will need to use latex math for this, just like we do in the Primer.) And fourth is a table with the parameter estimates. -->
...
<!-- XX. You will probably need to make this question much more specific. -->

What are the covariates for this problem?

```{r wisdom-6}
question_text(NULL,
	message = "XX. Answer should be something about the variables we have access to. Should be connected to the EDA or the chapter discussion.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- XX: Consider adding more questions about EDA with your data. In particular, add at least one question about the dependent variable in the model along with one or more questions about covariates. If there is a treatment variable, you must include a question about it. -->
