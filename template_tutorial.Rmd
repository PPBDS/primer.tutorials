...
# and tutorial.helpers are required for the tutorial to work at all. gt is often
...
<!-- The specific question is always either causal or predictive. (Note that few people use the term "predictive." We just use it to distinguih from "causal." Maybe "non-causal" would be a better term. If it is causal, you should use key causal language in the question, like "What is the causal effect of . . ." If the model is non-causal, the question should clearly compare two groups of units. "What is the difference in immigration attitudes between Democrats and Republicans?") -->
...
<!-- DK: How do we ask about mathematics?  -->

<!-- Recall this line: In data science, we deal with words, math, and code, but the most important of these is code. Need to ensure that, in the Courage section, students go through all three and understand how they are connected. Need to remind students about categorical variables turning into multiple 0/1 variables. Need to remind students that we can think in terms of individual rows or in terms of vectors. -->

<!-- I feel like we aren't giving the authors enough guidance about how to explain the math part of the model, both making the latex and also understanding it, especially why character/factor variables get changed. Same with the words part of the model. We need to help them translate code into words. We generally just tell them the code. -->

<!-- At the start of Courage, remind them of the outcome variable and the key covariate. Then we tell them the code for the model they will fit. Discuss what those extra variables are doing --- mainly they allow us to say that we have "adjusted for" (not "controlled for") them. -->
...
<!-- XX: Delete all these instructions once you are done with the tutorial. The only comments remaining should be ones that you wrote, comments specific to your tutoril --- like modeling approaches that you tried but did not use, other approaches that might be considered in the future and so on. -->
...
<!-- A plot can be a knowledge drop. -->
...
<!-- DK: Give examples of acceptable questions. -->
...
<!-- Every EDA should show a scatter plot of the outcome variable on the y-axis and one of (probably the most important or the treatment) covariates on the x-axis. There is no need for titles, axis labels or other prettiness. -->
...
There are generally two parts for a statistical model: the *probability family* and the *link* function. The probability family is the probability distribution which generates the randomness in our data. The link function is the mathematical formula which links our data to the unknown parameters in the probability distribution. 
...
<!-- XX: Give the probability family of your model. Explain how that choice is dictated by the values of your outcome variable. Give the mathematical formula, using your actual outcome variables. Examples:


$$ biden_i  \sim Bernoulli(\rho) $$

$$ att\_end_i \sim Normal(\mu)$$
...
<!-- XX: Give a simple example of a link function. It is unclear to me whether this should always be a "generic" link function which does not mention any of the variables in our actual problem or a specific link function which does use them. Here are the two key generic link functions:

$$
\rho = \frac{e^{\beta_0 + \beta_1 x_1 + \dots}}{1 + e^{\beta_0 +\beta_1 x_1 + \dots}}
$$
...
<!-- XX:  -->
<!-- ``` -->
<!-- Give them the math within these triple backticks so that they can just copy and paste it into their QMD. Use double dollar signs so that each formula is on its own line. -->
<!-- ``` -->
...
<!-- XX: Give an example of the actual math by replacing th x_0 variables in the generic link function with actual variables. Use your best judgment. You do need to give all the math, but you should give some. Examples:  
...
<!-- DK: Add some examples from a non-causal model. Our estimate or the mean of our posterior -->
