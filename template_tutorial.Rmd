...
Connect the Github repo to a project on your computer using `File -> New Folder from Git ...`. Make sure to select the "Open in a new window" box. 
...
In your QMD, load the **tidyverse** and the [XX: **primer.data**] packages in a new code chunk. Render the file.

Notice that the file does not look good because the code is visible and there are annoying messages. To take care of this, add `#| message: false` to remove all the messages in this `setup` chunk. Also add the following to the YAML header to remove all code echos from the whole file:
...
In the Console, type `?XX`, and paste in the Description below.
...
<!-- XX: We finished the Introduction with a broad area of interest and a relevant dataset. Now we need to iterate, moving back and forth between the data and the Preceptor Table until we have ones that seem to work well together. -->
...
```{r justice-1}
question_text(NULL,
	message = "Justice concerns five topics: the Population Table, validity, stability, representativeness, and unconfoundedness.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
For a multinomial outcome variable, we use a multinomial logistic regression model.
...
check_predictions(extract_fit_engine(fit_XX))
```
...
First question is always predictions(fit_XX) with the knowledge drop being:
...
Change code for other library loadings to the Console.

check_predictions(extract_fit_engine(fit_biden)) from easystats does not show the plot in two places that we really want it to: an exercise code block or a simple code chunk in the Rmd. Very annoying! It is not clear what to do about this. There are two ways to see the cool plot. First, we could run it in the Console. Second, we could add it, temporarily to the QMD, then render, then look at the plot, then remove it. Neither option is great!

If we run it in the Console, we need to create all the objects needed there, including libraries, any data cleaning and model creation. Recall that this is all occurring in another R session over which we have no control. If we do it in the QMD, we need to ensure that it is set up to work.

In the Console, there are two approaches. First, provide the students with a block of 10+ lines of code that they need to copy/paste into the Console just before the check_predictions() question. Second, instead of doing all the commands in exercise code blocks, as we currently do, we could have students do everything in the Console. So, like `library(tidymodels)` would occur in the Console. The main draw back of that approach is that it makes it harder to "know" that students did it. (Checking search() each time might be annoying, as is CP/CR each time. Or maybe not?) Of course, maybe there is no need to check since the check_predictions() command won't work if you don't do this.

The more that I think, the more reason there is to get rid of the "Put library(tidyverse) in the exercise code block." First, they have already done a ton of that in r4ds.tutorials. No need to do more. Second, this is not what data scientists do! We either put library commands in the Console or in the QMD. So, assume that we are getting rid of that.

This means that we need to set up the new Window at the start, which means moving up the repo creation. That is good stuff for the Introduction. -->


<!-- OPEN ISSUES: -->

<!-- 1) Combine the Question and Wisdom sections. That will lead to 20 questions by default, which is OK. Data science begins with a decision, not with a question. The Introduction section describes that decision, generally in the "Imagine that you are . . ." paragraph. Wisdom features an interplay between possible questions (and the resulting Preceptor Tables) and possible data sets. We go back-and-forth between them, until we finally settle it at the end. The Wisdom section ends with the Preceptor Table and the data. -->

<!-- In doing this, think harder about displaying iterations of the Preceptor Table. At the very least, we need to explicitly show, with gt() code, the final version of our beautiful Preceptor Table, the one we use going forward. But, as part of the back-and-forth iteration, we will also discuss earlier versions of the Preceptor Table. Maybe we just do so with words? Maybe we also use gt()? -->
