...
<!-- DK: Add more discussion about QoI. -->
...
Given our choice of treatment variable `XX`, how many potential outcomes are there for each unitXX? Explain why.
...
Write a sentence which speculates as to value of the XX different potential outcomes which we might observe in `XX: outcome variable` for each unitXX when we change the value of the treatment variable `XX`. 
...
<!-- Let's me give a couple of questions/answers to serve as examples, all based on this display. -->

<!-- fit_obj <- brm(formula = att_end ~ sex + treatment + att_start, data = trains) -->

<!-- > fixef(fit_obj) -->
<!--                  Estimate  Est.Error  Q2.5     Q97.5 -->
<!-- Intercept         2.57       0.462    1.676   3.4531-->
<!-- sexMale          -0.34       0.247   -0.833   0.1413-->
<!-- treatmentControl -0.92       0.248   -1.396  -0.434 -->
<!-- att_start         0.79       0.040    0.713   0.8690-->
<!-- >  -->

<!-- Q: Write a sentence interpreting the -0.34 estimate for sexMale. -->

<!-- A: When comparing men with women, men have a 0.34 lower value for att_end, meaning that they are slightly more liberal in immigration, relative to women, conditional on the other variables in the model. -->

<!-- Comment: Note how, whenever we consider non-treatment variables, we must never use terms like "cause," "impact" and so on. We can't make any statement which implies the existence of more than one potential outcome based on changes in non-treatment variables. We can't make any claims about within row effects. Instead, we can only compare across rows. Always use the phrase "when comparing X and Y" or something very similar. -->


<!-- Q: Write a sentence interpreting the confidence interval for sexMale. -->

<!-- A: We do not know the true value for the coefficient for sexMale, but we can by 95% confident that it lies somewhere between -0.83 and 0.14.  -->

<!-- Comment: Because we are Bayesians, we believe that there is a true value and that the confidence or uncertainty interval includes it at the stated level. These questions provide an occasion in the knowledge drop to compare/contrast the Frequenist interpretation. -->

<!-- Q: Write a sentence interpreting the -0.92 estimate for treatmentControl. -->

<!-- A: The causal effect of receiving the treatment of hearing Spanish-speakers on the train platform, relative to the control, is to have a 0.92 higher value for att_end, meaning that the treatment, relative to the control, makes one more conservative about immigration. -->

<!-- Comment: The interpretation of a treatment variable is very different than the intrepretation of a standard covariate. Whenever your model includes a treatment variable, you must always have a question about it and about the associated confidence interval. -->
