...
Edit the summary paragraph in your QMD as you see fit, but do not copy/paste our answer exactly. `Command/Ctrl + Shift + K`, and then commit/push.
...
<!-- XX: Not sure what to put here. I am still wrestling with the structure of this topic. I want to introduce the basic structure of the model first, without showing exactly which independent variables are included. Then, fit the model. Then show parameter values. But there is no (?) easy way to show the rough structure. So, instead, I sjust show, and explain, the exact model. -->
...
<!-- XX: Perhaps we should use the modelsummary package to make nicer looking tables? -->
...
<!-- XX: We still have some details to worry about in terms of what the table should look like. Not sure on the best approach. If we use the gt() package, we need to load it. Maybe we just give all the code, including library(gt), and then can just copy it in. Ought to get rid of the statistic and p.value columns. -->
