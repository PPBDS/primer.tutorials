...
<!-- Insert somewhere: 'We recommend the verb "adjust" in place of "control" when discussing the effect of including other variates in the model. Example: "The causal effect of exposure to Spanish-speakers is 1.5, adjusting for other variables like age and party." Adjusting is better than controlling because it demonstrates some humility. -->
...
<!-- The first sentence in the summary paragraph should be a simple reference to the overall topic and, perhaps, mention why it is important. This orients the reader to why the subject matters. The second sentence almost always begins "Using data from ...." and then gives a few words about the data source, followed by a comma. The rest of the sentence is something, "I/we explore the relationship between Y and X" where Y is the concept of the outcome variable, not the specific variable name in your data. -->

<!-- Five sentences: -->

<!-- 1) General introduction to the topic. Almost always along the lines of either a rhetorical question or a claim that Y is important or changing or interesting or . . . -->

<!-- 2) "Using data from (mention date, collection method (?) and organization (if famous)), I/we seek to understand outcome variable, which (give some information about outcome variable.), in order to (estimate the causal effect | compare two groups) for when/where. -->

<!-- 3) Mention a weakness involving the key assumptions. Be wary of using technical terms. -->

<!-- 4) "I/we model Y (the concept of the outcome, not the variable name; we almost never use any variable names, just like we never mention specific R code) as a linear/logistic/ordinal function of X (and maybe other covariates.)"  -->

<!-- 5) Describe a QoI, provide an estimate and the uncertainty associated with it. -->
...
<!-- XX: Example: Imagine that you are running for Governor of Texas in the next election. You have a campaign budget. Your goal is to win the election. Winning the election involves convincing people to vote for you *and* getting your supporters to vote. Should you send postcards to registered voters likely to vote for you? What should those postcards say?  -->
...
## simple histogram. Or it can include a covariate. The subtitle should
...
Add `library(marginaleffects)` to the `XX.qmd` setup code chunk. Create a new code chunk in `XX.qmd` which creates the `ndata` object. `Command/Ctrtl + Shift + K`.
...
Create a new code chunk in `XX.qmd`. Label it with `label: plot`. Copy/paste the code which creates your graphic. 
