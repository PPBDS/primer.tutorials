...
1) Basic intro, until mathematical structure of the model. Maybe also load up all the libraries, but not marginaleffects until Temperance.

2) Build a pipe to estimate one model. Three lines: model set up, fit and then tidy. (Or, if not tidy, something from easystats)

3) Change the code so that the model is different. Maybe ask a question about the two models. Maybe just tell the student what she should notice.

4) Add a third model. There is just no excuse not to have three models, even if they each only require  one or two questions.
...
<!-- DK: Maybe this should be easystats? Or broom, and sometimes broom.mixed? -->
...
<!-- The first (of three) versions of the model is a mathematical formula which includes parameters like \beta_1 as well as the error term. This model does not, yet, specify exactly which variables are included on the left-hand side, much less any transformations which are required.  What is the terminology which we use to describe this basic model, before any parameters are estimated? -->
...
<!-- XX: Provide a nice looking LaTeX version of the model. Then, show the code, within a four backtick environment, which generates that LaTeX. Then, provide some sensible discussion. Note that we can't (easily)use  a yes-answer exercise here because the answers inside an exercise chunk can't really be LaTeX. -->
...
<!-- DK: Need some examples here. How should look for the various sorts of common models? How would it look for random forests? -->
...
<!-- DK: We need to fit a couple of models. Then, we need to decide which of them to use. Then, we create the LaTeX code for the DGM. -->
...
<!-- XX: Note that, in some circumstances, functions from the marginaleffects package will not work in exercise code chunks. That is annoying and has something to do with weird interactions involving the shiny prerendered settings on tutorials. So, we always have students perform the calls to functions like plot_predictions() in the Console, and then have them CP/CR. (Note that there is rarely an response, so they are really just copying/pasting them command itself.) Note that we can't run these function in the tutorial itself for the same reason. Make sure that your knowledge drops explain clearly what the graphic which the students have generated means. -->
...
<!-- XX: Discuss more about what the model results are telling us. -->
...
<!-- XX: Give the final plot and our summary paragraph. And a note indicating how this information might be helpful to the Imagine person we created at the start. Or maybe do that in the last few questions in Temperance? -->
