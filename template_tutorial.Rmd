...
Imagine that you are ... <!-- XX: In a few sentences, provide background on the problem which you face. Do not discuss the data that you have. The problem is highly likely to be very general and multi-faceted. That is OK. That is an inherent aspect of reality. -->
...
The central problem for Wisdom is: Can we use data from [XX: describe your data] to predict behavior in [XX: describe your Preceptor Table]?
...
<!-- XX: Not sure what to put here. I am still wrestling with the structure of this topic. I want to introduce the basic structure of the model first, without showing exactly which independent variables are included. Then, fit the model. Then show parameter values. But there is no (?) easy way to show the rough structure. So, instead, I just show, and explain, the exact model. -->
...
The basic mathematical structure of the model is (mostly) determined by the type of outcome variable. 

<!-- DK: Provide the four possibilities for authors to choose from. -->
...
```{r}
extract_eq(fit_XX, wrap = TRUE, show_distribution = TRUE)
```
