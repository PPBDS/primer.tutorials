...
Because `biden` is a binary variable, we assume that the outcome of voting for Biden (or not) is produced from a Bernoulli distribution.
...
<!-- XX: Discuss the link function. This is a "generic" link function which does not mention any of the variables in our actual problem. 

Because we are using a Bernoulli (or Categorical) distribution, the link function is logit. That is:
...
<!-- XX: Mention other important details here. For example, in a categorial or ordinal models, the probabilities must sum to 1. Also, we might only give the formula for one $\rho$. Mention that the other formulas would be the same, but with different parameters. -->
