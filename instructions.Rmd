...
4. Use spacing around equal signs. For example, ggplot(aes(x = rtg)) NOT ggplot(aes(x=rtg)). 
...
The last section panel of your tutorial must be the Submit section. This prints out instructions for students to download an .rds file containing their hash to be uploaded to Canvas. Every details matters, including the context values and the lack of paratheses after `submission_ui`.
