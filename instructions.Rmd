...
Ex. This is what the top of the Chapter 3 tutorial looks like:
...
2. Copy/paste the code below. This is your "setup" code chunk. 
...
Every tutorial must load the **learnr** package. `library(primer.tutorials)` is always included because it contains the code, `submissions_functions.R`, which enables answer downloading. All our tutorials need the **tidyverse**. You may need to add other packages which are used in your tutorial.
...
Each side panel is composed of a series of numbered exercises.
...
3. Prevent quotes from turning into curly quotes by writing `"nnet"` instead of "nnet". You never want curly quotes in something which might be copy/pasted to the prompt. 
