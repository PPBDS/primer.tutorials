...
2. Next, you want to insert an "r-chunk" --- via Control-Option-i. Copy/ paste all the text below (from 'r-setup' to the last parentheses) into your r-chunk. This sets up the packages to make sure the tutorial functions appropriately. 
...
Every tutorial must load the **learnr** package. `library(primer.tutorials)` is always included because it contains the code, `submissions_functions.R`, which enables answer downloading. All our tutorials need the **tidyverse**. You may need to add packages which are used in your tutorial.

`echo = FALSE` is a handy default because, most of the time, we don't show students the code. Setting the `tutorial.exercise.timelimit` to 60 seconds is safe, but probably not necessary. `tutorial.storage` must be set to `local` to ensure that student work is saved between sessions.


## First Question

<!-- DK: Edit this once we decide on the best way. -->

The first question should ask students to give us their name and email. (This way, when we generate a hash of a student's responses, their name/email is included in the hash). Use this:
...
Tutorials are divided into sections that are seen on the side panel. To create these sections, we include a double hash (##) before the text for it to show up as a side panel. This is also called the Group Title. If possible, there should be a direct mapping of these headers to the sub-part of the chapter which they cover. (Ex. If there is an “Basic Commands” section within the chapter, you make this into a Group Title (## Basic commands). Use sentence case.
...
Each tutorial should have a preliminaries section which reminds students of any tricks which are needed here, even if we have already seen those functions in earlier chapters. This should be its own side panel.
...
2. Never instruct a student to replace an object with a different object with the same name. 

3. One way to having quotes turned into curly quotes is to write `"nnet"` instead of "nnet". You never want curly quotes in something which might be copy/pasted to the prompt. 

4. Use spacing around equal signs. For example, ggplot(aes(x = rtg)) NOT ggplot(aes(x=rtg)). And follow other aspects of our Code Style Guide.
...
<!-- DK: Doesn't need R text constant make this unecessary? -->
...
For unknown reasons, our usual way of including images in an Rmd file does not work in tutorials. (Probably cause is something to do with Shiny.) The workaround is to include the png and grid packages among the libraries and then use code like:
...
The last section panel of your tutorial must be the Submit section. This prints out instructions for students to download an .rds file containing their hash to be uploaded to Canvas. Every detail matters, including the context values and the lack of parentheses after `submission_ui`.
