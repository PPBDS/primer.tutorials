...
* Added a `getting-started-tutorial` tutorial, imported from `PPBDS/learnr2`'s `getting-started`. Unlike every other tutorial in this package it is a **learnr2** tutorial (`inst/tutorials/getting-started-tutorial/tutorial.qmd`, Quarto + `quarto-live` + WebR), not a learnr `.Rmd`: it runs entirely in the browser and teaches the mechanics of a learnr2 tutorial (student info, running `{webr}` cells, downloading answers). Because it is `.qmd`, `learnr::available_tutorials()` / `learnr::run_tutorial()` and the `tutorial.helpers` render test do not see it; run it with `learnr2::run_tutorial("getting-started-tutorial", package = "primer.tutorials")`, which vendors the `quarto-live` extension and renders on demand. `learnr2` added to `Suggests` and `Remotes` (`PPBDS/learnr2`).
