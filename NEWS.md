...
* Fixed a render failure in `dgm-logistic`: `plot_predictions(mod_logistic, condition = <continuous>)` on a parsnip `logistic_reg()` fit exhausts memory under `marginaleffects` 0.32.0 (the same call on an `lm` fit or via `extract_fit_engine()` is fine). Exercises 10 and 11 now build an explicit grid of hypothetical employees (`tibble()` / `expand_grid()`) and pass it to `predictions()`, which is equivalent, version-robust, and keeps the grid columns named exactly as the `ggplot()` code expects.
