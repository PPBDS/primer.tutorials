...
make_p_table <- function(table = "preceptor",
  ...
  # Validate inputs
  table <- tolower(substr(table, 1, 3))  # "pre", "pop", etc.
  type <- tolower(substr(type, 1, 3))    # "pre", "caus", etc.
  ...
  spanner_treatment <- if (is_causal) glue::glue('  tab_spanner(label = "Treatment", columns = c({treatment_col})) |>') else ""
  treatment_col_def <- if (is_causal) paste0(", ~`", treatment_label, "`") else ""

  # Generate column header line for tribble
  col_headers <- c(
...
{spanner_treatment}
  ...
  {if (is_causal) 'gt::tab_footnote(treatment_footnote, locations = gt::cells_column_spanners(spanners = "Treatment")) |>' else ""} 
