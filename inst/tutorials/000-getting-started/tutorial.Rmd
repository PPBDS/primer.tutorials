...
<!-- Add stuff about Terminating a tutorial. Restarting RStudio. All your work is there. Use images. Warn about that terminate window being hidden. Replaces Exercsie 4. VD: Done -->

<!-- Add an example which shows the Copy Code button and how to use it. VD: Done-->

<!-- Have them download both the rds and the html. Explain their different uses. Explain what happens when you download the answers twice by mistake. VD: I can't seem to replicate this error. -->
...
You should have already run `primer.tutorials::install_only_binary_packages()` as part the *Getting Started* chapter. To confirm that you have done so, type `options("pkgType")` in your RStudio Console and hit return. Copy/paste the command and the return value below. 
...
The return value should be "binary". If it is not, try rerunning `primer.tutorials::install_only_binary_packages()` in the R Console.
