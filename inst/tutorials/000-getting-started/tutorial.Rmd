...
<!-- Plan is that students are told to read the Getting Started chapter before the first class. We want them to, at least, install R/RStudio before the first class. Then, in class, we do this tutorial together. Given that structure, we should make this a bit longer and ensure that all the important things in Getting Started have, in fact, been done. Key example is usethis::blank_slate(). -->
