...
<!-- Plan is that students are told to read the Getting Started chapter before the first class. We want them to, at least, install R/RStudio before the first class. Then, in class, we do this tutorial together. Given that structure, we should make this a bit longer and ensure that all the important things in Getting Started have, in fact, been done. -->
...
The response should be TRUE. If it is not, you probably failed to complete the Getting Started chapter in the *Primer*. Quit the tutorial, restart RStudio, run `usethis::use_blank_slate()`, restart RStudio, restart the tutorial and then this should work.
