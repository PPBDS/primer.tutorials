...
Since you have already read and followed the instructions in the [Getting Started](https://ppbds.github.io/primer/00-getting-started.html) chapter from the *[Primer](https://ppbds.github.io/primer/)*, the following command should work:
...
You can generally call functions by just giving their name, like `prep_rstudio_settings()`. However, it can be safer to also give the name of the package in which the function is found. Since `prep_rstudio_settings()` is in the **primer.tutorials**, we can also call it using the double colon notation, as above.
...
Type `options("pkgType")` in your RStudio Console and hit return. Copy/paste the command and the return value below. 
...
The return value should be "binary". If it isn't, try re-starting your R session again.
