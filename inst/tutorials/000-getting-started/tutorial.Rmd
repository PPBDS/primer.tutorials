...
You should be running R version 4.1.0 or later. If not, reinstall R, following the instructions in [Getting Started](https://ppbds.github.io/primer/getting-started.html).
...
You should already have run the command `usethis::use_blank_slate()` from the R Console. This causes RStudio to neither save nor reload old janky data sets. To confirm that you have done this, run `rstudioapi::readRStudioPreference(name = "load_workspace", default = "Code failed.")` from the Console. Copy/paste the command and the answer below.
...
You should have already run `primer.tutorials::install_only_binary_packages()` as part 
