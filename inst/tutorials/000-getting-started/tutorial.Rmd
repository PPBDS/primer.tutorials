...
Let's practice some simple math. We've already assigned 5 to `x` and 4 to `y`. On the line below those assignments, add the two variables by typing `x + y`. Hit **"Run Code"** to see your results. Clicking "**Hint"** gives you a hint about what to type. 
...
Once you click **"Run Code"** the answer --- [1] 9 --- should appear below the code you entered. This is the same behavior you would see if you ran this code in the Console.
...
Let's check the version of R running on your computer. You can do this by copy-pasting `version[['version.string']]` into your console and then by hitting **"Run Code"**. 
...
`writeRStudioPreference("load_workspace", FALSE)` from the R Console. This causes RStudio to neither save nor reload old janky data sets. To confirm that you have done this, run `rstudioapi::readRStudioPreference(name = "load_workspace", default = "Code failed.")` from the Console. Copy/paste the command and the answer below.
...
The response should be FALSE. If it is not, you probably failed to complete the Getting Started chapter in the *Primer*. Please do so now.


### Exercise 8

You should have already run `primer.tutorials::install_only_binary_packages()` as part the *Getting Started* chapter. To confirm that you have done so, type `options("pkgType")` at the prompt and hit return. Copy/paste the command and the return value below. The return value should be "binary".
