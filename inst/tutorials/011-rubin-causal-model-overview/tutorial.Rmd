...
<!-- add an image of a receptor table and ask them to define it in their own words, then ask them to define the preceptor table using the primer--> 


### Exercise 1

In your own words, define the table below:

<!-- DK: Create table by hand, using gt package, just as we do in the Primer. -->

```{r preceptor-table-ex-1}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Go to the [Primer](https://ppbds.github.io/primer/rubin-causal-model.html) and define the preceptor table in your own words now:

```{r preceptor-table-ex-2}
question_text(NULL,
...
```{r preceptor-table-ex-3}
precep_table
```

Define what the column ID is for in your own words:

```{r preceptor-table-3-a}
question_text(NULL,
    message = "The first column 'ID' serves to label each unit. Each person is a unit.",
    answer(NULL, correct = TRUE),
    allow_retry = FALSE,
    incorrect = NULL,
    rows = 3)
...
We will always have an ID column in Preceptor Tables so that we can identify different units.

### 

Define what the column "Height(cm)" is in your words:

```{r preceptor-table-3-b}
question_text(NULL,
    message = "The second column 'ID' serves as the outcome of interest. In this example it would be the Height in cm.",
    answer(NULL, correct = TRUE),
    allow_retry = FALSE,
    incorrect = NULL,
    rows = 3)
...
To calculate the causal effect of positive causal effect, we need to compare the outcome for an individual in one possible state of the world (with treatment) to the outcome for that same individual in another state of the world (without treatment).

### Exercise 4

```{r causal-effect-ex-4}
...
We did not have to deal with the Fundamental Problem of causal Inference because we knew the outcomes for both treatment and control. Write a paragraph explaining the Fundamental Problem of Causal Inference. Include an example relating to your own life.

```{r causal-effect-ex-5}
question_text(NULL,
...
Many times if we don't know the outcomes for the source under both treatment and control so we are not able to ignore the Fundamental Problem of causal Inference. 
...
```{r average-treatment-effect-ex-1}
question_text(NULL,
...
```{r average-treatment-effect-ex-2}
question_text(NULL,
    message = "1.6",
...
```{r average-treatment-effect-ex-3}
question_text(NULL,
    message = "9.4",
    answer(NULL, correct = TRUE),
    allow_retry = FALSE,
...
```{r average-treatment-effect-ex-4}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
...
```{r average-treatment-effect-ex-5}
question_text(NULL,
