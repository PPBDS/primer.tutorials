...
author: Sophia Zhu, Arghayan Jeiyasarangkan, Mann Talati, and Anish Bellamkonda
tutorial:
...
<!-- Change the Preceptor/Population Table sections to match the book? Or is it good for the examples here to be different? -->
...
This tutorial covers [Chapter 1](https://ppbds.github.io/primer/rubin-causal-model.html) of [*Preceptor’s Primer for Bayesian Data Science: Using the Cardinal Virtues for Inference*](https://ppbds.github.io/primer/). This tutorial will review key concepts including the Preceptor Table, the Population Table, potential outcomes, causal effects, validity, stability, representativeness, and unconfoundedness. We assume that your have read the [chapter](https://ppbds.github.io/primer/rubin-causal-model.html).
...
Question marks are used in indicate missing data. It is tempting to "plug in" the data that we have for four of the brothers. For examle, don't we "know" that Robert is 178 centimeters tall? Can't we just replace "?" with 178 in our Preceptor Table?
...
It is so common to, implicitly, assume that nothing has changed between data collection and now --- or to whatever time period the question refers --- that we often go straight to a Preceptor Table which includes the data that we have.
...
```{r preceptor-table-6}
question_text(NULL,
	message = "Validity is the consistency, or lack thereof, in the columns of your dataset and the corresponding columns in your Preceptor Table. In order to consider the two datasets to be drawn from the same population, the columns from one must have a valid correspondence with the columns in the other.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
We hope that all this knew terminology is not overwhelming. The "[Key Concepts](https://ppbds.github.io/primer/key-concepts.html)" chapter in the [*Primer*](https://ppbds.github.io/primer/) provides an overview of all the most important terms and their definitions.
...
Much of what a good data scientist does is to try to think of reasons why she can **not** answer the question. Assumptions are rarely fully true. We need to test them. And the best way to start testing is to come up with (plausible) objections. We will practice this skill over and over again.
...
In your own words, describe the process by which we can go from the combined tables in the previous Exercise to the "informal" Preceptor Table with which we began:
...
Use the following "imaginary" Preceptor Table to answer some questions about "estimands," quantities in which we might be interested.
...
In your own words, write two sentences which explains the meaning of the phrase "No causation without manipulation."

<!-- Could make it a paragraph and ask for personal example . . . -->


```{r causal-effect-8-ex}
question_text(NULL,
...
```{r assumptions-1}
question_text(NULL,
	message = "The assumption of validity allows us ignore any variation in treatment or in any other variable. It allows us to 'stack' the rows from the data, the Preceptor Table, and the population into a single Population Table.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
