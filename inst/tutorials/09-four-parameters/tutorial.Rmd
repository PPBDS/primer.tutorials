...
*Among all people who have an `att_start` value of 10, what proportion are not liberal?*


*If there is a group of four people at the train station today, two of whom that have an `att_start` value of 13 and two of whom that have an att_start value of 8, how many will be liberal?*


### Exercise 1

Let's begin by answering the first question. To do so, create a tibble a named `newobs` that has one variable `att_start1` set to 10.
...
Copy/paste your code from above. Use `posterior_epred()`. The first argument to `posterior_epred()` should be `fit_1`. Also set `newdata` to `newobs`.
...
Pipe the results of `posterior_epred()` to `as_tibble()`. Save your results to an object named `pe`.
...
Pipe the results of `posterior_epred()` to `as_tibble()`.
...
Write a paragraph that interprets the tibble you just created. Make sure to answer the question in your response. 
...
Let's now answer the second question. Start by creating a tibble named `newobs` that has one variable `att_start`. Use the `c()` and `rep()` function to create two observations for each `att_start` value.
...
newobs <- tibble(att_start = c(rep(13, 2),
                            rep(8, 2)
```

### Exercise 7

Copy/paste your code from above. Use `posterior_predict()`. The first argument to `posterior_predict()` should be `fit_1`. Also set `newdata` to `newobs`. Then, pipe your results to `as_tibble()`. 
...
Continue your pipe with` mutate_all()` with the argument `as.numeric`. Then, use `rowwise()`. 
...
Lastly, use `mutate()` and set `total` to `sum(c_across)`.
