...
Use `posterior_epred()`. The first argument to `posterior_epred()` should be `fit_1`. Also set `newdata` to `newobs`.
...
Use `posterior_predict()`. The first argument to `posterior_predict()` should be `fit_1`. Also set `newdata` to `newobs`. Then, pipe your results to `as_tibble()`. 
