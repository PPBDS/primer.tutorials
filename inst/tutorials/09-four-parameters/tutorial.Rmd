...
Let's create the following posterior probability distributions.
...
Continue the pipe. Use `rename()` to rename the `(Intercept)` column as `female_years`. Also use `select()` to keep just two variables: `female_years` and `male_years`.
...
<!-- BG: Should we tell them the exact arguments here rather than just saying "same tricks" in the chapter? -->

### Exercise 11

Pipe the results above to `ggplot()`. Map `years` to the x-axis, and map `Parameters` to `fill`. Add the layer `geom_histogram()` with the same tricks we use in the chapter: `after_stat()`, `bins`, `alpha` and `position`.
...
Let's also change the y-axis values to show percents rather decimals. Use `scale_y_continuous()` and set `labels` to `scales::percent_format()`. 
...
Continue the pipe with `mutate()` to create a new variable `Idaho_males`. `Idaho_males` should be equal to the following argument: `(Intercept)` + stateIdaho + sexMale. Make sure you place backticks on either side of the parentheses that enclose (Intercept).
...
Continue the pipe and select `Idaho_females` and `Idaho_males`. Then, pipe those results to `pivot_longer()`. Within `pivot_longer()`, set `cols` to `Idaho_females` and `Idaho_males` (Make sure you insert a colon between them). `names_to` should be set to "Parameters" and `values_to` should be set to "years".
...
Pipe your code to `ggplot()`. Map `years` to the x-axis, and map `Parameters` to `fill`. Add the layer `geom_histogram()` with the same tricks we use in the chapter: `after_stat()`, `bins`, `alpha` and `position`.
...
Let's create the following graph that has the two key posteriors.
...
Continue the pipe and `select()` `CA` and `MA`. Then use `pivot_longer()` and set `cols` to `CA` and `MA` (Make sure you insert a colon between them). `names_to` should be set to "Parameters" and `values_to` should be set to "years".
...
Pipe your code to `ggplot()` to plot your data. Map `years` to the x-axis, and map `Parameters` to `fill`. Add the layer `geom_histogram()` with the same tricks we use in the chapter: `after_stat()`, `bins`, `alpha` and `position`.
...
Great! Write two sentences interpreting this graph.
...
Copy/paste your code from above and assign it to an object named `new_object`.
...
Start a pipe with `pp` and use `as_tibble()`. Then use `mutate()` with the argument `across(everything(), as.numeric)`. We do this to make sure the code runs smoothly.
...
Now pipe your results to `ggplot()`. Map `diff` to the `aesthetic`.
...
Now add the layer `theme_classic()`. Also use `scale_y_continuous()`. Within `scale_y_continuous()`, set `labels` to `scales::percent_format()`.
