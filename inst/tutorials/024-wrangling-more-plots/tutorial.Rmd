...
We will be using the same data that was used in the Michigan voter experiment histogram graph. In this one however, we will showing you how to use `group_by()` and `mutate()` more.
...
Then use `ungroup()` to remove the grouping. 
...
<<<<<<< HEAD
The `baby_data` data set comes from the SSA (Social Security Administration). After 1986, all baby's were required to be given a social security number. This data contains all the names, year born, and sex of the babies. The number of names is counted and is converted into a proportion. We will be using this data to analyze the popularity of top 10 names each year.
=======
The `babynames` data set consists of all the names, what year, what gender, the number of people that have the same name, and the proportions of people that have the name compared to everybody born in that year. We will be using this data to analyze the popularity of top 10 names each year.
>>>>>>> 673b96873d546e033252efd5cb9fe847e8f40ec9
...
Previously, we were able to graph our `babynames` data on a plot. Now, we're going to explore these graphs a little deeper and see what else we can do with it.
...
Use the previous code. Replace `geom_line()` with `geom_point()`. Then add `geom_smooth()` and set `method` = `lm`

```{r more-babynames-1, exercise = TRUE}

<<<<<<< HEAD
=======
```{r babynames-13, exercise = TRUE}
>>>>>>> 673b96873d546e033252efd5cb9fe847e8f40ec9
...
<<<<<<< HEAD
```{r more-babynames-1-hint-1, eval = FALSE}
=======
```{r babynames-13-hint-1, eval = FALSE}
>>>>>>> 673b96873d546e033252efd5cb9fe847e8f40ec9
babynames %>%
  group_by(sex, year) %>%
  top_n(n = 10, wt = prop) %>%
...
The `geom_smooth()` helps create the line of best fit and the method argument creates that aura around it

<<<<<<< HEAD
### Exercise 2

Save everything to `x`. Then a few lines underneath use `ggplotly()` and set the argument to `x`

```{r more-babynames-2, exercise = TRUE}
=======
### Exercise 14

Save everything to `x`. Then a few lines underneath use `ggplotly()` and set the argument to `x`

```{r babynames-14, exercise = TRUE}
>>>>>>> 673b96873d546e033252efd5cb9fe847e8f40ec9
...
<<<<<<< HEAD
```{r more-babynames-2-hint-1, eval = FALSE}
=======
```{r babynames-14-hint-1, eval = FALSE}
>>>>>>> 673b96873d546e033252efd5cb9fe847e8f40ec9
...
Our previous graph was pretty cool, we could interact with it. Now let's make it move and come to life
...
x + transition_reveal(year)
```

<<<<<<< HEAD
### Exercise 4

We have provided you with the beginning of the previous graph. Save everything to `why`. Then a few lines down, insert the `why` and add the layer `transition_reveal()` so that it can display data by year.

```{r more-babynames-4, exercise = TRUE, fig.height= 3, fig.width = 4}
...
<<<<<<< HEAD
```{r more-babynames-4-hint-1, eval = FALSE}
=======
```{r babynames-16-hint-1, eval = FALSE}
>>>>>>> 673b96873d546e033252efd5cb9fe847e8f40ec9
why <- ... %>%
  
why + transition_reveal(year)
```

### Exercise 5

Let's create this unique circle graph
...
Start a pipe with the `babynames` dataset. Then `group_by()` `sex` and `year`.

```{r more-babynames-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-6-hint, eval = FALSE}
 babynames %>%
  group_by(..., ...)
```
...
Use `filter()` to get only the male names

```{r more-babynames-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-7-hint, eval = FALSE}
... %>%
  filter(... == "...")
```
...
```{r more-babynames-8-hint, eval = FALSE}
... %>%
  top_n(n = ...)
```
...
Now use `ggplot()` to graph it. Map x to name. Then add the `ylim()` layer to limit the numbers from -50 to 100. Add `geom_bar()` and set fill equal to `alpha()` with the color "skyblue". Adjust the feel a little by using `theme_minimal()`. To get rid of the text, use `theme()` and inside set what needs to go to element_blank().

```{r more-babynames-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-9-hint, eval = FALSE}
... %>%
  ggplot(mapping = aes(x = name)) +
  ylim(-50,100) +
  geom_bar(fill = alpha("skyblue")) +
  theme(axis.title = element_blank(),
    panel.grid = element_blank(),
    axis.text.y = element_blank())
```
...
To get it into a circle shape, add `coord_polar` at the end

```{r more-babynames-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-10-hint, eval = FALSE}
... +
coord_polar()
```
...
Let's create this really cool graph

```{r}
clean_data <- babynames %>%
  filter(name %in% c("John", "William", "James", "Charles", "George", "Frank", "Joseph", "Thomas", "Henry", "David")) %>%
  filter(sex == "M") %>%
   ggplot(mapping = aes(x = year, y = n, fill = name, text = name)) +
    geom_area() +
    scale_fill_viridis(discrete = TRUE) +
    theme(legend.position = "none") +
    labs(title = "Popularity of American names in the past 30 years") +
    theme_ipsum() +
    theme(legend.position = "none")


ggplotly(clean_data, tooltip = "text")
```


### Exercise 12

Start a pipe with `babynames`, then `filter()` out the names "John", "William, "James", "Charles", "George", "Frank", "Joseph", "Thomas", "Henry", "David".

```{r more-babynames-12, exercise = TRUE}

```


```{r more-babynames-12-hint, eval = FALSE}
babynames %>%
  filter(name %in% c("...", "...", "...", "...", "...", "...", "...", "...", "..."))
```
...
Continue your pipe with `filter()` again but this time filter out sex by male. 

```{r more-babynames-13, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-13-hint, eval = FALSE}
... %>%
  filter(... == "...")
```
...
To plot the data, use `ggplot()` and map year to x, n to y, name to fill, and name to text. Then add `geom_area()`

```{r more-babynames-14, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-14-hint, eval = FALSE}
... %>%
  ggplot(mapping = aes(x = , y = , fill = ,text =)) +
    geom_area()
```
...
Now to finish it off. Add the layer `scale_fill_viridis()` and inside that set discrete equal to true. Then add the `theme()` and set legend.position = "none". After that use `labs()` to give your graph a title. To adjust the feel, use `theme_ipsum()`. Save everything to `clean_data()` then use `ggplotly()` to make it interactive.

```{r more-babynames-15, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r more-babynames-15-hint, eval = FALSE}
... +
scale_fill_viridis(...) +
    theme(... = "...") +
    labs(title = "...") +
...
ggplotly(clean_data, tooltip = "none")
```
