...
<!-- BL:I am adding the copy previous code for only the ones that need the previous code copied, that way it signals people that this is some new code chunk  -->

<!-- BL:You will also find that I am changing the wording so that it is more specific in what you have to do because I remember doing this tutorial and it being super confusing sometimes -->
...
<!-- BL: Just changed the wording here -->
### 
...
<!-- BL: Changed the wording here as well -->
### Exercise 3

We can see that there are some NA values in `nhanes`. Ideally, we want to get rid of NA values. 

Steps:

1. Start a pipe with `nhanes`
2. `select()` the three columns
3. Then use the function `drop_na()` to remove all NA values with the columns as the argument.
...
<!-- BL: Changed the wording here and added the copy previous code  -->
### 
...
<!-- BL: Added the copy preevious code button, changed the hint to make it a little bit more helpful if they're stuck -->
### 
...
<!-- BL: Added the copy previous code button, changed the hint a little bit -->
### 
...
```{r national-health-and--6-hint-2, eval = FALSE}
... + 
  facet_wrap(~...) 
```

<!-- BL: Added the copy previous code button, added the ~ in the facet_wrap hint thing to remind them once more -->
### 
...
<!-- BL: Added the copy previous code -->
### Exercise 8
...
```{r national-health-and--8-hint-2, eval = FALSE}
... +
  theme_classic() 
```

<!-- BL: Just changed the wording in this one and added the copy previous code button -->
### Exercise 9
...
<!-- BL: Changed the wording, added the copy previous code button, changed the hint so that it's easier for them to follow -->
...
<!-- BL: Changed the subtitle wording to make it comprehensible -->
### 
...
<!-- BL: Added the copy previous code button and added the ~ in the hint to remind them again -->
### 
...
<!-- BL: Added the copy previous code button -->
### Exercise 13
...
<!-- BL: Added copy previous code button, changed the hint for labs to make it easier -->
### Exercise 14
...
```{r national-health-and--16-hint-2, eval = FALSE}
  ... +
  scale_y_discrete(labels = c("No Days", "Several Days", "Most Days")) +
  ... +
```
...
knitr::include_app("https://www.youtube.com/embed/SBCw4_XgouA?start=5" )
...
Let's convert the voting records for the election before the 2006 primary into a binary system (using only 0 and 1 as answers). 

Here's what we're going to do.

1. We will use `pivot_longer()` to transform the six columns into one column
2. Make the change into binary
3. Then use `pivot_wider()` to transform the data back into the original format

*However*,  since there is no column that uniquely identifies each row, `pivot_wider()` cannot match data with the data that belonged to its original row. We've run into a problem.



### 

Let's solve this problem. We must create a variable that uniquely identifies each row. 

Steps:

1. Begin a pipe the `shaming` data set. 
2. Then use `mutate` to create the column `id`. Inside `mutate`, set `id` equal to the expression `1:length(primary_06)`(an expression for an integer series starting from 1 and ending with the number of rows in the column `primary_06`)
...
<!-- BL: I'm making the ones that a really long in words and instructions easier to comprehend -->

### Exercise 3

Now we can start making this a binary system. 

Steps:

1. Copy the previous code
2. We first use `pivot_longer()` to combine the contents of the columns from `primary_00` to `general_04` into one column. 
3. Then, we set the column names to go to a new column called `"election"` and the values to go to a column called, `"voted"`. Remember that you can use the `:` to make consecutive column selection easier. 
...
<!-- BL: Copy previous code button added, made it easier fro students to follow -->

### Exercise 4

Now it's time to change this into a binary system. 

Steps:

1. Copy the previous code
2. Use `mutate()` to change the `voted` column into a column of the same name, but which has `1` and `0` instead of `"Yes"` and `"No"`. 
3. Inside the `mutate()`, set `voted` equal to an `if_else()` function whose condition is `voted ==  "Yes"`. Set the results, for if true and false, to be `1` and `0`, respectively. 
...
1. Copy the previous code
2. Use `pivot_wider()` to transform the contents of `voted` back into six columns
3. Set the names of these new columns to come from `election` and their values to come from `voted`
...
<!-- BL: Copy previous code button added -->


### Exercise 7 

Copy all the previous code. Then use `mutate()` to change the `vote_sum` column into a column of the same name, whose contents are characters rather than numbers. Use the `as.character()` function. 
...
Copy the previous code. Now use a new `mutate()` to change the `primary_06` column into a column of the same name, whose contents are characters rather than numbers. Use the `as.character()` function. 
...
Copy the previous code. Now, `select()` the columns relevant for our plot: `age`, `vote_sum`, and `primary_06`.
...
Now it's time to graph it. Copy your previous code and continue your pipe with `ggplot()`. Map `age` to the x-axis, and `primary_06` to `fill`. Add the layer `geom_histogram()`.
...
Ok now what we're going to do is add the layer `scale_fill_manual` to change the legend options from `0` and `1` to `Did not vote` and `vote`. 

Steps:
1. Copy the previous code
2. Add `scale_fill_manual` and set the first argument `values`, which determines the fill colors, equal to `c("#F8766D", "#00BFC4")`, the default red and blue in ggplot. 3. Set the second argument `labels` equal to `c("Did not vote", "Voted")`
...
<!-- BL: Copy code button, made it easier for students to follow -->
...
<!-- BL: All I did was add the copy code button -->
...
Yay! We finished our first graph. I know I know, you've been through a lot. But we're halfway there. Oh OH! Living on a prayer! If you don't get that reference why are you even here. Come on, I've got a surprise for you if you finish this last graph.

<!-- BL: Sometimes you've got to keep them interested ya know -->
...
<!-- BL: Added copy past button -->

### Exercise 18

Ok now we're gonna add the layer `scale_fill_manual` to change the legend options from `0` and `1` to `Did not vote` and `vote`.

Steps:

1. Copy the previous code
2. Add the layer `scale_fill_manual` and set the first argument `values`, which determines the fill colors,  equal to `c("#F8766D", "#00BFC4")`, the default red and blue in ggplot. 
3. Set the second argument `labels` equal to `c("Did not vote", "Voted")`. 
...
<!-- BL: Made it easier to follow, added copy paste button -->
...
Okayyy so maybe I forgot and accidentally lied. I swear I actually forgot there was one more graph. It's ok, you just did one more graph, now do one more. I believe in you.
...
<!-- BL: Added copy paste button -->

### Exercise 22

Ok now we're gonna add the layer `scale_color_manual` to change the legend options from `0` and `1` to `Did not vote` and `Voted`.

Steps:

1. Copy the previous code
2. Use `scale_color_manual` and set the first argument `values`, which determines the fill colors, equal to `c("#F8766D", "#00BFC4")`, the default red and blue in ggplot. 3. Set the second argument `labels` equal to `c("Did not vote", "Voted")`
...
<!-- BL: Added copy paste button, made it easier to follow -->
...
We're on the last step. We can do this. Now adjust the feel of your graph by adding `theme_classic() `. Finally, add labels to your graph. Use `color` rather than `fill` as we have done above. Pay particular attention to how the y-axis label and subtitle should change compared to the previous plot. 
...
What did I tell you. You did it. Ok now for the surprise. Click the link since I can't embed it to claim it.

https://youtu.be/iik25wqIuFo

<!-- BL: I RICK ROLLED THEM!! -->
...
Based on the quartiles for enrollment, let's create a column that groups the classes into four different groups based on enrollment size. 

Steps:

1. Start a new pipe from `qscores`
2. Use `mutate()` to create a new column called `enrollment_groups`. 
3. Set `enrollment_groups` equal to the function `case_when()`, which can take multiple arguments of the syntax `enrollment >= 16 & enrollment < 30 ~ "16-30"`.The part left of the `~` is the condition, and the part right of the `~` is the result when the condition is met. Using this example as the first, create four groups, named "16-30", "30-50", "50-100", and "100+". 
...
Let's start making `enrollment_groups` a factor. 

Steps:

1. Use `mutate` to change the column `enrollment_groups` into a column of the same name, but with factor variables. 
2. Set `enrollment_groups` equal to `factor()`. 
3. Within `factor()`,  the first argument is the column we want to make a factor, and the second argument is `levels` which should be set equal to the four different enrollment group names in the order in which we want them to appear, `c("16-30", "30-50", "50-100", "100+")`. 
...
<!-- BL: Addec opy paste button, made it easier to follow -->
...
<!-- BL: Added copy paste button, made it a little easier to follow -->

### Exercise 5

Copy paste the code from above. Next `select()` the columns relevant for our plot `rating`, `enrollment_groups`, and `introduction`. 
...
Copy paste the code from above. Continue your pipe with `ggplot()`. Map `rating` to the x-axis, and `introduction` to `fill`. Add the layer `geom_histogram()`.
...
Copy paste the code above. Within `geom_histogram()`, set the argument `position` equal to "identity", set `alpha` to 0.5 set the argument `bins` = 30. 
...
Copy Paste the code above. Now add the layer `facet_wrap()` to facet the histogram by `enrollment_groups`. 
...
Copy Paste the code above. Add the layer `coord_cartesian` and  use the argument to limit the x-axis domain to a minimum of 1 and a maximum of 5. 
...
Copy Paste the code above. Now let's make the graph prettier by adding `theme_classic() `. Finally, add labels to your graph. Pay particular attention to how the y-axis label and subtitle should change compared to the previous plot. 
...
Copy Paste the code above. Add the layer `coord_cartesian` and  use the argument to limit the x-axis domain to a minimum of 2.7 and a maximum of 5, so that we can better focus on the peaks of the distributions. 
...
Guys we finished. We've moved onto the next exercise. Wow I'm so impressed by y'all. You guys grow up too fast :(
...
knitr::include_app("https://www.youtube.com/embed/mx6MzHYzEfY")
...
Based on the quartiles above, let's create a column that groups the classes into four different groups based on enrollment size.

Steps:

1. Start a new pipe from `kenya`. 
2. Use `mutate()` to create a new column called `poverty_groups`. 
3. Set `poverty_groups` equal to the function `case_when()`, which can take multiple arguments of the syntax `poverty >= .18 & poverty < .35 ~ "1st Quartile"`.The part left of the `~` is the condition, and the part right of the `~` is the result when the condition is met.
4. Using this example as the first, create four groups, named "1", "2", "3", and "4". 
...
2. Use `mutate` to change the column `poverty_groups` into a column of the same name, but with factor variables. 
3. Set `poverty_groups` equal to `factor()`. 
4. Within `factor()`, the first argument should be the column we want to make a factor.
5. The second argument is `levels` which should be set equal to the four different enrollment group names in the order in which we want them to appear, `c("1", "2", "3", "4")`.
6. The third argument is `labels` which should be set equal to the names we want the respective levels to have on the graph, `c("1st Quartile (Least Poverty)", "2nd Quartile", "3 Quartile", "4th Quartile (Most Poverty)")`
...
Copy Paste the code above. Based on the 50th percentile for `mean_age`, use `mutate()` and `case_when()` to create a new column called `age_groups`, which has a value of `"1"` when the value of `mean_age` is  below the 50th percentile and a `"2"` when the value of `mean_age` is above the 50th percentile. 
...
Copy Paste the code above. `select` the columns relevant for our plot `reg_byrv13`, `poverty_groups`, and `age_groups`. 
...
Copy Paste the code above. Within `geom_histogram()`, set the argument `position` equal to `"stack"` and set the argument `bins` equal to 500. 
...
Copy Paste the code above. Now let's make it into the graph we saw in the beginning. Add the layer `facet_wrap()` to facet the histogram by `poverty_groups`. 
...
Copy Paste the code above. Add the layer `coord_cartesian` and use the argument `xlim` to limit the x-axis domain to a minimum of 0 and a maximum of .15. Use the argument `ylim` to limit the y-axis range to a minimum of 0 and a maximum of 160. 
...
1. Add the layer `scale_fill_manual` to change the legend options from `1` and `2` to `"Below 50th Percentile"` and `"Above 50th Percentile"`. 
2. The first argument is `values` which determines the fill colors, set this equal to `c("#F8766D", "#00BFC4")`, the default red and blue in ggplot. 
3. Set the second argument `labels` equal to a vector of our new names. (Below 50th Percentile and Above 50th Percentile)
...
Copy paste the code above. Adjust the feel of the graph by adding the theme `theme_classic()`. To finish your plot, use `labs()` to give the graph a title, subtitle, axis labels, legend heading, and caption.
...
Feels good to complete a graph eh
...
Copy paste the code above. Now add the layer `facet_wrap()` to facet the histogram by `poverty_groups`. Add the layer `coord_cartesian` and use the argument `xlim` to limit the x-axis domain to a minimum of 0 and a maximum of .25. 
...
1. Add the layer `scale_x_continuous` and set the argument `labels` equal to `scales::percent_format()`. 
2. Then, add the layer `scale_fill_manual`.
3. Set `values` equal to `c("#F8766D", "#00BFC4")`, the default red and blue in ggplot and set `labels` equal to a vector of the new names, `"Below 50th Percentile"` and `"Above 50th Percentile"`.
...
Copy paste the code above. Continue your pipe with `ggplot()`. Map `reg_byrv13` to the x-axis, `age_groups` to the y-axis and `poverty_groups` to `fill`. Add the layer `stat_slab()`.  Within `stat_slab()`, set `position` to "dodge".
...
Copy paste the code above. Add the layer `coord_cartesian` and use the argument `xlim` to limit the x-axis domain to a minimum of 0 and a maximum of .25. 
...
Copy paste the code above. Now add the layer `scale_x_continuous` and set the argument `labels` equal to `scales::percent_format()`. 
...
1. Add the layer `scale_fill_manual` 
2. Set the argument `values` equal to `c("#F8766D", "#7CAE00", "#00BFC4", "#C77CFF")` which are the hex codes for the default red, green, blue, and purple, respectively in the legend. 
3. Set `limits` equal to the current legend options (which are identical to those in our data frame) in the order you want, `c("4", "3", "2", "1")`. 
4. Finally, set `labels` equal to a vector of the names which you want to appear in the legend, corresponding to the **new** order specified in limits. 
...
1. Add the layer `scale_y_discrete()`. 
2. Set the first argument to `limits` and set it equal to `c("2", "1")`. See how we reversed the order. 
3. Then set the second argument to `labels` and set it equal to `c("Above 50th Percentile", "Below 50th Percentile")`. 
...
One more to go. That's it. There's a surprise for you at the end of the next one so keep working little one.


## Michigan Voter Experiment Column Graph

### 

We will show you techniques helpful for making plots with `geom_col()` including the use of `group_by()` and `mutate()`. 

### 

In the Visulization C Tutorial, we use `geom_bar()` to make this graph. 
...
```{r geomcol-michigan-vot-1, exercise = TRUE}

```

```{r geomcol-michigan-vot-1-hint-1, eval = FALSE}
skim(...)
```
...
```{r geomcol-michigan-vot-2, exercise = TRUE}

```

```{r geomcol-michigan-vot-2-hint-1, eval = FALSE}
... %>%
  select(..., ..., ...)
```
...
```{r geomcol-michigan-vot-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-3-hint-1, eval = FALSE}
... %>%
  group_by(treatment)
```
...
```{r geomcol-michigan-vot-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-4-hint-1, eval = FALSE}
... %>%
  mutate(treatment_total = n())
```
...
Copy paste the code above. Then use `ungroup()` to remove the grouping. 

```{r geomcol-michigan-vot-5, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-5-hint-1, eval = FALSE}
... %>%
  ungroup()
```
...
Let's create a column which records the percentage of the treatment group a single individual represents. 

Steps:

1. Copy paste the previous code
2. Use `mutate()` to create a new column named `individual_perc` which is equal to `1 / treatment_total`.

```{r geomcol-michigan-vot-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-6-hint-1, eval = FALSE}
... %>%
  mutate(individual_perc = 1/treatment_total)
```

<!-- BL: Made it easier to read, added copy paste button -->

### Exercise 7

Copy paste your code. Continue your pipe with `ggplot()`. Map `race` to the x-axis, `individual_perc` to the y-axis, and `line` to `fill`. Add the layer `geom_col()`.

```{r geomcol-michigan-vot-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-7-hint-1, eval = FALSE}
... %>% 
...
Copy paste the code above. Now add the layer `facet_wrap()` to facet the histogram by `treatment`. 

```{r geomcol-michigan-vot-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-8-hint-1, eval = FALSE}
Within facet_wrap(), remember to put a ~ before the variable you want to facet by.
```

```{r geomcol-michigan-vot-8-hint-2, eval = FALSE}
... + 
  facet_wrap(~...) 
```
...
Copy paste the previous code. 

Steps: 

1. Add the layer `scale_y_continuous`. 
2. Set the argument `limits` equal to the vector `c(0, 1)` and set the argument `labels` equal to `scales::percent_format()`.


```{r geomcol-michigan-vot-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-9-hint-1, eval = FALSE}
... +
...
Copy paste the previous code. Adjust the feel of the graph by adding the theme `theme_classic() `.

```{r geomcol-michigan-vot-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-10-hint-1, eval = FALSE}
... +
  theme_classic() 
```
...
Copy paste the code above. To finish your plot, use `labs()` to give the graph a title, subtitle, axis labels, legend heading, and caption. To change the legend heading from the name of the tibble column, use the argument `fill`. 

```{r geomcol-michigan-vot-11, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r geomcol-michigan-vot-11-hint-1, eval = FALSE}
... +
  labs(... ,
     fill = "...",
     ...)
...
We've done it boys and girls. All this work, effort, and time and we've accomplished the impossible. I'm proud of y'all. It feels good to finish everything doesn't it. Now go ahead and download that rds file and submit your work to Preceptor


###

Sike here's a video of Vine clips to enjoy. You're welcome :)
...
<!-- BL: Added copy paste button, and added more for them to enjoy -->
