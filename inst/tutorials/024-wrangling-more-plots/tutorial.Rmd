...
Our first graph was fine, but what if we could see the actual percentage and which year each point was specifically from.
...
We have provided you with the previous code. Replace `geom_line()` with `geom_point()`. Then add `geom_smooth()` and set `method` = `lm`
