...
We have provided you with the beginning of the previous graph. Save everything to `why`. Then a few lines down, insert the `why` and add the layer `transition_reveal()` that it can display data by `year`.
...
###
`transition_reveal` have three arguments: `along`, `range` and `keep_last`. The `year` variable you have just imputed is the `along` variable. This variable specifies the dimension R needs to draw, and it's should be set the same as the x aesthetic. We did not include the `range` or `keep_last` argument so it's set to the default in which `range = NULL` and `keep_last = TRUE`.
