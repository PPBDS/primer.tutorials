...
### Exercise 8
 ...
 Now use `mutate()` to change the `primary_06` column into a column of the same name, whose contents are characters rather than numbers. Use the `as.character()` function. 
...
We have provided you with the beginning of code for the last plot. Continue your pipe with `ggplot()`.  Map `age` to the x-axis, `vote_sum` to the y-axis, and `primary_06` to `color`. Add the layer `stat_pointinterval()` and specify `position` to be `"dodge"`.
...
Adjust the feel of your graph by adding `theme_classic() `. Finally, add labels to your graph. Use `color` rather than `fill` as we have done above. Pay particular attention to how the y-axis label and subtitle should change compared to the previous plot. 
