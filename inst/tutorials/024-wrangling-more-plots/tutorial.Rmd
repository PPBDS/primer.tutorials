...
Adjust the feel of your graph by adding `theme_classic() `. Finally, add labels to your graph. Pay particular attention to how the y-axis label and subtitle should change compared to the previous plot. 
...
Adjust the feel of your graph by adding `theme_classic() `. Finally, add labels to your graph. You need to rename both `fill` and `color` with the same name. Pay particular attention to how the y-axis label and subtitle should change compared to the previous plot. 
...
## `geom_col()`: Michigan Voter Experiment
...
Now `select()` the `race`, `line`, and `treatment` columns. 
...
Let's create a column whose value is the total number of members in the treatment group of the unit, i.e. the person described by the row. First, group the data by the variable `treatment`.
...
Now use `mutate()` to make a new column called `treatment_total()` which is equal to the number of units in each group. 
...
Let's create a column which records the percentage of the treatment group a single indivual represents. Use `mutate()` to create a new column named `individual_perc` which is equal to `1 / treatment_total`.
...
Add the layer `facet_wrap()` to facet the histogram by `treatment`. 
