...
# DK: Add questions for loading up openintro and ranger. Done
...
nd <- datagrid(term = pctiles, newdata = loans)
...
plot_predictions(fit_loans, condition = "term", newdata = loans1)
```

```{r}
loans1 <- loans |> drop_na()
plot_predictions(fit_loans, by = "term", newdata = loans1)
```
...
<!-- DK: Add title and subtitle. Add caption which gives source. Make x-axis go to zero, highlighting the lack of loans less than 5%.done --> 
