...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"Loans"` -- and an author (you). Render the document and save it as `loans.qmd`.
...
Remove everything below the YAML header from your QMD and render the file. `Cmd/Ctrl + Shift + K` first saves the file and then renders it.
...
`Cmd/Ctrl + Shift + K` follows, perhaps with a show_file("analysis.qmd", start = -5)
-->
...
Add `library(tidymodels)`, and `library(marginaleffects)` to the `setup` code chunk in your QMD. `Cmd/Ctrl + Shift + K`.
