...
```{r}
pctiles <- quantile(loans$paid_interest, probs = seq(0, 1, length.out = 101), na.rm = TRUE)
nd <- datagrid(paid_interest = pctiles, newdata = loans)
plot_predictions(fit_loans, by = "paid_interest", newdata = nd)
```
...
plot_predictions(fit_loans, condition = "total_credit_limit", newdata = loans1)
```
...
```{r}
loans1 <- loans |> drop_na()
plot_predictions(fit_loans, by = "paid_principal", newdata = loans1)
```
...
>>>>>>> 206a4cb1834b7bf8f192a14a0c22b297de905ee4
```
...
<!-- SU: to clarify, do i need to talk about how "theoretically" we can manipulate whether a person has debt or not? -->
```{r the-question-5}
question_text(NULL,
	message = "Imagine a variable called `debt` which has a value of `1` if the person has debt, and `0` have no debt. We, meaning the organization in charge of making such phone calls, can choose which individuals to call, based on whether they have debt or not.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Specify two different groups of people which have specific value for `term` and which might have different average values for the `interest_rate`.  
