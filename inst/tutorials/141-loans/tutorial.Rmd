...
fit_loans_model <- rand_forest(mode = "regression", engine = "ranger", mtry = 4, trees = 5000, min_n = 4) 
  fit_loans <- fit_loans_model |> fit(interest_rate ~ balance + homeownership + verified_income + total_credit_utilized + num_historical_failed_to_pay + application_type + term + installment + loan_status + loan_purpose + loan_amount + total_collection_amount_ever + total_credit_limit + debt_to_income + annual_income, data = loans)
...
```{r}
loans <- loans[!is.na(loans$debt_to_income), ]
nd1 <- datagrid(debt_to_income = loans$debt_to_income, newdata = loans)
  
plot_predictions(fit_loans, by = "debt_to_income", newdata = nd) + geom_point(data = loans, aes(x = debt_to_income, y = interest_rate), alpha = 0.04) + geom_smooth(data = loans, aes(x = debt_to_income, y = interest_rate), se = FALSE, color = "orange") +
    labs(x = "Debt to Income)", y = "Predicted interest rate",
         title = "Predicted interest rate for different values of debt to income") +
    theme_bw()
...
plot_predictions(fit_loans, condition = "debt_to_income", newdata = loans1)
```

```{r}
loans1 <- loans |> drop_na()
plot_predictions(fit_loans, by = "debt_to_income", newdata = loans1)
```
