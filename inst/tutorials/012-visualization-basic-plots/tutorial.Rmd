...
No more "Good work." Drop some knowledge. || KX: DONE
Mention some functions. (Example: geom_col().)  || KX: DONE
Mention some arguments (like binwidth for geom_histogram). || KX: DONE
Mention issues associated with your code, like with values in your arguments. Example: color = "steelblue". Where did that color name come from? Where can I read more about colors in R? Where can I look up all the color names I can play with?  || KX: DONE-->

<!-- Links: -->
<!-- https://ggplot2.tidyverse.org || KX: DONE -->
<!-- https://ggplot2-book.org/|| KX: DONE -->
...
This tutorial will walk you through some of the fundamental types of plots found in the `ggplot2` package. Additional examples can be found at the package's (documentation website)[https://ggplot2.tidyverse.org]. The online ggplot2 book is also a handy resource for more detailed explanations.

## Basic Tidyverse

### 

<!-- KX: removed unnecessary tuts -->
...
If you do not specify a method, R will give you a warning that it has made some assumptions in the function. `"lm"` stands for Linear Model, thus giving us a linear trend curve.
...
This dataset used satellite-measured data to calculate global monthly temperature changes. The values show monthly temperature deviations in °C relative to the respective 1991-2021 average temperature.

### Exercise 1

Let's first explore the `globe_temps` dataset. Use the `str()` function.
...
The ISO 1806 standard is the convention for date-time formatting. Dates are represented in the format YYYY-MM-DD, as seen in the `date` observations in the `globe_temps` dataset.
...
ggplot(data = `globe_temps`, aes(x = ..., y = ...))
```
...
ggplot(data = `globe_temps`, aes(x = date, y = globe)) + 
  ...
```
...
By default, the method is set to `"loess"`, which is short for LOcal regrESSion, producing a non-linear trend curve. 
...
Good work. `binwidth` was a parameter we didn't use here, but could be useful in future plots. It overrides the `bins` argument and can be specified as a numeric value. For example, we could have achieved the same plot by including `binwidth = 1`.
<!-- KX: Redone -->
...
Nice! You created your first graph using `geom_bar()`. `geom_col()` is a similar function where instead, the dataset specifies the height of each bar.
...
<!-- Summarize the entire tutorial in three sentences. || KX: DONE -->
