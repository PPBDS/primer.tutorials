...
`tidyverse` is a collection of R packages. Learn more about them [here](https://www.tidyverse.org/).
...
Let's create the following line graph via the `temperature` dataset. We will use a modified version of the dataset, `temp_modified`. <!-- KX: since year and month variables need to be put together -->
...
Notice that the monthly temperature deviations relative to the average temperature is being recorded, rather than the average monthly temperature. `date_modified` It's also good to note that values are being measured in °C.
...
`temp_modified` combines the `year` and `month` variables from the original dataset into a single variable named `date`. This will make life easier for us when plotting the data. You will learn the skills to do this yourself in later tutorials.
