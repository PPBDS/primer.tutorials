...
<!-- Fix tutorial questions to follow current guidance. No more "Good work." Drop some knowledge. Mention some functions. (Example: geom_col().) Mention some arguments (like binwidth for geom_histogram). Mention issues associated with your code, like with values in your arguments. Example: color = "steelblue". Where did that color name come from? Where can I read more about colors in R? Where can I look up all the color names I can play with?  -->
...
This tutorial will walk you through some of the fundamental types of plots found in the `ggplot2` package.

## Basic Tidyverse

### 

<!-- KX: remove unnecessary tuts -->
...
<!-- KX: using mpg data set -->

### Exercise 1

First, let's `str()` the data set used above called `mpg`. What are the names of the two variables we will use?
...
`str()` is a method that displays the internal **str**ucture of an R object.
...
Congratulations on your first plot the Primer! 
...
Run `str(trains)` to further explore the data.
...
Alongside the variable names, `str()` also displays the data type and the first few values of each variable.

<!-- Notice something! || KX: DONE-->
...
<!-- DK: Add two sentences which connect to your opening two sentence, which draw the lesson to a close, which inpsire and bring tears to the eyes of your students. -->

## Summary

### 

Awesome, you've just finished your first tutorial on visualizing data! 

<!-- Summarize the entire tutorial in three sentences. -->
