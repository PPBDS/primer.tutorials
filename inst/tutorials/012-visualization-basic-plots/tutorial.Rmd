...
Run `glimpse(trains)` to further explore the data.
...
This process should feel familiar to past exercises. Nothing new, yet!

### Exercise 5

The plot so far looks neat, but a trendline would be helpful in identifying trends across many years. Add the layer `geom_smooth()`.
...
Nice! The blue line represents our regression line, and the shaded area around it is the standard error, essentially the margin of error.
...
If you do not specify a method, R will give you a warning that it has made some assumptions in the function. `"loess"` is short for LOcal regrESSion, giving us the curved line.
...
Also set `se` to FALSE to get rid of the grey area surrounds the trendline.
...
<!-- KX: "statistical measurements" - is this accurate? -->
...
The `data` parameter requires the object to be of type data.frame. If not, as with `savings`, the dataset will be converted to one by `fortify()`.
