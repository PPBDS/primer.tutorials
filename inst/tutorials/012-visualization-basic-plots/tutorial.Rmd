...
<!-- Replace iris with qscores or some other built in tibble. | ; using trees  -->

<!-- Tutorial should take about 90 minutes. -->

<!-- Does view() work within tutorials? |DONE; No, it doesn't work. I'm thinking to tell students to run view() in the console? -->
...
Let's create the following scatterplot from the `trees` dataset, which provides measurements of attributes from 31 black cherry trees.

```{r scat-p}
scat_p <- ggplot(data = trees, 
                 mapping = aes(x = Girth, 
                               y = Height)) +
  geom_point() +
  labs(title = "Measurements for Diameter and Height of Black Cherry Trees",
       subtitle = "Wider trees are generally taller",
        x = "Diameter (in)",
        y = "Height (ft)", 
       caption = "Ryan, Joiner, Ryan (1976)")

scat_p
```
<!-- KX: using trees dataset -->
### 
...
First, let's `view()` the data set used above called `trees`. What is the variable 'Girth' measuring?
...
The measurements for Girth are actually measuring the tree's diameter! We will correct this error in our plot.

### Exercise 2

Within `ggplot()`, set `data` to `trees`. 
...
Now, map `Girth` to the x-axis and `Height` to the y-axis. 
...
Now, use `labs()` to add a title and axis labels to the graph. Set the `title` to `"Measurements for Diameter and Height of Black Cherry Trees"` and the `subtitle` to `"Wider Black Cherry trees are generally taller"`. To label the the axes, set `x` to  `"Diameter (in)"` and `y` to `"Height (ft)"`. Set the `caption` to `"Ryan, Joiner, Ryan (1976)"`.  
...
```{r geompoint-6-hint-2, eval = FALSE}
... +
  labs(title = "...",
       subtitle = "...",
       x = "...",
