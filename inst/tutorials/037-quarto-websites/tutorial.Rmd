...
<!-- Always start with the main menu, in this case the file command. But drop the knowledge about other buttons in the spots after each exercise. -->

<!-- Switch "knit" to "render" throughout. Replace Distill with Quarto throughout. -->

<!-- Rationalize .qmd versus qmd. I suggest the latter. -->

<!-- File names should always be in backticks, right? -->

<!-- Maybe use dashes for all file names? We need to make a choice, across all the tutorials, and we need to be consistent. -->
...
Open index.qmd , which should be in the same directory as the .Rproj for
...
In the Terminal, run `tail index.qmd `. Copy and paste the output below.
...
Render the .qmd . In the R Markdown tab, you will see a report of the
...
In the Terminal, run `tail -n 20 index.qmd `. Copy and paste the output
below.
...
Open `about.qmd `, which should be in the same directory as your
`distill-tutorial.Rproj` file. (This is not the same file as the
`index.qmd ` which we just edited in the last section.) In the white
...
text. When you are finished, renderthe qmd.
...
**Your Name**, and I...". When you are finished renderthe qmd.
...
In the Terminal, run `cat -n about.qmd`. Copy and paste the result
below.
...
another asterisk. Render the qmd.
...
'"This is data science"'. Render the rmd.
...
`create_article("sources.qmd")`. This opens an R Markdown called
sources.qmd.

### 

You cannot make a new .qmd from the File Tab. This will cause your
...
this. If this happens, delete the problematic .qmd file and use
...
In the Terminal, run the command `ls sources.qmd`. Copy and paste the
result.
...
than index.qmd and about.qmd. Delete the author and date fields from
sources.qmd so that the file more closely resembles the other two pages
for our website. Render sources.qmd.

### 

In the Terminal, run the command `grep author sources.qmd`. Copy and
...
any broad challenges I faced while finding sources." Render sources.qmd.

### 

In the Terminal, run `tail sources.qmd`. Copy and paste the result.
...
When you save `_site.yml` the site is automatically rendered and opened.
...
This should return both source_A.qmd and source_B.qmd
...
three subsequent fields after author. Render the two .qmds.
...
In the Terminal, run the command `head source_A.qmd source_B.qmd`. Copy
