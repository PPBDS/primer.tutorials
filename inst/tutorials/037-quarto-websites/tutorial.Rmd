...
Open `index.qmd`. Delete everything from "This is a Quarto Website", and add a code chunk with the green c and a plus: 

```{r}
knitr::include_graphics("images/code-chunk.png")
```

Make sure you are on the `main` or `master` branch, otherwise you will not see a qmd.

###

The set of three lines at the very top is called a **YAML Header**. These are very sensitive about spaces, but luckily we are just changing the title and nothing else.
...
Render the qmd with the render button at the very top. It should show the plot
