...
In your browser, log in to your Github account. Navigate to your Repositories section. Create a new repository named `quarto-tutorial` by pressing the green button. Select the option to add a README.md file. 
...
Back in the RStudio session from which you are running this tutorial, click File -> New Project -> Version Control -> Git and copy and paste your url into the "Repository URL". Click "Create Project" and reopen this tutorial from the new "quarto-tutorial" project. 

### 

In the Terminal, run `pwd` to confirm that you are in the correct directory, i.e. a directory named `quarto_tutorial` in the place you normally keep R projects. CP/CR. (copy and paste command and result, CP/CR is what we'll be using.)
...
To make a Quarto Website, type `quarto create-project Title-Of-My-Final-Project --type website` in the terminal.
...
Within `Title-Of-My-Final-Project` you should see files that you may not have seen yet:

```{r}
include_graphics("images/files_for-site")
```

 
Knit the document. Now you should see another file called `_site`
