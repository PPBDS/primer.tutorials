...
Now, you should have a directory called `first-website` where you store your projects.

### Exercise 2

In the console, run `list.files(all.files = TRUE)` to view a list of all
the files in the directory. You should see hidden files like `.gitignore`, along with some other files/directories.
...
All of these have different uses, but the folder means you
...
In the page for the github repo, go to settings, and in the menu on the left choose pages. Once you are there it should bring you to a page that looks like this:
...
Where it says "None", choose main. It should show a new set of options, `/ root` or `/docs`. Select `/ root` if it's not already selected. **DO NOT CLICK SAVE!!!**.
...
Copy and paste the URL of your website below.
...
Let's learn how to switch branches, as you will need to know how to do that before we can add anything to the website.

### 

Go to your Git tab (if you followed instructions for the previous exercises, you should have a Git tab). Across from where you press commit, you should see a drop down menu that says `gh-pages `or `master`/`main`, sort of like the picture below.
...
If it says `gh-pages` you will need to switch branches.
### 

You do this by clicking on the branch name (`master`/`main`, or `gh-pages`) and selecting the branch you want.

### 

Select `master`/`main`. When you switch to `gh-pages`, all of the files will turn into html so github pages can read them and publish them easier. Do not freak out if you look at your repo on github and it says that it is JavaScript or HTML, because it has been converted when you switch branches.
