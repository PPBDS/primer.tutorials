...
In the Console, load the package `readr` and then run `read_lines(file = "index.qmd")``. CP/CR
...
Right now, the URL of your website should look like this: http://localhost:some-number/. Localhost means that the website that is currently live does not have all of the updates that you are adding and this version of the website is **only on your computer** until you publish to `gh-pages`. Do not freak out because the URL is not like this: https://miriamheiss.github.io/first-website/. When you publish, everything will be updated, not just on your computer.
