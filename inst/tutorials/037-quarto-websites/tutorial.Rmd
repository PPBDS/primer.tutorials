...
<!-- **DK: ADD COMMENTS (or else...)!** -->

## Interactive Pages

<!-- MH: The easiest way to make interactive pages is by using a little bit of JavaScript, otherwise you have to have a separate server using shiny with a bunch of errors that could pop up at any time, I don't know if I should do Java Script for this, because it is an R class, but the interactive pages look really cool, and the quarto guide says that this is an easier way to make them (without having a separate server for shiny) -->
...
<!-- MH: This is an R package so you can write in JavaScript, see comment above -->
...
Make a new Quarto document called `plane-passengers`. Save it, name it `plane-passengers`, and change the title in the Yaml Header to "Plane Passengers". The tutorial will be terminated when you create a new page, after you're done creating the page, the tutorial will keep your place.
...
Render the qmd. It should now show your graph!

### Exercise 4

Make another Quarto Document called `map-london`, save it and call it `map-london`, and change the title in the Yaml header to be "Map of London".
