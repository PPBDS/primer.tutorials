...
We will use `height` as our outcome variable which is measured in centimeters. (1 in = 2.5 cm).
...
Run `check_predictions(extract_fit_engine(fit_height))`.
...
$$\widehat{\text{height}} = 161.3 + 14.2 \cdot \text{sex}_{\text{Male}}$$
...
Create a new code chunk in your QMD. Add a code chunk option: `#| cache: true`. Copy/paste the code from above for estimating the model into the code chunk, assigning the result to `fit_height`. 
...
This shows the model’s estimates along with 95% confidence intervals. The `(Intercept)` term represents the average height for the baseline group—in this case, females. The `conf.low` and `conf.high` columns give the lower and upper bounds of the confidence interval for each estimate.

The `sexMale` row shows how much taller males are than females, on average. The estimate column shows the average difference, and the confidence interval gives a range of plausible values for that difference.
...
This filters the output to just the confidence interval for the intercept—the average height for females. The numbers are rounded for easier reading.
...
This filters to show the confidence interval for the `sexMale` coefficient. It tells us the model’s range of uncertainty when estimating how much taller men are than women.
...
tidy(fit_height, conf.int = TRUE) |>
  select(term, estimate, conf.low, conf.high) |>
  kable(
    caption = "Estimates and 95% Confidence Intervals for Predictors of Height\nSource: NHANES 2009–2011"
  )
```
...
At minimum, your table should include a clear title and a caption with the data source. Focus on key values like estimates and confidence intervals, and aim for simple, readable formatting. Use AI to streamline this—practice makes better.
...
Write a sentence interpreting the 174 prediction for a male in `predictions(fit_height)`

```{r temperance-4-question}
question_text(
  NULL,
  message = "The model estimates that a typical male in the data is about 174 cm tall.",
  answer(NULL, correct = TRUE),
  allow_retry = FALSE,
  incorrect = NULL,
  rows = 6
...
Write a sentence interpreting the 10.8 estimate for sexMale in `avg_comparisons(fit_height)`.

```{r temperance-6-question}
question_text(
  NULL,
  message = "On average, men are estimated to be 10.8 cm taller than women, based on the model.",
  answer(NULL, correct = TRUE),
  allow_retry = FALSE,
  incorrect = NULL,
  rows = 6
...
Write a sentence interpreting the 8.6 to 13.1 confidence interval for `sexMale` in `avg_comparisons(fit_height)`.

```{r temperance-6-question-2}
question_text(
  NULL,
  message = "The model estimates that a typical male in the data is about 174 cm tall.",
  answer(NULL, correct = TRUE),
...
Write a sentence interpreting the 1.07 estimate in the ratio output from `avg_comparisons(fit_height, comparison = "ratio")`.

```{r temperance-7-question}
question_text(
  NULL,
  message = "The model predicts that men are about 7% taller than women on average, using the ratio of their predicted heights.",
  answer(NULL, correct = TRUE),
  allow_retry = FALSE,
  incorrect = NULL,
  rows = 6
...
plot_predictions(fit_height, condition = "sex") |>
  labs(
    title = "Average Height in Men and Women",
    subtitle = "On average, men are taller than women by around 14 cm (6 in).",
    x = NULL,
    y = "Height (cm)",
    caption = "Data from a sample of 100 individuals from NHANES"
  )
...
Again, make the plot looks more professional by adding a title, subtitle, caption, and labels. Add `theme(axis.text.x = element_blank())` following `labs()` to remove the default "male" label. This is what your graph should look like:
...
Create a new code chunk. Copy/paste the code which creates your graphic.
