...
```{r}
key_number <- round(tidy(fit_height, conf.int = TRUE)[1,2][[1]])
```

Write a sentence interpreting the `r key_number` estimate for the Intercept.

```{r}
tidy(fit_height, conf.int = TRUE) |> 
  select("term", "estimate", "conf.low", "conf.high")

my_answer <- paste("This is the number", key_number, "and other stuff.")
```
...
```{r}
key_number <- tidy(fit_height, conf.int = TRUE) |> 
                filter(term == "sexMale") |>
                pull(estimate) |> 
                round()

answer <- paste("When comparing the average height of men versus women, our guess/estimate is that men are taller than women by", key_number, "centimeters.")
```

Write a sentence interpreting the estimate of about `r key_number` for the `sexMale`.
...
Write a sentence interpreting the 1.47 standard error for the estimate of the Intercept.

```{r courage-14}
question_text(NULL,
	message = "The end points of our confidence interval are simply the estimate plus/minus two times the standard error (1.47).",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r courage-15}
question_text(NULL,
	message = "We do not know the true value for the average height of all adult females in the population, but we can by 95% confident that it lies somewhere between 156 and 162.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r courage-16}
question_text(NULL,
	message = "We do not know the true value for the coefficient for sexMale, but we can be 95% confident that it lies somewhere between 14.1 and 22.0.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
question_text(NULL,
	message = "Knowing about the average height of adult men is important since it is a reasonable benchmark for us to assess our own growth.
Using the nhanes data set from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to understand height, which is of each individual in the data set and measured in centimeters, in order to estimate the average height of all men in the world right now. Since the participation for the survey is voluntary, it could mean that, for example, taller men are more likely to answer this question, thus reduces representativeness of the data. We model height as a continuous variable that follows normal distribution.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- GK: Revise question to make it consistent -->
...
question_text(NULL,
	message = "Knowing about the average height of adult men is important since it is a reasonable benchmark for us to assess our own growth.
Using the nhanes data set from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to understand height, which is of each individual in the data set and measured in centimeters, in order to estimate the average height of all men in the world right now. Since the participation for the survey is voluntary, it could mean that, for example, taller men are more likely to answer this question, thus reduces representativeness of the data. We model height as a continuous variable that follows normal distribution. Our best guess for the average height of all men on Earth in 2024 is 178.8698 cm, with standard error of 0.12 cm.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
