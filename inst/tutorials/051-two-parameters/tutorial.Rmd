...
plot <- plot_predictions(fit_height, newdata = expand_grid(sex = unique(x$sex)), condition = "sex")
```
...
Can we use data from `nhanes` to predict the average height of all men and women in the world now?
...
We do not need to load the **skimr** package in `analysis.qmd`. This package is used for exploring our data which can be useful in the tutorial, but we will not be using this package in `analysis.qmd` so there is no need to load it there. 
...
nhanes |> 
  ...
  labs(title = "Distribution of Height by Sex",
         subtitle = "On average, Males are taller than Females",
         x = "Height (cm)",
         y = "Number")
...
Pipe `nhanes` to `filter()`. Setting `age` greater than or equal to `18`. Then, continue the pipe to `select()`, putting `height` and `sex` as arguments. Finally, drop the NA's using `drop_na()`.
...
Pipe the previous code to `ggplot()`. Set `x` equal `height`, `fill` equal `sex` in the `aes()` argument. Add `geom_histogram()` with `bins = 50` and then finally add `labs()` layer with the `title`, `x` and `y` axis title, and a `caption`.
...
```{r justice-9}
question_text(NULL,
	message = "Knowing about the average height of men and women is essential for ordering the appropriate uniform for the bootcamp recruits in the Marine Corps. Using the nhanes data set from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to predict the average height of men and women. Since the participation for the survey is voluntary, it could mean that, for example, taller people are more likely to answer this question, thus reduces representativeness of the data.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- DK: This is good stuff, but could be improved. -->
...
answer <- paste("When comparing the average height of men versus women, our guess/estimate is that men are taller than women by", result, "centimeters.")
```

Write a sentence interpreting the estimate of about `r result` for the `sexMale`.
...
```{r courage-17}
question_text(NULL,
	message = "Knowing about the average height of men and women is essential for ordering the appropriate uniform for the bootcamp recruits in the Marine Corps. Using the nhanes data set from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to predict the average height of men and women. Since the survey is voluntary, it could be that taller people are more likely to answer this question. We model height as a linear function of sex.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r temperance-4}
question_text(NULL,
	message = "On the right-hand side, there are the Intercept and the variable sex. In most statistical models, the intercept is assumed to be there, so we do not need to include it. So, there is only one column for sex in our ndata object.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Here is the R code which creates the `newdata` object: `expand_grid(sex = unique(x$sex))`. Type it into the code exercise block and hit "Run Code."
...
Add `library(marginaleffects)` to the `analysis.qmd` setup code chunk. Create a new code chunk in `analysis.qmd` which creates the `ndata` object. `Command/Ctrtl + Shift + K`.
...
<!-- GK: What should the standard error be -->

```{r temperance-11}
question_text(NULL,
	message = "Knowing about the average height of men and women is essential for ordering the appropriate uniform for the bootcamp recruits in the Marine Corps. Using the nhanes data set from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to predict the average height of men and women. Since the participation for the survey is voluntary, it could mean that, for example, taller people are more likely to answer this question, thus reduces representativeness of the data. We model height as a function of sex. Our best guess for the average height of all men on Earth in 2024 is 175 cm.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
