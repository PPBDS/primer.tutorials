...
<!-- DK: Follow these instructions from the template. XX: Consider showing your table, and the code which you used to create it. For you, make it a very simple table, simple as possible, probably only using kable, from knitr. But you need a title, subtitle and caption, or the equivalent. AP: added to courage -->
...
<!-- AP: a little confused here, do I add a knowledge drop like below or a whole other set of questions to explain and answer this scenario?-->

This model shows associations, not causation. To answer a causal question—like the effect of taking a vitamin on height—we’d need a different model and more assumptions (e.g., randomization or controlling for confounders).
...
<!-- DK: easystats question. AP: Added above.-->

<!-- DK: Latex code here. AP: Added above. -->

### Exercise 12

<!-- DK: Make like template. AP: Done.  -->
...
<!-- DK: Interpretation questions belong in Temperance. -->

<!-- AP: If this is the code meant to be in Courage, how do I add the interpretation questions relating to it in Temperance? Do I delete the questions here and have the student run the same code again in Temperance to answer these? Or do I create new interpretation questions in Temperance as the code there is different? -->
...
question_text(
  NULL,
  message = "Our (best) guess/estimate for the average height of adult females is 161 cm (63in).",
  answer(NULL, correct = TRUE),
  allow_retry = FALSE,
  incorrect = NULL,
  rows = 6
...
<!-- Knowledge drop? AP: added-->

The intercept tells us the average height for the baseline group—here, adult **females**—since "sexMale" is the dummy variable.
...
The estimate for `sexMale` tells us how much taller, on average, **males** are compared to **females**, the reference group.
...
question_text(
  NULL,
  message = "There is a 95% chance that the true value of the `(Intercept)` is within the range 159 cm and 163 cm.",
  answer(NULL, correct = TRUE),
  allow_retry = FALSE,
  incorrect = NULL,
  rows = 6
...
Write a sentence interpreting the confidence interval for the estimate of `sexMale`.
...
<!-- DK: Knowledged drop. Here and elsewhere! AP: revised and fixed -->
...
Again, make the plot looks more professional by adding a title, subtitle, caption, and labels. Add `theme(axis.text.x = element_blank())` following `labs()` to remove the "male" text. This is what your graph should look like:
