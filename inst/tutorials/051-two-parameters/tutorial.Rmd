...
The reason for making models is not, primarily, that making models is fun — although it is! The reason is that the world confronts us. We are forced to choose — between options X or Y, between actions A, B, and C. To make informed decisions, we need a model of the world.

Imagine you're in charge of ordering uniforms for next year's Marine Corps bootcamp recruits. There are many factors to consider: the cost of different designs, the number of male and female recruits, the distributions of heights and weights, and so on. What should you order?
...
We will use a dataset from the U.S. National Health and Nutrition Examination Survey (NHANES), which includes information on body shape and related measurements from the 2009–2010 and 2011–2012 cycles.See this [link](https://www.cdc.gov/nchs/nhanes/index.htm?CDC_AA_refVal=https%3A%2F%2Fwww.cdc.gov%2Fnchs%2Fnhanes.htm) for further details about NHANES.
...
To ensure demographic diversity, NHANES oversampled some minority groups.
This caused an imbalance in the sample, so some observations from more common groups were resampled to adjust the data.
...
Let's imagine a brand new variable that **does not exist** in the data. It should meet two criteria:
1. It must be binary, meaning that it only takes on one of two values. 
2. It should also, at least in theory, by manipulable. In other words, if the value of the variable is "X," or whatever, then it generates one potential outcome and if it is "Y," or whatever, it generates another potential outcome.
...
There is no single correct answer here. What matters is that the variable you imagine could, in theory, be changed.
...
The same dataset can be used to build many different models—both causal and predictive—by simply changing the outcome variable, the treatment variable, or both. This is a *conceptual* framework we apply to the data. It is never inherent in the data itself.

### Exercise 7

Here’s a clearer version of that sentence without losing any detail:

In a few sentences, choose two different values for the imaginary treatment variable `vitamin` for one unit. Then, guess what the potential outcomes would be for each value, and use those guesses to determine the causal effect for that unit.
...
In predictive models, avoid using words like "cause," "influence," or "impact" that suggest causation. Instead, it’s best to describe results as "differences" between groups with different values of the covariate of interest.

You should only use causal language --- such as "affect," "influence," "be associated with," "cause,", "causal effect," et cetera --- in your question if you are creating a causal model. This means your model includes a treatment variable that could, at least in theory, be manipulated and has at least two potential outcomes.
...
Our Quantity of Interest might seem too narrow for such a complex topic—but that’s okay! There are many numbers we care about, but we don’t need to list them all here. We just need to pick one to help guide us when building the Preceptor Table and the model.
...
The central problem for Wisdon is: Can we use data from `nhanes` to predict the average height of all men and women in the world now? When was the data collected? What does each row in the data represent? Is it for height of each male and female in the world now?
...
<!-- DK: Can we get rid of the "Male" label on the x-axis?  -->
