...
In the Console, run `fit_height` and hit "Run Code."
...
plot_predictions(fit_height,
                 condition = "sex") + 
 labs(title = "Average Height in Men and Women",
...
Let's use `plot_comparisons()` to get a good estimate of the height difference between men and women, and the confidence interval thereof.
Enter this code into the exercise code block and hit "Run Code."
...
plot_comparisons(fit_height,
                ...
                type = "numeric",
                condition = "sex") + 
  labs(title = "Estimate of height difference between men and women",
       subtitle = "We are 95% confident that the real value of differences in average height \n lies between 10.7 and 16.8 cm",
       caption = "Data from a sample of 100 individuals from NHANES",
       x = NULL, 
       y = "Average height difference (cm)")
```
...
<!-- GK: This result in a plain graph, have not fixed yet -->
<!-- GK: Can add more question in the Temperance about comparison on the change on the predicted values when there is change in 1 unit of the independent variable  -->
<!-- set.seed(10) -->
<!-- x <- nhanes |> -->
<!--   filter(age >= 18) |> -->
...
<!-- fit_height <- linear_reg() |>  -->
<!--   set_engine("lm") |>  -->
<!--   fit(height ~ sex + age, data = x) -->

<!-- plot_comparisons(fit_height, -->
<!--   variables = list("age" = "sd"), -->
<!--   condition = c("height", "sex"),  -->
<!--   type = "numeric") -->
