...
ndata <- expand_grid(sex = unique(x$sex))

plot <- plot_predictions(fit_height, 
                         newdata = expand_grid(sex = unique(x$sex)),
                         condition = "sex")
```
...
<!-- DK: Get rid of coded answer creation in interpretation questions. Just hard code the number once! -->

<!-- DK: Get rid of ndata. Plot is: -->

<!-- plot_predictions(fit_height$fit, condition = "sex") -->
...
```{r}
result <- tidy(fit_height, conf.int = TRUE) |> 
                  filter(term == "(Intercept)") |> 
                  pull(estimate) |>
                  round()
answer <- paste("Our (best) guess/estimate for the average height of adult females is", result)
```

Write a sentence interpreting the `r result` estimate for the Intercept.
...
```{r}
ci_intercept <- tidy(fit_height, conf.int = TRUE) |> 
                filter(term == "(Intercept)") |>
                select(conf.low, conf.high) |> 
                round() |> 
                with(sprintf("[%d, %d]", conf.low, conf.high)) 

answer <- paste("There is a 95% chance that the true value of the Intercept is within the range", ci_intercept, "centimeters.")
```
...
```{r}
ci_sex <- tidy(fit_height, conf.int = TRUE) |> 
                filter(term == "sexMale") |>
                select(conf.low, conf.high) |> 
                round() |> 
                with(sprintf("[%d, %d]", conf.low, conf.high)) 

answer <- paste("There is a 95% chance that the true value of the coefficient of `sexMale` is within the range", ci_sex, "centimeters.")
```
...
```{r temperance-4}
question_text(NULL,
	message = "The ndata object should include all the independent variables in the model, i.e., all the variables after the `~` in the model formula. In this case, those variables are Intercept and sex. In most statistical models, the intercept is assumed to be there, so we do not need to include it. So, there is only one column for sex in our ndata object.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r temperance-5}
question_text(NULL,
	message = "The `sex` variable will be either Male or Female.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Here is the R code which creates the `ndata` object: `expand_grid(sex = unique(x$sex))`. Type it into the code exercise block and hit "Run Code."
...
Note that the `ndata` object has not existed in the qmd world yet, that's why we need to create it there. 
...
```{r temperance-11}
question_text(NULL,
	message = "Men and women vary in height. Using data from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to estimate the average height of men and women. Since the participation for the survey is voluntary, it could mean that, for example, taller people are more likely to answer this question, thus reduces representativeness of the data. We model height as a linear function of sex. Our best guest for the average height of women is 161 cm, we are 95% confident that its true value is between 158 and 164 while men, on average, are expected to be taller in women by 14.4 cm, and we are also 95% confident that the real value of differences in average height lies between 10.2 and 18.6.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Using data from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention, we seek to estimate the average height of men and women. Since the participation for the survey is voluntary, it could mean that, for example, taller people are more likely to answer this question, thus reduces representativeness of the data. We model height as a linear function of sex. Our best guest for the average height of women is 161 cm, we are 95% confident that its true value is between [158, 164] while men, on average, are expected to be taller in women by 14.4 cm, and we are also 95% confident that the real value of differences in average height lies between [10.2, 18.6]
