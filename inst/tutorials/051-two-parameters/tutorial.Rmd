...
Because height is a continuous variable, we assume it comes from a normal distribution:

$$ height \sim Normal(\mu, \sigma^2)$$

### 

The link function—the basic mathematical structure of the model—is largely determined by the type of outcome variable.

For continuous outcomes like height, we use a linear model as the link function:

$$\mu = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots$$ 

### Exercise 5

Add `library(tidymodels)`to the `setup` code chunk in your QMD. `Command/Ctrl + Shift + K`.
...
The code extracts the equation from the `fit_height` model, labels the `Intercept` as $\beta_0$ and substitutes the estimated coefficients from the model to the equation.
...
The **`marginaleffects`** package makes regression results easier to interpret by showing how changes in variables affect model predictions. In this tutorial, we’ll use `predictions()`, `plot_predictions()`, `comparisons()`, and `avg_comparisons()` to explore and summarize model output.
