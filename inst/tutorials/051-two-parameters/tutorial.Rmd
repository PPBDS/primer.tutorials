...
<!-- DK: Rework to be more consistent with the approach in https://ppbds.github.io/primer/cardinal-virtues.html#question. -->
...
To answer our general question, we would need to create a model of height. In order to make progress, we need drill down to a more specific question. Specifics help you to fix ideas as you start to work on a project. Just because you start looking for this number does not mean that we can’t consider other questions. Our specific question is:
...
Before we continue questioning our data, let's practice creating and publish a quarto document to answer the question we have in a professional way. Professional data scientists always store their work somewhere safe. This is so you don't lose your code even if something happens to your computer.
...
Now we have a Quarto document and we can start adding thing to it. Before that, we will have to make some changes to the `.gitignore`.
...
We have loaded all the libraries in the tutorial, but we will need to do the same thing in the `analysis.qmd`.

### Exercise 15

In `analysis.qmd`, add a new code chunk, load the **tidyverse** and **primer.data** packages. Render the file.
...
The `pattern = "tidyverse|primer.data"` will only return lines that have `tidyverse` or `primer.data` in `analysis.qmd`.
...
Now there is a graph in the `analysis.qmd` file. 
...
In `analysis.qmd`, add the summary from above at the end of the file. Make sure to change some things accordingly. Do not just copy the answer example. Render the file. 
...
Update the `analysis.qmd` file. In the code chunk with the other libraries, load the **tidybayes** and **brms** packages. Save the file and in the Console, run: 
...
The `brm()` function from the **brms** package is used to create models. 
...
Add a new code chunk to `analysis.qmd`.  Copy the code from above to that code chunk, but assign the result to a new object, `fit_male_height`. Add `#| cache: TRUE` at the beginning of the code chunk. Render the file, this will take a while because the object has to be created. Render it again and you will notice that it took significantly less time, this is because `#| cache: TRUE` saves the object. 
...
Now we have an object, `fit_male_height`, in `analysis.qmd`. We need to make that object in the tutorial as well. 

### Exercise 7

Copy the previous code and then set it to an object called `fit_male_height`. Type `fit_male_height` and hit "Run Code." This generates the same results as using `print(fit_male_height)`.
...
We will now make a table from `fit_male_height`. 
...
Copy the code from above and put it in the `analysis.qmd` file. Render the file and in the Console, run: 
...
We now have a table which shows some important numbers from our data. 
...
Add the code from above to the `analysis.qmd` in a new code chunk. Render the file and in the Console, run: 
...
Now your qmd file is complete. Look over the whole file to make any final edits. Render `analysis.qmd` and in the console, run:
...
Make sure to also go to the Git tab and commit the changes and push the commit. This makes sure that your project is saved if anything happens to your computer.
