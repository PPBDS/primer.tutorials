...
```{r justice-4}
question_text(NULL,
	message = "A potential reason why the assumption of stability might not be true in this case is that there might have been an outside variable that changed the height of male adults in America from 2009 to 2011. One possible reason may be the increase in immigrants which might impact the results. Regardless, these impats are probably not impactful enough for us to not assume stability.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- DK: make simpler and more direct. -->

<!-- MK: Is this better? -->

```{r justice-6}
question_text(NULL,
	message = "A reason why the assumption of representativeness might not be true in this case is that participation is voluntary. This means that there might be some groups of people who do not participate. Although this will impact out data, it is not major enough for us to not assume representativeness.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r justice-8}
question_text(NULL,
	message = "Our goal is to create a model of height for adult men which we will use to answer some related questions. We used the data to created a Population Table that can be used to accurately answer our questions. We made sure that the data is representing our population and our data is stable throughout time. This data can be used to make accurate interpertations of the population. ",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Run `tbl_regression()` on `fit_male_height`.
...
tbl_regression(fit_male_height)
```
...
```{r courage-14}
question_text(NULL,
	message = "So far, we have used a simple linear model to model the average male height. We now know that that the average adult male height is 175.87 cm with the standard deviation of the distribution being 7.48 cm. We first specify the mathematical formula which connects the outcome variable we are interested in with the other data that we have.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Create a model of height for adult men to answer three questions:
...
We will focus on the first question for now.

Pipe `fit_male_height` to `add_predicted_draws()`. Inside, have `newdata` set to `tibble()` with parameter `.rows = 1`.
...
Continue the pipe to `mutate()`. Inside, have `tall` set to `if_else()` with parameter `.prediction > 180, TRUE, FALSE`.
...
Pipe `fit_male_height` to `add_predicted_draws()`. Inside, have `newdata` set to `tibble()` with parameter `.rows = 4`.
...
Continue the pipe to `pivot_wider()`. Inside, have `values_from` set to `.prediction`, and have `names_from` set to `.row`.
...
End the pipe with `rowwise()`. connect `mutate()` after `rowwise()` by using `%>%`. Inside `mutate`, set `diff` to `max() - min()`. Inside both `max()` and `min()`, have the parameter ``c_across(`1`:`4`)``. set the whole thing to an object called draws. In a new line, type `draws`.
...
Unfortunately, this code can take a while to run, so we save the results in a permanent tibble named draws. This will be a common approach, just as we have saved the fitted model object as fit_male_height, or whatever, rather than recreating it each time.

### Exercise 9

Pipe `draws` to `ggplot()` with `x` set to `diff` in `aes()`. Then, add a `geom_histogram()` layer with `y` equal to `after_stat(count/sum(count))` and `bins` equal to `100`.
...
The final step is to use our graph to find the answer. 

Type `sum() / length()`. within `sum()`, have `draws$diff` be greater than `10`, and in `length()`, have `draws$diff` as the argument. 
...
The answer tells us that there is about an 78% chance that, when meeting 4 random men, the tallest will be at least 10 cm taller than the shortest.

### Exercise 11

Now we will answer the final question:

What is our posterior probability distribution of the height of the 3rd tallest man out of the next 100 we meet?

Copy your code from Exercise 7. Then change `tibble()` to make `.rows` now equal to `100` instead of `4`. 
...
Continue the pipe to `rowwise()`, use `%>%` to add `mutate()` with the argument `third_tallest` set to ``sort(c_across(`1`:`100`))[98]``. Set this equal to `draws.` Type `draws` in a new line. 
...
Pipe draws to `ggplot()` and have `x` equal to `third_tallest` and `y` equal to `after_stat(count / sum(count))`. Then add `geom_histogram()` using `bins` set to `100`. 
...
```{r temperance-14}
question_text(NULL,
	message = "We wanted to create a model of height for adult men so that we can use it to answer some related questions. We used The nhanes data set from the National Health and Nutrition Examination Survey conducted from 2009 to 2011 by the Centers for Disease Control and Prevention includes 15 variables, including physical attributes like weight and height. Then we created a Population Table that can be used to accurately answer our questions. Then we have used a simple linear model to model the average male height. We learned that that the average adult male height is 175.87 cm with the standard deviation of the distribution being 7.48 cm. Finaly, we used our data to answer our questions.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
