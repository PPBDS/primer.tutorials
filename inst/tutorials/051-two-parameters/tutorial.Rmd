...
```{r the-question-11}
question_text(NULL,
	message = "The question can be: If an individual is an adult Male, how higher would that person be compared to being an adult Female?",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Note that although we are comparing between two groups, we are not claiming any causal effect but rather predicting the differences. Since `sex` is something that we cannot manipulate, our model is just predictive, recall the slogan: [No causation without manipulation](https://ppbds.github.io/primer/rubin-causal-model.html).
...
```{r the-question-12}
question_text(NULL,
	message = "The Quantity of Interest for this question is the difference between the *expected* value of `height` for Male and *expected* value of `height` for Female",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Let's round up the results to 2 decimal places. Run the command `round()` with `fixef(fit_male_height)` as the first argument, and `2` as the second argument. 
...
round(fixef(fit_male_height), 2)
```

### 

The parameter we most care about is $\mu$. That is the parameter with a substantively meaningful interpretation. Not only is the meaning of $\sigma$ difficult to describe, we also don't particular care about its value. 

### Exercise 15

Write a sentence interpreting the 175.87 estimate for Intercept.

```{r courage-15}
question_text(NULL,
	message = "The average height of all males in the *population* is around 175.87 centimeters.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r courage-16}
question_text(NULL,
	message = "The end points of our confidence interval are simply the estimate $\pm$ two times the standard error (0.12).",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
