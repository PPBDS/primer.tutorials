...
```{r mutate-10-hint, exercise = TRUE}
... %>% 
  group_by(democrat, ...) %>% 
  summarize(mean_change = mean(...), 
            max_change = max(...))
...
```{r mutate-11-hint, exercise = TRUE}
... %>% 
  group_by(democrat, ...) %>% 
  summarize(mean_change = mean(...), 
            max_change = max(...))
...
Copy/paste your code. Use `arrange()` to arrange your summary table so that it starts with the row that contains the smallest value for `mean_change` and ends with the row that contains the largest. 

```{r mutate-12, exercise = TRUE}

```

```{r mutate-12-hint, exercise = TRUE}
... %>% 
  arrange(mean_change)
