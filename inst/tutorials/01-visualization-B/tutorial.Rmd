...
Copy/paste your code. Use `slice()` to make a tibble with the first 5 rows. The tibble we get when we run this, however, has 20 rows. This is because the data was grouped, and so `slice()` made a tibble of the rows containing the 5 highest `att_change` for each group. 
...
Copy/paste your code before `slice()` function. To ungroup the table, continue your pipe and use the function `ungroup()`. Now try to slice the first 5 rows. 
