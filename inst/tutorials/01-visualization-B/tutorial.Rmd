...
Let's practice using`tidyverse` functions that help manipulate datasets for use.
...
Great. Let's now build on our code to include data where `state` is "Massachusetts" AND where `ideology` is "Liberal". 
...
Let's now use `select()` to look at specific data columns of the `cces` dataset. Start your code with `cces`. Use the pipe operator `%>% `to add the function `select()`. Within `select()`, use the variables `gender` and `age`.
...
Let's organize the values of `age` in ascending order. Start your code with `cces` and use ` %>% ` to add the function `arrange()`.
...
```{r tv-10-hint-2, eval = FALSE}
Within `slice()` use the argument 1:200
```

### Exercise 11

Start your code with `mpg`. Use `mutate()` to create a new variable called `midsize`. Within `mutate()` set `midsize` to an `if_else()` statement where it it returns TRUE when the class of the car is "midsize" and FALSE otherwise. 
...
Let's now calculate the mean value of `age` in the `nhanes` dataset. Start your code with `nhanes` and use `%>%` to add `summarize()`. Within `summarize()`, set `mean` to the `mean()`. The first argument to `mean()` should be `age`. The second argument should be `na.rm = TRUE` to remove all NA values.
