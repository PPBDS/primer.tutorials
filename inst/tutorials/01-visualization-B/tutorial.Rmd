...
Let's create a new variable in the `mpg` tibble. Start your code with `mpg`. Use `mutate()` to create a new variable called `midsize`. Set  `midsize` to an `if_else()` statement where it returns TRUE when the `class` is "midsize" and FALSE otherwise. 
...
Let's now calculate the mean value of `age` in the `nhanes` dataset. Start your code with `nhanes` and use `%>%` to add `summarize()`. Within `summarize()`, use `mean()` with the argument `age`.
...
Notice an NA value appears as the mean value because NA values exist and interfere with the calculations. 


### Exercise 15

We must remove the NA values with `drop_na()` in order to ingnore them and get our mean values. Copy and paste your code from above. Use `%>%` to add the function `drop_na()`. 
