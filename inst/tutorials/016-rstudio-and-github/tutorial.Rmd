...
<!-- Do current RStudio and Friends tutorial. Make notes (and fix errors) relevant to these sections here. TM: Done-->
...
Up until now, you have been programming awesome plots on your own computer. But what if you want to share these visualizations and the code behind it with others? Well, that's where tools like GitHub and RPubs come in.

This tutorial will walk you through the process of creating accounts for GitHub and RPubs, then you will upload your first visualization onto the internet!
...
The next step is creating a GitHub account by going to the [GitHub mainpage](https://github.com/) and using their instructions. Follow **[this advice](https://happygitwithr.com/github-acct.html#username-advice) when choosing your username.** 
...
First lets make an R Markdown file. In your current project(First_Repo) press File, New File, then R Markdown. Change the title to Car_MPG and keep the other fields as the default values. Save the file and name it `mpg.Rmd`. Run `list.files()` in the Console and copy-paste the output below.
...
You should see `mpg.Rmd` somewhere in the output.

###

### Exercise 2

Knit this RMarkdown file. A new window should open. Run `list.files()` in the Console and copy-paste the output below. 
...
`mpg.html` should be one of the files outputted.
...
This should make your document accessible on the internet. Copy paste the RPubs link in the answer box below.
