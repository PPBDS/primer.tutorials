...
Start by opening up the Terminal pane in RStudio. This should be directly next to the console pane in the bottom left of your screen.
...
<!-- R: Change this to something like pwd instead, this isn't very useful -->
### 
...
If you do see `First_Repo.Rproj`, try clicking on the refresh button in the upper left or make sure you have it spelled correctly.
...
To do so, go to the Git tab in the top right of your screen. Check the checkbox next to the `.gitignore` file and then click on the `Commit` button.
...
Next, press `Push` with the green arrow on the top right. This pushes or uploads the files and changes to GitHub! Open up GitHub and refresh the project page to see the  commit message you just made at the top of your repo page.
...
First let's make a new Quarto document. Press File, New File, then Quarto Document. Change the title to "Example" and keep the other fields as the default values. Save the file ("ctrl + s" for Windows, "command + s" for Mac) and name it `example`.
...
Just like before, in the "Git" tab in the top right corner of your "First_Repo" RStudio session check the boxes next to ".gitignore", "example.qmd", and "example.html" to stage them. Press the commit button and add the commit message "published website".
