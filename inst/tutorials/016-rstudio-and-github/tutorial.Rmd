...
## Introduction
###
Up until now, you have been programming awesome plots on your own computer. But what if you want to share or save them online, in case your computer blows up? Well, that's where tools like GitHub and RPubs come in.

###

This tutorial will walk you through the process of creating a Github account, and uploading a visualization to the internet!
...
First lets make an R Markdown file. In your current project (First_Repo) press File, New File, then R Markdown. Change the title to "Example" and keep the other fields as the default values. Save the file("ctrl s" for Windows, "command s" for Mac) and name it `example.Rmd`.
...
Knit this R Markdown file by pressing the "Knit" button. A new window should open. Knitting is the process of converting your R Markdown file into [HTML](https://en.wikipedia.org/wiki/HTML), the programming language used to make web-pages.
...
Now press the `Publish` button in the top right corner of your knitted window. Choose `RPubs` and follow the prompts to click "Publish". You should already have an account from the "RStudio and Code" tutorial. Name your document, and add a slug. Hit `Continue`.
...
### Exercise 2
In the top right of you separate RStudio Session press the dropdown menu with the current heading "First_Repo". Select the project `project1` from the "RStudio and Code" tutorial. Let's try to publish our `FirstRScript.R` file as a Gist. We can do this by using the `gist_create()` function in the **gistr** package.
...
Good work on finishing this tutorial! You should now be able to make Version Control Projects with Github, which you can publish on the internet.
