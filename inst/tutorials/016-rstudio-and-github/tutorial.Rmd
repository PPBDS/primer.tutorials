...
First lets make a new Quarto document. Remember how we did this in the RStudio and code tutorial. An alternative way to do this is to press File, New File, then Quarto document. Change the title to "Example" and keep the other fields as the default values. Save the file("ctrl + s" for Windows, "command + s" for Mac) and name it `example.Rmd`.
...
Render this Quarto document by pressing the "Render" button. A new window should open in your browser, displaying the document. Rendering is the process of converting your Quarto document into [HTML](https://en.wikipedia.org/wiki/HTML), the programming language used to make web-pages.
