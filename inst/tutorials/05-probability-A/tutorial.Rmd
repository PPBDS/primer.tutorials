...
```{r tm-12, exercise = TRUE}

```

```{r tm-12-hint-1, eval = FALSE}
... +
  geom_histogram(..., ...)
```
...
Now use `scale_x_continuous()` so the x-axis has `breaks` at 0 and 1. Set `seq` to 1 and 2.
...
We begin our code the same way we began our code for the *unnormalized conditional distribution*. We have provided the code for you below. To begin, use `scale_x_continuous()` so the x-axis has `breaks` at 0 and 1. Set `seq` to 1 and 2.
