...
<!-- Add to list column questions something which explains why we need map_* functions for sampling from random variables? -->
...
Great. Now assign your work to an object named `tbl_x`. 
...
Great! Now use let's start a pipe with `tbl_x`. Use `summarize()` to create the column `range_x`, which is set to `range(x)` wrapped in `list()`. 
...
Recall that when you create a pipe, you need to use %>% before adding a function like summarize.
```
...
<!-- DK: These questions are weird and not very informative. What are we teaching? -->

### Exercise 6

Copy/ paste your code from above. Replace `str()` with `pull(range_x)` to pull out the `range_x` from `tbl_x`.
...
Good work. Copy/ paste your code from above. Save your work to an object named `tmp`. We will work with `tmp` in the following questions.
