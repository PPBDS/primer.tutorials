...
wiki_graph <- raw_data %>% rename(
  ...
  mutate(tile_num=ntile(population, 20)) %>%
  group_by(tile_num) %>%
  summarize(sum_rate=sum(death_rate), total_states=n()) %>%
  mutate(avg_rate=sum_rate/total_states) %>%
  mutate(percent_rate=avg_rate/sum(avg_rate)) %>%
  ggplot(aes(x=tile_num, y=percent_rate)) + 
  geom_col() + 
  ...
  theme_classic() + 
  labs(title="Distribution of US Death Rate\nbyQuantile of Population in 2015",
       ...
       y="Percent of Average Death Rate in Quantile",
...
The resulting tibble is your summarized data.
