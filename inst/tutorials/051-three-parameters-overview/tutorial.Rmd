...
pp_2 <- posterior_predict(fit_1, newdata = newobs) |>
    as_tibble() |>
...
newobs_2 <- tibble(treatment = c("Treated", "Control"))

pe <- posterior_epred(fit_2, newobs_2) |> 
    as_tibble() |> 
    mutate(ate = `1` - `2`)
...
newobs_3 <- tibble(age = 40)

pe_2 <- posterior_epred(fit_3, newdata = newobs_3) |> 
  as_tibble() 
...
newobs_4 <- tibble(income = 100000)

pe_3 <- posterior_epred(fit_4, newdata = newobs_4) |> 
  as_tibble()
...
newobs_5 <- tibble(income = c(rep(100000, 2),
                            rep(200000, 2),
                            rep(300000, 2),
                            rep(400000, 2)))

pp_4 <- posterior_predict(fit_4, 
                        newdata = newobs_5) |> 
  as_tibble() |> 
  mutate_all(as.numeric) |> 
  rowwise() |> 
...
In two sentences explain the columns that we would like to have in our Preceptor Table and what they are used for.

```{r wisdom-1}
question_text(NULL,
	message = "We would have two columns for our Preceptor Table: party and age of the train commuters. Our age column would represent our outcome variable and our party column would represent our covariate.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
```{r wisdom-3}
question_text(NULL,
	message = "We would need the columns of age and income within this scenario for the Preceptor Table. Our age column would represent our outcome variable and our income column would represent our covariate.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
```{r wisdom-4}
question_text(NULL,
	message = "We would need a column for income, and a column for whether someone is liberal or not. Our liberal column would represent our outcome variable and our income column would represent our covariate.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Next, let's take a look at the dataset we would like to use to answer these questions. We will be using `trains`. First, let's take a look at **trains** through the glimpse function.
...
Notice that `att_end` is an integer rather than a double and that `party` is a character while `treatment` is a factor. These choices are a mixture of sensible and arbitrary. Regardless, it is your responsibility to notice them. You can’t make a good model without looking closely at the data which you are using.
...
In one sentence explain whether we should use a *predictive* model or a *causal* model for this scenario with the *trains* data.

```{r wisdom-7}
question_text(NULL,
	message = "We need to use a causal model for this scenario because there is a treatment at hand.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
In two sentences explain the columns that we would like to have in our Preceptor Table and what they are used for with the *trains* data.

```{r wisdom-8}
question_text(NULL,
	message = "We will need the columns of age, income, party, liberal, att_end and treatment. With these columns we have several different potential outcomes because age, income, party, liberal, and att_end are all important to validate our data.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
In one sentence explain the units of this situation.

```{r wisdom-9}
question_text(NULL,
	message = "The units in this case would be the weeks per year because our potential outcomes and covariates of age, income, party, liberal, att_end and treatment.",
...
In one sentence explain the treatment and how it applies to the scenario.

```{r wisdom-10}
question_text(NULL,
	message = "The treatment depends on whether or not the individual was exposed to Spanish-speakers.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
In two sentences provide one reason why the *assumption of validity* might not hold and explain how it may not hold within our scenario.

```{r wisdom-11}
question_text(NULL,
	message = "The assumption of validity might not hold because the variables are too different from each other within the data and the Preceptor Table as the columns are not similar enough between each other. One possible reason we may not consider the data to be drawn from the same population as our Preceptor Table due to changes that may have occurred over time because of a belief that our larger population is unable to include both 2012 and 2021 for some reason such as if we believed that the political climate was far too different.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
In order for our *assumption of validity* to hold true we need to have our data to match up with our Preceptor Table which allows us to construct a Population Table. With the *assumption of validity* we are able to build a Population Table that analyzes our data.
...
With this knowledge, we must consider the *representativeness* of the data. In two sentences, give an example of why the rows in the Preceptor Table may not be representative of the overall population.

```{r age--party-3}
question_text(NULL,
	message = "Our rows in the Preceptor Table may not be representative of the overall population because the participation and submission of the form was voluntary. Therefore, not having a random sample of rows within our Preceptor Table with the voluntary and induced by payment method may not be representative of the overall population.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
In three sentences, give an example of why the rows in the data may not be representative of the overall population.
...
```{r age-party-graph-15, echo = FALSE}
age_party_graph
```
...
```{r age--party-15-hint-2, eval = FALSE}
... |> 
  labs(title = ...,
       subtitle = ...,
       x = ...,
...
pp_2 <- posterior_predict(..., newdata = ...) |> 
  as_tibble() |> 
  mutate_all(as.numeric)
```
...
```{r age--party-21-hint-2, eval = FALSE}
newobs <- tibble(party = c("Democrat", "Democrat", "Democrat", 
                           "Republican", "Republican","Republican"))
```
...
```{r age--party-24-hint-2, eval = FALSE}
... |> 
  rowwise() |> 
...
```{r age--party-26, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r age--party-26-hint-1, eval = FALSE}
... +
  labs(...) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme_classic()
...
Let's look at the *validity* of our model. Write three sentences discussing the meaning of receiving the treatment and some problems that we might run into when combining the `att_end` column from the data and our Preceptor Table. 
...
Next, consider the *stability* of our model. Write two sentences considering whether the *assumption of stability* may or may not hold and provide a possible reason for why we may not consider the relationship between `att_end` and `treatment` to hold with the *assumption of stability*.

```{r attend--treatment-2}
question_text(NULL,
	message = "Some reasons that you we can consider could be the controversial elections and events in the US that have occurred between 2012 and now that may have had vast affects on immigration views in the US. Therefore, the assumption of stability may not hold because of the change in elections and events that have caused a shift in the immigration views.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
While you have discussed a reason that the *assumption of stability* for the model may not hold, we will assume that this has not impacted our relationship and use the *assumption of stability* for the model as it will hold.
...
Lastly, write one sentence stating whether we will use a *linear* or *logistic* model and include what outcome variable will affect this model.
...
Set your outcome variable as `att_end`and your explanatory variable as `treatment` (Remember you must insert a tilde `~` between the two). Also, include a `-1` after `treatment` so we do not get an intercept.
...
Set `data` to `trains`, and `refresh` to 0. Set this to the `fit_2` and print it out.

```{r attend--treatment-wi-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r attend--treatment-wi-6-hint, eval = FALSE}
fit_2 <- stan_glm(... ~ ... -1,
                  data = ...
                  refresh = ...)
```
...
Use `pivot_longer()` with the argument of `cols` set to the "treatmentTreated" and "treatmentControl". Set the argument `names_to` to "Parameter", and `values_to` to "attitude".
...
Now, let's use `posterior_epred()` to answer this question. Create a tibble like we have done before with our desired ovservations. For this scenario, call it `newobs_2` and set `treatment` having two values, "Treated" and "Control".
...
```{r attend--treatment-14-hint-2, eval = FALSE}
pe <- ... |> 
  mutate(ate = `1` - `2`)
```
...
To answer this one, we will use `posterior_predict()` for two people, one who is treated and the other control. In other words, we can use the tibble `newobs` which we already created. Set the first argument to our fitted model and `newdata` to `newobs_2`. Make sure to use `as_tibble()` and to make all the values numeric as we have done previously.
...
Continue this pipe and add a column `te` which is the difference between the treated person and the untreated person. Remember the columns are unnamed and that you must use numbers. Store this as `pp_3`.
...
In order to do so, we can use the function `quantile()`. To do so, select the `te` column of `pp_3` and use this as the first argument of `quantile()`. Set the second argument `prob` to our desired percentile, 0.9.

```{r attend--treatment-21, exercise = TRUE}

```

```{r attend--treatment-21-hint-1, eval = FALSE}
quantile(pp_3$te, prob = 0.9)
```
...
```{r income--age-1}
question_text(NULL,
	message = "The assumption of stability may not hold because of the relationship between age and income and how that has changed between 2012 and now. Once again, let’s consider inflation and what would happen if the incomes were to increase at the rate of inflation, the income distribution would be drastically different than that of 2012.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Wages don’t tend to change as quickly as inflation does, so they likely did not change significantly and we can consider this model to be stable.

### Exercise 2

In two sentences, first, give an example of why the rows in the Preceptor Table may not be representative of the overall population. Second, give an example of why the rows in the data may not be representative of the overall population.

```{r income--age-2}
question_text(NULL,
	message = "First within our Preceptor Table our rows may not be representative of the overall population because of possible skewing in the age distribution and overestimate of younger people in the population. Second, our rows in the data may not be representative of the overall population because those who actually complete and submit the survey are not random which could alter our answers to our current questions.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r income--age-3-hint-2, eval = FALSE}
fit_3 <- stan_glm(income ~ age, 
                  data = trains,
                  refresh = 0)
```
...
Now that we have made our fitted model, let's make our posterior distribution. Start by making a tibble `newobs_3` with the observation `age` is equal to 40.
...
Now, recall that `posterior_epred()` is used when looking for expected values such as in this case. Therefore, use `posterior_epred()` with the first argument as our fitted model `fit_3` and the `newdata` argument as `newobs_3`. Remember to use `as_tibble()`. Store this as `pe`.
...
pe_2 <- posterior_epred(..., newdata = ...) |> 
  as_tibble()
```

```{r income--age-5-hint-2, eval = FALSE}
pe_2 <- posterior_epred(fit_3, newdata = newobs_3) |> 
  as_tibble() 
```
...
```{r income--age-6, exercise = TRUE}

```

```{r income--age-6-hint-1, eval = FALSE}
Remember that the columns are unnamed so you will have to call them using numbers.
```

```{r income--age-6-hint-2, eval = FALSE}
pe_2 |> 
  ggplot(aes(x = `1`)) +
  geom_histogram(aes(y = after_stat(count/sum(count))),
                 bins = ...)
...
```{r income--age-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r income--age-7-hint-1, eval = FALSE}
... +
  labs(...) +
...
```{r income--age-8}
question_text(NULL,
	message = "A 40-year old train commuter would earn around $140,000.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
```{r liberal--income-2}
question_text(NULL,
	message = "This is a logistic model because `liberal` is the outcome variable and it does not have a range of values, but rather only two.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Like always, to answer the question, we must first make a tibble with the observations we desire. In this case, make a tibble with `income` set to 100000, and store this as `newobs_4`.
...
newobs_4 <- tibble(income = ...)
```

### Exercise 5

Now, in order to get our posterior distribution, let's use `posterior_epred()` with the first argument as our fitted model, `fit_4`, and the second argument `newdata` as `newobs_4`. Remember to make this a tibble as well. Store this as `pe_3`.
...
pe_3 |> ggplot(aes(x = `1`)) + 
  geom_histogram(aes(... = after_stat(count/sum(count))),
                 bins = ...)
```
...
In order to answer this question, let's first make our tibble with the desired observations. In this case, we would like to have 2 people who make $100,000, $200,000, $300,000, and $400,000 each. So, in our tibble, set income to these values. Store this as `newobs_5`.
...
newobs_5 <- tibble(income = c(rep(..., 2),
                            rep(200000, ...),
                            rep(..., 2),
                            rep(400000, ...)))
```
...
Now, let's make our posterior distribution. Stary by using `posterior_predict()` with the first argument as our fitted model `fit_4` and the second argument `newdata` set to `newobs_5`. Make this a tibble and convert the values to be numeric.
...
Now, like we have done before, we are going to add a total column which is the sum of the entire row since every individual who is liberal in our groupings is represented by a `1` and everyone who isn't is represented by a `0`, so the total will give us the number of liberals in the group. Store this as `pp_4`.
