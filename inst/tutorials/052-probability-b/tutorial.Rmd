...
<!-- How do we we emphasize that the 10,000 sample one matches, almost exactly, the mathematical one? Not sure! -->

<!-- Second example. Show what happens when the mathematical assumptions are false, with rolling two dice. Use the weight argument in sample so that 6's have a 25% chance of happening while all other numbers have a 15% chance.. Maybe we do a similar plot, but with just three panes, 10, 50, 10,000.  Lesson: With little data, we can't be sure that our model is correct. With lots of data, we can be sure that the model is false. ->

<!-- Third example shows what happens when we directly compare data (which we know to be false) to simulations from the model (which we define as the truth.) With 10,000 draws, it is obvious that the data demonstrates that the model is false. (Maybe this is combined with second example. Maybe not.) Key is that we are comparing the model with the data and, on the basis of a discrepancy between the two, determining that we need to fix the model. Introduce the concept of model checking. -->
...
## Rolling two dice (False assumption)

Let's see what would happened if we make bad mathematical assumption. 
```{r}
