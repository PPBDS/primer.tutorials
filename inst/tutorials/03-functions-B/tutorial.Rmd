...
Awesome. Let's now  use `replicate()` to run our function multiple times. The first argument to `replicate()` should be 4. The second argument should be the function `prediction_game()`.
...
<!-- BG: each q small. step by step- replace cat with return, and so on.  -->

### Exercise 11

The code you have created so far is below. Replace both `cat()` functions with the `return()` function. Then, call `prediction_game()`.
...
Great. Copy/paste your code from above. Now place the `return()` functions inside their own `{ }`. 
...
Copy/paste your code from above. Now place each `return()` function inside their own `{ }`. 

```{r pg-13, exercise = TRUE}

```

```{r pg-13-hint-1, eval = FALSE}
...
 if(your_error < my_error) { 
   return("You win!")
 }
...
Let's change the input inside each `return()` function. For each `return()` statement, use the argument `paste()`. The first argument to `paste()` should be "Guess". The second argument should be `guess_1`(or `guess_2` based on the `if()` statement). The third argument should be "wins!"

Then, call `prediction_game()`.
...
So far, we have a function that tells us which guess (500 or 600) is closer to the sampled value guess for the variable `rv13` of  the `kenya` tibble. This function is pretty restricted. Let’s now adjust our function to allow for any guesses to be passed in as an argument (not just 500 and 600). lso, let's allow any tibble and variable to be used! 
