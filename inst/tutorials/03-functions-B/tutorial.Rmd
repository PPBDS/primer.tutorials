...
Awesome work. But we are not done yet! Let's add in some checks for our code.


### Exercise 21

We have provided the code you have completed so far. Below the code for `my_guess`, add the function `stopifnot()`. Within `stopifnot()` use `length()` with the argument `guesses`. Set the `length()` statement equal to 2.
...
Copy/paste your code from above. Add another `stopifnot()` function below the one you just created. Use the argument `all()`. Within `all()` use the argument `is_double(guesses)`.
...
Awesome work! Our function can now use any guess, tibble, or variable as arguments!
