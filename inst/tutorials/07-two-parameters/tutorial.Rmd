...
Using the concept of **Wisdom**, write 3-4 sentences about whether or not this data is relevant for the problem we face. See *The Primer* for guidance.
...
Great! Next, specify which are the two parameters we want to determine. Also mention which one we care most about and why, writing no more than 3 sentences.
...
Use `posterior_predict()` again with `fit_obj` as the first argument. This time, also add the `newdata` argument and set it equal to `newobs`.
...
Good job! We can clearly see that this only gives us 1 column as required. 

Copy your code from above and add another line with `as_tibble()`. Assign the result to an object named `pp`.
...
As a final step, let's continue the pipe even more. Use `summarize()` to make the `answer` equal to the `sum` of `gt_100` divided by the the function `n()`. 
