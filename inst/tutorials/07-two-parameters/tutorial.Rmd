...
<!-- DK: I am starting to think that code only runs if it is in either the setup chunk at the start or in am exercise setup chunk. Code in random code chunks does not run! Or, at least it does not run in such a way that students have access to the objects for later questions. Or maybe code chunks without chunk names work OK? Then again, the objects under Courage around line 490 seem fine. They are created in a named R code chunk and are available for later questions. -->
...
Awesome! We can also use `[ ]` to extract columns. This is especially useful when we want to extract variables not by their name, but by their position. 

Extract variables 1 to 3 from `nhanes` using this method.  
...
```{r brackets-hint-1, eval = FALSE }

nhanes[...:...]

```

```{r brackets-hint-2, eval = FALSE }
...
We can use the function `set_names()` to change the names of the variables in a tibble. For this, we simply place a vector with the desired names in set_names.

Choose any approach to extract the first two variables from the `nhanes` data set. Create a pipe and use `set_names()` to rename the variables to “year” and “sex”.
...
<!-- TW: It seems like the below section had the purpose to familiarize people with NHANES. I tried to use the data set in the preliminaries where I could, so I don't think we need this part anymore. If some of the functions used here might be good to know beforehand, we should rather put it in the preliminaries. -->
...
```{r classic-hint, exercise=TRUE, eval=FALSE}
...
Use `posterior_predict()` again with `fit_obj` as the first argument. This time, however, also add the `newdata` argument and set it equal to the tibble `newobs`.
...
Good job! We can clearly see that this only gives us 1 column as required. Copy your code from above, add another line with `as_tibble()` and save assign the result to an object named `pp`.
