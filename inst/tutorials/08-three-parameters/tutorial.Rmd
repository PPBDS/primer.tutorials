...
This tutorial needs to be updated. Outline:

- EDA of trains

- Create a model

- One question with posterior_epred(). Look at avg income
  for liberals vs non-liberals. Include overlapping plot.
  
- One question with posterior_predict(). Expected height 
  for a liberal and a non-liberal. This requires two rows. 
  Again, also add an overlapping plot.

- Another question with posterior_predict(). Should involve
  two people and some kind of manipulation, e.g. difference
  between values. This will lead to a plot with only a single
  distribution.
...
Let's also change the y-axis values for probability to show percents rather decimals (It is nicer to view probability this way). We do this by using `scale_y_continuous()`. Within `scale_y_continuous()`, set the`labels` to `scales::percent_format()`. Let's change the x-axis values of income to include dollar signs. We do this by using `scale_x_continuous()`, Within `scale_x_continuous()`, set the`labels` to `scales::dollar_format()`. 
...
Let's answer the following question:  _What is the probability that the next liberal who arrives at the train station today has an income greater than $100,000?_ . 
