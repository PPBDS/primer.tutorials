...
<!-- Neighbour and Control should be swtich on table -->
...
There's much more info when using ?shaming instead of glimpse(shaming). Try out the different commands when you get in later exercises!
  
  
### Exercise 4

Type `tibble(ID = "1")` in order to create the start of the pipe which we will build into the Preceptor Table which we showed you above. The number 1 is the way that we will refer to the first person in the table, since we do not know her name.
...
Notice how row 1 has several columns. The only one we care about is the type of treatment they received, and whether it impacted their voting. Reminder that `primary_06` is whether or not they voted.

### Exercise 5

Let's continue the table and add variable `ytreat`. We can see that in row 1, it contains treatment "Civic Duty", and has a `primary_06` value of 0. Since it isn't a control treatment, and has a value of 0, we will add 0 as the `ytreat`.  

<!-- add something that says that primary_06 is whether they voted or not -->
...
Next, let's add age and sex as variables Fill in the rows accordingly.
...
Let's get a bit more specific, let's breakdown ytreated in the four different treated categories: Civic Duty, Hawthorne, Self and Neighbors. Remove the column ytreated, and replace it with the four treatments. Change entries as necessary. 
...
Format these variables to look more professional by changing ycivic into $\sf{Y_{civic}}$ using `cols_label()` and using `<sub>`. 
...
cols_label(columnname = md("inserttext<sub>insertsubscript</sub>")
```
...
Drop knowledge abotu cols_label and style. More information about formatting and stylizing your table can be found here [https://gt.rstudio.com/reference/index.html] 

### Exercise 13

Add rows 3, 48, 51, 68 and 198 into the table! Fill in each column as necessary.  
...
### 
Try adding a title with `tab_header()`... explain more.
...
We will create a tibble with one ATE, which looks like this:
...
probably not here, but write something down about the importance of filtering, and how it's half the job. woo! Also about unnecessary info and sorting out the good stuff, 
...
Start your pipe with `shaming`. Once again, we just want to see the effect of the neighbor postcards against control. `Filter` the data accordingly, using `%in%` as part of the code. 
...
<!-- confidence intervals, itnroduce them somwhere alongside ATE-->
