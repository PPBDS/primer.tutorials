...
## Table of outcomes and covarients
...
Start a gt table by creating a `tibble()` with 1 variable called ID. ID will store our row number. Within ID, add a 1 to symbolize row 1. 
...
Notice how row 1 has several columns. The only one we care about is the type of treatment they received, and whether it impacted their voting. Reminder that primary 06 is whtehr they vote

### Exercise 5

Let's continue the table and add variable ytreat. We can see that in row one, it contains treatment "Civic Duty", and has a primary_06 of 0. Since it isn't a control treatment, and has a value of 0, we will add 0 as the ytreat.  add something that says that primary_06 is whether they voted or not
