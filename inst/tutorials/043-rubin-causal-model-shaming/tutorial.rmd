...
The purpose of this tibble is to estimate the ATE to see how the treatment `Neighbor` compares to `Control`. 
...
Start a pipe with `shaming`. `filter()` for the value of `Neighbor` and `Control`, which are located `%in%` treatment. 
...
ATE = Subtract the value of Control from Neighbors
```
...
Remember that ATE is the average treatment effect and is the difference between the *mean* of potential outcomes (effect of treatment `Neighbors` vs effect of `Control`). In this tibble, we take the mean of `primary_06` to figure out how likely a person is to vote when subjected to treatment `Neighbors`, and treatment `Control`. 
...
Notice how you can calculate multiple ATEs in one data set. Before, we were finding the ATE of Control and Neighbor, but now we're finding the ATE of each treatment pair. 
...
You calculate the ATE by finding the difference between Neighbors and Control, the two different treatment types.  
```
...
Using `mutate`, create a new column called ATE, which has the ATE of treatment Neighbors and Control. 
