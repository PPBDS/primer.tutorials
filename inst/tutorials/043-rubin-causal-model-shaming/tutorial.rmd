...
<!-- Why am I here? What is the purpose of this section? How does it connect to what has gone before or what will come after? -->
...
<!-- DK: Change to `vote.perc` perhaps. -->

Define `avg.percent` as the `mean()` of  `primary_06`. Use `summarize()` to add `avg.percent` to the tibble.
...
`avg.percent` is the percent of people who voted. But we don't want to know the overall percentage. We need the percent of people who voted for each level of `treatment`. We should also make the warning go away.



### Exercise 4

Remove the `summarize()` line from the pipe and use `group_by(treatment)` instead. 
