...
read_rds(
  systemt.file("www/03-data-response3.rds",
...
imf_data <- left_join(imf_raw, iso_codes, by = "iso2c")
...
On this page, we see a new url that allows us to make queries for JSON file responses: "https://bison.usgs.gov/api/search.json". Because this URL to make queries is one you will use often, assign it to an R object called `bison_base`.
...
Most likely, you do not have one yet. In this case the Terminal will return the message "No such file or directory". If you do have one, the Terminal will print the contents of your .Renviron file. 

### Exercise 3 

In your console, run the command `usethis::edit_r_environ`. This is a command that opens the .Renviron file so that we can edit it. If you do not already have a .Renviron file, it will create one for you.  Copy and paste the command into the box below. 
...
In the Terminal run `cat $HOME/.Renviron` again. Now that you have created the file, Terminal should return nothing, since the file contains nothing. Copy and paste the command and its result into the box below. 
...
You must restart R for the new environental variable to be able to be used. 
...
Use `left_join()` to join `iso_codes` to `imf_raw` by the shared column "iso2c". Save this to an object named `imf_data`.   
...
imf_data <- left_join(imf_data, iso_codes, by = "...")
```
...
Start a new pipe with `imf_data`. Rename the column `GGXWDG_GDP` to `dtg`.
...
Copy and paste the code above. Filter the data to keep rows only where year is equal to "2015". Assign this to an object named `imf_data`. 
...
```{r remove-rplots}
file.remove(system.file("tutorials/03-data-API/Rplots.pdf"))
```
