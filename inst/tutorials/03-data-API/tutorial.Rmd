...
`GET()` is the key command in the **httr** package and is used to get information from sites. It takes the URL to the API and optionally a query to define the limits of the data accessed for its arguments. 

###

Use `GET()` with the `url` argument set to the homepage for USGS BISON, "https://bison.usgs.gov/". Assign the resulting object to `my.response`. 
...
The URL which returns the response is the same one that we contacted. Status 200 mean success. The response is a html file of size 38kB  We can see a preview of the html file we recieved below.
...
Let's save a copy of `response1` in our working directory. First run `content()` with the argument `response1` with the second argument `"raw"`. View this output and then assign it to an object called `response1.bin`. 
...
In the Terminal, run `open response.html`. If this does not work, try to open the file in the file pane in RStudio or in your computer's file viewer. Copy and paste the terminal the line of the terminal command for your answer.

```{r httr-8, exercise = TRUE}

```

###

This command opens the file in the default html viewer on your computer, likely your default browser. Scroll past the many links at the top of the page, which are the navigation tabs for the website's UI. Read about USGS BISON.
...
When navigating an API page for the first time we have two goals. First, to figure out what the URL is at which we can make our queries and get data. Two, to figure out what parameters we should use to make a query for this API. Ideally, there is a page where all of this information is conveniently documented. 

###

Assign the "raw" version of the `content()` of the response to an object called `response2.bin`` 
...
my.response.bin <- content(my.response, "raw")
```
...
Use the up arrow in the Terminal to access your last command, `open response.html`, and run it. Copy and paste the line of the command for this answer. Read the first section, BISON Search. Look specifically for a URL which we contact to make data and parameters which we use to specify our request. 

```{r httr-12, exercise = TRUE}

```

### Exercise 13

On this page, we see a new url that allows us to make queries for JSON responses: "https://bison.usgs.gov/api/search.json". Because this URL to make queries is one you will use often, assign it to an R object called `bison_base`.
...
In the BISON Search section there are also several parameter options for requesting json files at the url, including species, type, and state.
...
The application type is also different. It is no longer an HTML file as before, but a JSON file of 15kB. JSON is a file which stores data. 
...
You should see lots of brackets, slahses and colons, the syntax of a JSON file. You should also see state and county names and numbers. This is data, and we will work on how to handle this type of data later in the tutorials. 
...
In the last exercise, you started from the homepage and had to find out how the link at which to access the API. In this exercise, we will give you the link at which to access the API from the start. 

###

If you were to naviagte from the home page looking for the ASOS API, you would eventually be redirected to this link, https://mesonet.agron.iastate.edu/request/download.phtml. While this site has some useful documentation such as the variable list at the bottom, it is designed for interactive use (hence phtml instead of plain html) in a browser and so is missing some important information, such as the access URL and the format for time parameters. One option would be to view the html file as a text file, where you can find the request link and variable names which undergird the UI. 

###

Though this cannot be done using `GET()`, another way to find information on an interactive site is to make a request on your browser and then observe the resulting link. Doing so, you will see both the link and the names for as many parameters as you selected. 
...
You will now have access to this object throughout the rest of this exercise. 
...
Note that the URL is still the base url, which indicates that there was no further query. We can confirm that by looking at the size of the response - 23 Bytes (our previous responses were in **kilo**bytes. `Status: 500` indicates that the query encountered an error. 
...
There are several different types of data to be had from this API, including the temperature in degrees celsius, which come with different `data` query arguments. See the discussion at the beginning of this section for where we would find this information on this site and others like it. 
...
Start a pipe with `denver_response`.  Pipe the `content()` function immediately after the `GET()` function. Then, pipe `read_csv()` to convert the comma separated value in our response into a tibble. 
...
There are so many labels for x-axis breaks that they are indistinguishable. Use `scale_x_discrete()` to set only three x axis breaks, `06-01-2017`, `06-01-2018`, `06-01-2019`.
...
If a function in a help page  has an argument that is not set equal to anything, that means there is no default for that argument and that the user must specify the value.
...
On the example page we saw a function with only one code, so it is safe to assume that it is not necessary to specify codes for every item in  `imf_codelists`.
...
This can akso be done, though with a more difficult to read view, in the 
Tutorials window. 
```
...
Start a new pipe with imf_data_named. Rename the column `GGXWDG_GDP` to `dtg`. When referring to columns that begin with capital letters, numbers, or special characters in code, you must surround them with backticks, e.g. "`GGXWDG_GDP`".
...
Let's learn more about the `rbison` package. In your console, run `help(package = "rbison")`. Click and read about the function `bison`, the function used to download data in this package. Also click and read about the function `bisonmap`.
...
Run `output$counties`, according to structure, this should return a data.frame with the columns `record_id`, `total`, `county_name`, and `state`.
...
The `rbison` package comes with a function that helps make a `ggplot()` map with bison data. Set the argument `input` to `output` and set the argument `geom` to `geom_jitter`. 
...
Run `GET()` with the URL we assigned to the object `bison_base`.
...
GET(url = "https://bison.usgs.gov/api/search.json")
```

### 

This request takes more time to load, you can stop it. If you wait the whole time, you will get the error message: "Empty reply from server". This url does not return a response. 
...
response <- GET(url = "https://bison.usgs.gov/api/search.json",
    query = list(species = "...",
                 type = "...", 
                 count = 500))
```
...
This response is a JSON file and 119kB in size. Notice how the addition of the question mark and our query list to the url. 
...
Run `output_api$states$data$Oregon`. 
...
Start a pipe with `output$states$data`. Add the function `as_tibble()`. 
...
`output$states$data` is a list of containing 45 two-item lists of the type we just saw for Oregon for every state. When we make this into a tibble, each list becomes a `list` column which contains its two character items. 

### Exercise 9

Copy/paste your code. Because the columns are list columns, they do not display well in our tibble. We can convert these to character columns, with the functions `mutate()` and `as.character()`. 

###

To avoid having to do a mutation for each column, we can nest the `across()` function within `mutate()`. The first argument in `across` specifies the columns you want,`everything()`. The second argument specifies the function we want to use, `as.character`, in this case without arguments or parentheses.  
...
Continue your pipe. Use `pivot_wider()` to transform the data so that values for `total` and `fips` are in separate rows. Set `names_from` equal to `names` and `values_from` equal to `values`. View this tibble and save it to an object called clean_data_api.
...
The ggplot function `map_data()` is used to download data to make maps which can be used in `ggplot()`. Run `map_data("states")` which returns a tibble to make a map of the contiguous 48 states. 
...
Use `labs()` to add a title, subtitle, axes labels, and a caption to your plot.  
