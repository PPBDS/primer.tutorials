...
base_response <- GET(url = "https://bison.usgs.gov/doc/api.jsp",
    query = list(species = "Accipiter", type = "scientific_name", count = 500))
```
...
## Bison Data with Package
###

Let's make the following plot with data from the USGS Bison API and the `rbison` package.
...
`bison` has lots of arguments, but has examples in which it specifies very few. 
...
`output` seems to be a complex ob ject
...
`output` seems to be an object containing lists and data frames. 
...
```{r show-bison-plot-dirty}
response <- GET(url = "https://bison.usgs.gov/api/search.json",
    query = list(species = "Accipiter", type = "scientific_name", count = 500))

output_text <- content(response, "text")

new_output <- fromJSON(output_text)

clean_bison <- new_output$states$data %>% 
  as_tibble() %>% 
  unnest(cols = c(Oregon, `North Carolina`, Vermont, Wyoming, Maine, Oklahoma, 
    Tennessee, `British Columbia Canada`, Arkansas, Washington, 
    Utah, `South Carolina`, Nebraska, `Ontario Canada`, Massachusetts, 
    Colorado, Missouri, `Quebec Canada`, `Alberta Canada`, `North Dakota`, 
    Arizona, Wisconsin, Nevada, `New York`, Montana, Kentucky, 
    Hawaii, Minnesota, California, Kansas, Florida, Delaware, 
    `New Jersey`, Pennsylvania, Michigan, `New Mexico`, Texas, 
    Illinois, Connecticut, `New Hampshire`, Ohio, Georgia, Maryland, 
    Virginia, Idaho)) %>% 
...
states <- map_data("state") %>% 
  left_join(clean_bison, by = "region")

states %>% 
  ggplot(aes(long, lat, group = group, fill = total)) +
  geom_polygon() +
  scale_fill_gradient(low = "blue", high = "red", na.value = "grey50") +
  labs(title = "Accipiter Bison in the Contiguous United States", 
       subtitle = "Alaska has 1, Hawaii 0", 
       x = "Latitude", 
       y = "Longitude", 
...
Use `GET()` to make contact with the base url for the USGS Bison API: "https://bison.usgs.gov/doc/api.jsp. View this output and assign it to an object named `base_response`
...
GET(url = "https://bison.usgs.gov/doc/api.jsp")
```

### 

We get back a response, of 69kB and of type html. 

### Exercise 2 

Use the function `class()` for the object `base_response`.
...
Use the function `content()` to view the content of `base_response`.
