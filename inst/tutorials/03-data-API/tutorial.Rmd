...
Finally, to convert this data to a more familiar format, add the the `content()` and `read_csv()` functions immediately following the `GET()` query to convert the jibberish from the query response into a nicely formatted data frame. Save this output as an object named 'denver'. 
