...
Let's make the following plot with data from the USGS Bison API. 
...
Let's learn more about the `rbison` package. In your console, run `help(package = "rbison")`. Click and read about the function `bison`, the function used to download data in this package.
...
We see that `bison` requires two arguments. A `database_id` which can be found with the function `imf_ids` and a string or vector of indicator IDs which can be found in the function `imf_codes`. 
