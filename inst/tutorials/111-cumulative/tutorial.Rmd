...
# fit_approve <- brm(formula = approval_ch ~ ideology,
#                    data = approve_data,
#                    family = cumulative(),
#                    refresh = 0,
#                    silent = 2,
...
There is no right answer! The `cces` tibble has 17 variables. For the rest of this tutorial, we will be focusing on `ideology` as the main explanatory variable, but our model will certainly include other variables. 
...
This problem is not causal so there is no treatment. In order to make causal relationships, the data needs to be manipulated. In `cces`, the data is observed, there was no manipulated variables.
...
<!-- MK: data shows "Strongly Disapprove" but that is not a value mentioned in the approval_ch description in ?cces -->
...
`glimpse()` gives us a look at the raw data contained within the `cces` data set. At the very top of the output, we can see the number of rows and columns, or observations and variables, respectively. We see that there are 617,455 observations, with each row corresponding to a unique respondent.

### Exercise 17

Pipe `cces` to `filter()` with `year` set to `2020` inside of it using `==`.
...
cces |> filter(year == 2020) |> select(approval_ch, education, faminc, ideology)
```
...
Behind the scenes of this tutorial, we have created an object called `approve_data` using the previous code. 

### Exercise 20

To see the object that was created in the background, run `approve_data`.
...
There are now only 54,839 rows which is significantly fewer than the whole `cces` data. Having more manageable data will make it easier for us to create models.

### Exercise 21

In `analysis.qmd`, add a new code chunk. Copy the code from two exercises ago which prepares the data and paste it in the code chunk. Set the code to an object called `approve_data`. 
...
<!-- MK: faminc is ordered weirdly/ not in order. approval_ch has "strongly disapprove" and does not have "not sure"  -->

Pipe `approve_data` to `summary()`.
...
Pipe `approve_data` to `ggplot()` with `x` set to `ideology` and `fill` set to `approval_ch`. Then add a layer of `geom_bar`.
...
```{r justice-6}
question_text(NULL,
	message = "The cces website claims that they designed the study to be representative of all adults. But, they could make mistakes while executing the study would make the sample not representative of the larger population. For example, the way they select people for the survey might be flawed.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
A large portion of the CCES respondents are YouGov panelists. This group might not representative of the whole population because they might be more politically active and care more about politics than other people do.
...
```{r justice-9}
question_text(NULL,
	message = "Using the CCES, which is one of the largest political surveys in the United States, we seek to understand the relationship between presidential approval and political ideology in 2020. One concern is that survey respondents might be systematically different from other Americans.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Create a model using `brm()` from the **brms** package. Your arguments should be `formula = approval_ch ~ ideology + faminc + education`, `data = approve_data`, `family = cumulative()`, `refresh = 0`, `silent = 2`, and `seed = 19`. 
...
Behind the scenes, we have assigned the result of the `brm()` call to an object named `fit_approve`. Type `fit_approve` and hit "Run Code." This generates the same results as using `print(fit_approve)`.
...
<!-- XX: Drop some knowledge about what you have learned by looking at the resulting table. Of course, you could have learned the same thing when you first took a look at fit_approve. But the table makes it easier to see the relationships between the variables and the outcome. With luck, students will take the hint when they answer the next question. -->
