...
<!-- 6. Replace slice(21:25) with lice(seq(1, 25, by = 5)). All adds up to 100%. Because a person in a given category must offer one of the five allowed answers. -->
...
This is the first row and first column of, 125 different rows. What this represents is what the the model factors in for ie. we are looking to determine the probability that someone in the Strongly Disapprove group would approve of Trump.
...
Let's look at the 2nd column in our `plot_predictions()`.
...
This is weird! why is our model, utilizing the 'education' variable when we only specified the condition of ideology. Well, recall our DGM from Courage, since we fit_approval, predicts the log odds of approval, with relation to ideology and education. Plot_predictions utilizes 'high school graduate' as the baseline, since there is no fit_approval that independently predicts the log odds of ideology, or education.
 ...
 condition = c("ideology"), 
    draw = FALSE) |> 
     select(ideology) |> 
        slice(1) 
```
This column represents the specific type of ideology we are predicting. Imagine plot_predictions as a modelling tool that helps of visualize and interpret our logistic regression algorithm `fit_approval()`. As we can see this column/row pair represents conservative, but later as we see all of the rows/columns it will make more sense.
 ...
 condition = c("ideology"), 
    draw = FALSE) |> 
     select(estimate) |> 
        slice(1) 
```
This column represents the estimate, given all of the previous columns, remember "conservative, high school graduate, and strongly disapprove" the probability of approval is 0.1089.

### Exercise 8

Now that we have seen what the model entails, let's view all of the column/row pairs.
...
plot_predictions(fit_approval, 
condition = c("ideology"), 
draw = FALSE) |> select(group, education, ideology, estimate) |> head(25) 
```
...
plot_predictions(fit_approval, 
  condition = c("ideology"), 
    draw = FALSE) |> 
      select(group, education, ideology, estimate) |> head(25) 
```

### 
This is all starting to make a little more sense, so what our model is telling us is essentially all the possible combinations of approval groups, AND ideology, and their associated predicted log odds for approval. Logically, we can see that liberals and very liberals do have a lower rate of approval EVEN if they are in the group.
