...
Before we create the model, we need to make our data even smaller because `brm()` takes a very long time to run when the data is large.

To solve this issue, create a new object called `brm_data` and set it to `approve_data |> slice_sample(prop = 0.2)`. 

On a new line, call on the `brm_data` object. 
...
brm_data <- approve_data |> slice_sample(...)
brm_data
```

```{r courage-5-test, include = FALSE}
brm_data <- approve_data |> slice_sample(prop = 0.2)
brm_data
```

### 

The `brm_data` object only has 20% of the data from `approve_data`. This will make the `brm()` call run much faster.
...
Behind the scenes, we have used `brm()` to create an object named `fit_approve`. This version was created using `approve_data` so it has more rows than the one you created in the previous exercise.
...
Using 20% of the data instead of the whole data should not make a huge difference in the model because we chose a random sample. 
...
Again, `fit_approve` will have more rows than the one you have created because you used only 20% of the data to save time while creating the fit object using `brm()`.
...
```         
<!-- XX fix $$approval_ch ~ ideology + faminc + education$$ -->
```
...
<!-- MK: There will be an error message which says: Unable to identify the list of variables.  -->
