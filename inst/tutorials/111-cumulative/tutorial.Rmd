...
tidy_approval <- read_rds("C:/Users/922485/primer/primer.tutorials/inst/tutorials/111-cumulative/data/tbl_approval.rds")

# tbl_approval <- tbl_regression(fit_approval)
# write_rds(tbl_approval, file = "data/tbl_approval.rds")
tbl_approval <- read_rds("C:/Users/922485/primer/primer.tutorials/inst/tutorials/111-cumulative/data/tbl_approval.rds")
...
Update your previous plot by adding `labs()`. Insure that your graph includes a descriptive title, subtitle, x-axis label, y-axis label, fill, and caption. Make sure each label clearly describes what is shown in the plot.
...
# MASS::polr(formula = approval ~ ideology + faminc + education, data = x)
```

<!-- AT: The object "faminc" isn't found. Its causing an error.  -->
...
For this exercise, you will use the `plot_predictions()` function from the marginaleffects package to generate predictions from your model. Start by running `plot_predictions()` with your fitted model as the first argument. Specify `condition = c("education")` so that the function produces predicted values across different education levels, and use `draw = FALSE` to get a data frame instead of a plot. Next, use the pipe operator to send the results to `select()`, keeping only the columns for `group, education, ideology, and estimate`. Finally, use `slice(seq(1, 25, by = 5))` to display every fifth row out of the first 25, so you can examine a representative sample of your model’s predictions. Click on the "hint" button to see an example of this code. 
...
Now that the `dataset` is ready, begin constructing a plot. Set x to `ideology`, y to `estimate`, and `fill to group` in the aes() function. This will lay the foundation for the visualization. Also, add a `geom_bar(stat = "identity")` layer to your previous plot code. This will tell ggplot to display a bar for each combination of ideology and group, with the height of each bar set by the estimate value from your data.
...
<!-- AT: Plots an empty graph. Fixed I think.-->
