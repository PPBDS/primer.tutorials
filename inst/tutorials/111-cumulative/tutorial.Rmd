...
<!-- MP: The central problem for Wisdom is: Can we use the total completion time of approximately 30 minutes across both waves of the survey, along with the points-based compensation system (which allows respondents to exchange points for gift cards and other prizes), to predict respondent behavior? -->
...
<!--MP: The Preceptor Table does not include all the covariates which you will eventually include in your model. It only includes covariates which you need to answer your question. -->
...
<!--MP: This problem is predictive so there is no treatment. In order to make causal relationships, the data needs to be manipulated. In `ces`, the data is observed, there was no manipulated variables. -->
### Exercise 4
...
<!--MP: "ADD THIS" Specifying the Preceptor Table forces us to think clearly about the units and outcomes implied by the question. The resulting discussion sometimes leads us to modify the question with which we started. No data science project follows a single direction. We always backtrack. There is always dialogue. -->
...
<!--MP: Regardless, the central lesson is always the same: *You can never look at your data too much.* -->
...
<!--MP: Because this is a predictive model, none of the covariates are considered to be treatments. Remember that a treatment is just another covariate which, for the purposes of this specific problem, we are assuming can be manipulated, thereby, creating two or more different potential outcomes for each unit. -->
...
<!--MP: However, if our question were causal, then if the treatment variable is continuous (like income), then there are lots and lots of potential outcomes, one for each possible value of the treatment variable.  -->
...
<!-- MP: "ADD STILL? Iteration makes perfection", For a causal model, can you really manipulate the treatments? In reality or in theory? How? What details would need to change to make this more or less plausible? -->
...
<!--MP: //REPLACE? Validity is always about the *columns* in the Preceptor Table and the data. Just because columns from these two different tables have the same *name* does not mean that they are the same *thing*. -->
...
<!--MP: //REPLACE? Justice is about concerns that you (or your critics) might have, reasons why the model you create might not work as well as you hope.  -->
...
<!--MP: "Could be added, although it is a logical conclusion without addition" The reason that representativeness is important is because, when it is violated, the estimates for the model parameters might be biased.  -->
...
I suggest maybe the addition of some form of hypo-testing, since this is a course on Bayesian Statistics, maybe some cool posterior distribution tutorials would be valuable. It doesn't have to be super indepth and we can abstract the complicated parts through the examples:
Exercise 9:

In Bayesian data analysis, a posterior distribution represents our updated beliefs after we incorporate data. It is created by combining what we knew before (our prior) with what the data shows (the likelihood).

Fit a Bayesian model that predicts presidential approval based on ideology.
Summarize the posterior distributions. This summary tells us what the model believes about the parameters after seeing the data. 

##INSERT CODE:

Exercise 10:
The posterior distribution tells us how likely different parameter values are, based on both our prior knowledge and the data.

Extract the posterior samples for the coefficient representing ideology (i.e., how ideology affects presidential approval).
View these samples and note how they spread across different values.
Exercise 11:
Now, let’s visualize the posterior distribution to see how the model estimates the effect of ideology on presidential approval.

Use the mcmc_areas() function to plot the posterior distribution for the ideology coefficient.
Look at the shape of the distribution. Are the estimates spread out or closely grouped? What does that tell you about the certainty of the effect?
ALSO:
How about some other types of models, besides ordinal regression? Maybe logistic regression, Poisson are some that seem valuable. If we wanted to find a specific variable for example sex, Linear Regression could also be variable.

Tutorials are about opportunity cost. Time is our enemy as we linear progress through time, learning diminishes, would it be better to possibly cut down on the Wisdom tutorial and prioritize some more forms of modelling in courage? a
-->
