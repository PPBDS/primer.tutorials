...
MP: 
Here's some stuff i've gotten from reading the marginal effects library; 1. Predicted_draws function, this maybe able to fix the weird approval rating stuff. 
I can get it to work with ggplot trickery, similar to exercises 14-17

The three questions, will be at the bottom.
-->
...
plot_predictions is showing the probability of approval ratings. We can see that as we progress in education levels. It might seem confusing but each point represents the given probability that someone with no education level would have an approval rating for.
...
```{r temperance-14-hint-1, eval = TRUE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), position = position_stack(vjust = 0.5), size = 3) +
  coord_flip() +
