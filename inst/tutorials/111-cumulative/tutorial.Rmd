...
<!-- DK: Read the latest version of template tutorial. MP: Done. -->

<!-- DK: Change this tutorial to match, exactly, the new version of the template. MP: Done. -->

<!-- DK: Start experimenting with cool marginaleffects plots. 

MP: Kind of confused, does this mean a completely new different type of model? or just plots as in using the model with ggplot, if so this is also done?
Here's some plots that I worked on that could classify under this:

So far I have created a stacked bar plot, and a line plot. I have created associated exercises BUT I couldn't find anything in template_tutorial for plots specifically in temperance, so I mirrored it. The tutorials for the stacked bar plot are created but not for the line plot. I have also experimented with slopes() in marginal effects but it takes a century to run.
-->
...
<!-- DK: The tutorial introduces a treatment in the context of ads earlier, but it should be clarified that the ads are not considered the treatment in the analysis. This clarification will help prevent confusion about the role of ads in the context of the exercise. MP: Done.-->
...
<!-- DK: Test that this works in student QMD file. MP: Done -->
...
<!-- DK: Some notes: 
MP: Done additional tutorials have been created.-->
...
### 
<!-- MP: Exercises 4-12 need to be deleted, and I can reformat the tutorials I have created to both, A. Match the template_tutorial appropriately, and B. Maybe modify it to make it more efficient -->
...
<!-- MP: I added a few more tutorials, I tried to follow the wording of the template_tutorial as best as possible but could not find anything talking about ggplot specifically. -->
### Exercise 14
...
```{r temperance-21, exercise = TRUE}

```

```{r temperance-21-hint-1, eval = TRUE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), position = position_stack(vjust = 0.5), size = 3) +
  coord_flip() +
...
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  ...
  coord_flip()
```
<!--MP: code for the line plot: USING plot_predictions.
fit_approval$data$ideology <- factor(fit_approval$data$ideology,
levels = c("Very Liberal", "Liberal", "Moderate", "Conservative", "Very Conservative"))

predictions_df <- predictions(fit_approval, newdata = datagrid(ideology = levels(fit_approval$data$ideology)))

predictions_df <- as.data.frame(predictions_df)

ggplot(predictions_df, aes(x = ideology, y = estimate, color = group, group = group)) +
  geom_line(size = 1) +                 
  geom_point(size = 3) +                
  scale_color_brewer(palette = "RdBu") +
  labs(
    title = "Predicted Probabilities of Approval by Ideology",
    x = "Ideology",
...
-->
<!-- Using Slopes, I created a line plot as well (problem is it takes extremely long to run), I got it to run with my HPC access... :
library(marginaleffects)
library(ggplot2)

marginal_slopes <- slopes(fit_approval, variables = "ideology")

ggplot(marginal_slopes, aes(x = ideology, y = estimate, group = education, color = education)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  labs(title = "Marginal Effects of Ideology on Approval Ratings by Education Level",
       x = "Ideology",
       y = "Marginal Effect (Estimate)",
