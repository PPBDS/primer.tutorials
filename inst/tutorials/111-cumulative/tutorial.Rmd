...
<!-- MP:
8/15 Madhavan
This is an ideal plot

Where is the column for ideology??
Here are all the problems I was noticing, and I need some help to fix
1. There should not be p-values of zero, for the strongly disapprove/disapprove/somewhat approve columns. (model overfitting)
2. Infinite S-values, this could suggest that the model is not doing calculations correctly
3. I think the problem with the model is confidence intervals might be overlapping?
4. I don't think this is a algorithm problem, a simple check with summary of fit_approval() shows that it does work as intended
5. For example if you run this code the probability is completley bonkers
ggplot(predictions_output, aes(x = ideology, y = estimate, fill = approval)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Predicted Approval Ratings by Ideology",
       x = "Ideology",
       y = "Estimated Approval Probability") +
  theme_minimal()
...
# A tibble: 5 × 5
# Groups:   ideology [5]
  ideology          approval             mean_estimate min_estimate max_estimate
  <fct>             <ord>                        <dbl>        <dbl>        <dbl>
1 Very Liberal      Approve / Somewhat …           0.2      0.00437        0.913
2 Liberal           Approve / Somewhat …           0.2      0.00739        0.851
3 Moderate          Approve / Somewhat …           0.2      0.0204         0.489
4 Conservative      Approve / Somewhat …           0.2      0.0122         0.605
5 Very Conservative Approve / Somewhat …           0.2      0.00567        0.805
the mean_estimate is not supposed to be the exact same..

here's the list of problem's im encountering, models having the same estimate, the original problem, and models that are completley wrong "the one bonkers example that I gave", the part I think that's confusing me is stuff like this: 

# A tibble: 5 × 2
  ideology          total_estimate
  <fct>                      <dbl>
1 Very Liberal                   1
2 Liberal                        1
3 Moderate                       1
4 Conservative                   1
5 Very Conservative              1
  -->

<!-- I might have fixed it with this plot:
temp is just a random thing I made.
...
<!-- We only need `group`, `estimate`, `ideology`, `conf.low` and `conf.high` from the object `fit_approval`. Only keep these columns using the command `select()`. Assign the result to a new object called `z`.  -->
...
<!-- z <- plot_predictions(fit_approval, -->
<!--                  condition = c("ideology"), -->
<!--                  draw = FALSE)  -->
<!-- z |> select(group, estimate, ideology, conf.low, conf.high) -->
<!-- ``` -->
...
Let's make a nice plot to visualize our results. Continue a pipe with `ggplot()`, setting `x` equals `group`, `y` equals `estimate`, and `color` equals `ideology` in the `aes()` function. Note that this will return a plain graph since we have not mapped any data points to the graph yet.
...
plot_predictions(fit_approval,
                 condition = c("ideology"),
                 draw = FALSE) |> 
  ggplot(aes(x = group, y = estimate, color = ideology))
```
...
<!-- GK: Got some error with this command since it does not show the uncertainty range -->
...
<!-- GK: Got this error, so I commented out the code.  -->
...
<!-- ### Exercise 8
MP: Potential ways to structure the exercises:

Let’s modify the factor levels of the variable group in our data. Set the levels to a specific order: "Strongly Disapprove", "Disapprove / Somewhat Disapprove", "Neither Approve nor Disapprove", "Approve / Somewhat Approve", and "Strongly Approve". This will ensure that our plot later displays these levels in the intended sequence.
...
### Exercise 9
Let’s create a bar plot showing the distribution of approval levels by ideology. Use ggplot() with aes(x = ideology, y = estimate, fill = group) to set up the data, and then add a geom_bar() layer with stat = "identity".
...
### Exercise 10
Now let's add labels to our barplot using the function geom_text. Use geom_text with aes(label) to create appropriate labels for our plot.
...
```
### Exercise 11
Now let's have our model overlap the objects, to do this we will use the function position_stack with the call of vjust to help move our text around. An appropriate vjust could be 0.5.
...
### Exercise 12
Let’s apply a color palette to differentiate the approval levels visually. Use scale_fill_brewer() to add a predefined color scheme to the plot. We’ll use the "RdBu" palette, which provides a red-to-blue gradient ideal for contrasting approval levels. This step enhances the readability and interpretation of the graph by making each approval group easily distinguishable. Add theme_minimal() as well.
```{r}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), 
...
Now let’s rotate the x-axis labels for better readability and flip the coordinates for a horizontal bar chart. Use theme(axis.text.x = element_text(angle = 45, hjust = 1)) to rotate the labels and add coord_flip() to flip the graph.
...
```
### Exercise 15
Let's now add labels to our ggplot(), with labs() your model should look similar to this, add coord_flip() to swap the x and y axes, making it easier to read the categorical labels when there are many groups.
