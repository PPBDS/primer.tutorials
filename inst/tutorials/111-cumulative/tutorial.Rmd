...
# AP: there is an error for select() because of clashing packages. I potentially cannot load MASS as a library
...
tidy_approval <- read_rds("data/tidy_approval.rds") #AP: there is an issue with this for me. Says data doesn't exist.
```
...
<!-- Add question to load MASS. Change call to just polr. AP: Done, but a problem-->
...
### Exercise 5
<!-- AP: potentially a conflict error
...
Add the following code for our data to  `setup` code chunk in your QMD. Run the code chunk. `Cmd/Ctrl + Shift + K`.
...
<!-- DK: Move to this to QMD -> Console. AP: Done.-->

Continue the `fit_approval` pipe with `tidy(conf.int = TRUE)`. Run the code chunk. This might take a minute or two.
...
```         
tutorial.helpers::show_file("analysis.qmd", pattern = "tidy_approval")
```
...
<!-- DK: Explain need a new code chunk. Run this code in that chunk. Not an exercise code chunk. Use show_file(..., chunk = "Last") AP: ok, fixed.-->
...
<!-- AT: In the tutorial I got an error on this exercise: "object 'x' not found."  AP: fixed.-->
...
<!-- DK: Too fast. Need tidy_approval object in QMD and console. Then show it to AI to make a table. AP: ok, fixed above-->
...
<!-- This function takes too long to run for us to test it here. AP: working on it, do I have them run only this code in the QMD/Console or all marginaleffects questions? -->
