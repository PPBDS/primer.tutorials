...
In this case, we can again see that family income has no correlation with presidential approval. From now on, we can drop `faminc` from the model because it has no relationship with `approval`. 
...
We saw from the `fit_approve` that `faminc` does not have any relationship with `approval`. We will have it in our `newdata` object but we will not include it in our graph.
...
We can choose any value for `faminc` and that would not change our estimates of the predictions as there is no relationship between `faminc` and `approval`. 
