...
<!-- AP: since we dont use tidymodels, should i explain why not here?-->
...
<!--AP: which modeling package are we using? -->
...
For an ordinal outcome variable, we use a cumulative logit model:

$$
\log\left[ \frac{P(\text{approval} \leq k)}{P(\text{approval} > k)} \right] = \theta_k - (\beta_1 \cdot \text{ideology} + \beta_2 \cdot \text{education})
$$

The link function for an ordinal variable is built out of a log-odds structure.
