...
ndata <- expand_grid(ideology = unique(x$ideology),
...
We will create a model that estimates `approval` as a function of `faminc`, `ideology`, and `education`.
...
The `|` regular expression in the `show_file()` call allows us to just pull out the two lines in the QMD which we want. The `|` is used like or, so the code returns any lines that have `brms` or `tidybayes`.
...
$$ approval_i  \sim Cumulative(\rho_{Strongly Approve}, \\ \rho_{Approve / Somewhat Approve}, \\ \rho_{Neither Approve Nor Disapprove}, \\ \rho_{Disapprove / Somewhat Disapprove}, \\ \rho_{Strongly Disapprove}) $$        
Approvals add to 1.
...
$$
\rho = \frac{1}{1 + e^{-(\beta_0 + \beta_1 x_1 + \ldots) }}
$$
...
<!-- MK: do the questions and the answers make sense. -->

### Exercise 14

Write a sentence interpreting the `0.17` estimate for `educationHighSchoolGraduate`.

```{r courage-14}
question_text(NULL,
	message = "When comparing high school graduates with other education levels, high school graduates have a 0.17 value for approval, meaning that they are more likely to have more approval for the president, relative to other education levels because the estimates for the other educational levels are all negative.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Since `education` is not a binary variable, interpreting the estimates becomes a little more complex. Larger coefficients (either positive or negative) indicate a stronger relationship between the predictor and the latent approval score.

### Exercise 15

Write a sentence interpreting the `0.27` to `1.44` confidence interval for `ideologyLiberal`. 

```{r courage-15}
question_text(NULL,
	message = "We do not know the true value for the coefficient for ideologyLiberal, but we can be 95% confident that it lies somewhere between 0.27  and 1.44.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
If the credible interval does not include zero, the effect is likely to be significant.

### Exercise 16

Write a sentence interpreting the `6.23` estimate for `ideologyVeryConservative`.

```{r courage-16}
question_text(NULL,
	message = "If we compare people who are very conservative with the other ideologies, the very conservative people have, on average, a 6.23 more approval toward the president, adjusting for other individual characteristics.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Here is the R code which creates the `newdata` object: `expand_grid(ideology = unique(x$ideology), education = unique(x$education), faminc = unique(x$faminc))`. Type it into the code exercise block and hit "Run Code."
...
expand_grid(ideology = unique(x$ideology),
            education = unique(x$education),
            faminc = unique(x$faminc))
```
