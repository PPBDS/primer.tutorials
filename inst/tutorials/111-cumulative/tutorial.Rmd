...
<!-- DK: All data manipulation in EDA section. -->
...
To solve this issue, create a new object called `brm_data` and set it to `x |> slice_sample(prop = 0.2)`. 
...
The `brm_data` object only has 20% of the data from `x`. This will make the `brm()` call run much faster.

<!-- DK: Split above into two questioms. First, build the pipe. Second, tell them you have assigned the result of the pipe to an object, have them print that object. Name of object should be `x` or something similarly short and bland. Or, rather, do all this earlier.-->
