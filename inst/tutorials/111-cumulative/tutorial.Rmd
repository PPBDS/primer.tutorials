...
# Danger! There is a select() function in MASS.
...
Add `library(MASS)` and `library(broom)` to the `setup` code chunk in `analysis.qmd`. Copy and paste the below code for the mathematical structure of the model to the body of `analysis.qmd`. `Command/Ctrl + Shift + K`. 
...
```
tutorial.helpers::show_file("analysis.qmd", pattern = "MASS|broom|\\$\\$")
```
...
Create a model using `polr()` from the **MASS** package. Your arguments should be `formula = approval ~ ideology + faminc + education` and `data = x`. 
...
polr(formula = approval ~ ... + faminc + education,
    ... = x)
```
