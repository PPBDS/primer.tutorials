...
<!-- DK: FYI: We don't use tidymodels on purpose. Make sure students understand why. AP: added in courage-->

<!-- DK: Check the template_tutorial and add "Imagine that . . ." section. AP: fixed
...
<!-- DK: A little too loosey goosey. AP: fixed i think-->
...
A version of the data from XX is available in the `ces` tibble.
...
<!-- DK: The below plot is not working because plot_predictions() uses different variable names for its output than what the previous code did, so there is no "predicted" variable. AP: should be fixed -->
...
ggplot(results, aes(x = ideology, y = estimate, fill = group)) +
  ...
  labs(
    ...
    subtitle = "Approval is higher among some ideological groups",
    x = "Ideology",
    y = "Probability",
    fill = "Approval"
  ) +
...
<!-- DK: Add to template when you figure out these, so that future tutorial writers benefit. AP: will do soon once figured out completely-->
...
Add `library(broom)` to the `setup` code chunk in your QMD. `Command/Ctrl + Shift + K`.
...
### Exercise 6
<!--AP: need to reorder some things-->
...
In the Console, run `check_predictions(extract_fit_engine(fit_approval))`. CP/CR.
...
check_predictions(extract_fit_engine(fit_approval))
```
...
In the Console, run `tidy()` on `fit_approval` with the argument `conf.int` set equal to `TRUE`. The returns 95% intervals for all the parameters in our model.
...
```{r temperance-5-hint-3, eval = FALSE}
avg_comparisons(fit_approval, variable = "ideology")
```
...
```{r temperance-5-hint-4, eval = FALSE}
plot_predictions(fit_approval, condition = "ideology")
```
...
<!--AP: I might delete these instructions below as I followed the template for the exercises above, just wanted confirmation (Everything above is according to the template, I am considering deleting everything below until Summary)-->
...
```{r temperance-21-hint-1, eval = FALSE}
...
```{r temperance-22, exercise = TRUE}

```

<button onclick="transfer_code(this)">Copy previous code</button>

```{r temperance-22-hint-1, eval = FALSE}
...
```{r temperance-23-hint-1, eval = FALSE}
ggplot(temp, aes(x = ..., y = ..., fill = ...)) +
  geom_bar(stat = "...") +
  scale_fill_brewer(... = "...")
...
```{r temperance-24, exercise = TRUE}

```

<button onclick="transfer_code(this)">Copy previous code</button>
```{r temperance-24-hint-1, eval = FALSE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
...
```{r temperance-25, exercise = TRUE}

```

<button onclick="transfer_code(this)">Copy previous code</button>

```{r temperance-25-hint-1, eval = FALSE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), position = position_stack(vjust = 0.5), size = 3) +
  coord_flip()
...
```{r temperance-26, exercise = TRUE}

```

<button onclick="transfer_code(this)">Copy previous code</button>

```{r temperance-26-hint-1, eval = FALSE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), position = position_stack(vjust = 0.5), size = 3) +
  coord_flip() +
...
```{r temperance-27, exercise = TRUE}

```

```{r temperance-27, eval = TRUE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), position = position_stack(vjust = 0.5), size = 3) +
  coord_flip() +
