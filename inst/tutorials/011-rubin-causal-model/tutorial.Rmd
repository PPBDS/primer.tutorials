...
<!-- 5 to 10 questions like these? -->

<!-- Add some questions in which you give students a scenario --- here is a question, here is some data, here is our approach --- and then ask for a reasoning why a given assumption (specifying the assumption) does not hold. -->

<!-- Example: You want to know the distribution of wealth in NYC today. You have some data from income tax returns 5 years ago. What is one reason why the validity assumption might not hold? Answer: Income (the data you have) is different enough from wealth (the data you want), that validity may not hold, i.e., you can't "stack" the data and the Preceptor Table on top of each other to construct the Population Table. -->

<!-- 5 questions at end of Preceptor table in which you describe a scenario and they tell you, in words, what the Preceptor Table looks like in terms of units, outcomes, treatments, and covariates. -->

<!-- Make "default" questions and their answers more like the template_tutorial versions. -->

<!-- Need five questions at the end of causal effects in which a student is given a scenario and asked: Should you use a causal or predictive model? Why?  -->

<!-- We then give a great answer to the above. -->
...
<!-- We want to make sure students understand all the key concepts as mentioned in the introduction. That is the goal of this tutorial. -->

<!-- It is not important to make any part of the tutorial mimic the chapter. We will eventually change the chapter to be more like the tutorial.  -->

<!-- We should copy/paste from template_tutorial all the standard questions. They answer these questions every single time!  -->

<!-- But, even though there is not a lot of other material, we can't ask these questions more than once. -->

<!-- But, we can provide lots of simple examples to make sure students understand the concepts. That, is: Here is a situation. Describe the Preceptor Table for this scenario. Or, here is a situation. Describe a reason why representativeness might not hold. We could go through three or four of these. Only in the first one would we confirm that students can define terms in their own words. Afterwards, we just walk through a new situtation, from start to finish.  -->
...
<!-- More detail on the Preceptor Table. How exactly does one construct one, step-by-step. Use the same approach as in the template tutorial. Maybe do that set of question 2 or 3 times, each time with a different example. How to make a Preceptor Table  was totally opaque to students this year, even after doing the tutorial. -->
...
```{r preceptor-table-4}
question_text(NULL,
	message = "What if the brothers are children and the data were collected in 2020? Do we really know that, for example, Robert's current height is 178? No! We don't. This (potentially) makes answering our question much more difficult.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r}
precep_1 |> 
  cols_add(Year = rep(2025, 5), .after = id) 
```
...
We hope that all this new terminology is not overwhelming. The "[Cardinal Virtues](https://ppbds.github.io/primer/cardinal-virtues.html)" chapter in the [*Primer*](https://ppbds.github.io/primer/) provides an overview of the most important terms and their definitions.
...
```{r preceptor-table-7}
question_text(NULL,
	message = "What if the data were collected while the brothers had their shoes on but the question about average height refers to height without shoes? In that case, the meaning of 'height' would be different between the data and the Preceptor Table.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
In particular, what do we need to assume to use, for example, Robert's measured height in 2020 to fill in Robert's height in the Preceptor Table in 2025?


```{r preceptor-table-9}
question_text(NULL,
	message = "The key assumption is that height is constant, at least for the 4 years we care about and for these men. That is, although we have not measured Robert's height in 2025, we assume that it is the same as the height we measured in 2020: 178 centimeters.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
You want to predict the final college GPA of students at a specific University. You have data involving college GPA from another university. Describe what the Preceptor Table should look like for this example.

```{r preceptor-table-10}
question_text(NULL,
...
Using the example in Exercise 10, come up with a scenario for this problem in which the assumption of validity might not hold.

```{r preceptor-table-11}
question_text(NULL,
	message = "What if the data were collected with colleges using a 5.0 scale but the question about final college GPA refers to a 4.0 scale? In that case, the meaning of GPA would be different between the data and the Preceptor Table.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Using the example in Exercise 12, come up with a scenario for this problem in which the assumption of validity might not hold.

```{r preceptor-table-13}
question_text(NULL,
...
Using the example in Exercise 14, come up with a scenario for this problem in which the assumption of validity might not hold.

```{r preceptor-table-15}
question_text(NULL,
	message = "What if the marketing campaign 3 years ago offered customers a 20% discount code in the email, and the current campaign is just a product announcement? Validity would not hold in this case.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- Include Population Table somewhere here so that the students don't have to cycle back to the previous page to see it -->
...
Beau, along with the other brothers, will appear multiple times in the Population Table. The `Time` 2025 will appear multiple times, but there will only be one row for Beau in 2025.

### Exercise 5

In your own words, explain why "time is always a lie" in a Population Table.

<!-- SD: It would be better to make this more of a knowledge drop so it isn't as textbook-intensive -->

<!-- TJ: I agree with this, the answer should be a lot shorter -->
...
<!-- DK: Could add more discussion/explanation here. -->
...
<!-- DK: Format of gt_obj is messed up in Estimand column. -->
...
<!-- TJ: We should add the table again after every question so that the students don't need to go back up. -->
...
<!-- DK: This quiz is unnecessary, the one before covers negative causal effect fine. Delete. -->
...
<!-- Students need a pointer? -->

<!-- DK: Include this table in the tutorial, with a full set of five questions leading to its creation, just like in the template. -->
...
You want to predict the final college GPA of students at a specific University. You have data involving college GPA from another university. In one sentence, explain which type of model the college-GPA example involves and why.

```{r causal-effect-9}
question_text(NULL,
	message = "The college-GPA problem involves the creation of a predictive model because there is only one outcome: final college GPA.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- AC: It does say in the question to "not use actual numbers, use 'tau'" so perhaps changing that in Exercise 1 and 2's answer might be good -->
