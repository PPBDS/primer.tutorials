...
Create a new variable, `t_outcome` which takes the value of `y_t` if `treatment` equal TRUE and zero if it is not. Use `if_else()`. The three arguments for `if_else()` will be `treatment`, `y_t` and `0`.
...
As a special property of NA anything operation applied NA will result in NA, so we must first maniputate the `y_t` and `y_c` column before we calculate the estimated causal effect.
...
Start a pipe with `all_assignments`, use `filter()` and filter out `assignment` number `10` and let's see if we can manipulate other assignment just like what we doe with the experiment data.
...
```{r all-assignments-10-hint, eval = FALSE}
all_assignments %>%
...
```{r all-assignments-11-hint, eval = FALSE}
all_assignments %>%
  filter(assignment == 10) %>%
  mutate(Y_t = c(13, 14, NA, NA, 3))
```
...
```{r all-assignments-12-hint, eval = FALSE}
all_assignments %>%
  filter(assignment == 10) %>%
  mutate(Y_t = c(13, 14, NA, NA, 3)) %>%
  mutate(Y_c = c(NA, NA, 6, 12, NA))
...
Use `mutate()` create a variable `t_outcome` and set it equal to `if_else()` which has three arguments in order, `treatment`, then `Y_t` , then `0`.
...
```{r all-assignments-13-hint, eval = FALSE}
all_assignments %>%
  filter(assignment == 10) %>%
  mutate(Y_t = c(13, 14, NA, NA, 3)) %>%
  mutate(Y_c = c(NA, NA, 6, 12, NA)) %>%
...
###
This is an column with two NA values, two zeros and a value of 13 for Yao.
...
```{r all-assignments-14-hint, eval = FALSE}
all_assignments %>%
  filter(assignment == 10) %>%
  mutate(Y_t = c(13, 14, NA, NA, 3)) %>%
  mutate(Y_c = c(NA, NA, 6, 12, NA)) %>%
...
Use `summarise()` create an variable `est_effect` represents estimated treatment effect, and set it equal to the `mean()` of `t_outcomes` minus the `mean()` of `c_outcomes`. You should get NA as your results. 
...
```{r all-assignments-15-hint, eval = FALSE}
all_assignments %>%
  filter(assignment == 10) %>%
  mutate(Y_t = c(13, 14, NA, NA, 3)) %>%
  mutate(Y_c = c(NA, NA, 6, 12, NA)) %>%
...
```{r null-hypothesis-1-hint, eval = FALSE}
...
```{r null-hypothesis-2-hint, eval = FALSE}
...
```{r null-hypothesis-3-hint, eval = FALSE}
...
```{r null-hypothesis-4-hint, eval = FALSE}
...
```{r null-hypothesis-5-hint, eval = FALSE}
...
```{r null-hypothesis-6-hint, eval = FALSE}
...
```{r null-hypothesis-7-hint, eval = FALSE}
...
```{r null-hypothesis-8-hint, eval = FALSE}
...
```{r null-hypothesis-9-hint, eval = FALSE}
...
```{r null-hypothesis-10-hint, eval = FALSE}
...
expand_grid(Yao     = c(TRUE, FALSE),
  ...
  mutate(assignment = rep(1:32, each = 5)) %>%
  relocate(assignment, .before = name) %>%
  mutate(outcome = rep(c(13, 14, 6, 12, 3), 32)) %>%
  unnest(outcome) %>% 
...
<!-- sample(c(TRUE, FALSE), size = 155, replace = TRUE), probbaly use map functions. -->
