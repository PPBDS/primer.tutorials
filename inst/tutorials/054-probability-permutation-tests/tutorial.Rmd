...
<!-- What does this mean and why are you showing it to me? -->
...
<!-- Note that you get an error if you use `NA` instead of  -->
<!-- Teach them about NA_real. -->
...
```{r null-hypothesis-1}
question_text(NULL,
	message = "The two potential outcomes for each person would be the same and the column of causal effect estimates would all be zero. Drop the knowledge of the Preceptor Table.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
### 
This `combine_outcome` column is our treatment and control outcomes under null hypothesis, as they are exactly identical we don't need to create a new column (even though we could) to specify treatment or control.
...
Use to `group_by()` to create a group by `assignment`
