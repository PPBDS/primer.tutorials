...
<!-- Want students to understand NA versus NA_real_, and at least to mention the other types of NA. And to see an example of the sort of error message which shows up when you make a mistake. -->

<!-- Want students to understand the difference between if_else and ifelse. The matter is "safer" in that it tries to enforce that the option for FALSE is of the same type as the option for TRUE. The latter is easier because it just tries to do the easy/reasonable thing. -->
...
Include the video that you like in your knowledge drops. -->
...
```{r full-example-11-hint, eval = FALSE}
tibble(ID = 1:5000)
```
...
```{r full-example-12-hint, eval = FALSE}
group_by(ID)
```
...
```{r full-example-13-hint, eval = FALSE}
... %>%
  mutate(treatment = map(ID, ~sample(c(TRUE, FALSE), size = 115, replace = TRUE)))
```
...
```{r full-example-14-hint, eval = FALSE}
... %>%
  mutate(comb_outcome = map(ID, ~c(trains$att_end)))
```
...
```{r full-example-15-hint, eval = FALSE}
... %>%
...
```{r full-example-16-hint, eval = FALSE}
... %>%
  mutate(control = ! treatment) 
```
...
```{r full-example-17-hint, eval = FALSE}
relocate(control, .before = att_end)
```
...
```{r full-example-18-hint, eval = FALSE}
... %>%
  mutate(treatment_outcome = if_else(treatment, att_end, NA_real_))
```
...
```{r full-example-19-hint, eval = FALSE}
... %>%
  mutate(control_outcome =   if_else(control,   att_end, NA_real_))
```
...
```{r full-example-20-hint, eval = FALSE}
... %>%
  summarise(mean_difference = mean(treatment_outcome, na.rm = TRUE) - 
                              mean(control_outcome, na.rm = TRUE))
```
...
```{r full-example-21-hint, eval = FALSE}
... %>%
  ggplot(aes(x = mean_difference)) +
   geom_histogram()
```
...
```{r full-example-22-hint, eval = FALSE}
... +
...
```{r full-example-23-hint, eval = FALSE}
... +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   binwidth = .05, 
                   color = "white") 
...
```{r full-example-24-hint, eval = FALSE}
... + 
  scale_y_continuous(labels = scales::percent_format(...))
```
...
```{r full-example-25-hint, eval = FALSE}
...%>%
  annotate('segment', 
             x = S_obs, 
             xend = S_obs, 
...
```{r full-example-26-hint, eval = FALSE}
... +
  annotate('text', 
             x = S_obs, 
             y = 0.016, 
...
Generally here is the step for a permutation test,
1. Calculate the estimated treatment effect of the actual data . 
2. Apply null hypothesis to replace the unknowns in the dataset.
3. Use null hypothesis to calculate the estimated treatment effect of all/random possible assignments.
4. Construct the permutation distribution.
5. Determine if the experiment ETE is unlikely to appear under null hypothesis. 
6. Determine if null hypothesis is correct.
7. Determine if the given treatment is effective.
