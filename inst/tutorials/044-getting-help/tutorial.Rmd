...
Create a simple plot using the `ggplot()` function by setting `data` to `mtcars` then mapping `x` to `mpg` and `y` to `cyla` using the `aes()` function. Then add the `geom_point()` function so that the graph can actually be shown.
...
ggplot(data = mtcars, aes(x = mpg, y = cyla)) +
  geom_point()
```
...
<!-- Copy into new R file, tell them to delete it later -->
...
<!-- Show them how library calls are missing and it won't work -->
...
<!-- Post to RStudio thread, just using https://community.rstudio.com/t/thread-for-student-to-practice-posting-questions-please-do-not-answer-them/107564 from the bootcamp -->
...
```{r reprex-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reprex-8-hint-1, eval = FALSE}
library(tidyverse)
...
```{r reprex-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reprex-9-hint-1, eval = FALSE}
...
```{r reprex-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reprex-10-hint-1, eval = FALSE}
...
```{r reprex-12, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r reprex-12-hint-1, eval = FALSE}
