...
Add `shape_file` and set it to `starts_with("US_tract_1910")`.
...
Add the `scale_fill_viridis_c()` layer  setting `options` to `"magma"` and `labels` to `scales::percent` so our axis has percentages.  Run `readLines("NHGIS_data.Rmd")` in the console and copy/paste the output below. 
