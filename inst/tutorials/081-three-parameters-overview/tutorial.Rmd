...
With this knowledge, we must consider the representativeness of the data. Write 2 sentences discussing an assumption that we must make given the voluntary participation used to collect the data.

```{r age--party-2}
question_text(NULL,
	message = "Since the participation was voluntary, we must assume that those who decide to fill out the survey will not be very different from those who don't decide to fill out the survey. This is because it is possible that those who fill out surveys are different than those who don't, so in order for us to use this data and extend our conclusions to U.S. train commuters today, we must assume that train commuters regardless of whether they fill or don't fill our the surver are not significantly different to alter the data. Another way to address this issue would be to narrow our population down to train commuters in the U.S. today who are willing to fill out the surveys.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Write 2 sentences discussing the validity of `party`. Consider if the meaning of being a Democrat or a Republican changed between 2012 and now. 

```{r age--party-3}
question_text(NULL,
	message = "We will be considering it to be valid enough, however, it's important to take the time between 2012 and now and how political lines may have changed between then and now into consideration. Between then and now, there have been divisive elections and national events bringing greater attention to social and political issues which may have changed the meaning of aligning with a party.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
```{r age--party-11-hint-2, eval = FALSE}
... %>% 
  labs(title = ...,
       subtitle = ...,
       x = ...,
...
Here is the subpart of our population table relevant to our current questions.
...
First, write a paragraph discussing if the data is representative of the population. Can we treat the rows of our data as if they are random samples from the population table? Think specifically in terms of `att_end` and `treatment`. 
