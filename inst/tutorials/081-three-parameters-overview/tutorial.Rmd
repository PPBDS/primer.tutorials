...
Recall that this study was done in Boston, MA in 2012. Write a paragraph discussing whether you believe it's reasonable to consider this data to be drawn from the same population as our Preceptor Table. Identify at least one reason that you may not consider the data to be drawn from the same population.

```{r wisdom-8}
question_text(NULL,
	message = "One possible reason we may not consider the data to be drawn from the same population as our Preceptor Table due to changes that may have occurred over time, and a belief that our larger population is unable to include both 2012 and 2021 for some reason such as if we believed that the political climate was far too different.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Now, let's make our posterior distribution using 'stan_glm()'. Set your outcome variable as `age`and your explanatory variable as `party` (Remember you must insert a tilde `~` between the two). Also, include a `-1` after `liberal` so we do not get an intercept.  Set `data` to `trains`, and `refresh` to 0. Additionally, since it's a linear model, set `family` to gaussian.

<!-- MM: break question into multiple sections -->
