...
Let's consider the validity of our data. Recall that validity pertains to the columns of the data and whether they have the same meaning down the columns of the population table. Write two sentences discussing if `party` has the same meaning in the Preceptor Table and the Population Table. In other words, does it mean the same thing to identify as a Republican or Democrat in 2012 as it does in 2021?
...
With this knowledge, we must consider the representativeness of the data. Write 2 sentences discussing whether we can assume the data of 2012 Boston train commuters is representative of 2012 US train commuters. Consider specifically the location of train commuters in our data as opposed to our population.
...
Finally, let's also consider our DGM and whether it will be linear or logistic and explain why. Also, state the family that you would use in `stan_glm` when making the fitted model.
...
Let's look at the validity of our model. Write a paragraph discussing the meaning of receiving the treatment and some problems that we might run into when combining the `att_end` column from the data and our Preceptor Table. 
...
```{r attend--treatment-11-hint-2, eval = FALSE}
pe <- ... %>% 
  mutate(ate = `1` - `2`)
```
