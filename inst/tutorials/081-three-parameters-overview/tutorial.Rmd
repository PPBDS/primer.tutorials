...
Let's say we are trying to answer the following questions about train commuters in the U.S.:

1. *What is the probability that if a Democrat shows up to a train station, he will be over 50 years old?*
...
In order to answer the first two questions, what columns would we like to have in our Preceptor Table. 


```{r wisdom-ex-1}
question_text(NULL,
	message = "We would use a predictive model and would ideally have data on both the political affiliation and age of train commuters.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Next consider the third and fourth question and the columns needed in the Preceptor Table to answer them.
...
```{r wisdom-ex-3}
question_text(NULL,
	message = "For question 5, the columns we would need would be age and income. For questions 6 and 7, we would also need a column for income, but we would additionally need a column for whether someone is liberal or not.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
Recall that this study was done in Boston, MA in 2012. Discuss whether you believe it's reasonable to consider this data to be drawn from the same data set as our Preceptor Table. 
...
First, let's make this into a tibble and remove `sigma` since it is not of relevance to us. Remember that we need to use `as_tibble()`.
...
Now, let's plot this posterior and make the following plot.
...
Plot the column representing age to the x-axis. Then use geom_histogram and use `aes()` in as the first argument to make the y-axiz represent the probability. Additionally, set `bins` to 100.
...
Now, let's use the distribution to answer our questions by plotting it. To do so, we will create the following graph.
...
Now, let's plot this and make the following graph.
...
Plot `attitude` on the x-axis and set `fill` to `Parameter`. Add the layer `geom_histogram()` and make y into a probability. Set `alpha` to 0.5, `bins` to 100, and `position` to "identity".
...
Now, let's graph this to make the following plot.
...
Map `ate` to the x-axis and add the layer `geom_histogram()`. Make the values on the y-axis probabilities and set the argument `bins` to 100. 
...
```{r liberal--income-ex-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-3-hint, eval = FALSE}
...
```{r liberal--income-ex-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-4-hint, eval = FALSE}
...
```{r liberal--income-ex-5, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-5-hint, eval = FALSE}
pe <- posterior_epred(..., newdata = ...) %>% 
  as_tibble()
```
...
```{r liberal--income-ex-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-6-hint, eval = FALSE}
pe
```
...
```{r liberal--income-ex-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-7-hint-1, eval = FALSE}
Remember that your columns are not named, but numbered.
```

```{r liberal--income-ex-7-hint-2, eval = FALSE}
pe %>% ggplot(aes(x = `1`)) + 
  geom_histogram(aes(y = after_stat(count/sum(count))),
                 bins = ...)
```
...
```{r liberal--income-ex-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-8-hint-1, eval = FALSE}
Use `scale_x_continuous` and `scale_y_continuous`. Also, set `accuracy` to 1 in order to make the number an integer.
```

```{r liberal--income-ex-8-hint-2, eval = FALSE}
... +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
```
...
```{r liberal--income-ex-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-9-hint, eval = FALSE}
... + 
  labs(...) + 
  theme_classic().
```
...
```{r liberal--income-ex-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-10-hint-1, eval = FALSE}
`rep()` can be used to replicate items in the tibble.
```

```{r liberal--income-ex-10-hint-2, eval = FALSE}
...
```{r liberal--income-ex-11, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-11-hint, eval = FALSE}
posterior_predict(..., newdata = ...) %>% 
  as_tibble() %>% 
...
```{r liberal--income-ex-12, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-12-hint, eval = FALSE}
... %>% 
  rowwise()
```
...
```{r liberal--income-ex-13, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-13-hint-1, eval = FALSE}
You can use `c_across` to go across all the columns when taking the sum
```

```{r liberal--income-ex-13-hint-2, eval = FALSE}
... %>% 
...
```{r liberal--income-ex-14, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-14-hint-1, eval = FALSE}
To make y a probability, use `aes(y = after_stat(count/sum(count)))` as the first argument of `geom_histogram()`.
```

```{r liberal--income-ex-14-hint-2, eval = FALSE}
pp %>% 
  ggplot(aes(x = total)) %>% 
  geom_histogram(aes(y = after_stat(count/sum(count))),
                 bins = 100)
...
```{r liberal--income-ex-15, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-15-hint, eval = FALSE}
... + 
  scale_x_continuous(labels = scales::number_format(accuracy = 1)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
```
...
```{r liberal--income-ex-16, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r liberal--income-ex-16-hint, eval = FALSE}
... +
  labs(...) + 
  theme_classic()
```
