...
<!-- Check for default garbage. It should all be gone! Like, from index.Rmd, "Learn more about creating websites" We check that they did this by grepping for "Learn more" and expecting it to return nothing.  -->
...
We'll now examine each file starting with `index.Rmd`. The goal is for the home page to not include more than 30 seconds worth of material.
  ...
  If you haven't already, start by including your four sentence summary from the previous section after the header of your `index.qmd` file.
...
This should only return one line. If it returns multiple you still haven't followed the instructions above. You should make another page for any extra plots by creating another Quarto Document and adding it to the `_quarto.yml` file like in the Quarto Websites tutorial.
