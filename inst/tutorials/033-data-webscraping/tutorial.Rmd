...
In this tutorial, you will use the **rvest** package to read the HTML of websites and extract data. First, load the **rvest** package.

You may need to install the **rvest** package in RStudio's console if the code box below outputs an error. If so, use `install.packages("rvest")` in Console. Do not issue this command within the tutorial itself.
...
**[rvest](https://rvest.tidyverse.org/)** is part of the **tidyverse**, a collection of packages, but it is not part of the core **tidyverse** package so you have to load it in separately.
...
<!-- DK: Pipes beginw ith objects. Should be raw_html_2 |> minimal_html(). -->

Start a pipe using the function `minimal_html()` and `raw_html_2` as its argument
...
Note that there is an older function called `html_text()`, but its default behavior is much less convenient than `html_text2()`. You should always use `html_text2()` unless you have a good reason not to.
...
Start a pipe using `minimal_html()` and `raw_html_3` as its argument.
...
You now loaded in the html in `anchor2`.
...
<!-- DK: Should we be using backticks rather than quotes for idenifying weird column names? -->
