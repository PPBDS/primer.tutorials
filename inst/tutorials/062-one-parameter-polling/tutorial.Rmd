...
# 20 posterior predictive distributions for IOP: 
iop_rooms <- tibble(num_people = 1:20) %>%
...
# 20 posterior predictive distributions for class: 
class_rooms <- tibble(num_people = 1:20) %>%
  mutate(predictions = map(num_people, ~ 
...
both_polls <- inner_join(class_rooms, iop_rooms, by = c("num_people", "total")) %>%
  rename(class_draws = num_draws.x, iop_draws = num_draws.y) 
```
...
<!-- DK: The reasons why we can't consider the data and the Preceptor Table to be from the same population is if validity fails, if, that is, we can't combine the two tibbles into one tibble because the columns can not be stacked. -->
...
In 2 sentences, explain how the choice of poll question may challenge the *validity* of the approval column, meaning the Preceptor Table and data could not come from the same population. 


```{r wisdom-6}
question_text(NULL,
	message = "A poorly worded question can heavily influence people's responses, and may not accurately reflect wether or not someone approves of President Obama. The more biased the question is, the more the meaning of approval in the survey will differ from meaning of approval needed to answer our question, and the columns could not be stacked together to be considered coming from the same population.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
It is important to state that we care about the attitudes of young Americans specifically on November 15, 2013, as that is the date the question asks us, and we acknowledge that President Obama's approval rating will likely change from day to day.  
...
```{r justice-5}
question_text(NULL,
	message = "The Population would be larger than the Preceptor Table and have rows for all young Americans aged 18-29, including multiple rows for a single individual as their opinion may have changed a few days before or after the poll. There should also be rows for people who may turn 18 a few days after the poll, and also people who turned 30 after the poll was conducted.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
For presidential approval polls, time essentially creates a new population each day. For example, from October 1st to October 17th, 2013, the U.S government shut down under President Obama causing his approval rating to dip. The pervasiveness of an event like this heavily influences people's beliefs, and it is fairly certain that some people would have differing sentiments about President Obama during and after the shutdown, showing that individuals from these two time periods could be considered entirely different populations alltogether. 
...
Almost no sample in the real world is ever perfectly representative. In fact, *representativeness is always a lie* to a certain extent. 
...
Rows with 1's represent respondents who approved of President Obama's performance, and 0's represent those who disproved. 
...
It's always a good idea to save your posteriors after turning them into tibbles, as they will always be utilized in predictive functions like `posterior_predict()` needed to answer our questions. 
...
Because we want to find the probability of each possible Obama approval rating, we should convert our y-axis to percentage values. 
...
At the exact same time as the Harvard IOP poll, a high school government class conducted their own Obama approval poll using the exact same methodology as the Harvard Institute of Politics poll, the only difference being that they polled 200 respondents instead of the 2,089 polled by the Harvard IOP. 
...
Copy your code from above, and continue your pipe using `ggplot()`. Map `approval` to the x-axis, and `poll` to the fill. 
...
We set `position` to "identity" and `alpha` to .5 to have the bars overlap in order to visualize the blended color, and `bins` to a large number like 100 to space the bars apart on the graph, to make it clear which bar belongs to which x-axis value. 
...
You have successfully created the posteriors for two different Obama approval polls. The next step is to compare them using the strategies stated earlier: 
...
```{r smaller-sample-study-19, exercise = TRUE}

```

```{r smaller-sample-study-19-hint-1, eval = FALSE}
mean(ob_iop_ppd$...)
mean(class_ppd$...)
```
...
```{r simple-prediction-10-hint-1, eval = FALSE}
... +
   scale_y_continuous(labels = ...)
```

```{r simple-prediction-10-hint-2, eval = FALSE}
... +
   scale_y_continuous(labels = scales::percent_format(accuracy = ...))
```
...
```{r simple-prediction-11, exercise = TRUE}
...
> Imagine that it is November 15, 2013. What is the probability that a room of IOP poll respondents and a room of the Gov't class poll respondents would have the same number of people who approve of President Obama? 
...
both_polls %>%
  ...
  facet_wrap(~num_people) +
  geom_col(position = "identity", alpha = .5) +
  scale_fill_manual(values = c("#e01919", "#213fd1"), labels = c("Class respondents", "IOP respondents"), name = "Group") + 
  labs(title = "Similarities and Differences for 2 Different Rooms",
       subtitle = "Each frame represents a different room size",
       y = "Number of Posteiror Draws",
       x = "Number of Obama Approvers in Room")
...
A mostly purple bar indicates that the room of IOP respondents and class poll respondents are likely to have a similar number of Obama approvers designated by the x-axis. Red bars indicate that the room with class poll respondents is more likely to have the number of Obama supporters designated by the x-axis, while blue bars indicate the same phenomena with IOP respondents. 

### 

Notice how there is more cumulative non-purple area as the room size increases: as room size increases, the room of IOP respondents and the room of class poll respondents become more different. 
...
Create a column named `lrg_poll` using `mutate()`, and set it equal to the `max()` function. Within `max()`, use `c_across()` to create a vector with 2 elements: "class_draws" and "iop_draws". 
...
Create another column named `sml_poll` using `mutate()`, and set it equal to the `min()` function. Within `min()`, use `c_across()` to create a vector with the same 2 elements: "class_draws" and "iop_draws". 
...
The `pct` column quantifies how similar or different the rooms of IOP respondents and class poll respondents are. If the two rooms are predicted to have a similar number of Obama approvers (area bar is mostly purple), then `pct` will be closer to 1. If the two rooms are less similar and have drastically different predictions for a number of Obama approvers, `pct` will be closer to 0. 

### Exercise 17

Let's visualize the `pct` differences between the room of IOP respondents and class poll respondents. Copy your code from above, and use `ggplot()` to map `total` to the x-axis and `pct` to the y-axis. Add the layer `geom_col()`, and `facet_wrap()` by `num_people` (Make sure to include a ~ in `facet_wrap()`). To finish, use `labs()` to give your graph the appropriate title, subtitle, and axis labels.
...
both_polls %>%
  rowwise() %>%
  mutate(lrg_poll = max(c_across(c("class_draws", "iop_draws")))) %>%
  mutate(sml_poll = min(c_across(c("class_draws", "iop_draws")))) %>%
  ungroup() %>%
...
What is the probability that on November 15, 2013, a single person randomly chosen from a group of IOP respondents and a group from the class poll respondents would both either approve of disapprove of President Obama?  Respond in one sentence.

<!-- RS: I need to fix this question because it's not technically correct. The posteiror factors in uncertainty, so having the question be about poll respondents who we know the true values of would be incorrect.  -->

<!-- If we knew the actual percentage of young Americans and all Americans who approved of Obama, how do you think our answer would change, if at all? -->

<!-- If we knew the actual percentage of young Americans and all Americans who approved of President Obama on November 15, 2013, the probability would most likely be much higher, because the answer would become a simple probability calculation that would not have to factor in uncertainty regarding the actual value of each percentage. -->

```{r probability-20}
question_text(NULL,
	message = "There is about an 81% chance that the two people would both either approve or disapprove of President Obama.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
