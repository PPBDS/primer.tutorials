...
iop_data <- tibble(approve = c(rep(1, round(.41*2089)), rep(0, round(.54*2089))))
...
class_data <- tibble(approve = c(rep(1, .32*200), rep(0, .52*200)))
...
young <- tibble(num_people = 1:20) %>%
  mutate(predictions = map(num_people, ~ posterior_predict(iop_fit, newdata = tibble(constant = rep(1, .))))) %>%
...
all <- tibble(num_people = 1:20) %>%
  mutate(predictions = map(num_people, ~ posterior_predict(class_fit, newdata = tibble(constant = rep(1, .))))) %>%
  mutate(total = map(predictions, ~ rowSums(.))) %>%
  unnest(total) %>%
  group_by(num_people, total) %>%
  summarize(num_draws = n(), .groups = "drop") 
...
First, you will examine a Harvard poll to determine wether or not it can answer our questions. Next, you will calculate feasible values for the percentage of young Americans who approved of President Obama in the fall of 2013, by creating a posterior distribution using the professional **rstanarm** statistical library.  You will then create another posterior using a different poll, and learn to compare them using their means and variation. Finally, you will answer a challenging probability question by calculating the area under a graph. 
...
In 2-4 sentences, examine *validity* across polls by considering how classifying non-responses as 0's (disapprovals) could cause different mathematical results compared to excluding them all-together.
...
In the Harvard IOP poll was conducted between October 29 and November 13 of 2013. Keeping this in mind, in 2 sentences examine the *stability* of this poll across time periods. Do you believe the results could be used to draw conclusions about how young adults aged 18-29 feel about President Obama in the present day? 
...
However, how should the 5% of people who failed to respond be represented? Because non-responses are the fault of experimental design, and not a reflection of people's actual beliefs, they should be excluded. For example, people who fail to respond could be busy, or may not want to reveal their beliefs. 

###

First, input respondents who approved of President Obama's performance into your data. 

Use `tibble()` and set `approve` equal to `c()`. Include `rep()` as the first element in the vector. Within `rep()`, pass in `1` as the first argument, and the rounded value of $.41 * 2,089$ as the second argument (use the `round()` function to round). 

```{r iop-1, exercise = TRUE}

```

```{r iop-1-hint-1, eval = FALSE}
tibble(... = ...)
```
...
```{r iop-1-hint-3, eval = FALSE}
tibble(... = c(rep(..., ...)))
```

###

Your tibble should have `r nrow(tibble(rep(1, round(.41*2089))))` rows, representing the 41% of the 2,089 survey respondents who approved of President Obama's performance. 

### Exercise 2

Next, input the 54% of respondents who disapproved of President Obama's performance.

###

Copy your code from above. Add another `rep()` function as the second element in the vector. Within `rep()`, pass in `0` as the first argument, and the rounded value of $.54 * 2089$ as the second argument (use the `round()` function to round). 
...
```{r iop-2-hint-2, eval = FALSE}
c(..., rep(..., ...))
```

###

Your tibble should now have `r nrow(tibble(c(rep(1, round(.41*2089)), rep(0, round(.54*2089)))))` rows. Rows with 1's represent respondents who approved of President Obama's performance, and 0's represent those who disproved. 

The `r nrow(tibble(c(rep(1, round(.41*2089)), rep(0, round(.54*2089)))))` individuals in our tibble is less than the 2,089 individuals who initially completed the poll. This is because we excluded people who failed to respond, as we do not know whether or not they approved of Obama.  
...
We will eventually compare the Harvard IOP poll with another poll conducted by a government class using their centers and spreads in order to estimate the percentage of a population who approved of President Obama in fall of 2013, and determine the uncertainty surrounding that estimate. 

###

In the fall of 2013, a high school government class conducted their own Obama approval poll. The class randomly dialed U.S households, requesting to speak to an adult with the most recent birthday, and asked the following question:
...
The poll was given to random U.S households. Assuming the dialing was truly random, and only adults answered the call, then the population of the survey would be all U.S adults in the fall of 2013. 
...
First, input the 32% of respondents who approved of President Obama's performance into your data.

Use `tibble()` and set `approve` equal to `c()`. Include `rep()` as the first element in the vector. Within `rep()`, pass in `1` as the first argument, and $.32 * 200$ as the second argument.
...
```{r class-poll-2-hint-3, eval = FALSE}
tibble(... = c(rep(..., ...)))
```

###

Your tibble should have `r nrow(tibble(rep(1, .32*200)))` rows, representing the 32% of the 200 class poll respondents who approved of President Obama's performance.

### Exercise 3

Next, input the 52% of respondents who disapproved of President Obama's performance.

###

Copy your code from above. Add another `rep()` function as the second element in the vector. Within `rep()`, pass in `0` as the first argument, and $.52 * 200$ as the second argument.
...
```{r class-poll-3-hint-2, eval = FALSE}
c(..., rep(..., ...))
```

###

Your tibble should now have `r nrow(tibble(c(rep(1, .32*200), rep(0, .52*200))))` rows. Rows with 1's represent respondents who approved of President Obama's performance, and 0's represent those who disproved. 
...
> In the fall of 2013, if there were two rooms of the same size, one filled with all American adults, and one filled with just 18-29 year olds, what is the probability that the rooms will have the same number of people who approve of Obama?
...
```{r prob-1-hint-2, eval = FALSE}
Use : to create a range of integers. 
```
...
Copy your code from above, and replace `iop_fit` with `class_fit`. Also change the object name from `young` to `all`.
...
A mostly purple bar indicates that the room of young Americans and all Americans are likely to have a similar number of Obama approvers designated by the x-axis. Red bars indicate that the room with all Americans is more likely to have the number of Obama supporters designated by the x-axis, while blue bars indicate the same phenomanan with young Americans. 
...
To finish operations across rows, add `ungroup()` to your pipe. Next, create a new column called `pct`. Set `pct` equal to `min` divided by `max`. 
...
both_polls %>%
  ...
  mutate(min = min(c_across(c("all_summary", "young_summary")))) %>%
  ungroup() %>%
  mutate(pct = min/max) %>%
...
Copy your code from above, but delete `ggplot()` as well as the `geom_col()` and `facet_wrap()` layers (we no longer wish to visualize the percent differences).  Instead, group your tibble by `num_people` using `group_by()`. 
