...
>>>>>>> 8d8c5f6535f6b32343df0fde1feddae27be2b926
### 
...
<!-- DK: I lied! The reasons why we can't consider the data and the Precetor Table to be from the same population is if validity fails, if, that is, we can't combine the two tibbles into one tibble because the columns can not be stacked. -->
...
<!-- DK: I liked! Move validity up to Wisdom. -->
>>>>>>> 8d8c5f6535f6b32343df0fde1feddae27be2b926
...
```{r justice-5}
question_text(NULL,
	message = "The Data source would have 2,089 rows, one for each poll respondent.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
In general, we would include all the rows from the data in the Population Table. But what if the question was about people aged 18 to 22? In that case, we might exclude poll data from older respondents even if they are available in the data set.
...
Almost no sample in the real world is ever perfectly representative. In fact, *representativeness is always a lie.* 
...
<!-- clean_iop <- iop %>%  -->
<!--   drop_na() %>%  -->
<!--   mutate(approval = if_else(response == "Approve", 1, 0)) %>%  -->
<!--   select(approval) -->
