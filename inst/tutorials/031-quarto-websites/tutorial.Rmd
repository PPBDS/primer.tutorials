...
In the Console (not the Terminal), run `usethis::use_git()`. But read these instructions first!

You will be asked two questions. The first, asked about the files in the project, is "Is it ok to commit them?" Answer some version of "No." (Annoyingly, **usethis** provided jokey options like "Absolutely not." Any negative answer is fine.)

The second question --- "Restart now?" --- concerns restarting RStudio. Agree with that. Doing so will probably generate a message requiring you to Terminate the tutorial. This always happens when RStudio is restarted, which is required with `use_git()`. Go ahead click "Terminate Jobs".

### 

We answer "No" to the first question because we want to modify the `.gitignore` file before committing anything.

We answer "Yes" to the second question because a restart is required in order to see the Git pane.
...
Edit the `.gitignore` to add `*Rproj`. The Rproj file includes our personal settings. We generally don't like files like that to become public on Github.

After saving the new `.gitignore`, commit all the files in the project.

###

Run `cat .gitignore` from the Terminal. CP/CR.
...
`use_git()` initializes a Git repository and creates a `.gitignore`. Notice that the default contents of this `.gitignore` are different from the contents of a `.gitignore` created when we use a Github repo to start a new R Project.
...
There are multiple ways to make citations, you can make them manually, or with a citation manager like [Zotero](https://www.zotero.org/). We will be making them manually.
