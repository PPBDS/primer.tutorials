...
Although the Tidyverse includes hundreds of commands, the most important are `filter()`, `select()`, `arrange()`, `mutate()`, `group_by()`, and `summarize()`. Whenever you face a new problem, try to think about which one of these commands might be a good way to start.
...
Above, we calculated the average `age` for the entire `trains` tibble. Now, let's do the same calculation for men and women separately. Pipe `trains` to `group_by()` with `gender` as the argument. Stop there and look at the result

```{r tidyverse-13, exercise = TRUE}

```

```{r tidyverse-13-hint-1, eval = FALSE}
trains |> 
  group_by(...)
```
...
Notice the top of the printout:

`# A tibble: 115 × 14`
`# Groups:   gender [2]`

Without the `group_by()` statement, this would just show the dimensions of the tibble. The `Groups: gender [2]` information is new. It means that this is not a simple tibble. Instead, it is a "grouped" tibble. It is grouped by the "gender" variable, which has "2" levels. This attribute will change how subsequent functions will act.

### Exercise 14

Start with the pipe from the previous exercise. Pipe the result to `summarize()` and create a new variable, `avg`, which is the mean age.
...
There are some subtleties in whether or not the tibble which results from a call to `summarize()`/`group_by()` is itself still grouped. See `?summarize` for details. The `.groups` argument to `summarize()` controls this behavior. The most common approach is to use `.groups = "drop"` so that the resulting tibble is no longer grouped. Using the `ungroup()` command has the same effect.
