...
<!-- AM: Most exercise descriptions simply tell the user what to do without really giving an insight as to what the user is doing. I will be adding in little "words of wisdom" after exercises, where applicable, to help the user understand what the code chunk just did if it is not obvious. For example, I would not leave any words of wisdom for exercise 2 of Mutating Strings because it is straightforward, but I will for exercise 3 of Mutating Strings. -->
...
<!-- AM: Suggesting rewording: This tutorial will explore various functions that can manipulate strings. -->
<!-- AM: Start off the tutorial with x <- read_csv("https://raw.githubusercontent.com/lawanin/PA-policing-data/master/philly-clean.csv") without specifying col_types = cols(), in order to help the user better understand the difference. -->
...
<!-- AM: This is probably a really minor change, but shouldn't the dataset be named something more meaningful than "x"? -->
...
<!-- AM: I don't know if the copy button is perfected yet (Instantly copies in the previous exercise's code WITHOUT automatically running) but it will be useful here. <button onclick = "transfer_code(this)">Copy previous code</button> -->
<!-- AM: It will also be useful for every single exercise after Exercise 1. Please figure this out as I did not create the copy button. -->

<!-- AM: No text block should not be any longer than 2 sentences. -->
<!-- AM: Suggested rewording: Copy-paste your pipe from the previous exercise and use `mutate()` and `str_detect()` to create a new column called `race` with value `TRUE` when the column `summary` contains the string `"race"` and `FALSE` if it does not. The first argument of `str_detect()` is the column in which you want to detect a string and the second argument is the quoted string you want to detect. -->
...
<!-- AM: I don't believe the hints should be this straightforward. -->
...
Let's tidy this into long data by making each religion have 10 rows, with each observation of religion having an income group and count in the same row.  
