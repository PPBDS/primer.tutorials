...
We will be working with data about police complaints. Type `police` to take a look.
...
Start a new pipe with `x` by starting a new line and typing `x` followed by `%>%` on the new line. Select `summary`. 
...
Continue your pipe and use `mutate()` and `str_detect()` to create a new column called `race` with value `TRUE` when the column `summary` contains the string `"rac"` and `FALSE` if it does not.  

```{r mutating-strings-ex-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r mutating-strings-ex-3-hint-1, eval = FALSE}
...
```{r mutating-strings-ex-3-hint-2, eval = FALSE}
... %>% 
