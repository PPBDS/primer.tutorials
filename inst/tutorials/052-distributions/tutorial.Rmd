...
tibble(ID = 1:10000) %>%
  ...
  mutate(door_open = case_when(
                  ...
                  door_with_car == 2 & door_chosen == 3 ~ 1,
                  door_with_car == 3 & door_chosen == 2 ~ 1,
                  door_with_car == 1 & door_chosen == 3 ~ 2,
                  door_with_car == 3 & door_chosen == 1 ~ 2 )) %>%
  mutate(switch = map_lgl(ID, ~ sample(c(TRUE, FALSE), 1))) %>%
  mutate(door_chosen = as.double(door_chosen)) %>%
  mutate(contestent = if_else(switch,
                          case_when(
                            door_with_car == 1 & door_chosen == 1 & door_open == 2 ~ 3,
                            door_with_car == 1 & door_chosen == 1 & door_open == 3 ~ 2,
                            door_with_car == 2 & door_chosen == 2 & door_open == 1 ~ 3,
                            door_with_car == 2 & door_chosen == 2 & door_open == 3 ~ 1,
                            door_with_car == 3 & door_chosen == 3 & door_open == 1 ~ 2,
                            door_with_car == 3 & door_chosen == 3 & door_open == 2 ~ 1,
                            door_with_car == 1 & door_chosen == 2 & door_open == 3 ~ 1,
                            door_with_car == 2 & door_chosen == 1 & door_open == 3 ~ 2,
                            door_with_car == 2 & door_chosen == 3 & door_open == 1 ~ 2,
                            door_with_car == 3 & door_chosen == 2 & door_open == 1 ~ 3,
                            door_with_car == 1 & door_chosen == 3 & door_open == 2 ~ 1,
                            door_with_car == 3 & door_chosen == 1 & door_open == 2 ~ 3),
                          door_chosen)) %>%
  mutate(result = case_when(
    contestent == door_with_car ~ "Win",
    contestent != door_with_car ~ "Lose",
  )) %>%
  ggplot(aes(x = as.factor(result), 
             y = as.factor(switch))) +
  geom_point() +
  geom_jitter(alpha = .3)+
