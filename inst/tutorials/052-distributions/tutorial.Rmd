...
tibble(ID = 1:10000) %>%
  ...
  mutate(door_final = 
           case_when(all(change_doors, c(c(door_chosen, door_open) %in% c(1, 2))) ~ 3L,
                     all(change_doors, c(c(door_chosen, door_open) %in% c(1, 3))) ~ 2L,
                     all(change_doors, c(c(door_chosen, door_open) %in% c(2, 3))) ~ 1L,
                     TRUE ~ door_chosen)) %>%
