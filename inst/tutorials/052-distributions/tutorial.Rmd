...
#                        TRUE, FALSE)) %>% 
#   mutate(door_opened = if_else(! doors_match, 
#                                (1:3)[! (1:3 %in% door_with_car & 
#                                             door_chosen))], NA))
#   
#              case_when(door_with_car == door_chosen ~
#                        ~ sample((1:3)[! (1:3 %in% door_chosen)]),
#             TRUE ~ NA))

# Next column. action = map(ID, ~ sample(c("switch", "stay"), 1))

#Next column is result = car or goat.

#Choose car door
# car_door <- sample(1:3,  size = 1, replace = FALSE)
# choose_door <- sample(1:3, size = 1, replace = FALSE)
# door <- seq(1:3)
# 
# choose_door <- as.numeric(choose_door)
# 
# if (car_door == choose_door) { 
#   data <- data.frame(door)
# x <- data %>% 
#   filter(!door %in% choose_door)%>%
#     select(door)
# seq_remaining <- as.numeric(x$door)
# monty_select <- sample(seq_remaining, size = 1, replace = F)
#   
# } else {
#   data <- data.frame(door)
# y <- data %>%
#   filter(door != choose_door) %>%
#   filter(door != car_door) %>%
#     select(door)
# seq_remaining <- as.numeric(y$door)
# monty_select <- sample(seq_remaining, size = 1, replace = F)
# }
# 
# data_monty <- data.frame(door)
# 
# contestent_choice <- data_monty %>% 
#   filter(door != monty_select) %>% 
#   filter(door != choose_door) %>%
#   select(door)
# contestent_choice <- contestent_choice$door
# 
# 
# cat("Contestant choose:",choose_door,"\n")
# cat("Contestant choice:",contestent_choice,"\n")
# cat("Car Door:",car_door,"\n")
# 
# car <- sample(1:3,  size = 1, replace = FALSE)
# choose <- sample(1:3, size = 1, replace = FALSE)
# seq_door <- list(seq(1:3))
# 
# tibble(car = sample(1:3,  size = 10000, replace = TRUE),
#         choose = sample(1:3, size = 10000, replace = TRUE),
#           seq_door = list(seq(1:3))) %>%
#   unnest(seq_door) %>%
#   mutate(seq_door = as.numeric(seq_door)) %>%
#   mutate(car = as.numeric(car)) %>%
#   mutate(choose = as.numeric(choose)) %>%
#   mutate(monty = if_else(car == choose, 
#                          seq_door %>%
#                        filter(seq_door != choose)%>%
#                        select(seq_door))  %>%
#                        sample(seq_door, size = 1, replace = FALSE),
#        seq_door %>%
#          filter(seq_door != choose) %>%
#        filter(seq_door != car) %>%
#        select(seq_door) %>%
...
# tibble(ID = list(sample(1:3, size = 1,replace = TRUE))) %>%
#   mutate(car = map(ID, ~sample(car_door, size = 4000, replace = TRUE))) %>%
#   mutate(choose = map(ID, ~sample(choose_door, replace = TRUE,size = 4000))) %>%
#   mutate(selection = map(ID, ~list(sample(monty_select, replace = TRUE)))) %>%
#   mutate(switch_yes = rep(c("Yes"))) %>%
#   mutate(result_yes = map(ID, ~sample(contestent_choice, replace = TRUE, size = 4000))) %>%
#   mutate(switch_no = rep(c("No"))) %>%
#   mutate(result_no = map(ID, ~sample(contestent_choice_no, replace = TRUE, size = 4000))) %>%
#   unnest(c(car, choose, result_yes, selection, result_no)) %>%
#   summarise(car, choose, result_yes, result_no)
