...
```{r}
portland_data <- 
  read_csv("https://projects.oregonlive.com/weather/pdx_temps.csv",
...
portland_p <- 
  ggplot(data = portland_data, 
         mapping = aes(yday, tmax, color = year)) +
  ggforce::geom_mark_ellipse(
    aes(fill = new_record, label = new_record, filter = new_record != ""),
    alpha = 0, show.legend = FALSE) +
  geom_point(alpha = .5, size = .9) +
  geom_point(data = filter(portland_data, new_record != ""), aes(color = year), size = 1.3) +
...
portland_p + scale_color_viridis_c(option = "turbo", direction = -1, name = NULL, breaks = c(1938, seq(1950, 2010, by = 10), 2021))
```
...
Good! Note: the temperatures are all in Fahrenheit, and `tmax` and `tmin` represent the highest temperature and the lowest temperature for those days.
