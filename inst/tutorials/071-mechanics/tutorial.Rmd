...
# AT: Keeping brm() code here for future reference. Most likely will delete later
...
# AT: Discuss how to deal with loo()
...
<!-- DK: Replace brms with tidymodels. -->
<!-- DK: Use broom not broom.mixed. -->
<!-- DK: Add test code chunks for anything not too slow. -->
<!-- DK: Delete all data/*rds which are no longer used. -->
...
Create a linear regression model using the ***tidymodels** framework. Use the formula `income ~ age` with the `trains` data. Assign the result to an object called `fit_1`.
...
The value of $\beta_0$, the intercept in the regression, is `r scales::comma(round(tidy(fit_1)["Intercept", "Estimate"], 0))`. This represents the estimated average income for a person with an age of zero, which is awkward and useless since there are no people of zero age in our data. 
...
```{r transforming-variables-14-test, eval = FALSE}
fit_1_s <- linear_reg() |>
  set_engine("lm") |>
...
augment(nhanes_1, new_data = no_na_nhanes) |> 
...
augment(nhanes_2, new_data = no_na_nhanes) |> 
  arrange(age) |> 
  ggplot(aes(x = age, y = height)) +
    geom_point(alpha = 0.1) +
...
augment(nhanes_3, new_data = no_na_nhanes) |>
  arrange(age) |>
  ggplot(aes(x = age, y = height)) +
    geom_point(alpha = 0.1) +
    geom_line(aes(y = .pred), color = "red") +
...
```{r selecting-variables-3-test, eval = FALSE}
...
Consider the first model, which seeks to explain the attitudes towards immigration among Boston commuters, using political ideology (`liberal`) (variable) as the explanatory variable. Run a linear regression using the tidymodels (framework). Use the formula `att_end ~ liberal` (formula) with the `trains` (data) data. Assign the result to an object called `fit_liberal` (object).
...
<!-- DK: Replace `tidy()` and similar functions with tidy() equivalents. -->
...
The most obvious criterion for comparing models is the accuracy of their predictions. For example, consider using `liberal` to predict `att_end`. Pipe trains to `mutate()` and use `augment(fit_liberal)` to get the predicted values. Assign the predicted values to a new column called `pred_liberal`.
...
trains |>
  mutate(... = augment(..., new_data = ...)$.pred)
```

```{r comparing-models-in-theory-5-test, include = FALSE}
trains |>
  mutate(pred_liberal = augment(fit_liberal, new_data = trains)$.pred)
```
...
The most sensible way to test a model is to use it to make predictions and compare those predictions to new data. After fitting the model using ***tidymodels**, we use the `augment()` function to generate predicted values for new or existing cases.

### Exercise 12

Consider another model, using `att_start` to forecast `att_end`. Pipe `trains` to `mutate()` and use `augment(fit_att_start, new_data = trains)` to get the predicted values. Assign the predicted values to a new column called `pred_as`.
...
trains |> 
  mutate(pred_as = augment(..., new_data = trains)$`.pred`)
```

```{r comparing-models-in-theory-12-test, include = FALSE}
trains |> 
  mutate(pred_as = augment(fit_att_start, new_data = trains)$.pred)
```
...
Continue the pipe with `mutate()`. Set `pred_lib` equal to the predicted values from `augment(fit_liberal, new_data = trains)`.
...
... |> 
  mutate(pred_lib = augment(..., new_data = trains)$`.pred`)
```

```{r comparing-models-in-theory-19-test, include = FALSE}
trains |> 
  select(att_end, att_start, liberal) |> 
  mutate(pred_lib = augment(fit_liberal, new_data = trains)$.pred)
```
...
Continue the pipe with `mutate()`. Create a column named `lib_err` and set it equal to `(pred_lib - att_end)^2`. First, get the predictions using `augment(fit_liberal, new_data = trains)$.pred`.
...
Continue the pipe with `mutate()`. Create a column named `as_err` and set it equal to `(pred_as - att_end)^2`. First, get the predictions using `augment(fit_att_start, new_data = trains)$.pred`.
...
<!-- First, we will familiarize ourselves with our first model, `fit_liberal`. Run `tidy()` on `fit_liberal` to print out the previous result. -->
...
<!-- Now, we will perform `loo()` on our model and look at the results. Run the `loo()` command on `fit_liberal` and assign the result to an object called `loo_liberal`. -->
...
<!-- <!-- DK: Comment out anything that does not work after you remove brms. --> -->
...
<!-- Let’s turn our attention to our second model. To begin, let’s observe the qualities of `fit_att_start` once again. Run `tidy()` on `fit_att_start` to print out the previous result. -->
...
<!-- Great! Now, let’s perform `loo()` on this model. Run the `loo()` command on `fit_att_start` and assign the result to an object called `loo_att_start`. -->
...
<!-- <!-- AC: It says that loo_liberal and loo_att_start cannot be found.  --> -->
