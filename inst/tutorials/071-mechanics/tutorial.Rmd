...
<!-- Change the sections (topics). One should be "Log Transformations". Do this first, Next is "Scaling". -->
...
Copy your previous code, but set `formula` equal `income ~ c_age`, and `data` equal `trains_2`. 
...
`The `performance()` function from ***easystats** summarizes how well your model fits the data. It reports key measures like AIC (Akaike Information Criterion), BIC (Bayesian Information Criterion), R² (explained variance), and RMSE (root mean squared error). These metrics help you compare models and decide which one provides the best fit or most accurate predictions.
