...
#Do we need this or we can lay out the code in the code chunk for students and all they need to do is to hit "Run Code". The reason is the nhanes dataset will be heavy and even heavier after adding more variables. The second reason is if we lay out the code right there, some knowledge drop related to the code can be dropped. 
...
nhanes_3 <- brm(formula = height ~ I(ifelse(age > 18, 18, age)),
                data = no_na_nhanes,
                family = gaussian(),
                silent = 2,
                refresh = 0,
...
### Exercise 21
<!-- GK: The fourth mutate in the textbook is `fit_liberal`, should be `fit_att_start` -->
