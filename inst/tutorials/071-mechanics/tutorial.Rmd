...
```{r selecting-variables-5-test2, include = FALSE}
linear_reg() |>
  fit(att_end ~ treatment + att_start, data = x) |>
  tidy(conf.int = TRUE)
...
#AT: The label 'vars' is automatically changed to 'test' when running check_current_tutorials(). The tutorial still works normally either way.
...
<!-- AT: # `ci_msg` should work in the code chunk below, I tested it and had no issues.  -->
...
In your own words, explain what the `predict()` function does and how adding predictions to your data can help you evaluate the performance of a model. Why is this step important in data science?
