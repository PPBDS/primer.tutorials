...
# AT: Added code to display numbers straight from the data instead. 
...
```{r scaling-17-test, include=FALSE}
...
```{r scaling-21-test, include=FALSE}
...
```{r selecting-variables-5-vars, include=FALSE}
...
```{r selecting-variables-14-test, include=FALSE}
...
question_text(NULL,
	message = "The coefficient for `liberalTRUE` is {coef_liberal} (negative), indicating that the liberals in our data set make less on average than non-liberals.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r selecting-variables-15-test, include=FALSE}
...
```{r selecting-variables-16-test, include=FALSE}
age_ci <- fit_2_model |>
  tidy(conf.int = TRUE) |>
...
Ask AI to help you create a visualization that compares model predictions from your `fit_att_start` model to the actual `att_end` values, using the `x` data. Your plot should display the predicted values for `att_end` from the `fit_att_start` model on the x-axis and the actual `att_end` values on the y-axis. Use `geom_jitter()` to show overlapping points clearly, and add a red reference line to highlight where predictions perfectly match actual values. Be sure your plot includes appropriate axis labels, a descriptive title, and a helpful subtitle.
...
<!-- DK: List specific functions. `performance()`, `compare_models()`. Done  -->

To choose the best model for predicting attitudes toward immigration, we need to compare their performance using reliable statistical tools. The **easystats** package, especially its `performance()`, `compare_performance()`, and `compare_models()` functions, helps us evaluate, compare, and formally test our models. In this section, you’ll use these functions to review model quality metrics, compare models side by side, examine confidence intervals, and run formal statistical tests to decide which predictor is most useful.



<!-- AT: Is the paragraph above good as an introduction? No. Should mention the functions, not the statistical measures. Done -->
...
<!-- DK: Discuss how confidence intervals aid in model choice. Done? -->
