...
# DK: Do we need all the variables in trains? Probably not. In one step, create new 
# x (you can even just give them the code to execute, and then, 
# Behind the scenes.)

# DK: Do we need a fit_1 (and other) objects? Not if they are just used in one pipe.
...
# DK: I don't think we need to create these objects.
...
<!-- DK: Change the sections (topics). One should be "Log Transformations". Do this first, Next is "Scaling". -->

<!-- AT: Currently just a handful of excercises use 'scale()' should I add more before making a seperate section for it? -->

DK: Cool approach. Delete the fit_ objects from scaling section.
...
<!-- DK: Missing a lot of copy code buttons. -->

### Exercise 10

One way to make the intercept more meaningful is to transform `age`. Pipe `trains` to `mutate()` to create a new column `c_age = age - mean(age)`. Assign the result to `x`.
...
Copy your previous code, but set `formula` equal `income ~ c_age`, and `data` equal `x`. 
...
Behind the scenes, we have assigned the result of the pipe to an object named `fit_1_c`. Type `fit_1_c` and hit "Run Code." This will print a summary of the fitted model.
...
<!-- DK: Or maybe we keep them to be able to do this? -->
...
<!-- DK: Fix, here and elsewhere. Intructions, not questions. -->

Pipe `linear_reg()` to `fit()`, with the formula `income ~ s_age` and `data = trains_3` as arguments.
...
Behind the scenes, we have assigned the result of the previous code to an object named `fit_1_s`. Type `fit_1_s` and hit "Run Code." This will print a summary of the fitted model.
...
Pipe `linear_reg()` to `fit()`, with the formula `income ~ z_age` and `data = trains_4` as arguments.
...
Behind the scenes, we have assigned the result of the previous code to an object named `fit_1_z`. Type `fit_1_z` and hit "Run Code." This will print a summary of the fitted model.
...
<!-- DK: Behind the scenes everywhere! -->
...
<!-- DK: Don't think this works. Need `conf.int = TRUE`. -->
...
<!-- DK: Stop telling students to assign the result of a pipe to X. -->
...
<!-- DK: We do not make the student create plots line by line. Can tell them to ask AI to make a plot. Then show them your plot, and the code which created it, and maybe drop some knowledge about why your plot is good. -->
...
<!-- AT: Is the paragraph above good as an introduction? -->
...
<!-- DK: Discuss confidence intervals aid in model choice. -->
