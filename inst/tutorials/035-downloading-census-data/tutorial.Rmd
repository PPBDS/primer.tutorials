...
<!-- AG: With the introduction of IPUMS, there's a small conundrum. You need to download absolutely massive files if you want to use IPUMS (3199967 rows! 280.8 MB! 13 variables!), which makes it fundamentally incompatible with the tutorials. As such, I'm making this a separate project that they then publish. This is also a great time to walk them through dealing with large files but without the commit reset. -->
...
First, load the **tidyverse** and **ipumsr** packages.

```{r ipums-data-1, exercise = TRUE}

```

```{r}
source("data/usa_00001.R")
```


<button onclick = "transfer_code(this)">Copy previous code</button>

```{r ipums-data-1-hint, eval = FALSE}
...
Second, we need to create an IPUMS account using 

```{r ipums-data-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r ipums-data-2-hint, eval = FALSE}

```

###

<!-- this is a knowledge drop and probably an unfunny joke -->
...
<font size = "1"> There goes my dreams of being a mad (data) scientist. </font>
