...
<!-- Do all the commands in Chapter 2 of the book. Force students to type out each command. Hints can't be that good. SG: Done -->
...
Smaller geographies like Census tracts can also be subsetted by the `county` argument. 
...
Finally, we are going to learn about the `get_flows()` function. This function allows for analysis of immigrants, emigrants, and net migration for a given geography using data from a given 5-year ACS sample.
...
This should return a tibble with 6 columns, one of them being "variable". We can see variabled such as "MOVEDIN" and corresponding estimates like 152 for the "MOVEDIN" variable.
