...
Note: Throughout this section we will be working through content from [this](https://walker-data.com/census-r/an-introduction-to-tidycensus.html#an-introduction-to-tidycensus) book.

### 

<!-- DK: Make clear that this is chaoter 2 from the book. SG: Done -->

<!-- DK: Review for discussion the Maps and IPUMS appendices. SG: Done-->

The Census API works by accessing Census databases and tables. We can find these tables by visiting [data.census.gov](data.census.gov). For this tutorial, we'll be using the **American Community Survey**, the **Decennial Census**, and the **Population Estimates Program**.

By default, when using `tidycensus` to get data from these databases, they are downloaded in "tidy" format, meaning each variable is a column and each observation is a row. Unless you have an explicit reason not to, keep this default.
...
Congrats! You just donwloaded your first Census data from the Decennial Census.
...
```{r getting-data-from-th-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-4-hint, eval = FALSE}
get_acs(
  ... = "...", 
  variables = "...", 
  survey = "...",
...
```{r getting-data-from-th-5, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-5-hint, eval = FALSE}
get_acs(
  ... = "...", 
  table = "...",
  ... = ...
...
```{r getting-data-from-th-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-6-hint, eval = FALSE}
get_acs(
  ... = "...", 
  variables = "...", 
  ... = "WI",
...
```{r getting-data-from-th-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-7-hint, eval = FALSE}
get_acs(
  geography = "...", 
  variables = "...", 
  state = "...", 
...
```{r getting-data-from-th-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-8-hint, eval = FALSE}
...
```{r getting-data-from-th-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-9-hint, eval = FALSE}
get_acs(
  geography = "...",
  state = "...",
  ... = c(medinc = "...",
...
```{r getting-data-from-th-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-10-hint, eval = FALSE}
get_estimates(
  geography = "...",
  product = "...",
  ... = "...",
...
```{r getting-data-from-th-11, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r getting-data-from-th-11-hint, eval = FALSE}
...
Census and ACS data are associated with geographies, which are units at which the data is aggregated. These geographies are in [US Census Bureau's TIGER/Line database](https://www2.census.gov/geo/pdfs/maps-data/data/tiger/tgrshp2019/TGRSHP2019_TechDoc.pdf). 

###

Working with spatial datasets from this database is quite complex; fortunately, R offers the `tigris` package which does a lot of the work for us. 

###

**When using tigris, use `options(tigris_use_cache = TRUE)` so it doens't take forever to download data.** Don't worry about it in this tutorial, but keep this in mind when working on your own. 

Let's explore the basic functions of `tigris`.
...
```{r census-geographic-da-1-hint, eval = FALSE}
... <- states()
class(...)
```
...
```{r census-geographic-da-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r census-geographic-da-2-hint, eval = FALSE}
plot(st$...)
```
...
```{r census-geographic-da-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r census-geographic-da-3-hint, eval = FALSE}
nm_counties <- ...("NM")
plot(...$geometry)
```
...
```{r census-geographic-da-4-hint, eval = FALSE}
...
```{r census-geographic-da-5-hint, eval = FALSE}
dc_roads <- ...("DC")
plot(...)
```
...
```{r census-geographic-da-6-hint, eval = FALSE}
...
```{r census-geographic-da-7-hint, eval = FALSE}
la_tracts <- tracts("NM", "Los Alamos")
ggplot(...) + 
  geom_sf()
```
...
```{r census-geographic-da-8-hint, eval = FALSE}
... +
  theme_void()
```
...
```{r census-geographic-da-9-hint, eval = FALSE}
la_tracts <- tracts("NM", "Los Alamos")
mapview(...)
```
...
```{r census-geographic-da-10-hint, eval = FALSE}
yearly_plots <- map(seq(1990, 2020, 10), ~{
  year_tracts <- tracts("TX", "Tarrant", year = .x,
                        cb = TRUE)
...
```{r census-geographic-da-11-hint, eval = FALSE}
... 
(yearly_plots[[1]] + ...[[2]]) / 
  (yearly_plots[[...]] + ...[[...]])
I sense a pattern...
...
```{r census-geographic-da-12-hint, eval = FALSE}
...
```{r census-geographic-da-13-hint, eval = FALSE}
... + 
  coord_sf(crs = ...)
```
...
```{r census-geographic-da-14-hint, eval = FALSE}
... + 
  coord_sf(... = ..., datum = ...)
```
...
```{r census-geographic-da-15-hint, eval = FALSE}
us_states <- states(cb = ...)
ggplot(...) + 
  geom_sf() +
  theme_void()
...
```{r census-geographic-da-16-hint, eval = FALSE}
... + 
  coord_sf(crs = "...")
```
...
```{r census-geographic-da-17-hint, eval = FALSE}
us_states <- states(cb = TRUE)
...
```{r census-geographic-da-18-hint, eval = FALSE}
us_states <- states(cb = TRUE)
us_states_shifted <- shift_geometry(...)
ggplot(...) +
  geom_sf() +
...
```{r census-geographic-da-19-hint, eval = FALSE}
...
```{r census-geographic-da-20-hint, eval = FALSE}
...
...
**Whenever you use tidycensus to map data, make to load the library and type the following command: `options(tigris_use_cache = TRUE)`**. For this tutorial, we have already done that for you. 
...
```{r mapping-census-data-1-hint, eval = FALSE}
dc_income <- get_acs(
  geography = "...", 
  variables = "...",
  ... = "...", 
...
```{r mapping-census-data-2-hint, eval = FALSE}
plot(dc_income["..."])
```
...
```{r mapping-census-data-3-hint, eval = FALSE}
us_median_age <- get_acs(
  geography = "...",
  variables = "...",
  ... = 2019,
...
```{r mapping-census-data-4-hint, eval = FALSE}
us_median_age <- get_acs(
  geography = "...",
  variables = "...",
  ... = 2019,
...
```{r mapping-census-data-5-hint, eval = FALSE}
...
```{r mapping-census-data-6-hint, eval = FALSE}
... +
...
```{r mapping-census-data-7-hint, eval = FALSE}
... +
  labs(title = "...",
       caption = "...",
       fill = "ACS estimate") + 
...
```{r mapping-census-data-8-hint, eval = FALSE}
...
```{r mapping-census-data-9-hint, eval = FALSE}
hennepin_black <- hennepin_race |> 
  filter(variable == "...")
```
...
```{r mapping-census-data-10-hint, eval = FALSE}
tm_shape(...) + 
  tm_polygons()
```
...
```{r mapping-census-data-11-hint, eval = FALSE}
... + 
  ...(col = "...")
```
...
```{r mapping-census-data-12-hint, eval = FALSE}
... + 
  ...(col = "...",
          style = "...",
          ... = 5,
...
```{r mapping-census-data-13-hint, eval = FALSE}
... + 
  tm_layout(title = "...",
            frame = ...,
            legend.outside = ...)
...
```{r mapping-census-data-14-hint, eval = FALSE}
...(hennepin_race) + 
...
```{r mapping-census-data-15-hint, eval = FALSE}
... + 
  tm_fill(... = "percent",
          style = "...",
          ... = 6,
...
```{r mapping-census-data-16-hint, eval = FALSE}
tm_layout(... = "grey", 
            ... = c(-..., ...),
...
```{r mapping-census-data-17-hint, eval = FALSE}
...
mapview(..., zcol = "...")
```
...
```{r mapping-census-data-18-hint, eval = FALSE}
tmap_mode("...")
tm_shape(...)
```
...
```{r mapping-census-data-19-hint, eval = FALSE}
...
... + 
  tm_fill(col = "...",
          palette = "...",
...
```{r mapping-census-data-20-hint, eval = FALSE}
...
```{r mapping-census-data-21-hint, eval = FALSE}
leaflet() |> 
...
```{r mapping-census-data-22-hint, eval = FALSE}
... |> 
  addPolygons(data = ...,
              ... = ~pal(estimate),
              weight = ...,
...
```{r mapping-census-data-23-hint, eval = FALSE}
... |> 
  addLegend(
    ... = "bottomright",
    pal = pal,
