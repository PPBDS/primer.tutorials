...
Use `ggplot()` and `geom_jitter()` to make a jittered scatterplot. Map `weight` to the x-axis, `height` to the y-axis, and `bmi` to `color`.
...
Add a trend line with `geom_smooth()`. Set the argument `se` to FALSE and set `method` to "gam".
...
`filter()` `kenya` to only the rows with a `treatment` of "control," "local," "SMS," or "canvass." Use the `%in% `operator with `c()` to do so.
...
Continue your code and `select()` the `treatment`, `mean_age`, and `reg_byrv13` columns. 
...
Use `%>% `to add  `ggplot()`.  Use`fct_reorder()` to reorder the `treatment` variable by `mean_turnout` and map this to the x-axis. Also, map `treatment` to the y-axis.
...
Now let's practice using summary statistics with our data. Take the `mean()` of the `hours` column. Use the `$` operator to extract the `hours` column.
