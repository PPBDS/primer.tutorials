...
Run `c(min(qscores$enrollment), mean(qscore$enrollment), max(qscores$enrollment))` to produce a vector with the minimum, mean and maximum enrollment value in this data. Run the equivalent command for `rating`.
...
Continue your pipe with `ggplot()`. Map `age` to the x-axis, and `introduction` to `fill`. Add the layer `geom_histogram()`.
...
We have provided you with the beginning of code for the last plot. Continue your pipe with `ggplot()`.. Map `rating` to the x-axis, `enrollment_groups` to the y-axis, `introduction` to `fill`, **and** `introduction` to **color**. Add the layer `geom_density()`. Within `geom_density()`, set `alpha` equal to 0.5.
...
Run `c(min(qscores$poverty), mean(qscore$poverty), max(qscores$poverty))` to produce a vector with the minimum, mean, and maximum poverty rate in this data. Run the equivalent function for `mean_age`.
...
... %>% 
mutate(enrollment_groups = case_when(poverty >= .18 & poverty < .35 ~ "1",
            ...)
```
...
... %>% 
 mutate(enrollment_groups = factor(enrollment_groups,
                                   levels = c("16-30", "30-50", "50-100", "100+"), 
                                   labels = c("1st Quartile (Least Poverty)", "2nd Quartile",
                                              "3 Quartile", "4th Quartile (Most Poverty)")))
```
...
Add the layer `facet_wrap()` to facet the histogram by `mean_age()`. 
...
kenya_p2 <- kenya %>% 
  ...
  geom_histogram(bins = 500, position = "stack") +
  facet_wrap(~ poverty_groups) +
  coord_cartesian(xlim = c(0, .15), ylim = c(0, 160)) +
  scale_x_continuous(labels = scales::percent_format()) +
  scale_fill_manual(values = c("#F8766D", "#00BFC4"), 
                    labels = c("Below 50th Percentile", "Above 50th Percentile")) +
  theme_classic() +
  labs(title = "Distribution of New Registered Voters by Poverty and Age Groups",
       subtitle = "The poorest districts have fewer zero and near-zero percent \n increases and more districts of unusually high increase",
       x = "Percent of New Registered Voters", 
...
Add the layer `facet_wrap()` to facet the histogram by `mean_age()`. Add the layer `coord_cartesian` and use the argument `xlim` to limit the x-axis domain to a minimum of 0 and a maximum of .25. 
...
We have provided you with the beginning of code for the last plot. However, because we are no longer faceting, we have removed the lines where we mutated `enrollment_groups()` into a factor. 

Map `reg_byrv13` to the x-axis, and `age_groups` to `fill`.Continue your pipe with `ggplot()`. Add the layer `geom_density()`.  Within `geom_density()`, set `alpha` equal to 0.5 **and** set `color` = `NA`.

```{r kenya17, exercise = TRUE}
 qscores %>% 
  mutate(enrollment_groups = 
           case_when(
            enrollment >= 16 & enrollment < 30  ~ "16-30",
            enrollment >= 30 & enrollment < 50  ~ "30-50",
            enrollment >= 50 & enrollment < 100 ~ "50-100",
            enrollment >= 100 ~ "100+")) %>% 
  mutate(enrollment_groups = 
           factor(enrollment_groups, 
                  levels = c("16-30", "30-50", 
                             "50-100", "100+"))) %>% 
  mutate(introduction = str_detect(name, "Introduction")) %>% 
  select(rating, enrollment_groups, introduction)
```
...
Use `limits` and `labels` in `scale_y_discrete()` to reverse the order and change the names from `1` and `2` as we have done above. The syntax is identical to the previous exercise, except that `scale_y_discrete()` does not require the `values` argument. 
