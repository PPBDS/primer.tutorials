...
Add the layer `scale_x_continuous` and set the argument `labels` equal to `scales::percent_format()`. Then, add the layer `scale_fill_manual`. Set `values` equal to `c("#F8766D", "#00BFC4")`, the default red and blue in ggplot and set `labels` equal to a vector of the new names, `"Younger Half"` and `"Older Half"`.
