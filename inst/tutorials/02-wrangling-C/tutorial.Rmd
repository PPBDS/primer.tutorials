...
Calculate the 95th confidence interval of `qscores$hours` using the `c()` function. The first argument to `c()` should take the `mean()` and add it to two times the value of `sd()`.  The second argument to `c()` should take the `mean()`of  and subtract the value of 2 times the `sd()`.

```{r qscores22, exercise = TRUE}

```

```{r qscores22-hint, eval = FALSE}
c(mean(qscores$hours) + 2 * sd(qscores$hours), 
  mean(...) - 2 * sd(...))
```

### Exercise 3

Now that we have explored our data a little, let's build our plot. Start a pipe with `qscores` and `select()` the `department` and `hours` columns.
