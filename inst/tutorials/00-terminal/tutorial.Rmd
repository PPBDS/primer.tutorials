...
Let's figure out where we are. To see your current location, type the command `pwd` (**p**resent **w**orking **d**irectory) in the Terminal. Hitting return runs the command. Copy/paste your work.
...
Run `ls` (list) to see all the items that are located in the directory in which you are currently located. 
...
Let's make a folder called `my-class`. Run command `mkdir my-class`. Then run `ls` to see your newly created folder. 
...
Run the command `cd my-class` in order to **c**hange **d**irectories into the directory which we just created. There should not be anything in this directory. Type `ls` to be sure. Notice the change to the prompt after changing directories.
...
Notice that the prompt has shortened to only the name of the user and the $. The home directory always ends  the prompt.
