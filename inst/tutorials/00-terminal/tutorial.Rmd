...
<!-- Teach a couple of simple uses of grep for looking for something in the AUTHORS file. -->
...
By default, the files pane (bottom right corner) in RStudio shows the contents of this directory. Towards the top, you will see a path of directories on your computer starting from a home icon. This should match the latter part of the path generated by `pwd`. 
...
Use `mkdir` to make a directory within `my-class` called `lessons`. 
...
The file is only named it as if it were a picture. `touch` still creates a text file. 
...
<!-- Should be two exercises  -->

Let's change our working directory to `lessons`. Run `cd ..` to go to the folder immediately above the working directory. Use `pwd` to confirm that you are in the right directory. Create a folder called `tbd`.
