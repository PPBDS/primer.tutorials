...
Use `touch` to make a text file inside `fruits`, named `apple.txt`.
...
Run `ls` to check if the file is inside `options`. If you did the previous exercise correctly, you should **not** see the new file. This is because we prefixed the name with  `.`,  which hid the file from normal view.  Run `ls -a my-class`. Using the **option** `-a`, you should be able to see **a**ll the files in `my-class`.
...
Let's make a copy of the `AUTHORS` file from `$R_HOME/doc` in `options`. Use the command `cp`, whose syntax is identical to `mv`, and the `.` shorthand. 
...
Use `tails` and `>>` to append the last 11 lines of `AUTHORS` to `AUTHORS-fake`. Refer to Exercise 18 if needed.
...
Wildcards can be used with more than `ls`. Let's  move hat.txt and cat.txt together into one directory. Make a directory called `suess`. Run `mv *t.* suess`
...
Let's search the text file for lines containing the letter "a". Run `grep a afiles.txt`
...
Let's search through the text file for lines that begin with the letter "a". Run `grep ^a afiles.txt`
...
Use `ls` and the environmental variable `$R_HOME` to make a list of the contents of `$R_HOME` and use `>` to save this to a new file named `rfiles.txt`. Use `cat` to print this file. 
...
Use `grep` to search `rfiles.txt` for lines that contain `r`
...
Use `grep` and `^` to search `rfiles.txt` for lines that begin with `R`
...
Use `grep` and `$` to search `rfiles.txt` for lines that end with `c`. Remember that `$` comes at the end. 
...
Use `grep` to search `rfiles.txt` for lines than contain the string `ib`. 
