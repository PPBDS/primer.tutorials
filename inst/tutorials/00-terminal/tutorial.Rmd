...
Use `cd` to change back to `options` in your default working directory. Use the absolute path from Exercise 1 if you need it.
...
Try to use `cp` to copy the `doc` directory into `options`. You will get an error informing you that the thing you tried to copy was a directory. Run the same command using the option `-r`.
...
Let's view just the initial lines of the `AUTHORS`. Use the function `head` to view the first ten lines of the file. 
...
Use `rm -r` to remove the `options` directory. 
...
Let's read the file `AUTHORS`. First, make sure this is in the working directory. Then, run `cat AUTHORS`. 
...
<!-- Change to be consistent with introductory style. Remove preface -->

### Exercise 2

Let's view just the initial lines of the file. `cat` prints the whole text file in the Terminal. This is often not the most convenient way of viewing. Use the function `head` to view the first ten lines of the file. 
