...
Let's create the following posterior probability distribution for $\beta_0$ and for $\beta_0 + \beta_1 $.

<!-- BG: Although this is how it is in the book, should I replace this with using newobs/ posterior_epred() approach? -->
...
```{r jc-ex-9-hint-2, eval = FALSE }
Remember to place backticks outside the parentheses that enclose Intercept (i.e. `(Intercept)`)
```

### Exercise 10

Pipe your results to `pivot_longer()`. Set `cols` to `female_years` and `male_years`. (Remember to insert a colon between them.) `names_to` should be set to "parameters" and `values_to` should be set to "years".
...
Pipe the results above to `ggplot()`. Map `years` to the x-axis, and map `parameters` to the  `fill` aesthetic. Use `geom_histogram()` with the same tricks we use in the chapter: `after_stat()`, `bins`, `alpha` and `position`.
...
Use `scale_x_continuous()` to put the x-axis in number format. Within `scale_x_continuous()`, set `labels` to `scales::number_format()`. Within `number_format()` set `accuracy` to 1.


```{r jc-ex-13, eval = FALSE}
...
In our last section, we used an “on average” interpretation of the question: *How would the lived_after values vary between the female candidate and the male candidate, both 5 years older than the average age of candidates?* 
