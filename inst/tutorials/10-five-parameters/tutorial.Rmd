...
Use `posterior_predict()`. The first argument to `posterior_predict()` should be `gov_2`. Also set `newdata` to `newobs`.
...
posterior_predict(gov_2, 
                  newdata = newobs)
```

### Exercise 3

Pipe the results of `posterior_predict()` to `as_tibble()`. Also use `rename()` to rename the column "Male" as `1` and the column "Female" as `2`. Save you results to an object named `pe`.
