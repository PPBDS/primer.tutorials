...
description: "Chapter 7 tutorial part A"
---
...
```{r preliminaries-ex-1-hint-1, eval = FALSE}
ts %>% 
  rowwise() %>% 
  mutate(avg = ...)
```

```{r preliminaries-ex-1-hint-2, eval = FALSE}
ts %>% 
  rowwise() %>% 
  mutate(avg = mean(c_across(...)))
```

```{r preliminaries-ex-1-hint-3, eval = FALSE}
ts %>% 
  rowwise() %>% 
...
```{r courage-ex-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r courage-ex-2-hint, eval = FALSE}
nhanes %>% 
  filter(...) %>% 
  select(...) %>% 
  drop_na() -> ch7_female
...
<!-- GP: I don't even know the answer to this myself. Check out chapter 2 Distributions. -->
...
```{r mean}
question_text(NULL,
	message = "The median is less affected by outliers; it is a more stable measure of the center of the distribution.",
	answer(NULL,
	correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
...
<!-- GP: Note to self, Temperance section may need some reordering in order to better communicate information. -->
