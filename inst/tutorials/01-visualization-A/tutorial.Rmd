...
Data frames can be created using `tibble()`. Assemble the vectors below into a data frame using `tibble()`with the column names `numbers`, `logicals`, `strings`. Assign the data frame to the object named `data_f`.
...
```{r lists4-hint, eval = FALSE}
Remember to use the assignment operator "<-" when assigning values to an object.
```
...
Let's extract the strings column of the `data_f` data frame using the `$` operator. See the **The Primer** for guidance. 
