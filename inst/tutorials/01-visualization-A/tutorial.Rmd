...
Before you start coding, you must load the packages you are going to using In the code chunk below, use the function `library()` to load the `tidyverse` package.
...
Because the names are characters, make sure to use quotation marks inside the brackets such as vec2["..."]
```
...
R comes with many toy **data sets** to explore. You can simply examine datasets by typing their name. Let's examine the contents of `iris` data set. Type `iris` in the line below.
...
Type iris inside the glimpse() function.
```
...
Within `geom_jitter()` set the `width` to `.2` to adjust the amount the points are jittered.
...
Use the dataset `cces` to make a density plot with `age` on the x axis.
...
Great! Now set the `alpha` parameter in `geom_density()` to 0.1.
