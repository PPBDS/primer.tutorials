...
Now use `labs()` to add a title and axis labels to the graph. Set the `title` to `"Measurements for Different Species of Iris"` and the `subtitle` to `"Virginica has the longest sepals"`. To label the the axes, set `x` to  `"Sepal.Length"` and `y` to `"Sepal.Width"`. Set the `caption` to `"Fisher (1936)"`.  
...
One method to fight overplotting is to make each point **semi-transparent**. The code chunk below provides the code used to create the graph above. Change the transparency of the points by setting `alpha` equal to 0.2 within `geom_point()`.
...
Change the color of the bars by setting `fill` equal to `"red"` within `geom_histogram()`. 
...
Notice how the values of `count` increased when we decreased the number of `bins`. This is because, more ratings are included in a single bin (e.g. ratings of 4.0-4.9 all included in the fourth bin). 
...
By default `position` is set to `"stacked"`. `"dodge"` puts different colors side by side. 
...
ggplot(data = trains, 
  ...
  geom_jitter(width = .2)  +
    geom_smooth(method = "lm", formula = y ~ x) +
  labs(title = "Starting and Ending Attitudes of Participants", 
       subtitle = "Most partipants had similar starting and ending attitudes",
       x = "Starting Attitude", 
       y = "Ending Attitude", 
       caption = "Enos (2014)")
```
