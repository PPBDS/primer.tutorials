...
Let's save the results of `rnorm(100, mean = 100, sd = 15)` to an object named `results`. Remember to use the assignment operator `<-`. See **The Primer** for guidance. 
...
Use the funciton c(...) and place the values inside.
```
...
Use `glimpse()` to look at the `trains` data set. We already loaded the `tidyverse` and `PPBDS.data` packages.
...
Data frames can be created using `tibble()`. Assemble the vectors below into a data frame using `tibble()`with the column names `numbers`, `logicals`, `strings`.
...
As you can see, when we use the `tibble()` function, we can see the tibble. However, when we assign the tibble to an object named `data_f`, we do not see the actual tibble. We are just running the code to assign the values. 

Run the code below. Notice there is nothing that prints out for us to see.

```{r diff-tibble, exercise = TRUE}
data_f <- tibble(numbers, logicals, strings)
```

### Exercise 4

After you assign values to an object, you must type the name of the object to see results. Now run the code below. Notice that we can now see results.
...
Let's now extract the `strings` column of the `data_f` data frame using the `$` operator. See the **The Primer** for guidance. 
...
Load the `PPBDS.data` package. Use `view()` to look at the `qscores` data set.
...
Nice! Now load the `ggplot2` package. On the line below, use the `ggplot()` function to create a scatterplot using the `qscores` data set. Map `rating` to the x-axis and `hours` to the y-axis. Add the layer `geom_point()`. Remember when you add a layer you need to include `+`. 
...
The following scatterplot, which was created using the `mpg` tibble, has overplotting.
...
Let's fix the overplotting issue. The code for the graph above has been provided for you below. Replace `geom_point()` with `geom_jitter()`. 
