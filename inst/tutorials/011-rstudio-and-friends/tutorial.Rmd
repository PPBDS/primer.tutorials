...
<!-- Do not waste time fussing with images. DONE - just went with what looked correct on my screen. -->

<!-- DK: File names use lower case and underscores. DONE -->
...
<!-- Ask on RStudio Community. Send me the link when you do. Is there a way to set up this default, like we do with blank_slate(). Put that in Getting Started tutorial. I'm going with the getwd() perspective since this is weird. I can't find the directory preference that it puts projects in. DONE -->

<!-- Case studies provide occasions for teaching. Note how I remove all the YAML info except the document type when I am planning to publish on RPubs because that stuff is ugly. Explain what this does. DONE -->
...
<!-- Add a case study which teaches about inline R code. Maybe integrate into R markdown session. Also teach r scales::comma(x) DONE -->
...
<!-- AG: I'm assuming that it's ok to ask them for their PAT as it was done in another tutorial. -->
...
<!-- You can't use a console command here as it cannot open the connection. -->

Run `cat "$HOME/.Renviron"` in the Terminal. Copy-paste the command and the output in the space below.
...
This should return the contents of your .Renviron file. Packages like `usethis` will read your .Renviron file and access the `GITHUB_PAT` variable, so make sure that you use the correct formatting.
...
<!-- AG: Are there restrictions with the length of the response? This could be pretty long so... DONE -->
...
<!-- AG: I'm not sure whether there should be a full case study for inline code as it's inline with the R Markdown files. DONE -->
...
<!-- AG: Add the YAML modification part somewhere in here. It's useful, I just need to change the images to make it fit somewhere. DONE -->
...
<!-- AG: I'm contemplating relocating this section to the GitHub section since that area actually deals with GitHub. Not at this stage though since the current structure only has 1 case study. DONE - It's staying here as it acts as the publishing method for R scripts. -->
...
<!-- AG: You may need to use the GITHUB_PAT envvar to use gistr. It's recommended in the documentation at least. Please figure this out. Probably use GITHUB_PAT. DONE - see Connecting to GitHub section -->
...
<!-- AG: I'm not elaborating because it feels too long and we haven't even explained how to fork and everything in a normal repository, nevermind why a gist is different. Adding final GitHub repo to resolve these issues -->
...
<!-- AG: Just wanted to add this since it distinguishes the gist from a github repo. The transition sucks but it's still an extremely common use of gists. DONE - fixed transition -->
