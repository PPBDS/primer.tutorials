...
###
`range()` eturns a *vector* containing the minimum and maximum of all the given arguments.
...
###
Note that the x value inside `mean()` ,`median()` and `sd()` has to ba a numeric vector, not a factor or logical. 

### Exercise 5

Within the `summarise()`, add a new argument seperated by comma, `med_dev`, and set mad equal to the standard deviation of `numbers` using `mad()`. 
...
Let's simulate a flipping a coin, use `sample()`, within `sample()`, set the first argument to 0 and 1, where 0 represent tails and 1 represent heads, second argument to repeat 100 times, and third argument set `replace` ` equals `TRUE`, so that we can generate different values.
...
After finish this tutorial you should be able to 
