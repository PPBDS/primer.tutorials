...
The `scale_fill_viridis_c()` function is extremely versatile for when you want to provide map fill colors. This has a few different color palettes that can be selected with the option argument, all of which are easily distinguishable both when displayed in black and white and for people with common forms of colorblindness. You can also reverse the default order of the colors with the direction = -1 option. This function is for continuous variables such as prop_rural; if you have a discrete variable, you can use the analogous scale_fill_viridis_d().
...
Start a new R code chunk and copy the previous code, including the part where we defined `rural`. Add the argument `shift_geo = TRUE` to the `get_decennial()` function and rename the new `rural` to `rural_shifted`.
...
Finally, change the map to use `rural_shifted` instead of `rural`. It should look like this:
...
This option removes Puerto Rico from your tibble altogether, so it is not a good choice if you want to show data from Puerto Rico.
...
Use the `get_acs()` function in a new R code chunk, setting the `geography` arguments to `"tract"`, the `variables` argument to the vector you created in the last exercise, the `year` to `2018`, the `state` to the state that you live in, the `county` to your county, the `geometry` to TRUE, and `summary_var` to `"B02001_001"`, the total population of your county.
