...
<!-- Create setup_script.R which creates all the objects which the tutorial loads. DONE (see below) -->

<!-- AG: I thought that it would be better to split the code into different files and then save it in another folder. That way it's all split up and you can run the different files separately without having to load everything every time that you make a small change to one graph. This is especially important for the big graphs as they take years to finish, so running the setup file ends up being very annoying. --> 

<!-- Perhaps of interest: https://milospopovic.net/how-to-make-choropleth-map-in-r/ -->

<!-- Also: https://github.com/r-spatial/mapview -->

<!-- AG: As for publishing, I'm going with the method of creating one page for every graph in the section plus a home page. This allows us to publish everything rather than one single graph which is I feel more rewarding since there's less wasted effort. It's not hard anyways and it teaches them about modifying _site.yml more. -->
...
<!-- This exercise prevents the home page from being completely blank. -->
...
<!-- AG: Should we walk them through finding the variables again? I will, but that's a final cleanup thing and not necessary as of right now. -->
...
<!-- AG: Personal note - Change this so that it's an R script and RDS rather than an R Markdown, it's necessary for something this big. It'll also act as a good way for people to learn about using LFS for Git. -->
...
<!-- Address to lat long convertor found here: https://www.census.gov/programs-surveys/geography/technical-documentation/complete-technical-documentation/census-geocoder.html may be a good idea to download a bunch of addresses and then map them. Library found here: https://cran.r-project.org/web/packages/tidygeocoder/tidygeocoder.pdf -->

<!-- The plan is map Mcdonalds locations vs avg income by county and see how they differ. Or something similar to show how there are a bunch of food deserts based on income, forcing people to eat unhealthy foods. It's an interesting concept that I feel suits a final project.-->

## Publishing

<!-- This section was created by following the processes described here: https://bookdown.org/yihui/rmarkdown/rmarkdown-site.html#fig:site-build and here: https://resources.github.com/whitepapers/github-and-rstudio/ -->

We're going to publish this file to a GitHub Pages website so that we can publish everything on different pages.

<!-- AG: Is this tutorial before or after the Distill tutorial? -->
...
<!-- AG: Not sure if this is exactly the most professional way to read the build type, but it's the closest I can find and it's guaranteed to work. -->
...
<!-- AG: I'm wondering if I should walk them through the advanced YAML stuff like bibliographies. It's not totally necessary (at least without IPUMS which requires you to use a bibliography), but it may be nice. -->
...
<!-- I can't explain indentation, needs refining. -->
...
This isn't the limit of YAML files. Visit one of the websites below to learn more about publishing websites and using YAML files.
...
It's time to publish our website to GitHub pages.
