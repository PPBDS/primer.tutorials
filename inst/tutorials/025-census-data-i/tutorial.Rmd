...
We will be using `geom_pointrange`, which will show us the margin of error and the mean household income.
...
We will be using the *tigris* package
...
Run `usethis::edit_r_profile()` in the Console. Then put `options(tigris_use_cache = TRUE)` in your `.Rprofile` file. Restart R for the changes to take effect. (The tutorial may be terminated, once R is restarted, just come right back to the tutorial, it should save your spot.)
...
<!-- MH: Just have to update rds, everything should work as normal (also do same layout as visualization tutorials: This is the thing we are making, step 1, step 2, etc.) -->
