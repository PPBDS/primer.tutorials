...
To start, create a new subdirectory inside your regular R project folder for this class named `census_tutorial`. All files made in this tutorial should be placed in this subdirectory.
...
**This will come back, keep it in the back of your mind.**
...
Recall that in the previous section, we said: **"This will come back, keep it in the back of your mind."** In this section, we are going to learn what a margin of error (moe) is, and how we can analyze and visualize it.
...
Copy/paste your code from above. Add the `moe_level` argument and set it to `99`.
...
```{r margin-of-error-4-hint, eval = FALSE}
...
We can error bars for the margin of error by using `geom_errorbar()`.
...
Add the `geom_ribbon` layer. Inside `aes()` set `xmin` to `estimate - moe` and `xmax` to `estimate + moe`.
...
... + 
  geom_ribbon(aes(xmin = estimate - moe,
                  xmax = estimate + moe))
```
...
```{r}
include_graphics("images/moe_ot_plt.png")
```
...
Create a new R Markdown file titled `tworaces.Rmd` in your `census_tutorial` subdirectory and delete all of the text except for the setup chunk and the YAML header. Then, load `tidyverse` and `tidycensus`.
...
You can see what the variable refers to by looking at the `label` column. 
...
The variable `B02001_001` has a label of `Estimate!!Total`, showing how it represents the total estimated population.
...
In this chunk, create a new variable called `continental` that contains the value `state.name[! state.name %in% c("Alaska", "Hawaii")]`.
...
The `state.name` variable is built into R, and it includes the name of every US state. In this case, we're filtering out Alaska and Hawaii so that they aren't included in our search. 
...
Run `get_acs()`  using the `continental` vector we created earlier as the `state` argument and the Census variable that described 2 races as the `variables` argument. Additionally, since we're looking at the entire country, remove the `county` argument. Save this to a variable named `races`.
...
Much like before, this changes the data into a percentage that lets us clearly distinguish between the different data points. **Percentages are sometimes more useful than real numbers.**

### Exercise 6

Continue the pipe and create a map using the `ggplot()` setting `fill` to `Percent`. Add the `geom_sf()` layer.
### 
...
By setting size to 0.003, we can create thin outlines around our census tracts. This makes it easier to see individual tracts.
...
You may have noticed how applying `theme_void()` gets rid of the x and y axes. 

This is especially important for maps because the x and y axes represent latitude and longitude lines, which aren't very useful when you're trying to create infographics and don't need the exact coordinates of a location.
...
Let's save our graph to an RDS file. Save your graph to a variable, then write that variable to an RDS file named `races_map.rds` using the function `write_rds()`.
...
This saves our map as an RDS file for future use. 

Keep in mind that RDS files are unique to R and can't be distributed to other people. If you want to save the graph and send it to somebody else, try saving it as an image using the `ggsave()` function.

### Exercise 11

While we did save our R object, we never actually use it.
