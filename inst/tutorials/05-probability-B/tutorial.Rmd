...
# ...%>%
# mutate(tt= map_dbl(tt, ~ tt))
```


```{r sz-9-hint-2, eval = FALSE}
# ...%>%
# mutate(...= map_dbl(N, ~sample(c(...: tt), size = 1)))
```
...
# ...%>%
# mutate(...= map_dbl(N, ~sample(c(tt:tt), size = 1)))
```
...
# ...+
# geom_histogram(binwidth = tt, color = "tt") +
#   ...
```
...
tt+
scale_x_continuous(breaks = seq(tt,tt, tt), labels = tt:tt) 
```
...
# ...+ 
#   scale_y_continuous(labels = scales::percent_format(tt))
```
