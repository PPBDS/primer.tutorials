...
<!-- DK: Split this into 3 or 4 separate questions. And then, ask this question. Firts, question has answer of rep(10, 3). Second question has answer of tibble(N = rep(10, 3)). Third question has answer of tibble(N = c(rep(10, 3), rep(500, 2))). Fourth question has answer  tibble(N = c(rep(10, 3), rep(500, 2))) %>%  mutate(result = x) -->
...
x %>%
mutate(x= map_dbl(x, ~ x))
```


```{r sz-9-hint-2, eval = FALSE}
x %>%
mutate(x = map_dbl(N, ~sample(c(x:x), size = 1)))
```
...
x +
geom_histogram(binwidth = x, color = "x") +
  x
```
...
x +
scale_x_continuous(breaks = seq(x,x, x), labels = x:x) 
```
...
x + 
  scale_y_continuous(labels = scales::percent_format(x))
```
