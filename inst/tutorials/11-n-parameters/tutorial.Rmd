...
Now write two sentences, using your own words, that explains how you calculate the percent voting in the Neighbors group.
...
Look at the results above. Write two sentences, using your own words, that explain the meaning of the value of `soloTRUE:treatmentNeighbors`.
...
Let's create the following posterior distribution for the likeliness of voting for solo households.
...
Continue the pipe and use `mutate()` to create the variable `Control`, which should be equal to the following argument: `soloTRUE`. Make sure you place backticks around "soloTRUE".
...
Continue the pipe and use `mutate()` to create the variable `Neighbors`, which should be equal to the following argument: `soloTRUE` + `soloTRUE:treatmentNeighbors`. Make sure you place backticks around "soloTRUE" and "treatmentNeighbors".
...
temp_p <- posterior_epred(sham_2,
 ...
 separate(Group, c("Solo", "Age"), "_") %>% 
  ...
  labs(title = "Treatment Effects of Different Post Cards on Various Types of People",
       subtitle = "People who do not live alone are more likely to vote",
       x = "Treatment Effect",
       y = "Age")
...
Great. The function `expand_grid()` allows us to create a tibble from all combinations of inputs, which is our goal. Use `expand_grid()` with the arguments `treatment`, `solo`, and `age`. 
...
Pipe the results of `expand_grid()` to `mutate()`and set `names` to the function `paste()`. Within `paste()` use the arguments `treatment`, `solo`, and `age`. Also, set `sep` to "_". Save your code to an object named `newobs`.
...
Note: We use back ticks with `not-solo_60` because the hyphen - is an irregular character so we need to tell R to ignore it.
```
