...
Remembder you are adding a layer here so use +
```
...
Recall that you are adding a layer here!
```
...
Use the data set `trains`. Put `att_start` on the x axis and `att_end` on the y axis. Use `geom_smooth()`, and set its argument `method` to`"lm"`. and set `se` to FALSE.
...
Use the `qscores` tibble. Put `rating` on the x-axis and `number` on the y-axis. 
...
Also set the `fill` argument to `cut`. 
