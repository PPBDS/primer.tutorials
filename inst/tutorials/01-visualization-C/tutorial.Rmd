...
Continue your pipe using `%>%` and select the columns `height` and `weight`. 
...
Extend your pipe using `%>%` and add `ggplot()`. Map `weight` to the x-axis axis and `height` to the y-axis. Also, add the layer `geom_point()`.
...
Continue your pipe and add `ggplot()`. Map `price` to the x-axis, and `carat` to the y-axis. Then, add the layer `geom_jitter()`.
...
Notice that there are only two different years, 1998 and 2008.
...
Start a new pipe with `ggplot()`. Map the variable `race` to the x-axis. Also add the layer `geom_bar()`.
...
Select the columns `att_end` and `att_start`. 
...
Let's add a theme to our graph. Add the layer `theme_fivethiryeight()` to your code. 
...
To finish your plot, use `labs()` to give the graph a title, subtitle, axis labels, legned labels, and caption of your choosing.
...
Set the `alpha` argument to .5 within `geom_density()` 
...
Arrange the tibble by `congress` in descending order to find the number of the most recent congress in the data set. 
...
Drop any rows with `N/A` values in the `nominate_dim_1` and `nominate_dim_2` columns. 
...
Select the columns `nominate_dim_1`, `nominate_dim_2`, `party`, `name`, `state`, and `chamber`. 
...
Use `ggplot()` and `aes()` to map `nominate_dim_1` to x and `nominate_dim_2` to `y`. Add the layer `geom_point()`. 
...
Within `aes()` in `ggplot()` add the argument `text` and set this equal to the function `paste`. 
...
Use `labs()` to add a title, axes labels, legend labels, and a caption to your plot. `plotly()` does not support subtitles. 
...
Copy/paste the code from above. On a new line not piped to the previous object, type the function `ggplotly()`. The first argument should be the name of the ggplot object, `nominate_p`. Make the second argument `tooltip = "text"`. This tells `ggplotly()` to make the label based on what we have in the `text` argument in `ggplot()`.
