...
In this section, we will mimic physical sampling with virtual sampling, using a computer. In a real sampling activity, we would perform tactile sampling. In this we use a physical urn of beads and a physical shovel which is done by hand so that we could develop our intuition about the ideas behind sampling.
...
Now pipe your results to `mutate()` to create the variable `shovel`, which is set to `map()`. The first argument to `map()` should be `trial_ID`. The second argument should use `slice_sample()`. Within `slice_sample()`, the first argument should be `urn` (the data we want to sample from), and then set `n` equal to our sample size (we want a sample size of 50).
...
Within `geom_histogram()` set `binwidth` to .01, `boundary` to .4, and `color` to "white".
...
Continue your pipe with `mutate()` to create the variable `numb_red`, which will tells us the number of red beads present. Set `numb_red` equal to the function `map_int()`. The first argument to `map_int()` should be `shovel`. The second argument should take the `sum()` of where the column `color` of `shovel` is equal to red.
...
<!--# TJ: is it possible to explain this better? -->
...
<!--# TJ: I feel we should have the definitions before the question itself to help out the student -->
...
In two sentences, provide a reason why the assumption of stability might not hold. (Your counter-example can be far-fetched.)
