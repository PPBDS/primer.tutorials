...
# DK: Write about what this is and why it is here.
...
<!-- Look through entire tutorial and add knowledge drops and fix exercises based on setup tutorial. Done -->
...
The argument `skip` is used to skip rows, but to skip columns, you can't set `col_types` to `col_only` to only read the specified columns you want. 
...
Write code to make sure the column `grade` appears as an integer variable (`<int>`), and `student` as a character variable (`<chr>`).
...
Now read the file `"data/test_bad_names.csv"` file using `read_csv()`. Then pipe it `clean_names()` which is a function from the **janitor** package.
...
Write code for reading the second sheet of the file into R by setting the `sheet` argument to "sheet 2" in `read_excel()`.
