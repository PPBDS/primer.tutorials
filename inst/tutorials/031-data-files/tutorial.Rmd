...
<!-- AG: I fixed the typos and made a few of the current lines more clear, but I didn't fix the actual issues or add more explanation. Just for reference, I took about 45 minutes to complete this tutorial so it should take ~90 for a completely new student. -->
...
<!-- AG: This does not work for me on a Windows PC. The error message is: "Cannot open connection". However, it does show the file when I run list.files() in the above exercise. Seems like it's a problem with where the written files are stored. You may have to combine Exercise 10 and 11 to resolve the issue. -->
...
```{r csv-files-15, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r csv-files-15-hint-1, eval = FALSE}
read_csv(file = "data/test_bad_names.csv", 
...
<!-- AG: Much like in the CSV section, I didn't see "test_1.rds" in the output. -->
...
<!-- AG: Like in Exercise 1, I didn't see the file. I only saw data and exercise.Rmd. Not sure how to fix this and I'd recommend against writing files in tutorials anyways. -->
...
<!-- AG: Arrays are never explained right now, which is definitely confusing for somebody who's never coded before. Add an explanation about arrays or store something different. -->

You can also write arrays with the same number of rows and columns to a JSON format using `toJSON()`. 
...
<!-- DK: The below is wrong! readr 2.0.0 does much better on this. Delete this example? Rework it? -->

<!-- AG: This entire section doesn't work anymore. There are hundreds of parsing failures and the exercises don't do anything to fix them. Please rework it to work with readr 2.0.0 -->
...
<!-- AG: There are no parsing failures with this, which is weird. -->
