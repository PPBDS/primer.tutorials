...
<!-- Own every word here. Do the entire tutorial again. -->

<!-- Add copy buttons as appropriate. -->

<!-- Exercise 2 should use `show_col_types = FALSE` to get rid of that ugly guessing. -->
...
Make the warning message disappear with `show_col_types = FALSE`.
...
As you can see, `fromJSON()` can display arrays as well. 

### Exercise 5

You can also write arrays with the same number of dimensions to a JSON format using `toJSON()`. Display a 3x3 array in JSON format.
