...
#uses the cache of the new email to login without the user doing anything
options(gargle_oauth_email = "tutorialsprimer@gmail.com")
...
We have many files, but we will focus on the files with "similar" in their names.
...
We will use the Google sheets provided from the `googlesheets4` documentation, so use `read_sheet()` and use `url` in it. You might be prompted to sign into your google account.
...
The `googlesheets4` package also has example sheets we can use. Use the function `gs4_example()`, input in the name `"deaths"`, and pipe it with `read_sheet()`
