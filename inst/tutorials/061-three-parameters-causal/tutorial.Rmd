...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"XX"` -- and an author (you). Render the document and save it as `immigration.qmd`.
...
```         
show_file("immigration.qmd", start = -5)
```
...
`att_end` will be our outcome variable.
...
Given our (imaginary) treatment variable `phone_call`, how many potential outcomes are there for each [XX: unit]? Explain why.
...
<!-- DK: Insert at least two questions which explore the data. Provide knowledge drops which highlight important aspects. Don't forget to `tutorial.helpers::check_current_tutorial()` when you are done so that all the subsequent exercises are renumbered correctly. -->
...
What we really want is the effect of hearing Spanish-speakers on voting behavior. That us what matters. But, in this imperfect world, we will have to use information about its effect on attitudes toward immigration.
...
In almost all practical problems, the data was gathered at a time than that to which the Preceptor Table refers.

<!-- DK: Could make this plot nice. Use Count. Show all possible values, including zero bars. -->
...
What is question we will try to answer?
...
We need `fit_att` to exit in Console World. Copy/paste this code into the Console and execute it.
...
<!-- DK: Should discuss how the simulated data is not integers and extends beyond [3, 15]. -->
...
<!-- DK: Might this table better than that in Tutorial 5. Maybe: -->

<!-- tidy(fit_att, conf.int = TRUE) |>
  select(term, estimate, conf.low, conf.high) |>
  mutate(across(c(estimate, conf.low, conf.high), round, digits = 2)) |>
  gt() |>
  tab_header(
    title = "Model Estimates for att_end ~ treatment"
  ) |>
  cols_label(
    term = "Term",
    estimate = "Estimate",
...
First question is always predictions(fit_att) with the knowledge drop being:
...
In the Console, run `plot_predictions()` on `fit_att` with [XX: these arguments/values].
