...
<!-- DK: Lots of your knowledge drops could be better. -->
...
**Covariates** can be any variable that can influence the outcome. Be careful that  **covariates** may have different meanings depending on the problems so it's best to consider specific context when discussing this term, for more detailed explanation, visit [Cardinal Virtues](https://ppbds.github.io/primer/key-concepts.html) section.
...
In this data set, in each row, we can only have either the outcome for **treated** or **control**, not both. Since the respondent cannot be in both group to fill in the survey, recall the **Fundamental Problem of Causal Inference**. 
...
The data we have were collected from commuters on nine train platforms around Boston, Massachusetts. However, were the people on the trains in Boston similar to people at Chicago? And, were the people took the train that day and those took it tomorrow similar? We do not know. 
...
Let's practice creating and publishing a quarto document to answer the question we have in a professional way. Professional data scientists always do and store their work on Github, or a similar "source control" tool. If your computer blows up, you don't want to lose your work.

### Exercise 16

Create a Github repo called `causal-effect`. Make sure to click the "Add a README file" check box. Copy/paste the URL for its Github location.
...
```         
https://github.com/giakhang1906/causal-effect.git
```
...
Consider the same case, if we are aware of this potential factor which may affect the outcome and decide to record the rush hour when collecting the data. In that case, `rush hour` is a covariate which is measured but is not the outcome, which is the same as `treatment`.  
...
With the same case again, when putting variables to our DGM, we decide to only include `treatment`, but not `rush hour`, then just only `treatment` is considered a covariate.
...
Discussing covariates in the context of the Preceptor Table is different than discussing covariates in the context of the data. Recall that the Preceptor Table is the smallest possible table, so we don’t need to include every relevant variable but the ones that are necessary to answer the question.

### Exercise 23

As we render the file, it also shows the code we did to load up the packages in our document. Add `#| echo: FALSE` to prevent the code from appearing in our rendered document. Save and render the file again, the code will go away.
...
As we the outcome we care about is the attitude toward immigration at the end of the experiment for both the **treatment** and the **control** group, we only need to keep these two variables. 
...
Going back to the `analysis.qmd`, at the end of the file, type your adjusted answer to this question based on our suggestion. Don't forget to save the file. 
...
<!-- GK: Not sure about the word "pattern" -->

**Stability** looks across time periods, **Representativeness** looks within time periods, allowing us to make connection between the data we have and the population, and the population to the Preceptor Table. **Unconfoundedness** to rule out any pattern in our data that can affect the potential outcome. 
...
The Population Table look like this: 
...
The easiest way to ensure unconfoundedness is to assign treatment randomly. In our experiment, this may involve giving each person a number and then randomly pick a number to assign the treatment. 
...
Update `causal-effect.qmd` paragraph accordingly. Do not just copy/paste our example answer, obviously!  
...
The mathematical formula we will use for our model is: 
...
In the `analysis.qmd`, load the **brms** and the **tidybayes** packages in the library setup code chunk. Save the file and in the Console, run: 
...
The mathematical formula we are using is a standard regression model, we use the word "standard" since the model only has one variable. $y_i$ is the attitude toward immigration for person $i$ after being exposed to the treatment or control while $x_i$ corresponds to the treatment status for person $i$
...
Note that other numbers assigned to the `seed` argument will not affect the result since it only for the purpose of eliminating the effect of randomness in the modeling fitting process. If you and your friend are running the above code in your own computers and want to cross-check the result, remember to set the same value for `seed`. 
...
Selecting a model sometimes can feel like a trial and error process in which we first come up with a model to estimate our quantities of interest, test the model and try a different one until we fine the most suitable model to answer our problem. 
...
Running `#| cache: TRUE` will save the results of the code chunk the first time it is run, and subsequent rungs will use the cached results instead of re-executing the code. 
...
Recall that choosing a model is a trial and error process, if the model we use generate an outcome that is very different from the data, it might be because of the assumption we use. We assume the default value of `family`, `gaussian()`, in our model, perhaps changing the value for `family` might have. 
...
In the `analysis.qmd`, add a new code chunk, type `tbl_regression(fit_gauss)`. Below that code chunk, add a sentence to describe the results provided by the command. Save the file and in the Console, run: 
...
Note that the phrase "confidence interval" is usually given a "Frequentist" interpretation: if we were to apply the procedure we used in an infinite number of future situations like this, we would expect the true value to fall within the calculated confidence intervals 95% of the time. However, at the end, both interpretation refer to our degree of uncertainty in a sample statistic, and thus we have the general term which is "uncertainty interval."
...
Behind the scene, we have created that tibble for you and store it in an object called `ndata`. Type `ndata` and hit "Run Code".
...
In the `analysis.qmd`, add a new code chunkg and type `ndata`. Render the file, note that it will return an error. 
...
This is because the **qmd** is a different world in which we have not assigned anything to `ndata`.
...
Each row will generate a posterior for the attitude towards immigration. Let say we want our model to perform 4,000 draws for each group, we will have 8,000 posteriors/ rows in our `ndata` object.
...
```{r temperance-20, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r temperance-20-hint-1, eval = FALSE}
... |> 
  summarize(ave = ..., ... = party)
```

```{r temperance-20-test, include = FALSE}
trains |> 
  filter(treatment == "Control") |> 
...
In the `analysis.qmd`, continue the paragraph with your adjusted answer to this question based on our suggestion. Save the file and in the Console, run: 
