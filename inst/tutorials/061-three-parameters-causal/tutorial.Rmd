...
ndata1 <- tibble(treatment = c("Treated", "Control"))
```
...
We will explore some functions within this package. Pipe `trains` to `tbl_summary()`. Put `include = c(att_end, treatment)` within the argument. Assign the result to an object called `table1`. Type `table1` in a separate line print out the result. 
...
<!-- Ex 2 fit_guass -->
<!-- Ex 3 tibble(treatment = c("Treated", "Control")) -->
<!-- Ex 4 ndata1 -->
<!-- Ex 5 add_pred -->
...
Pipe `fit_gauss` to `add_epred_draws()` with `newdata = tibble(treatment = c("Treated", "Control")))`
...
Continue the pipe with `pivot_wider()`, with the `id_cols = .draw, names_from = treatment, values_from = .epred` argument
...
Run this code to see the Posterior Distribution of the treatment effect. 
