...
<!-- GK: Not sure about the interpretation -->

A confidence interval, and its associated confidence level, tells us how likely the truth is to lie within a specific range. In this case, we are 95% confident that the true value of each parameter to lie between each above range.

### Exercise 12

Run `fixef()` on `fit_gauss`. The `fixef()` returns information about the **fix**ed **ef**fects in the model.
...
<!-- GK: Add interpretation of the result  -->

Our model is a fixed-effect model because we assume the relationship among the columns in the Population Table stable over time. In this case, the estimates of the fixed-effects parameters from the model is approximately $10$, with a $95%$ Confidence Interval between $9.2$ and $11$. 
...
Create a tibble with the treatment column take on either the value "Treated" or "Control".
...
We have 8,000 rows. The first four thousand rows are draws from the first row of newdata, meaning **treatment** equals **"Treated"**. The next four thousand rows are draws for **treatment** equals **"Control"**.

### 

### Exercise 6

In the current `newdata` tibble, the "Treated" and "Control" posteriors are stacked up on each other, which makes it difficult for us to calculate the average treatment effect. Continue the pipe with `pivot_wider()`, with the `id_cols = .draw, names_from = treatment, values_from = .epred` argument. 
...
The resulting table has only 4,000 rows because we have put the posteriors for Treated and Control next to each other.

### 

### Exercise 7

The easiest way to calculate the causal effect is to subtract the posterior for the outcome under Control from the posterior of the outcome under Treatment. Continue the pipe with `mutate()`, and set `causal_effect = Treated - Control` as an argument.
...
The `pivot_wider()` function will first take each unique value in the `.draw` column to become a unique row and then each unique value in the `treatment` column to become a separate column. Finally, the values from the `.epred` column will be used to fill in those newly created columns accordingly. 
...
```{r temperance-9-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r temperance-9-hint, eval = FALSE}
... |> 
  ggplot(aes(x = ...)) + 
    geom_histogram(...(y = ...))
```
...
```{r temperance-12-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r temperance-12-hint, eval = FALSE}
... + 
  theme_classic()
```
...
```{r temperance-13-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r temperance-13-hint, eval = FALSE}
... + 
  labs(title = ...,
       subtitle = ...,
       x = ...,
...
<!-- GK: Not sure how to reasonable come up with a new confidence interval -->
```{r temperance-11}
question_text(NULL,
	message = "As we assumed Stability in our model, which is certainly false, the estimates of the average treatment effect, of exposing people to Spanish-speakers, on their attitudes toward immigration might be wrong. A better estimate would be about 1.5, with a confidence interval of 0 to 3.  ",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
