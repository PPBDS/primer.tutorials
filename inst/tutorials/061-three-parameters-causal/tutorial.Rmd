...
author: David Kane and Gia Khang and Satvika Upperla
tutorial:
...
### 
<!-- SU: Does this work? should it not be the data is available in the trains tibble from primer.data? The order does not make sense to me, but it was in the template so putting here for now.--> 
...
```{r the-question-3}
question_text(NULL,
	message = "Data for attitudes toward immigration-related policies, both before and after an experiment which randomly exposed a treated group to Spanish-speakers on a Boston commuter train platform. See Enos (2014) for background and details [(pdf)](https://github.com/PPBDS/primer.data/blob/master/inst/papers/trains.pdf). Individuals with a treatment value of 'Treated' were exposed to two Spanish-speakers on their regular commute. 'Control' individuals were not.",
...
The data we have were collected from commuters on nine train platforms around Boston, Massachusetts in 2014.
...
question_text(NULL,
	message = "Imagine a variable called `o_m (how open minded a person is)`. We can manipulate the value of `(o_m)`, at least in theory, by adding o_m to the treatment group while not adding to the control group.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
### 
<!-- SU: The last two sentence kind of use fancy wording, can we make it simpler to understand? *from template tutorial -->
...
### Exercise 7
<!-- SU: I'm confused is att_start the attitude at the start and then att_end after treatment or no treatment has been applied? but then shouldn't att_start and then att_end with control be the same value? -->
...
### Exercise 10
<!-- SU: I didn't know what to add here, but pls check if its accurate -->
...
```{r the-question-10}
question_text(NULL,
	message = "XX: Some people might have a value for party of Republican. Others might have a value of Democrat. Those two groups will, on average, have different values for the outcome variable.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
### Exercise 11
<!-- SU: is it fine to day the covariate of interest is treatment? bcuz in the example given below the key covariate used was the treatment of the problem this tutorial was working on -->
