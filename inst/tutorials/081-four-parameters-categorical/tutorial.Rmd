...
Covariates are what we usually are comparing the outcome to.
...
Since this represents a categorical or predicative model, there are no treatments being observed.
...
```{r wisdom-9}
question_text(NULL,
	message = "The data has many variables, but only a will be used: 'year', 'sex' and 'pres_vote'",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
We see many variables in this data set, but, we do not need any of the other ones as they do not relate to our model.
...
What does the dependant variable represent in this data?
...
The dependent variable is what we are measuring, so that would be the candidate which was voted.

### Exercise 12

<!--# TJ: I need help coming up with a question about the covariate variable -->
...
```{r wisdom-15}
question_text(NULL,
	message = "People may claim that they voted for one candidate when they really voted for another.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
More people claim to have voted for the winning candidate than the actual number of votes that they recieve.
...
Using data from the National Election Studies survey of US citizens, we seek to understand the relationship between voter preference and sex in the 1992 Presidential election. In particular, were men or women more likely to vote for Ross Perot??

```{r wisdom-16}
question_text(NULL,
	message = "Men were much more likely",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r justice-7}
question_text(NULL,
	message = "Representativeness might not be true in this case due to potential differences between voters who responded to the NES survey and those who did not.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
The sample size that we are using may not accurately represent the rest of the population.
...
Since this is not a causal model, we do not need to worry about this.
