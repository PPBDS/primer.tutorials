...
```{r the-question-4}
question_text(NULL,
	message = "We will be using the count of each unique value in `pres_vote` as an outcome variable.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 2)
...
question_text(NULL,
	message = "For a given individual, assume that the value of the treatment variable might be `receive phone call` or `no phone call`. If the individual gets `receive phone call`, then their probability of voting for Bush is 0.7. If the individual gets `no phone call`, their probability of voting for Bush is 0.5. The causal effect on the probability of voting for Bush of `receive phone call` versus `no phone call` is 0.7 - 0.5 --- i.e., the difference between two potential outcomes --- which equals 0.2, which is the causal effect.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Can we use data from `nes` to predict the voting behavior of men and women in the US this year? When was the data collected? Is the question in the survey across year the same? 
...
Some variables of interest are `sex` and `pres_vote`. We will look at these more in depth later on. Before we continue questioning our data, let's update our `analysis.qmd` file. Professional data scientists always store their work somewhere safe. This is so you don't lose your code even if something happens to your computer.
...
Provide one reason why the assumption of validity might not hold for the outcome variable: `pres_vote`. Use the words "column" or "columns" in your answer.
