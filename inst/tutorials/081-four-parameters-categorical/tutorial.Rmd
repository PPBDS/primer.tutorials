...
<!--TJ: I need help with this question -->
```{r wisdom-9}
question_text(NULL,
	message = "XX",
	answer(NULL, correct = TRUE),
...
Pipe `summarize()` to nes, set the `.by` argument equal to `sex`
...
```{r wisdom-15-hint-2, eval = FALSE}
nes |> summarize(...)
```

```{r wisdom-15-test, include = FALSE}
nes |> summarize(.by = sex)
```

### 

We can see that there are four possible values for sex: Female, Male, NA and Other.
### 
...
<!---TJ: I am not sure if this is correct, I copied it from the textbook --->

```{r justice-3}
question_text(NULL,
	message = "Because this project is historical, the Population Table has the same number of rows as the Preceptor Table. We are not interested in these voters outside of the 1992 election. So, this Population Table, unlike most, does not require a Time column.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- Search and replace "fit_nes" as appropriate. -->
