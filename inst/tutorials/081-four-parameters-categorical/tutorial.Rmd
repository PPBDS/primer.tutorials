...
# 3. parsnip:::form_xy(...)
# 4. parsnip:::xy_xy(...)
# 5. parsnip:::check_outcome(env$y, object)
```
...
question_text(NULL,
	message = "Which candidate is mostly voted by men.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
> *Which candidate do men mostly support?*
...
```{r justice-9}
question_text(NULL,
	message = "Understanding the voter preference of different genders is essential for a candidate to design a campaign strategy. Using data from the National Election Studies survey of US citizens, we seek to understand the relationship between voter preference and sex in the 1992 Presidential election. ",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
The link functions for 3 categorical variables are: 

$$
\log \left( \frac{P(Y = 1 \mid X)}{P(Y = 3 \mid X)} \right) = X \beta_1
$$

$$
\log \left( \frac{P(Y = 2 \mid X)}{P(Y = 3 \mid X)} \right) = X \beta_2
$$

where:
- $P(Y = 1 \mid X)$ and $P(Y = 2 \mid X)$ are the probabilities of outcomes 1 and 2 given predictors $X$,
- $P(Y = 3 \mid X)$ is the probability of the baseline outcome 3,
- $X$ is the vector of predictor variables,
- $\beta_1$ and $\beta_2$ are the coefficient vectors for outcomes 1 and 2, respectively.
...
<!-- GK: There is an error when running this code.  -->
<!-- This is the basic mathematical structure of our model: -->

<!-- ```{r} -->
<!-- extract_eq(fit_nes, wrap = TRUE, show_distribution = TRUE) -->
<!-- ``` -->
...
<!--   no applicable method for 'extract_lhs' applied to an object of class "c('multinom', 'nnet')" -->
  
### Exercise 5

Add `library(tidymodels)`, `library(gtsummary)`, and `library(equatiomatic)` to the `setup` code chunk in `XX.qmd`. `Command/Ctrl + Shift + K`.

At the Console, run:

```
tutorial.helpers::show_file("XX.qmd", pattern = "tidymodels|gtsummary|equatiomatic")
```
...
Because our outcome variable is multinomial, start to create the model by using `multinom_reg(engine = "glmnet")`.
...
<!-- GK: Also got the same error as above. -->
...
<!-- GK: The table results looks weird -->
...
<!-- > tidy(fit_nes, conf.int = TRUE) -->
<!-- # A tibble: 4 × 8 -->
<!--   y.level term        estimate std.error -->
<!--   <chr>   <chr>          <dbl>     <dbl> -->
<!-- 1 Bush    (Intercept)    0.852     0.107 -->
<!-- 2 Bush    sexMale       -0.420     0.144 -->
<!-- 3 Clinton (Intercept)    1.31      0.101 -->
<!-- 4 Clinton sexMale       -0.676     0.137 -->
<!-- # ℹ 4 more variables: statistic <dbl>, -->
...
```{r temperance-3}
question_text(NULL,
	message = "What was the relationship between sex and voting in the 1992 US Presidential election among supporters of the three leading candidates: Clinton, Bush and Perot?",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- GK: Got this error:  -->
<!-- Error: One of the `condition` and `by` -->
<!--   arguments must be supplied, but not -->
<!--   both. -->
