...
<!-- DK: Discuss in various places that, unlike usually, the data comes after the Preceptor Table. So the Population Table looks different. -->

<!-- DK: Also discuss how post-hoc survey data can be unreliable. Validity concern is that who you say you voted for is not the same as who  -->

<!-- DK: Fix up the template tutorial to handle multinomial. -->
...
<!-- IS: causing error, not sure why -->

### Exercise 11

In the Console, run `check_predictions(extract_fit_engine(fit_nes))`. CP/CR.
...
# check_predictions(extract_fit_engine(fit_nes))
```
...
Most of the actual data points fall within the range of the simulated data, suggesting that our data generating mechanism (DGM) is a reasonably good fit. While the model may not capture every nuance, the alignment between the observed and simulated values indicates that the model is likely good enough for drawing conclusions about the relationship between sex and voting preference in the 1992 Presidential election.
...
<!-- more marginaleffects functions -->
