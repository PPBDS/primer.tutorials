...
```{r wisdom-9}
question_text(NULL,
	message = "This dataset is from the American National Election Studies (ANES), a survey that has been conducted since 1948 before and after each presidential election, and combines questions about voters' political attitudes with extensive biographical information.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Your code chunk should look like this: Copy and paste this into a the code chunk in the `.qmd` file. Render it to make sure it works.
...
Create a new code chunk. Add `#| label: loading`, `#| echo: FALSE` and `#| message: FALSE` to the top of it once again. Copy and paste the code to make above into the code chunk. 
...
Make sure to copy and paste this into the code chunk. Skip a line and add `fit_nes` to print the model.
...
Make a new code chunk. Make sure to add `#| label: temperence`, `#| message: FALSE` and `#| echo: FALSE`
...
Finally, add `scale_x_continuous()` to the function. Set `breaks` to `c(0.05, 0.3, 0.6)` and `labels` to `scales::percent_format()`
