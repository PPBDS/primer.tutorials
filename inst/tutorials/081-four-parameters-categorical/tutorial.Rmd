...
Make a new GitHub repository and connect it to a new RProject. Title the repository "Tutorial 8: Four Parameters: Categorical." We will use this when making our model later
...
Go to `File -> New File -> Quarto Document`. Title it "Tutorial 8". Make yourself the author

### Exercise 2

Add a code chunk. Add `#| label: setup` to the top of it. Don't forget to set message and echo to false.


### Exercise 3

Load the **brms**, **tidybayes**, **tidyverse** and **primer.data** packages into this code chunk. Load just the **brms** and **tidybayes** packages here.
...
Make a new code chunk, set the label to "cleaning-data". Set message and echo to false as usual. Type nes in this code chunk.
...
Finish the pipe with `mutate()`. Set the `pres_vote` argument to `case_when()`. Inside of `case_when()`, change `pres_vote` from the name of the political party to the name of the candidate that was voted. (Look in the textbook to find the associated political parties). Type the code in the exercise here to make sure it works before copy/pasting it into the quarto document.
...
Set this equal to a new object called `nes_92`. Make sure to do this in the quarto document as well.
...
Assign the result to an object called `fit_nes`. Make sure you are copy and pasting this into the quarto document.
