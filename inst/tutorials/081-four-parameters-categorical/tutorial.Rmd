...
In this case, we have the simplest possible formula. `pres_vote` and `sex` are both categorical variables. `pres_vote` is the dependant variable and `sex` is the independant variable.
...
<!-- TJ: Did I do this correctly? I mostly just copied from the Primer -->

Write the mathematical formula for your model, using $\LaTeX$ math notation.

<!-- TJ: Also, won't it be tough for them to use LaTeX math notation? -->
...
We want to know how "different" units behave, where the very definition of "different" is that they do not have identical values for all the variables used in `fit_nes`.
