...
<!-- VD: Change file/folder names to be less confusing in this tutorial. Also change references to these files in RStudio and Git tutorial -->
...
Quarto is a file format for making dynamic documents with R. It builds upon the R Markdown format to create a cleaner, better, and more useful file format. Quarto documents utilize markdown (an easy-to-write plain text format), which contain chunks of embedded code. These code chunks can be written in quite a few languages, including R and Python.

These files are special because unlike R scripts they can be **knitted** into *reports* (or HTML files) that are used in websites. These reports can have both regular text, and code blocks. We'll be publishing a Quarto document to RPubs.

To learn more about Quarto and how to use it, check out the [official webpage](https://quarto.org/)!

### Exercise 1

Unlike R Script, Quarto does not come with R Studio, so we will have to install it ourselves. 

To install Quarto, follow [this link](https://quarto.org/docs/get-started/), and download the appropriate version of Quarto for your operating system. Open the file and follow the instructions to install Quarto.

<!-- VD: Add pictures, potentially add exercises -->

### Exercise 2

Now that we have installed Quarto, we can create a new Quarto document in R Studio. To do this, click the dropdown menu at the top left of your R Studio window, and click "Quarto Document". Name the file `quarto1`.

<!-- VD: Update gif to show Quarto instead of Markdown. As noted earlier, change file name. -->
...
<!-- VD: Update images to show Quarto instead of Markdown.-->
...
This allows you to use R to create graphs then display it in a Quarto document with plain text descriptions.
...
The main function of a Quarto document, which differs it from a Script, is to knit into a **report**. To knit our Quarto Document, click the knit button at the top of the file, or use the shortcut cmd + shift + k.
...
Now let's try to add code chunks into our Quarto Document. Under the text that we added earlier, add a code chunk either by pressing the Add Chunk command at the top of the editor (the green C button with a plus sign next to it) or with the shortcut `option + cmd + i`. The code chunk should look like this:
...
Let's create the following scatter plot in an Quarto Document:
...
Inline R code is essentially a way for you to put simple R calculations in the same line as your text. Create the following code chunk in your Quarto Document.
...
Into your Quarto Document, directly after the code chunk we just made, copy the following text and inline R code.
...
Right now, our Quarto Document looks a little ugly. We don't want our code to be displayed in the knitted report. We can get rid of it using code chunk arguments.
