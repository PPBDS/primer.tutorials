...
Look at the top left of your R Console. Here you should see the version of R you are running.
...
Shortly, we will create a new project for you to start working in. Before we do this, it might be useful to realize where you are right now.

In the R Console, run `get_wd()`. Copy and paste both the command and the result into the space below. 
...
This function returns the path to your current R Project. A path is simply the path you would use to reach a location on your computer. In this case, if you wanted to find the folder for your current R Project, you would follow the path returned by `get_wd()`. 

### Exercise 2

Whenever you start a new R Project, you will want to store it somewhere in which you can find it easily. If we look back at the path from the previous exercise, we can see that the path likely leads somewhere which is not very easy to find. For this reason, we will make a `projects` folder, for you to store all future projects.

For a windows computer, [this](https://support.microsoft.com/en-us/office/create-a-new-folder-cbbfb6f5-59dd-4e5d-95f6-a12577952e17) resource should help you in creating a folder. 

<!-- VD: Add a more detailed explanation for windows computers -->

For a computer running macOS, you can make a folder in your desktop by simply right clicking on your computer wallpaper. This should bring up a simple menu, in which you can click "New Folder". An unnamed folder will then be created. To change the folder's name, right click it again, and click "Rename". Change the folder's name to "r-projects". We do not use spaces in file or folder names, as it can lead to issues down the line.

### Exercise 3
### 

Now return to your R Studio session. Let's make our new R Project. This can be done by clicking on the dropdown menu at the top right of R Studio, and clicking "New Project". This will likely prompt you to terminate running jobs. Click terminate if you see this screen. Next click "New Directory" and then "New Project". 

Name the Project "First-R-Project" and change the project subdirectory to your new "r-projects" folder. Lastly, check the box to open the project in a new R Session in the bottom left corner. If you don't do this, this tutorial will close and you will need to reopen it.

<!-- VD: Add Image Here. Expand on this process. -->
...
Good Work! You've created a new R Project. For the rest of this tutorial we will be working in "First-R-Project".

###

In "First-R-Project", run the `get_wd()` function that we used earlier. Copy and paste both the command and the result into the space below. 
...
The path should be different than the one we found earlier. If it isn't you are likely still in your original R Session. 

The path should now end in something similar to "r-projects/First-R-Project", telling you that "First-R-Project" is in the "r-projects" folder.
...
<!-- VD: Shorten this description -->

### Exercise 1

<!-- VD: Try moving Exercise 1 and Exercise 2 to the R Projects section. It might make more sense. -->
...
This should return only the `First-R-Project.Rproj` file. 
...
From the summary we can tell that `ggsave()` is used to save plots as a graphic in the form of a .png file. 
...
Now using the `ggsave()` function we can create a png of our plot from our code. In `ggsave()`, set `plot` to `hist_p`, `file` to `"hist-harvard"`. 
