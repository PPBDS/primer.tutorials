...
You may notice that there are curly braces `{}` at the top of the chunk, with an `r` inside. The `r` says that the language is `r`. In the next line, use `#| label: setup`, because that is where we are loading the libraries in.

### 

We don't need very many libraries for this, as we are just including images, but we do need the package **knitr**. Load **knitr** with `library(knitr)` in your setup chunk. 
...
Putting the character string `library(knitr)` inside the `setup` code chunk is enough to "load" **knitr** in QMD World because, whenever we render a file, all of it is set to R for processing. But just putting `library(knitr)` in `quart
