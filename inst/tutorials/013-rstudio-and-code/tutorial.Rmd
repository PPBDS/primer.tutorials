...
<!-- This tutorial should be easy for someone who has not done anything else! -->

<!-- Make the plot creation interactive! Pretend like you are sitting there with them, showing them how to move back and forth between the script and the Console. -->
...
<!-- Add something about code chunk options. Good example is warning=FALSE, which is needed when working with data with missing values, like nhanes$height. Start with echo = TRUE or FALSE. Explain what is going on with the setup R code chunk. VD: Done-->
...
This should return only the file "project1.Rproj". 
...
Run your R Script file to see your completed plot. In the Console tab, run `readLines("FirstRScript.R")` and copy-paste the output into the space below.
...
You can learn more about how to write R Markdown files with [RStudio's introduction to R Markdown](https://rmarkdown.rstudio.com/articles_intro.html).

### Exercise 1

First things first, let's create an R Markdown file. Click the dropdown menu at the top left. Click "R Markdown". Name the file `markdown1`

```{r}
include_graphics("images/new_markdown.gif")
```

### 

Save the R Markdown file by clicking the save icon, or with the shortcut cmd + s.

Notice that you will be once again asked to name the file. Name the file `markdown1`. The first name you gave the file is the name it shows as `title` of our YAML header.

### 

Now that the file is saved, run `list.files(). Copy and paste the output into the space below.
...
This should now return `markdown1.Rmd`, as well as `script1.R` and `project1.Rproj`. Notice the different suffixes used for our different types of files.
...
As you can see, a lot of the text is actually regular plain text, not code. However, there are code chunks that display R code. Here's an example of an R code chunk.
...
Delete everything except the YAML header (contained by dashes) and the first code chunk (we will often call this the setup code chunk), then add "This is my first R Markdown text." at the bottom.

### 

Save the file and run `readLines("markdown1.rmd") |> tail()`. Copy and paste the output into the space below.
...
The main function of an R Markdown file is to knit into a **report**. To knit our R Markdown file, click the knit button at the top of the file, or use the shortcut cmd + shift + k.

You should now see a fairly empty file which contains

In the following box, copy all of the text from your knitted R Markdown.
...
This should show your markdown file's name, the date, and "This is my first R Markdown text."

### Exercise 4

In R Markdown files, you can also use Markdown syntax. You can use this to create **bold** or *italic* text, and and to format your file with things like headers. This is useful for formatting in your report.

### 

Make the text created in the previous section a header by adding `##` at the beginning of the line, then adding this text under that as a description: `You can do a lot of cool things in R Markdown like **bold** and *italic* text.` Knit the file again and make sure that everything is showing up correctly.

### 

Run `readLines("markdown1.rmd") |> tail()` in the Console, then copy-paste the command and the output below.
...
Now let's try to add code chunks into our R Markdown file. Under the text that we added earlier, add a code chunk either by pressing the Add Chunk command at the top of the editor (the green C button with a plus sign next to it) or with the shortcut `option + cmd + i`. The code chunk should look like this:
...
Save the file, and run `readLines("markdown1.rmd") |> tail()` in the Console, then copy-paste the command and the output below.
...
First of all, we need to load the libraries that we need. In the setup code chunk, use the `library()` function with the argument **tidyverse**.  Your setup chunk should look like this:
...
Run `readLines("markdown1.rmd") |> head(15)` in the Console, then copy-paste the command and the output below.
...
Whenever we load libraries in our R Markdown files, we will use the setup code chunk. Do not load libraries in the same chunk as your plots.
...
Knit the file. You should now see your R Code Chunk, as well as a blank plot, as we have not added a geom layer. 

Run `readLines("markdown1.rmd") |> tail(10)` in the Console, then copy-paste the command and the output below.
...
Knit the file again. You should now see that our plot is no longer empty, and now shows a scatterplot.
...
Knit the file to see our completed graph.
...
Inline R code is essentially a way for you to put simple R calculations in the same line as your text. Create the following code chunk in your R Markdown File.
...
Knit the file, then in the console, type `x` to verify that x has been defined. Copy-paste the command and the output below.
...
In your R Markdown file, type the following.

The variable x has a value of: `` `r knitr::inline_expr("scales::comma(x)")` ``.

Knit the file. You should see that the line now looks like,
...
Instead of hardcoding values into text in your Markdown file, you should calculate values on the fly using inline R Code. This ensures that values will always be accurate. For more info on inline R Code, look at the [R Markdown Guide](https://rmarkdown.rstudio.com/lesson-4.html).

### Exercise 12

Right now, our R Markdown file looks a little ugly. We don't want our code to be displayed in the knitted report. We can get rid of it using code chunk arguments.

Code chunk arguments are a way to tell a chunk what it should do when knitted. Code chunk options are written after the `{r}` in our code chunk. Let's change `echo` to `FALSE` to stop the code from showing in our report. 

Run `readLines("markdown1.rmd")` in the Console, then copy-paste the command and the output below.
...
Some more useful code chunk options to look into include `warning`, and `message`. For a full list of code chunk options, check out the [R Markdown Reference Guide](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf).
...
In your new code chunk, use the function `include_graphics()`. Set the argument to the name of your .png. Remember to use quotes. The command should look something like `include_graphics("hist_p.png")`.

### 

Knit the file, and take a look at your completed R Markdown. It should now include your `iris` graph, the `x` text, and your `qscores` graph.
...
You should see a `markdown1.html` file, which generates upon knitting your markdown.
...
Notice a blue icon in the upper right-hand corner of your knitted R Markdown that reads `Publish`. Click it.
