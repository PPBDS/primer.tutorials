...
<!-- Need shorter version of "Copy and paste both the command and the result into the space below." How about CP/CR? Mention the abbreviation in the first question of all tutorials: Getting Started, this one and RStudio and Github. -->
...
<!-- Do everything in source at the beginning. Then (maybe?) explain visual. Or do visual with first Quarto document and then source with the second?  -->
...
<!-- Make a second script. And maybe a third. Maybe next summer? -->

## Introduction

### 

Up until now, you have been typing your code into the neat boxes provided by our tutorials. What if you want to start working on your own? In this tutorial you will learn how to use R scripts and Quarto documents, as well as how to publish your work using RPubs!

## Checking your setup

### 

This tutorial assumes that you have read and completed all the steps in the [Getting Started](https://ppbds.github.io/primer/getting-started.html) chapter of the *Primer*. We will not be using "copy and paste the command and the result into the tutorial into the space below", as that is very long. Instead, we will be using a much tidier abbreviation: CP/CR.
...
Copy and paste both the command and the result into the space below.
...
It would also be a good idea to to store this folder on your desktop, so that it can be found easily. **NOTE**: Do not store this folder inside of a cloud storage directory such as OneDrive. This can cause heaps of unforseen problems for you in the future.
...
Run `getwd()`. Copy and paste both the command and the result into the space below.
...
Run `list.files()`. Copy and paste both the command and the result into the space below.
...
Type `readLines("analysis-1.Rproj")` in the R Console and hit enter. Copy and paste both the command and the result into the space below.
...
Delete all code currently in your R Script file. Then click the save icon below the Script title, or use the shortcut cmd + s to save the Script file. The first time you save a file, you will need to name it. Name the file `script-1`. The `.R` suffix will be added automatically.
...
These code chunks allow you to use R to create plots and other graphics, then display them in a rendered Quarto document with plain text descriptions.
...
In Quarto documents, you can also use Markdown syntax. You can use this to create **bold** or *italic* text, and headers. This is useful for formatting in your reports, to make them cleaner and easier to understand.
...
<!-- VD: Need to finish section -->

Now let's make another Quarto document, and explore some more options!

### Exercise 1

Make a new Quarto document. If you need a refresher on how to make the document, take a minute to check the previous section.
...
Save the document as `quarto-2` and run `readLines("quarto-2.qmd")`. Copy-paste the command and the output below.
