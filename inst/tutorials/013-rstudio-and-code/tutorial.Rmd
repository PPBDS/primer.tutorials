...
The `5 * 5` should have run in your R Console. The return value is `25`. Whenever you run an R script file, it will send all the code in the file to the R Console, as if you had retyped it all there and hit return.

### Exercise 2

<!-- DK: I cut out ", this time using the shortcut cmd/ctrl + enter" because it is poorly explained. -->

Go back to the first line in your R script. Change `5 * 5` to `x <- 5 * 5`, thereby creating an object named `x` with a value of 25 . Run the script again. You should not see any value returned.
