...
<!-- Grab anything useful from the Primer sections on R Markdown and Creating R Projects sections from Getting Started. -->

<!-- Essentially force them to create ~/Desktop/projects and place projects there. -->
...
<!-- Use ls() as an interactive command. Talk about environment. -->
...
This should return only the file "FirstRProject.Rproj". 
...
In "FirstRproject", run the `getwd()` function that we used earlier. Copy and paste both the command and the result into the space below. 
...
Type `readLines(FirstRProject.Rproj)` in the R Console and hit enter. Copy and paste both the command and the result into the space below. 
...
Create an R Script by going to File -\> New File -\> R Script.
...
You should now see `FirstRScript.R` in the output as well as the `FirstRProject.Rproj` file. Recall when we ran this in the R Projects section of this tutorial, we only saw the `FirstRProject.Rproj` file.
