...
Create an R script by clicking the drop down menu at the top left of your RStudio Session, and selecting "R Script".
...
In the Console tab, run `list.files()`. CP/CR.
...
Remember that `ggplot()` will only generate a blank plot until a `geom` layer is added. 
...
Run the file. This should generate columns on your plot. Did you remember to place a `+` after the `ggplot()` call and before the `geom_histogram()` layer?
...
Here we can use the `ggsave()` function. Type `?ggsave` in the R Console. This will open the help page for `ggsave()`. Copy and paste the Description from the help page into the space below.
...
You should see the `hist_p.png` file containing your plot. In the bottom right corner of your workspace in the `files` tab, click on the png file to view your plot.
...
Click the dropdown menu at the top left of your RStudio window, and click "Quarto Document". You will be offered the option of providing a "Title" for the document. **Note that the title of a document has no necessary connection to the name of the file itself.**
...
Save the Quarto document by clicking the Disk icon at the top of the file, or by using the shortcut Cmd/Cntrl + s. Name the file `quarto-1`.
...
This should return `quarto-1.qmd`, as well as `script-1.R` and `analysis-1.Rproj`. Notice the different suffixes used for our different types of files.
...
Quarto documents utilize Markdown (an easy-to-write plain text format), which can contain chunks of embedded code. These code chunks can be written in many languages, including R and Python. These allow you to use R to create plots and other graphics, then display them in a rendered Quarto document.
...
Notice how the rendered document --- which is an html file in this case --- appears in the Viewer tab in the Files pane. 
...
Make the text created in the previous section a header by adding `##` at the beginning of the line, then adding this text under that as a description: `You can do a lot of cool things in R Markdown like **bold** and *italic* text.` Render the file again. Does everything look good in the Viewer?
...
In the top left corner of the Code pane, you should be able to see two buttons: "Source" and "Visual". "Source" shows you all the code while "Visual" shows you (mostly) how the document will look after being renderedt. You can edit a Quarto document while in either "Source" or "Visual" mode.

### 

We are going to switch to "Visual" and explore how we can edit while still in "Visual". Switch to "Visual".
...
<!-- DK: This is doing a poor job of explaing where your libraries have not been loaded, that there is a difference between the current workspace and the Quarto document itself. -->
...
Whenever we load libraries in our Quarto documents, we will use a setup code chunk. Do not load libraries in the same chunk as your plots.
...
Code chunk arguments are a way to tell a chunk what it should do when rendered. Code chunk options are written after the `{r}` in our code chunk. Let's change `echo` to `FALSE` to stop the code from showing in our report. `echo` like `label` is a code chunk option. Code chunk options are prefaced with a `#|`, called a "hash-pipe." After the name of the option, you place a colon, followed by the value for the option.
...
We are going to make another fake setup chunk, where you can load all the libraries you need. Add a new code chunk with the green C at the top of the page.

### 

You may notice that there are curly braces `{}` at the top of the chunk, with an `r` inside. The `r` says that the language is `r`. After the `r`, give your chunk a name, like `libraries`, because that is where we are loading the libraries in.

### 

We don't need very many libraries for this, as we are just including images, but we do need the package **knitr**. Load **knitr** with `library(knitr)` in your setup chunk. Once all that is done, run `search()` in the Console. CP/CR
...
Go back to the `quarto-1` document and copy and paste the code for the iris plot into the `quarto-2` document into a new code chunk named `iris`. In the curly braces after the code chunk name, add a comma and `echo = FALSE`.
...
We are going to do it the same way as the Harvard plot. In a new code chunk named `iris_save`, type `ggsave()`. Set `plot` equal to `iris_plot`, and `file` equal to `iris_plot.png`. Now you have another .png to include in your `quarto-2` document.
...
Now, we are going to include more inline R code. First, save `quarto-2` and restart the R session. 

### 

In a new code chunk set `x` to `1234567890` with `<-`. Make sure `echo` is `FALSE`.

### 

Now, we want the inline code to show in the document. Type x in the console to make sure it is `1234567890`.

### 

Now, you need to **TYPE** this into your document. Type it underneath the `iris_plot` code chunk. This does dot go in a code chunk though:
...
Run `readLines("quarto-2.qmd")` into the console. CP/CR.

### Exercise 8

Render the file, and take a look at your completed Quarto document. It should now include your `iris` graph, the `x` text, and your `qscores` graph.
...
Render the `quarto-2` file. By rendering, you are generating a report written in HTML, which is what RPubs publishes on the web.
...
You should see a `quarto-1.html` file, which generates upon knitting your qmd. (Note that the "qmd" file extension stands for **Q**uarto **M**arkdown **D**document.)
