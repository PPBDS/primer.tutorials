...
Delete everything except the YAML header (the text in the box at the top of the file, as well as the dashes) then add "This is my first Markdown text." at the bottom. Markdown is the plain-text language we are writing in.
...
The main function of a Quarto document, which differs it from a simple R script, is to create something which is a mixture of words and code. To render our Quarto document, click the "Render" arrow at the top of the Code pane, or use the shortcut Cmd/Cntrl + shift + k.

###

To render is to transform a document from Markdown to some other format, like html, PDF, Microsoft Word and so on.
...
In Quarto documents, you can use Markdown syntax. You can use this to create **bold** or *italic* text, and headers. This is useful for formatting in your documents and reports, to make them cleaner and easier to understand.
...
Whenever we load libraries in our Quarto documents, we will use the fake setup code chunk. Do not load libraries in the same chunk as your plots.
...
We don't need very many libraries for this, as we are just including images, but we do need the package **knitr**. Run `install.packages("knitr")` in the console. Once that is done loading, restart the r session.

### 

After you've done that, load the library **knitr** in your libraries chunk. Once all that is done, run `search()` in the Console. CP/CR
