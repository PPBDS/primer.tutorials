...
pe_rep <- pe %>% mutate(ID = "rep")

pe_unrep <- pe2 %>% mutate(ID = "unrep")

pe_combo <- merge(pe_rep, pe_unrep, all = TRUE)
...
combo_plot <- pe_combo %>% 
  ggplot(aes(x = `1`, fill = ID)) + 
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   ...
                   alpha = 0.5) + 
    labs(title = "Posterior for Expected Distance",
         subtitle = "The expected distance for polling locations with a poverty rate of 0.2\n is about 1.5 km higher in our representative data than in our unrepresentative data.",
         x = "Poverty Rate",
         y = "Probability",
         fill = "Type of Data") +
    scale_y_continuous(labels = scales::percent_format()) +
...
<!-- bind_rows(list(rep = pe, unrep = pe2), .id = "type") %>% slice_sample(n = 10) -->
...
<!-- MM: Break into more questions (model and data, then add refresh, then add family argument) -->
...
To do so, we will first start by adding a column to our posterior distribution tibbles. For `pe`, which was from our representative data, add a column called `ID` that has a value of "rep" for every row. Store this as `pe_rep`.
...
For `pe2`, add a column also called `ID`, however this will have a value of "unrep" for every row. Store this as `pe_unrep`
...
```{r compare-2-hint, eval = FALSE}
pe2 %>% mutate(ID = ...)
```

### Exercise 3

Now, we are going to combine both our `pe_rep` and `pe_unrep` tibbles into a single tibble with two columns, one for `ID` and the other for our value. To do so, use `merge()`, with the first argument as `pe_rep`, the second as `pe_unrep`, and then set all to `TRUE`. Store this as `combo_pe`.
...
```{r compare-3-hint, eval = FALSE}
pe_combo <- merge(..., ..., all = TRUE)
```

### Exercise 4

View `pe_combo` by running `pe_combo`.
...
```{r compare-4-hint, eval = FALSE}
pe_combo
```

###

Notice we have 8,000 rows in this tibble, 4,000 from each posterior distribution, and that the `ID` column now indicates whether that value is from the representative data or the unrepresentative data.

### Exercise 5

Let's plot both posteriors onto one plot using our tibble `pe_combo`. Use `ggplot()` and `aes()` to map distance to the x-axis and set `fill` to ID. Then, add the layer `geom_histogram()` and map probability to the y-axis by setting the first argument to `aes(y = after_stat(count/sum(count)))`. 
