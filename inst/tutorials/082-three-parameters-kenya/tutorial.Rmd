...
exp_poverty_2 <- pe2 %>%
  ...
  geom_histogram(aes(y = after_stat(count/sum(count))),
                 bins = 100) +
  labs(title = "Posterior for Expected Distance",
       subtitle = "Polling locations about 10.5 km away on average from those assigned to them have poverty rates of 0.2",
       x = "Poverty Rate",
       y = "Probability") +
  scale_y_continuous(labels = scales::percent_format()) +
  theme_classic()
...
View a sample of `pe_combo` by starting a pipe and using `sample_slice()`. 
