...
```         
show_file("urn_simulation.qmd", start = -5)
```
...
In the Console, run `check_predictions(extract_fit_engine(fit_color))`. CP/CR.
...
<!-- DK: Add a new question which shows the plot and then asks for a two sentence interpretation. -->
...
Create a new code chunk in your QMD. Add a code chunk option: `#| cache: true`. Copy/paste the code from above for estimating the model into the code chunk, assigning the result to `fit_color`. 
...
In the Console, run `tidy()` on `fit_color` with the argument `conf.int` set equal to `TRUE`. The returns 95% intervals for all the parameters in our model.
...
```
tutorial.helpers::show_file("urn_simulation.qmd", end = -10)
```
...
<!-- DK: Show a simple table. Show the code for the simple. -->

<!-- DK: Need to tell students that you are using gt and that, in your QMD, you would need to add library(gt) to the setup code chunk. -->
...
```{r temperance-2}
question_text(NULL,
	message = "The parameter estimate for the intercept is 0.66, with a 95% confidence interval from 0.09 to 1.27. This represents the log-odds of scooping a red bead with the shovel sample.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r temperance-3}
question_text(NULL,
	message = "The 95% confidence interval for the intercept ranges from 0.09 to 1.27. This means we are fairly confident that the true log-odds of scooping a red bead with the shovel falls within this range.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r temperance-4}
question_text(NULL,
	message = "The estimated log-odds of drawing a red bead with the shovel is 0.66. The confidence interval from 0.09 to 1.27 reflects the uncertainty in this estimate due to sampling variability.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- plot_predictions(fit_color, type = "prob",  -->
<!-- condition = "group", draw = FALSE) -->
...
preds |> 
  ...
  geom_pointrange(size = 0.9, color = "firebrick") +
  labs(
    title = "Proportion of Red Beads Scooped by the Shovel",
    y = "Estimated Proportion of Red Beads",
    x = "Bead Color"
  ) +
...
This plot gives our best estimate for the proportion of red beads scooped, with the line showing the uncertainty around that estimate. The true value could be anywhere in that range.
...
```{r temperance-11}
question_text(NULL,
	message = "Our main quantity of interest is the proportion of red beads that we scoop, which we estimate to be about 0.66.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
