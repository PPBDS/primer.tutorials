...
We refer to this method as a census which, assuming zero error when counting, will return the **exact** portion of red beads in the urn. However, it can be quite expensive in terms of time, energy and money when the population is large or even impossible. 
...
question_text(NULL,
	message = "A sample is generalizable if any results based on the sample can generalize to the population.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
With the `urn` variable start a pipe and set it to `sample_frac()`.
...
Run `expand_grid()` with two arguments. The first argument is `trial_ID = c(1:100)`. The second argument is `shovel_size = c(1:100)`. We need a thousands trials to draw reasonable conclusions about what happens when the size of the shovel varies between 1 and 100.
