...
In your own words, describe the act of **Sampling**. 

<!-- TJ: We should tell them to give the context of sampling in our problem, otherwise they will not write the second sentence. -->
...
<!-- TJ: We should again make the question specific to the problem -->
...
The `slice_sample()` function is like a shovel in real sampling in which it goes into the `urn` and take out a desired number of samples. In this case, as we set `n` equals to 1, the output creates a 1x1 tibble.
...
<!-- TJ: Do we need to add mutate again? -->
...
<!-- TJ: The x-axis label does not have the proper $\hat{p}$, or so it seems -->
...
<!-- TJ: I feel we shouldn't be adding examples into the answers when not asked in question -->
...
<!-- TJ: We should tell them to use the '~' when necessary, otherwise they could get confused -->
...
<!-- TJ: Again, do we have to call mutate twice? -->
...
<!-- TJ: We should probably mention that the code takes a while to run otherwise they may think that something is not right. -->
...
<!-- TJ: Is this supposed to say console? -->
...
<!-- TJ: I feel it would be better if we used ### and broke it up a little -->
