...
```{r the-question-5}
question_text(NULL,
	message = "Imagine a variable called `senior`, indicating whether or not the person is a senior citizen. We can manipulate this, at least in theory, by checking if senior citizens are more or less likely to get arrested during a traffic stop.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
There can be many good answers for a question like this. However, for the rest of this section, you should stick with our treatment variable of `senior`.

Recall that in a treatment variable, when manipulated, we look for the difference in arrest rate to see whether or not being a senior citizen results in a higher chance of getting arrested.
...
> By how much does being a senior citizen in New Orleans affect the likelihood of getting arrested at a traffic stop?

Given this choice of treatment variable `senior`, how many potential outcomes are there for each arrest? Explain why.

```{r the-question-6}
question_text(NULL,
	message = "There are 2 potential outcomes because the treatment variable `senior` takes on 2 posible values: either being a senior citizen or not being one.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Write a sentence which speculates how our 2 potential outcomes would change if we manipulated our treatment variable of `senior`. 

<!-- SD: Maybe I should make it shorter and remove the reasoning for why I think senior citizens are less likely to be arrested - wdy think Preceptor? -->
```{r the-question-7}
question_text(NULL,
	message = "We expect that the likelihood of getting arrested would decrease When `senior` is true, as the cops may be more likely to trust the senior citizens. Likewise, when `senior` is false, we expect that the likelihood of getting arrested would be higher.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r the-question-8}
question_text(NULL,
	message = "For a given arrest, assume that the value of the treatment variable might be `is a senior citizen` or `isn't a senior citizen`. If the person `is a senior citizen`, then the likelihood of getting arrested would be 2%. If the person `isn't a senior citizen`, then the likelihood of getting arrested would be 15%. The causal effect on the outcome of a treatment of `senior = TRUE` versus `senior = FALSE` is 2 - 15 --- i.e., the difference between two potential outcomes --- which equals -13, which is the causal effect.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
> The likelihood of getting arrested during a traffic stop in New Orleans is reduced by 13% if the person is a senior citizen.
...
```{r the-question-9}
question_text(NULL,
	message = "Let's consider the variable `senior` again. Note that now, instead of being a treatment, it's our key covariate, whose connection to the outcome variable we most want to explore.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 2)
...
Using the same variable allows us to see the true differences between the two model types, based on similar questions and the same variables. Of course, you may have said something else, and that is completely fine, but we should stick with `senior` for the rest of this section.


### Exercise 10

Write a few sentences which specify two different groups of traffic stops with different values for `senior`. Explain how the average value of `arrested` might differ between these two groups of traffic stops.

```{r the-question-10}
question_text(NULL,
	message = "XX: Some traffic stops might have a value for `senior` of `TRUE`. Others might have a value of `FALSE`. Those two groups will, on average, have different values for `arrested`.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r the-question-11}
question_text(NULL,
	message = "What is the likelihood that a 50 year old Black Male is arrested at a traffic stop in New Orleans?",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- SD: Need help fixing the question and the Quantity of Interest -->
...
> What is the likelihood that a 50 year old Black Male is arrested at a traffic stop in New Orleans?

<!-- XX: Repeat the question with which you ended The Question topic. -->

<!-- Old specific question: How does race impact the likelihood of getting arrested in a traffic stop? -->
...
<!-- Not only are you discussing the data, but you are also discussing potential problems with the data, problems which will be more fully fleshed out in the Justice section. You want to pick 5 to 10 interesting facts about the data.
