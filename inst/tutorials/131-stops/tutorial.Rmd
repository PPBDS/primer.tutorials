...
<!-- DK: Change to a logistic regression. Which means you should change the code which creates x so that arrested becomes a factor variable with Yes/No values. -->
...
Remove everything below the YAML header from `stops.qmd` and render the file. `Cmd/Ctrl + Shift + K` renders the file, this automatically saves the file as well.
...
Edit the summary paragraph in `stops.qmd` as you see fit, but do not copy/paste our answer exactly. `Cmd/Ctrl + Shift + K`, and then commit/push.
...
Add `library(tidymodels)` and `library(broom)` to the `setup` code chunk in `stops.qmd`. `Cmd/Ctrl + Shift + K`.
...
<!-- DK: Print out all the rows. Probably not. Print out the top 5 rows and then note you have cut it off. -->
