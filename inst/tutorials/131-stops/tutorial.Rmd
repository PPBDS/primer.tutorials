...
<!--     sex + race*zone -->
<!--     draw = FALSE -->
<!--     plot just black versus white estimates by hand, with zone ordered by something interesting. -->
...
```{r wisdom-6}
question_text(NULL,
	message = "There are two outcomes for `arrested`: either getting arrested or not getting arrested.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r wisdom-13}
question_text(NULL,
	message = "FIX - XX. Make sure your words given an excellent description of the Preceptor Table which you are about to show the student.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- FIX - Insert introductory comments about the data -->
...
Add `library(tidymodels)`, `library(broom)`, and `library(equatiomatic)` to the `setup` code chunk in `stops.qmd`. `Command/Ctrl + Shift + K`.
...
<!-- DK: You can use exactly the knowledge drops from the template, but with the variable names/appearence edited to match your model. -->
...
Create another code chunk in `stops.qmd`. Add the chunk option: `label: math`. In that code chunk, add something like the below.
...
<!-- FIX - XX: You may use other arguments to tidy if that is useful. -->
...
It is best to ensure that the variable types in `ndata` match the variable types in `stops`. The easiest way to ensure that is by using `stops` itself in the creation `ndata`. 
