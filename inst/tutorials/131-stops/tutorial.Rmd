...
date_error <- stops |> select(arrested, date) |> mutate(all = sum(arrested), .by = date) |> ggplot(aes(x = date, y = all)) + geom_point()
...
<!-- SD: Should I add more to the EDA, and if so, what else should I add?
 - I thought of removing the first 6 months from `stops` but figured that it makes no difference to our model. -->
