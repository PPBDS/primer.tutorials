...
fit_stops <- linear_reg() |>
    set_engine("lm") |>
    fit(arrested ~ sex + race*zone, data = stops_new)
...
<!-- DK: Check the instructions for writing the summary paragraph in the template. They are much improved! Adjust your questions and answers accordingly. - DONE -->
...
<!-- DK: Fix. Probability of arrest for a Black driver would not be a bad QoI. - DONE -->

```{r the-question-12}
question_text(NULL,
	message = "A quantity of interest which might help us to explore the answer to our question would be the probabilily of arrest for a black driver.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
plot_predictions(fit_arrested,
                      newdata = expand_grid(zone = unique(stops_new$zone)),
                      condition = c("zone")) +
  coord_flip() +
  scale_x_reordered() +
  labs(x = "Zone", 
...
It is best to ensure that the variable types in `ndata` match the variable types in `stops_new`. The easiest way to ensure that is by using `stops_new` itself in the creation `ndata`. 
...
Add `library(marginaleffects)` and `library(tidytext)` to the `stops.qmd` setup code chunk. Create a new code chunk in `stops.qmd` which creates the `ndata` object. Don't forget to add `#| label: ndata` to this chunk. `Command/Ctrtl + Shift + K`.

At the Console, run:

```
tutorial.helpers::show_file("stops.qmd", pattern = "marginaleffects|tidytext|ndata")
```
...
If we used `data.frame()` instead of `expand_grid()` when creating the `ndata` object, we would've got an error. This is because the `tuition` and `selectivity` columns are not equal in length. Instead of finding all the possible combinations, `data.frame()` makes only one row for each value in the column. Example:

```
data.frame(race = unique(stops_new$race),
           sex = unique(stops_new$sex),
           zone = unique(stops_new$zone))
```
...
```{r}
message("Error in data.frame(race = unique(stops_new$race), sex = unique(stops_new$sex),  : 
...
We can now proceed to order our data. I want to arrange the graph in ascending order for arrest rates by sex.

To do this, pipe to `mutate()` and set the argument `zone` to equal `reorder_within(zone, estimate, race)`.
...
plot_predictions(fit_stops,
                 ...
                 condition = c("zone", "race", "sex"),
                 draw = FALSE) |> as_tibble() |> 
  mutate(zone = reorder_within(zone, estimate, race))
```
...
Now we should plot our findings. Continue the pipe to `ggplot()`, and inside aesthetics, map `x` to `zone`, `y` to `estimate`, and `color` to `race`.

Then add `geom_point()` and `geom_errorbar()`, with the arguments `aes(ymin = conf.low, ymax = conf.high)` and `width = 0.2`.
...
... |>
  ggplot(aes(x = ..., y = ..., color = ...)) +
    geom_point() + 
    geom_errorbar(...(ymin = conf.low, ... = conf.high), ... = 0.2)
```
...
plot_predictions(fit_stops,
                 ...
                 draw = FALSE) |> as_tibble() |> 
  mutate(zone = reorder_within(zone, estimate, race)) |>
  ggplot(aes(x = zone, y = estimate, color = race)) +
    geom_point() + 
...
Let's make 2 plots, one for each value of `sex`. Add `facet_wrap()`. Inside this, add `~sex` and set `scales` to equal `"free_y"`.

Additionally, add `coord_flip()` to put the variables on the y-axis.
...
Now let's do something about those ugly y-axis labels.
...
Second, let's make the labels a bit smaller so that they're easier to read. To do this, add the following to the previous code: `theme(axis.text.y = element_text(size = 5))`.
...
plot_predictions(fit_stops,
  ...
  ggplot(aes(x = zone, y = estimate, color = race)) +
    ...
    scale_x_reordered() +
    theme(axis.text.y = element_text(size = 5)) +
    labs(x = "Zone", y = "Estimated Arrest Probabilty",
         title = "Predicted Arrest Rate of New Orleans Motorists by Zones",
