...
Change the call to `fit()` to `fit(arrested ~ race, data = x)`.
...
tidy(fit_stops_logistic, conf.int = TRUE) |>
  ...
  gt() |>
  tab_header(
    title = "Logistic Regression Estimates for Arrest Probability"
  ) |>
  tab_source_note(
    source_note = "Source: Traffic stops dataset (filtered for Black and White drivers)"
  ) |>
  cols_label(
    term = "Variable",
    estimate = "Estimate",
...
In the Console, `predictions()` on `fit_stops`.
