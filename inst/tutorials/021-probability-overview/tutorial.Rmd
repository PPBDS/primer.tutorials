...
Write a short paragraph explaining when you would use a `Prediction` model in your life and a `Casual` model in your life.

```{r wisdom-2}
question_text(NULL,
	message = "A predictive model in my life can be seen as determining what school supplies I will need for my next year based on the data that I already have regarding my past years. A casual model in my life could be shown as the weather forecast which is predictive but when one value changes from the input we can figure out the output value.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
`Prediction` models aim to forecast outcomes based on existing data without considering the `causal` relationship between variables,focusing on estimating one potential outcome for each observation. `Causal` models aim to determine the effect of manipulating a specific variable on the outcome by comparing multiple potential outcomes for each observation. The difference between `prediction` models and `causal` models is that the former have one column for the outcome variable and the latter have more than one column.

### Exercise 3

Write a short paragraph explaining the concept of `Validity`.

```{r wisdom-3}
question_text(NULL,
	message = "Validity refers to the extent to which the data we have accurately reflects the underlying construct or phenomenon of interest. It involves assessing whether the variables and measures used in our analysis align with the research question and the Preceptor Table. Validity also relies on assuming that the data we have and the Preceptor Table are drawn from the same population, allowing for meaningful inferences to be made.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Key in making decisions are based on the assumption of `validity`. We need to understand whether the columns in the `Preceptor Table` match the columns in the data.
...
Write a two-sentence summary explaining the concept of `Stability`.

```{r justice-2}
question_text(NULL,
	message = "Stability assumes that the relationship between the outcome variable and the covariates remains consistent over time. It acknowledges the temporal nature of real-world data science problems and the assumption that the future will be similar to the past.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Write a two-sentence summary explaining the significance of `Representativeness`.

```{r justice-3}
question_text(NULL,
	message = "It is important for the data available to be representative of the population for which parameters need to be calculated. Random sampling from the population is ideal but often not feasible. Ensuring representativeness is crucial for drawing accurate inferences.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
`Stability` and `representativeness` are about the rows in our `Population Table`.

### Exercise 4

Write a two-sentence summary explaining the significance of `Unconfoundedness`.

```{r justice-4}
question_text(NULL,
	message = "Unconfoundedness is related to the absence of confounding variables that may bias the relationship between the outcome variable and covariates. It implies that there are no hidden factors influencing both the treatment (if applicable) and the outcome.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
