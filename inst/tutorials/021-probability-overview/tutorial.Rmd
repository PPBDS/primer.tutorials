...
In one sentence, provide a reason why the `assumption of validity` might not hold with the data from immigrants with Spanish-speakers on the train to our scenario of giving citizens a speech to be more kind. In the second sentence, explain how that failure might bias the results.

```{r wisdom-9}
question_text(NULL,
	message = "The assumption of validity might not hold because the tretaments are not close for it to apply as one scenario revolves around citizens that are directly interacting with Spanish-speakers, while one scenario revolves around a speech given by the mayor about immigrants. The failure might bias the results as we never expected the certain scenario to result in one way which will effect the way that we view our data.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
In one sentence, provide a reason why the assumption of `Representativeness` might not hold. In the second sentence, explain how that failure might bias the results.

```{r justice-3}
question_text(NULL,
	message = "The assumption of representativeness might not hold in this scenario because we do not have a random sample for our data. The failure might bias the results because our data will be tainted without a random sample. With a random sample we are able to accurately visualize the data.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
