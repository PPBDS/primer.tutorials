---
title: 'Probability'
author: Mann Talati, Yuhan Wu, and David Kane
tutorial:
  id: probability
output:
  learnr::tutorial:
...
Within the `summarise()`, add a new argument separate by comma, `stan_dev`, and then set `stan_dev` equal to the standard deviation of `numbers` using sd().Recall standard deviation is measuring how spread out our numbers are.
...
Within the `summarise()`, add a new argument separated by comma, `med_dev`, and set mad equal to the median absolute deviation of `numbers` using `mad()`. 
...
Last but not least, within `summarise()`, add `quantile`, use `quantile()`. Within it, set the first argument to `numbers`, and the second to `prob` which is equal to `c(0, 0.5, 1)`.
...
This will create another two rows, because quantile has three values in it's column, yet if we want to keep it in one rows, we can add `list()` around `quantile()`, so that we create a list-column for column quantile and keep everything else in 1 row.

### Exercise 7

Start a new pipe using `tibble()`, make the first column name as `ID`, and set it equal to the consecutive integer from 1 to 100. Then make a second column named `scores`, and set it equal using `rep()` where value `1350` occurs 20 times, value `1380` occurs 30 times, `1440` occurs 40 times, and `1520` occurs 10 times. Remember to group the `rep()` together using `c()`.
...
Now let's say you are an admissions officer, and here's the score, now you want to look at the percentile of that score. Use the summarize function to make a new column called "percentile", and set equal to `quantile()`, first argument is scores, the second is `prob`, and set it equal to (.35, .50, .75).
...
Now let's say instead of looking at the percentile you want to randomly pick 10 people from 1350 to 1520 score range to admit. Within `summarise()`, replace `percentile` to `admit`, and set it equal to `sample()`, within `sample()`, the first argument is `scores`, second set `size` equal to 10, and third `replace` which you set to `FALSE`.
...
We need to expand our collection of R tricks by learning about list-columns and `map_*` functions. Recall that a list is different from an atomic vector. In atomic vectors, each element of the vector has one value. Lists, however, can contain vectors, and even more complex object, such as elements.
...
Now assign the list we just created to an variable `x`, then run `x[[1]]`. It should returns the first element which are the 3 numbers.
...
With a positive test, you can be almost 70% sure that you have Probaphobia, however there is a good chance about 30% that you receive a false positive, so don’t worry too much there is still about a third of hope that you get the wrong result.
...
Now set `in_sample` to the function `map_int()`. The first argument to `map_int()` should be `in_bag`. The second argument should be `rbinom()` where `n` is set to 1, `size` is set to 3, and `p` is set to `./2`.
