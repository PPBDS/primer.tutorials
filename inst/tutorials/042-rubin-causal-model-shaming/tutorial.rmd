...
Notice that we haven't (and won't) calculate the casual effect, which is the difference between the potential outcomes in treatment group, and the control group. It is possible by figuring out the tao value [like so](https://ppbds.github.io/primer/rubin-causal-model.html#a-single-value-for-tau), but it's unnecessary since we wouldn't be able to get a good estimate for each question mark.
...
```{r percentage-ate-2-hint-3, eval = FALSE}
... %>% 
mutate(age =ifelse(age > 64, "old", "young"))
```
### 
