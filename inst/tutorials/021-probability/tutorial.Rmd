...
```{rpds pd-for-flipping-a-coin-6-test, include = FALSE}
  tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   binwidth = 0.5, 
...
<!-- DK: Change as you see fit. I feel like questions 3 and 4 are unnecessary. We should instead have two questions like number 5 where one is a causal model and another is predicative. Done -->

<!-- TJ: Decided to keep them both.  -->
...
<!-- TJ: Replaced number 7 to make it more consistent with the template  -->
...
<!-- TJ: Deleted question 8 because it was not in the tutorial. -->
