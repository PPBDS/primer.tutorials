...
Create a `tibble()`, with one variable `numbers`, and set `numbers` equal to the consecutive integers from 1 to 101 inclusive using `:`.
...
<!-- TJ: Is this a better knowledge drop? -->
...
<!-- TJ: changed variable name to make it less confusing -->
...
Now let's say you are an admissions officer, and here's the score, now you want to look at the percentile of that score. Use the `summarize()` function to make a new column `percentile`, and set equal to `quantile()`, first argument is `scores`, the second is `prob`, and set it equal to `c(.35, .50, .75)`.
...
Make a new `tibble()`, set the value `heads` equals to `rbinom()`, with the first argument equal to `1000`, second argument `size` equal to `1`, and the third argument `prob` equal to `0.5`.
...
We can give a quick look at what the graph looks like, pipe `tibble()` to `ggplot()`, within `ggplot()`, use `aes()` and set `x =  heads`, then add the layer `geom_bar()`. Set `binwidth` to 0.5
...
```{r pds-with-flipping-a-coin-1-test, include = FALSE}
...
```{r pds-with-flipping-a-coin-2-test, include = FALSE}
tibble(results = sample(c(0, 1), 100, replace = TRUE))
```
...
```{r pds-with-flipping-a-coin-3-test, include = FALSE}
tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram()
```
...
```{r pds-with-flipping-a-coin-4-test, include = FALSE}
tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram(aes(y = after_stat(count/sum(count))))
```
...
Also set `binwidth` to .5 and `color` to "white" within `geom_histogram()`.
...
```{r pds-with-flipping-a-coin-5-test, include = FALSE}
  tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   binwidth = 0.5, 
...
```{rpds pds-with-flipping-a-coin-6-test, include = FALSE}
  tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   binwidth = 0.5, 
...
```{r pds-with-flipping-a-coin-7-test, include = FALSE}
  tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   binwidth = 0.5, 
...
```{r pds-with-flipping-a-coin-8-test, include = FALSE}
 tibble(results = sample(c(0, 1), 100, replace = TRUE)) |> 
  ggplot(aes(x = results)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   binwidth = 0.5, 
...
```{r pds-with-rolling-two-dice-1-test, include = FALSE}
tibble(ID = 1:100)
```
...
<!-- ```{r pds-with-rolling-two-dice-2-test, include = FALSE} -->
<!-- tibble(ID = 1:100) |> -->
...
```{r pds-with-rolling-two-dice-3-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)))
```
...
```{r pds-with-rolling-two-dice-4-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
...
```{r pds-with-rolling-two-dice-5-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            die_2 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            sum = die_1+die_2)
...
```{r pds-with-rolling-two-dice-6-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            die_2 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            sum = die_1+die_2) |>
...
```{r pds-with-rolling-two-dice-7-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            die_2 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            sum = die_1+die_2) |>
...
```{r pds-with-rolling-two-dice-8-test}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            die_2 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            sum = die_1+die_2) |>
...
```{r pds-with-rolling-two-dice-9-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            die_2 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            sum = die_1+die_2) |>
...
```{r pds-with-rolling-two-dice-10-test, include = FALSE}
tibble(ID = 1:100) |>
                     mutate(die_1 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            die_2 = map_dbl(ID, ~sample(c(1:6), size = 1)),
                            sum = die_1+die_2) |>
...
```{r pds-with-rolling-two-dice-11-test}
tibble(ID = 1:100) |> 
  mutate(die_1 = map_dbl(ID, ~ sample(c(1:6), size = 1))) |> 
  mutate(die_2 = map_dbl(ID, ~ sample(c(1:6), size = 1))) |> 
  mutate(sum = die_1 + die_2) |> 
...
```{r pds-with-engine-displacement-1-test, include = FALSE}
glimpse(mpg)
```
...
```{r pds-with-engine-displacement-2-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) 
```
...
Use `select()` to pick out the `displ` column which is what we want. Then use `drop_na()` to drop the rows that contains NA which we don't want. 
...
```{r pds-with-engine-displacement-3-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) |>
  select(displ)|>
  drop_na()
...
```{r pds-with-engine-displacement-4-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) |>
  select(displ)|>
  drop_na() |>
...
```{r pds-with-engine-displacement-5-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) |>
  select(displ)|>
  drop_na() |>
...
```{r pds-with-engine-displacement-6-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) |>
  select(displ)|>
  drop_na() |>
...
```{r pds-with-engine-displacement-7-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) |>
  select(displ)|>
  drop_na() |>
...
```{r pds-with-engine-displacement-8-test, include = FALSE}
mpg |>
  filter(drv == "f", cty >= 16) |>
  select(displ)|>
  drop_na() |>
...
Use `mutate()` to create a new column called `N`, set `N` equal to `sample(rnorm(10),size=1))`. 
...
<!-- TJ: Since there are many different plots, I did not make one here -->
...
```{r working-with-pds-with-rbinom-1-test, include = FALSE}
# tibble(heads = rbinom())
```
...
```{r working-with-pds-with-rbinom-2-test, include = FALSE}
...
```{r working-with-pds-with-rbinom-3-test, include = FALSE}
tibble(heads = rbinom(n = 10000, size = 10, prob = 0.5)) |>
  ggplot(aes(x = heads))
```
...
```{r working-with-pds-with-rbinom-4-test, include = FALSE}
tibble(heads = rbinom(n = 10000, size = 10, prob = 0.5)) |>
  ggplot(aes(x = heads)) + 
  geom_bar()
```
...
```{r working-with-pds-with-rbinom-5-test, include = FALSE}
tibble(heads = rbinom(n = 10000, size = 10, prob = 0.5)) |>
  ggplot(aes(x = heads)) + 
  geom_bar() +
...
```{r working-with-pds-with-rbinom-6-test, include = FALSE}
tibble(heads = rbinom(n = 10000, size = 10, prob = 0.5)) |>
  ggplot(aes(x = heads)) + 
  geom_bar() +
  labs(title = "Flipping a Fair Coin 10 Times",
...
```{r working-with-pds-with-rbinom-7-test, include = FALSE}
...
```{r working-with-pds-with-rbinom-8-test, include = FALSE}
tibble(heads = rbinom(n = 10000, size = 10, prob = 0.75)) |>
  ggplot(aes(x = heads)) + 
  geom_bar() +
  labs(title = "Flipping a Fair Coin 10 Times",
...
<!-- TJ: I feel like questions 3 and 4 are unnecessary. We should instead have two questions like number 5 where one is a causal model and another is predicative-->
