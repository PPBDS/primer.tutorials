...
The `knitr` package gives the tools to embed both tables and pictures into our website. There are also several other packages which can render tables with different features. 
...
In your console, run `install.packages("knitr")`. Then, in the setup chunk, load the library `knitr`. Save `index.Rmd`
...
In your console, run `install.packages("rmarkdown")`. Then, in the setup chunk, load the library `rmarkdwon`. Save `index.Rmd`
...
`paged_table` produces a table similar to the RMarkdown table output. Its chief advantage is that it allows for the table to have pages and take up less space. 
...
For an even wider window for rendering objects in Distill, use `layout = "l-page"`. 
...
In your console, run `install.packages("gt")`. Then, in the setup chunk, load the library `gt`. Save `index.Rmd`
...
To add an image to a Distill website, we can use the `include_graphics()` function which is part of the `knitr` function.
...
In the Terminal, run `pwd` to confirm you are in the main project directory. Then run `mkdir images`. Then run `ls` and make sure images is one of the files in this list. Copy and paste these commands and their results in the box below. 
