...
<!-- DK: Instead of "other window," we should use a phrase like "other RStudio instance" or whatever.  -->

<!-- DK: Need more advice/discussion/warnings about what happens if the Build tab does not appear. This happened to several students. Tell them to close RStudio. Then use upper right project list to open --- by clicking the image on the right --- the test distill which they were working on. -->

<!-- DK: .gitignore should disappear  Should be test_distill.Rproj (or whatever the name is) should disappear. -->

<!-- DK: Need images to walk students through the Github pages stuff. -->

<!-- DK: Is the sdiff stuff really necessary? Don't keep around questions just because you wrote them. -->

<!-- DK: Need to cover the problems we saw in class. Get them to create the error by hand. Then have them fix it by hand. The cause is unclear to me. It is either having a random Rmd file in the project or it is messing up the yml file, which is easy to do. Or it is the failure to use create_article(). -->
...
In the other window that you just opened. Go to File and the New Project. Select Version Control and then Git. Paste the URL to clone the Github repo into the repository URL box. The name should be automatically filled in based on the name of the repository. In the bottom right corner, select the option to open this project in a new session. 

###

In the Terminal of the other window, run `pwd` to confirm that you are in the correct directory, i.e. a directory named `distill_tutorial` in the place you normally keep R projects. Copy and paste the command and result below.
...
In the Terminal of the other window, run `ls -a` to view a list of all the files in the directory. You should see a `README.md` which we made when we created the github repo. You should also see two hidden files, `.gitignore` and `.git`. 
...
In the Console of the other window, load the `usethis` package. Then run the command `git_sitrep()`. Copy and paste the output below. 
...
To set up and manage the files needed to create a website with R Markdown files, we use the package `distill`. In the Console of the other window, use the `library()` function to load the package. 

###

To confirm that you have successfully installed and loaded the package run the function `search()` in the console of the other window. Copy and paste the output below. 
...
In `index.Rmd` (in the other window), replace the example description with the string "This is where I will describe my final project." Knit this file. This opens a preview of what this page of the website will look like.

###

In the Terminal of the other window, run `head index.Rmd`. This command prints the first ten lines which should include the text of your new description. Copy and paste the command below. 
...
Building the website rebuilds all the documents necessary to deploy the site, which live in your `docs` directory. In the Terminal of the other window, run `ls -l docs`.  Copy and paste the output below.
...
In the Terminal of the other window, run `cat .gitignore`. This should return the contents of your edited `.gitignore` file. Copy and paste the output below.
...
In the Terminal of the other window, run `git log`. Copy and paste the output below. 
...
In the other window, open index.Rmd, which should be in the same directory as the .Rproj for your website. In the setup chunk, load the library `tidyverse` and the library `ggthemes`. 
...
Here you will learn how to create new pages for your wesbite. You will learn how to make these accessible to users of your website through the navigation bar. 
...
In the setup chunk, load the library `rmarkdwon`. Save `index.Rmd`
...
`md` stands for markdown and allows the same formatting syntax as used in markdown to be used in the string. You can also itaclicize using this function and only one asterisk on each side.  
...
In the Terminal, run the command `grep -F "@" source_A.Rmd` Copy and paste the result. 
