...
```{r pfile-1-setup}
file.copy(system.file("www/ex_1.csv", package = "primer.tutorials"), ".")
```
...
```{r pfile-6-setup}
file.copy(system.file("www/ex_2.csv", package = "primer.tutorials"), ".")
```
...
Continue your pipe with `mutate()`. The first argument to `mutate()` should be `b`. The second argument should be `format`. Set `format` to `%Y%M%D%`.
