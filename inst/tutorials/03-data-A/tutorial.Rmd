...
Set the argument `x` to the tibble you want to save (i.e. tbl_2)
```

```{r ex-9-hint-2, eval = FALSE}
Set `file` to the path where you want to save the tibble on your computer.
```

```{r ex-9-hint-3, eval = FALSE}
...
Use `write_rds()` to save this tibble to a file named "tbl_1.rds". 
...
Set `x` to the tibble you want to save (i.e. tbl_1)
```

```{r rds-1-hint-2, eval = FALSE}
Set `file` it to the path where you want to save the tibble on your computer.
```
