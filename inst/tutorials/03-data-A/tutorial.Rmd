...
To look at the problems in more depth, use the function `problems()`with the argument `p_file`. 
...
If we look at the last few rows, you’ll see that they’re dates stored in a character vector. Use the function `tail()` with the argument `p_file`.
...
Notice that here we need to use a date parser instead. 
