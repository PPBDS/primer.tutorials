...
Use `write_csv()` to save this tibble to a .csv file named "tbl_2.csv". 
...
```{r ex-1-hint, eval = FALSE}
First use the `x` and set it to the tibble you want to save (i.e. tbl_2)
```

```{r ex-1-hint, eval = FALSE}
Use the `file` argument and set it to the path where you want to save the tibble on your computer.
```


### Exercise 9

Great? Now run `list.files()` in your console and paste the result in the space below. 
...
<!-- DK: One question for write_delim().  -->
...
Use `write_rds()` to save this tibble to a .rds file named "tbl_1.rds". 
...
```{r ex-1-hint, eval = FALSE}
First use the `x` and set it to the tibble you want to save (i.e. tbl_1)
```
...
Now run `ls()` in your console and paste the result in the space below. 
...
<!-- BG: paste file in r chunk here -->

Great. Let's now use `read_rds()` to read in the newly created file!

```{r ex-3-rds, exercise = TRUE}

```

```{r ex-1-rds-hint-1, eval = FALSE}
Use the file argument and set it to "tbl_1.rds"
```
