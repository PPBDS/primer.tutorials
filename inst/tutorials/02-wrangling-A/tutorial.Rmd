...
Use `str_detect()` to create a vector with value `TRUE` for states which contain the pattern "ana" and `FALSE` otherwise. The first argument to `str_detect()` should be `state.name`. The second argument set `pattern` to "ana".
...
Great. Now use  `str_detect()` on `state.name` to create a vector with value `TRUE` for states which contain the pattern "do" and `FALSE` otherwise.
...
To represent a wildcard in a serach term, i.e. any single character, use a `.`. Use `str_detect()` to find states that match state names that have a “a,” followed by any single character, followed by “o.” 
...
Great. Now use `str_sub` to create a character vector that contains only the first three letters of each state. The first argument should be the character `position` from where you want to start, and the second argument should be the character position where you want to end. 
...
Collapse `state.name` using `str_c()`. Use the argument `collapse` to assign how you want to separate the items of the vector. Separate them with a comma that is followed by a whitespace. This should create a single character object with all the states.
...
Use `str_replace()` to replace the pattern `North` with `N.`. Set the second argument `pattern` to  the string in the current vector which you want to change. Set the third argument `replacement` to the string which you want to replace the pattern with. For example, North Carolina becomes N. Carolina.
...
Use `str_replace()` to replace the pattern `South` with `S.`. For example,  South Carolina becomes S. Carolina.
