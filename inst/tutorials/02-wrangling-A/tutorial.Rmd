...
<!-- Need to deal with police data. Either replace it and put it in primer.data. -->
...
Run the following code: `x <- read_csv("https://raw.githubusercontent.com/Financial-Times/police-misconduct-complaints-analysis/main/output/philly_clean.csv", col_types = cols())`. This downloads data on 2016 PA police complaints and saves it to an R object named `x`. For your convenience, this code will be included in all the following exercises. 

```{r str-1, exercise = TRUE}
x <- read_csv("https://raw.githubusercontent.com/Financial-Times/police-misconduct-complaints-analysis/main/output/philly_clean.csv", col_types = cols())
```

### 

The argument `col_types = cols()` tells R to use its default reasoning to assign column types. R will still do this if this argument is ommitted, but will also output a lengthy message telling you each column type it assigned.

### Exercise 2

Start a new pipe with `x`, i.e type `x` on a line below the `read_csv()` and then place the `%>%` operator after this `x`. Select the columns `po_race`, `district_occurence`,`investigative_findings`, and `summary`. 
...
x %>% 
  select(po_race, district_occurence, investigative_findings, summary) %>% 
...
Now use `nlevels()` with the argument `cces_filt$ideology`. Notice that the same number of levels pop up.
