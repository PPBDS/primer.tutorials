...
<!-- DK: The model fitting process can't take more than a minute or two. The easiest way to speed things up is to just use a random subset of the data. Update ch10_data creation code. -->

<!-- DK: Make The Question section. Read the randomling discussion template_tutorial. Look at Chapter 9 example. -->
...
Set all of this to an object `ch10_data`. When you run again, nothing should print out because the code is being set to an object.

### Exercise 24

Start a new pipe with `ch10_data` to `sample_frac()` with `0.5` as the parameter.
...
Add the code from Exercise 23 to have a `ch10_data` object in a new code chunk in `causal_effect.qmd`. Also add the code from the previous exercise to add the graph in the file. Render the file. 
