...
Now that we have the `newdata` object, we can create a pipe which uses out fitted model to answer our question. Begin by typing `fit_vote` and clicking "Run Code".
...
Pipe `fit_vote` to `add_epred_draws()` with the argument `newdata = ndata`.
...
fit_vote |> add_epred_draws(newdata = ndata)
```
