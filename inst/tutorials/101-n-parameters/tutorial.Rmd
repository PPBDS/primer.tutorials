...
Let's consider a *predictive* model. Which variable in `shaming: the tibble` do you think might have an important connection to `primary_06: the outcome variable`? (If you don't see a reasonable variable in the data, you can just name a variable which *might have been* included in the data.)
...
Again, there is no right answer. There are other interesting variable that we might also explore, including voting history, donations to political campaigns, age, race, and, obviously, any postcard received. 

There are many variables which might be connected to someone's decision to vote. But, again, in this tutorial, we have selected the outcome and the key covariate which will be used.
