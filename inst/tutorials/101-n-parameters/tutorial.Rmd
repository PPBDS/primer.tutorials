...
mod <- logistic_reg(penalty = 0.001) |> 
	set_engine("glmnet") |> 
	fit(voted ~ sex + age + treatment, data = x) 
...
mod_lm <- linear_reg() |> 
	fit(primary_06 ~ sex + age + treatment, data = x) 


# DK: Save these objects out.	

# DK: Does plot_predictions not work with logistic!	
# But it works with glm!

z <- glm(primary_06 ~ 
        age + sex + treatment*voter_class, data = x)			
...
coefs <- tidy(fit_vote, conf.int = TRUE) |>
...
> x
A tibble: 344,084 × 6
   primary_06 treatment   sex      age civ_engage voter_class   
        <int> <fct>       <chr>  <int>      <int> <fct>         
 1          0 Civic Duty  Male      65          4 Sometimes Vote
 2          0 Civic Duty  Female    59          4 Sometimes Vote
 3          1 Hawthorne   Male      55          4 Sometimes Vote
 4          1 Hawthorne   Female    56          4 Sometimes Vote
 5          1 Hawthorne   Female    24          4 Sometimes Vote
 6          0 No Postcard Male      25          1 Rarely Vote 
````

Note how the process of cleaning the data starts to make the data, which is just a table with rows and columns, look like the Preceptor Table. This is necessary since we soon, conceptually, stack the Preceptor Table on top of the data in order to make the Population Table.
...
3) Change the code so that the model is different and/or more complex. Maybe ask a question about the two models. Maybe just tell the student what she should notice.
...
<!-- DK; Add AI version here and in template_tutorial. -->
...
```
tutorial.helpers::show_file("postcards.qmd", pattern = "tidymodels|broom")
```
...
Continue the pipe to `fit(as.factor(primary_06) ~ sex, data = x)`.
...
logistic_reg(engine = "glm") |> 
	fit(as.factor(primary_06) ~ sex, data = x)
```

### 

If you tried, `fit(primary_06 ~ sex, data = x)` instead, you would get an error message: "For a classification model, the outcome should be a <factor>, not an integer vector." We can fix this by either transforming `primary_06` into a factor on-the-fly, as we do here, or by going back to our data cleaning code and fixing it there.
...
The meaning of the magnitude of the coefficient values are much harder to interpret in logistic models then they are in linear models. But, the fact that the coefficient is positive means that men are more likely to vote than women, and the fact that the confidence interval excludes zero means that we can be fairly certain of that claim.

### Exercise 9

Change the call to `fit()` to `fit(as.factor(primary_06) ~ voter_class, data = x)`.
...
logistic_reg(engine = "glm") |> 
	fit(as.factor(primary_06) ~ voter_class, data = x) |> 
	tidy(conf.int = TRUE)
```
...
Because the coefficients of the dummy variables for "Sometimes Vote" and "Always Vote" are positive, and their confidence intervals exclude zero, we can be fairly certain that people in these categories are more likely to vote than "Rarely Vote" voters.

### Exercise 10

Change the call to `fit()` to `fit(as.factor(primary_06) ~ sex + age + treatment*voter_class, data = x)`.
...
Make sure it works. You might need to add some new libraries, e.g., **tinytable**, **knitr**, **gt**, **kable**, **flextable**, et cetera, to the `setup` code chunk, if you use any functions from these packages, all of which have strengths and weaknesses for making tables.
...
Interpret -2.43, the estimate for the `(Intercept)`.

```{r temperance-2}
question_text(NULL,
	message = "The value for the intercept covers the case for all the variable levels which were 'dropped' during estimation. In this case, it is for women (since a dummy variable for Male is included) who did not receive a postcard and who rarely vote, and whose age is zero. Of course, the last fact makes interpreting the exact value of the Intercept largely nonsense. Also, since this is a logistic model, the magnitude of any parameter is difficult to interpret in terms of changes to the outcome, in this case someone's likelihood of voting.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Interpret 0.28 and 0.44, the confidence interval for the estimate for the coefficient for `treatmentNeighbors`.
...
<!-- DK: Does plot_predictions not work for logistic models?!? -->
