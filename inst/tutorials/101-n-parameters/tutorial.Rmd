...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"XX"` -- and an author (you). Render the document and save it as `postcards.qmd`.
...
`Cmd/Ctrl + Shift + K` follows, perhaps with a show_file("postcards.qmd", chunk = "Last")
-->
...
In the Console, `predictions()` on `fit_vote`.
...
In the Console, run `plot_predictions()` on `fit_vote` with [XX: these arguments/values].
...
# plot_predictions(fit_vote, condition = "XX")
```
