...
<!-- SJ: Both tutorial 9 and 10 have the same questions on what is wisdom/courage/justice/temperance and like defining a preceptor table. If you asign both tutorials for one homework assignment, it could get slightly repetitive. -->
...
Why do we ask this, and a score more of other questions, in each tutorial? Because the best way to (try to) ensure that students remember these concepts more than a few months after the course ends is with [spaced repetition](https://en.wikipedia.org/wiki/Spaced_repetition), although we focus more on the repetition than on the spacing.
...
<!-- SJ: This question was pretty confusing and some additional guidence like an example for a different data set would be nice or a more in-depth explanation of what you want here. -->
...
<!-- Sj: This question just seems unneccessary. A binary variable will always only have 2 outcomes and we could just say that instead of a question. That might just be me being more famillar with this stuff however. -->
...
<!-- SJ: For me, the add a gitignore file was off by default and I had to manually turn it on. We should specify to turn on the gitignore file. -->
...
<!-- SJ: I was pretty confused with the whole latex thing here even with AI. Additional help might be needed. -->
...
In the Console, run `check_predictions(extract_fit_engine(fit_vote))`. (This will produce an error.) CP/CR.

<!-- SJ: This worked just fine for me so idk. -->
...
This is a good summary plot because it allows us to explore a wide variety of causal effects, under the assumption that the causal effect of a postcard depends both on the content of the postcard and on the type of person receiving it. However, the plot is far from perfect!

* We don't need to see both the probability of voting and the probability of not voting. The former is enough.

* We are interested in causal effects, not just predictions. So, instead of looking at the probability of voting for those who get no postcard (the control group) and the probability of voting for each type of postcard (the four different treatments), we really want to plot the *difference* between the the probability of voting for a given treatment and the probability of voting for the no postcard people.

* Plot would be easier to interpret if all the y-axes were the same.
...
<!-- SJ: Plot predictions should be explained better. -->
### Exercise 10
...
```{r temperance-12}
question_text(NULL,
	message = "Receiving a postcard specifying how their neighbors voted increase the likelihood of voting for highly engaged citizens by 8%, plus/minus 1%.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
How did we come up with this from that graphic. Well, you can sort of eyeball that quantity, looking at the "Always Vote" points. Also, if you run:
...
You get a data.frame with the relevant data.
...
<!-- SJ: I couldn't find how to publish the QMD and positron is still relativly new so the internet wasn't of much help. We could maybe explain how to publish. I could also just be stupid. -->
