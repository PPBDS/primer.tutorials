...
Variables of particular interest to us are sex, the size of the respondent’s household, and whether or not the respondent voted in the 2006 primary election. These covariants are included in the data and they could be helpful for understanding the data.
...
Add the code from Exercise 23 to have a `data` object in a new code chunk in `causal_effect.qmd`. Also add the code from the previous exercise to add the graph in the file. Render the file. 
...
<!-- MK: not sure if this is the best reason, does this make sense as a reason why unconfoundedness might not be true or is this more of validity?-->

```{r justice-8}
question_text(NULL,
	message = "A reason why unconfoundedness might not be true is because not everyone reads the postcards and they might just throw away the postcard. Or maybe something inturepted the postcard from reaching its destination. This might could mess with our data but, for the most part, we do not have to worry because random assignment ensures unconfoundedness.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
