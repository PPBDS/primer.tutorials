...
data <- shaming |> mutate(p_00 = (primary_00 == "Yes"), p_02 = (primary_02 == "Yes"),
...
voter_class = factor(voter_class, levels = c("Rarely Vote", "Sometimes Vote", "Always Vote")), age_z = as.numeric(scale(age))) |> rename(voted = primary_06) |> select(voted, treatment, sex, age_z, civ_engage, voter_class) |> drop_na()
```
...
```{r wisdom-8}
question_text(NULL,
	message = "Covariates which might be useful include voting history, donations to political campaigns, age, race, and, obviously, any postcard received.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
There are many variables which might be connected to someone's decision to vote. For this question, we must, at least, have information about treatment received for each individual and, given the question, some variables related to political engagement.
...
```{r wisdom-9}
question_text(NULL,
	message = "The treatments are postcards of various types.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Our question is focusing on postcards. But if we were concerned with the effect of website advertisement, then our treatments would be the types of advertisement. 
...
We cannot chose a variable that we cannot change to be the treatment. For example, household income might have an impact on the person's choice to vote or not. But we can't magicaly change someone's salery for the sake of an experiment. We have to choose something that we can actually manipulate. We have random assignments so we can ignore the other variables and treat them as though they are the same for everyone. This way, we can focus solely on the relationship between the treatment an the outcome.
...
```{r wisdom-15}
question_text(NULL,
	message = "This data set is from a field experiment prior to the August 2006 primary election in Michigan. A total of 180,000 households were randomly assigned to either a control group or one of four treatment groups. The participants' past poitical engagement is also measured.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Variables of particular interest to us are sex, the size of the respondent’s household, and whether or not the respondent voted in the 2006 primary election.
...
x <- shaming |> mutate(p_00 = (primary_00 == "Yes"),
         p_02 = (primary_02 == "Yes"),
         p_04 = (primary_04 == "Yes"),
         g_00 = (general_00 == "Yes"),
         g_02 = (general_02 == "Yes"),
...
Converting the Yes/No columns to binaries with the function we made. Note that primary_06 is already binary and also that we don't need it to construct a variable based on previous voting behavior.
...
In this graph, we are trying to see if the civic engagement of the individual has an impact on weather or not they voted. 
...
```{r wisdom-27-test, include = FALSE}
data |> sample_frac(0.5) |> 
  ggplot(aes(x = civ_engage, y = voted)) +
    geom_jitter(alpha = 0.03, height = 0.1) +
    scale_x_continuous(breaks = 1:6) + 
...
Add the code from Exercise 23 in a new code chunk in `causal_effect.qmd` to have a `data` object. Also add the code from the previous exercise to add the graph in the file. Render the file. 
