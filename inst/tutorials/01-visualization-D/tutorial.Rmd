...
Notice that pulse has 1437 missing values. 

### Exercise 2

Filter `nhanes` so that it contains only rows whose `age` column has a value greater than or equal to 18. Save the result to a new object called `nhanes_a`
...
The mean and median are both different ways to measure a central value of data. Their values are similar, but not exactly the same. In the plot below, the blue line represents the mean, and the red line represents the median. Both values are  close to the mode, the pulse value which occurs most often. 
...
The two outer lines in red show the approximation of the 95% confidence interval by the median average deviation. The MAD deviation an even worse approximation of the real intervals for this data than the standard deviation. 
