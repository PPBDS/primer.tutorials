...
```
If you want to select a column based on the starting alphabets of the column name then you could use `starts_with()` within filter.
###

### Exercise 2

If you know what order the columns are in and are too lazy to type out every column name then within `select()`, type out the starting column then a semicolon telling R it is inclusive,and the ending column. When it is run, you will get all the columns that are in between the start and end. Select the colums between year and day.
...
Now if you want to select all the columsn except the colums from year to day(inclusive), then we add a `!` to imply that we dont want the column from year to day.
...
If you want to select colums whose type is a character, then we can use `where(is.character)`, so go edit the code to find the columns whose type is a character.
...
```
if want to select colums which go in a range with the same starting alphabets then you could use `num_range()` and example would be `num_range("x", 1:3)`: which matches x1, x2 and x3
###
