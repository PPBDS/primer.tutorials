...
The main function of `filter()` is display rows based on the arguement it gets. First create a pipe with the "flights" tibble and use `filter()` to find all the flights that arrived more than 120 minutes.
...
In `filter()`, we can use multiple conditions like "<",">","==",">=","<=" to find the rows that we need. Now Filter dosen't modify/change the original tibble, it only finds rows based on the arguement. 

###
### Exercise 3

Let's modify the previous code, change the arguement in `filter()` to find the flights on the first of January. We should have two variable in the arguement which are "month" and "day". To have the function give us the flights on Jan 1st, we need to use the "and" operator `&` which gives us data for Jan 1st only.
...
If we want flights from either January or the first of any month, we could use the "or" opearator `|` like "filter(month == 1 | month == 2)"
...
<!-- AB: Add an introduction just like all the other sections -->
...
<!-- AB: Maybe talk about the function `colnames()` which is used to change column names -->
