...
<!-- DK: Do chapters 9 and 10. SG: Done-->
...
Your request should be queued for a little. You should receive a notification in your email when your request is ready and it should say completed in the "Extracted Requests" page. Download your "TABLES" and "GIS" files in the same directory (where you normally keep your R projects).
...
Create a new R markdown document in your R project folder titled "NHGIS_data". Click the blue save icon in top left and save the document in the same name. Run `list.files()` in your console and copy/paste the output below.
...
"NHGIS_data" should be one of the files listed. The csv.zip and shape.zip files should also be listed as you should have downloaded them to this folder. If they are not listed use `setwd()` in the console to set your working directory to your normal R project directory. 
...
Make sure your RStudio is running in your normal R project directory and the csv and shape files are within the R directory. 
...
You will need to unzip your shape.zip file. If on mac, open the zip file with "archive utility" and it should create a new subdirectory that ends in shape, not shape.zip. If you open this subdirectory you will see a file that says "nhgis0001_shapefile_tl2000_us_tract_1910.zip".
...
<!-- SG: How should I tell them to unzip for windows? -->
...
Create a new R Markdown file titled "IPUMS_Graph". Then save the file and name it "index.Rmd". Make sure this is in your regular R project folder.
...
This will be the file that we're coding in for the rest of this tutorial. 
...
You will need to first download the data via Download.DAT, then you will need to save the DDI and R links (via ‘save as’) in the same location as your .dat.gz file. Then relocate these files into your R project folder.
...
Then, filter out the state and county so that you only request data about Illinois and Cook County. Make sure to use acronyms like "IL" and "COOK" rather than the full name. Also, remember to press "Enter" to apply the filter. Otherwise, you may pull out data about the entire country rather than your county.
...
Create a new R Markdown file titled `deserts.Rmd` and delete all of the text except for the setup chunk and the YAML header. Then, load the **tidyverse**, **tidycensus**, **jsonlite**, and **ggthemes** libraries into the setup chunk using the `library()` command. Make sure this is in your regular R project folder.
