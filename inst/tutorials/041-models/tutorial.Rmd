...
If this checks out, Commit & Push your changes to GitHub. REMEMBER TO ADD A COMMIT MESSAGE!


### Exercise 6

<!-- DK: This is not how things work! Read the template_tutorial! You only show this around Exercise 12, after all the questions about it. How else will students learn to build their own? - DONE -->
...
Paste your sentence from the last exercise into your `models.qmd` file.
...
At the very top, add `#| cache = true`. 
...
Now run the code again. See how much quicker that was! This is because of the `#| cache = true` argument, where it stores the outcome after running it, making it quicker to run the same code multiple times. 


### Exercise 9

Add this `brm()` model to the end of your `models.qmd` file. 

Run `show_file("models.qmd", start = -10)` in the Console. CP/CR.
...
<!-- XX Say some general words about the object. Note that we are about to go through the top 4 rows. - DONE -->

The “Formula” is `biden ~ 1`, which is what we passed in to the formula argument when we called `brm()`.

<!-- Add summary() Exercise if it gives a different answer - NOT NEEDED -->
...
It would be a good idea for us to include this expression in our `models.qmd` file. So, add the following line at the bottom of the file (doesn't need to be in a code chunk): `$$ biden_i  \sim Bernoulli(\rho) $$`
...
Note that what you entered is in LaTeX math.
...
<!-- DK: Agreed on below. Do it. - DONE -->

`pp_check()` is a function from the *bayesplot* package, which is a completely different way (from ggplot2) of making graphs.
...
Now replace your old paragraph in `models.qmd` with this new paragraph.
...
Here's our Population Table from earlier:
...
You might need to refer to this later.
...
We use `select()` so it only selects the `.epred` column, instead of displaying the entire dataset.
...
Now add this graph to the bottom of your `models.qmd` file. We'll be checking the file itself rather than making you use `show_file()`.
...
Now replace our previous paragraph with this paragraph.
...
Now re-arrange the different pieces in your `models.qmd` file so that the model is at the top, followed by the math expression, and then the paragraph at the end.

Finally, publish your `models.qmd` file to RPubs and title it `models`. Paste the link to your RPubs doc in the box below.
