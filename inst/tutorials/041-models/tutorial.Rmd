...
fit_biden <- glm(biden ~ 1, data = poll_data, family = binomial)
...
The biden variable contains "Yes"/"No" responses, stored as a factor. When we use it in a logistic regression model, R automatically converts it into a dummy variable, treating one level (typically "Yes") as 1 and the other ("No") as 0.
...
In a logistic regression model, the intercept (β₀) represents the log-odds of voting for Biden when all predictor variables are 0. The slope (β₁) shows how those log-odds change when the predictor increases by 1 unit.

### Exercise 9

Let’s fit the final version of our model — just the intercept. This tells us the ***overall level of support** for Biden in the poll.

Change the call to fit() to `fit(biden ~ 1, data = poll_data)`
...
This final model includes no predictors — just an intercept. That’s all we need to estimate overall support for Biden.

We’re not explaining why people support him. We’re just estimating how many do.
...
In the Console, run `check_predictions(fit_biden)`. CP/CR.
...
The model uses only an intercept, so it predicts the same probability of Biden support for everyone—just the overall average. It's a simple baseline but doesn't explain any variation. To improve it, we’d need to add predictors like age or education.
...
Note the differences. First, we have replaced the parameters with our best estimates. Second, we have dropped the error term because this is a formula for predicting the value of our outcome variable. Third, the left-hand side variable is $\widehat{\text{biden}}$ instead of $\text{biden}$ because this formula generates our estimated `biden.` A hat indicates an estimated value.
...
This result shows the overall predicted probability of supporting Biden across the full sample. The estimate is the model’s best guess for average support, and the confidence interval gives the range of plausible values. Since the model has no predictors, the estimate is the same for everyone — there’s no variation by age, sex, or any other factor..

### Exercise 5

Due to the simplicity of our model, `plot_predictions()` will not work. This is because it doesn't include any predictors like age or sex. Instead, we can use `avg_predictions()` which is also from the marginaleffects package. 
...
avg_predictions(fit_biden) |>
  ggplot(aes(x = "Yes", y = estimate)) +
  geom_col(width = 0.4, fill = "#1f77b4") +
  labs(title = "Probability of Supporting Biden",
       y = "Probability",
       x = NULL) +
  theme_minimal() +
