...
<!-- DK: Deal with Adding missing grouping variables: `.row`message in Temperance plot. - DONE -->
<!-- SD: This should have been fixed as of my last PR, where it was a typo in the instructions that caused the error. -->

<!-- DK: This tutorial does not exactly follow the new template tutorial. Edit it to do so. - DONE -->

<!-- Some concerns that this is too long. -->

<!-- DK: Need more knowledge drops during Courage.  The Courage section of each chapter is the most complex because modeling is hard. Use opportunities for knowledge drops, like this one, judiciously. - DONE -->
...
```{r wisdom-5}
question_text(NULL,
	message = "The outcome is whether the vote was cast for Biden or not.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- AC: The numbers seem not to match up? I'm not sure tho because I'm unsure how to read the table. - DONE  -->

**Estimate:** The estimated intercept is `-0.32`. In the context of a Bernoulli model, this is the log-odds of the outcome (support for Biden). To convert this to a probability, the baseline probability of support for Biden is approximately 41%. Also note the low **Standard Error* of `0.04`, indicating that this figure has relatively low variability.

Note that these numbers reflect those that you got when you ran `fixef()` on `fit_bern`.
...
Update `models.qmd`. First, add `library(gtsummary)` to the `setup` code chunk,. Second, add the mathematical formula, in $\LaTeX$ and surrounded by double dollar signs, for your model. Third, add a new code chunk which creates the table of model parameters. `Command/Ctrl + Shift + K` to ensure that everything works.
...
```{r temperance-4}
question_text(NULL,
	message = "We only need one row because we are only measuring one value: the probability that Biden becomes President. Since our question asks if he would be president or not, we rely on his popularity to indicate whether he would become President.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
