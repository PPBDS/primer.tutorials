...
Place your cursor in the QMD file next to `poll_data`. Use `Cmd/Ctrl + Enter` to execute. It will automatically execite all three lines.
...
This problem is predictive so we are interested in how the outcome (a vote for Joe Biden) changes when we see which voters were exposed to a Trump ad or not. 
...
<!-- DK: This is wrong! We don't have age! -->
...
Provide one reason why the assumption of validity might not hold for the outcome (Biden support) or for one of the covariates. Use the words "column" or "columns" in your answer.
...
```{r}
knitr::include_graphics("images/biden-support.png")
```

Use AI to help. Your plot does not have to look exactly like ours. 

Note that this plot takes the output of `avg_predictions(fit_biden, type = "prob")`, filters out just the row for `group == "Yes"` and then pipes to `ggplot()`
