...
An interesting thing to note in this model is the inclusion of `seed`. In this case, the argument `seed = 9` is setting a variable that we can call back at any time, where it will always display the same answer if the same seed is used.
...
<!-- DK: This is good material, but too long. And too complex for this first pass. - DONE -->

<!-- SD: Old Knowledge Drop: -->
...
<!-- SD: New Knowledge Drop: -->
Here, `bernoulli()` indicates a Bernoulli distribution, which is a probability distribution for a random variable that has exactly two possible outcomes. It is used to model situations with binary outcomes, such as success/failure, yes/no, or true/false.


### Exercise 13

<!-- DK: Always tell them to `Command/Ctrl + Shift + K`. Check to see if the mathematical formula looks correct. - DONE -->
