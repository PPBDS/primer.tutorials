...
<!-- SD: This is more of a comment on the Primer itself, but it would be easier to understand if the 'components' were called 'steps' instead -->
...
<!-- SD: I might be being nitpicky at this point, but this section of the Primer doesn't say that it's the smallest possible table, and this point has to be recalled from memory (or is this the point of its lack of inclusion?) -->
...
<!-- SD: Again, this section doesn't explain how the units are exactly rows -->
...
<!-- SD: This question was a bit confusing as there is only 2 rows, tricking the reader into think that there is only one outcome instead of the 2 options. On a side note, what about Independent Candidates? A reader could easily question this, making this scenario as a whole pretty confusing-->
...
<!-- SD: Again, what about Independent? -->
...
<!-- SD: addition to the instructions: "Write about the source & date of the data." -->
...
<!-- SD: This sentence is extremely confusing, as 'we' also relates to how 'we' speak in English. A better term would be "Data Scientists" instead of "we". -->
...
Provide one reason why the assumption of validity might not <!-- SD:          ^ "(from the data)" -->
hold for this problem.

<!-- SD: without that insertion, I was thinking of how the real-life conditions might change between the time of this survey and the actual election -->
...
<!-- SD: {"Write a goal statement on how you will use this..."} instead of {Summarize the state of your...} -->
Summarize the state of your work so far in one or two sentences. Make reference to the data you have and to the question you are trying to answer.                      <!--   ^ "(i.e. "we seek to)" -->
...
<!-- SD: This is a bit confusing as the primer doesn't necessarily state this, and rather focuses on the data that we have, want to have, and don't care about -->
...
<!-- SD: THIS is the way that it should be put!!! Well done! -->
...
<!-- SD: Missing correct answer, possibly because the primer shows multiple reasons but no direct copy-paste answer. -->
...
question_text(NULL,
	...
	# SD:                                 ^ ", and this assumption is only relevant for casual models."
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- SD: the correct answer: "Since this is a predictive model, we do not have to worry about unconfoundedness." -->
...
<!-- SD:                                       ^ "IN DATA SCIENCE!!!" IT'S SO CONFUSING OTHERWISE!!! -->
```{r courage-1}
question_text(NULL,
...
<!-- SD: Is there any way to allow the reader to progress without completing this? It just takes so much time so it would be nice for the reader to work on the next problem while this one still renders -->
...
<!-- SD: Note: this creates a tibble with 0 rows, and is indeed blank (but doesn't have 2 rows) -->
...
<!-- SD: Note that this should be "Continue the pipe with..." instead of "Add..." for simplicity's sake -->
