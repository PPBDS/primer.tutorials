...
This creates an empty tibble with two rows. Because `fit_bern` is such a simple model, one with no independent variables, the `newdata` object is also extremely simple. In this case, we are interested in the two people we might meet (and who they voted for) so we need two rows in `newdata`.
