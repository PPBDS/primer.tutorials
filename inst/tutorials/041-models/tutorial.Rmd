...
Imagine you are a political strategist in early 2024. The election is close, and resources are limited. To allocate them wisely, you need to estimate Joe Biden’s true level of voter support—not just in a single poll, but the underlying probability that a voter will choose him. 
...
A version of the data from **YouGov's March 2024 poll** is available in the `poll_data tibble`. This tibble consists of 1,559 rows and 1 column.

### Exercise 3

After creating `poll_data`, examine its structure. In the Console, type `glimpse(poll_data)` and describe the data structure below.
...
The data comes from a YouGov poll conducted March 10-12, 2024, surveying 1,559 US adult citizens about their voting intentions for the 2024 presidential election. The specific question asked was: 
...
**Models** is the broad topic of this tutorial. Given that topic, which variable in `poll_data` should we use as our outcome variable?
...
```{r the-question-7}
question_text(NULL,
	message = "For a given voter, the treatment variable might be **exposure to a pro-Trump advertisement** or **no exposure**. If exposed, their support for `biden` is `0`; if not, it's `1`. The causal effect is `0 - 1 = -1` for this voter.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r wisdom-5}
question_text(NULL,
	message = "The units are individual survey respondents.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
