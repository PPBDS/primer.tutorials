...
<!-- DK: This is not how things work! Read the template_tutorial! You only show this around Exercise 12, after all the questions about it. How else will students learn to build their own? -->
...
<!-- DK: Where is the question! -->

Every good Data Scientist is familiar with all of their data. Let's take a look at our data for today: 

The data we have come from a YouGov poll [pdf](https://github.com/PPBDS/primer/blob/master/sources/you-gov-2024-03-12.pdf) of 1,559 US adult citizens, conducted March 10 - 12, 2024. 

### 

There are many questions in the poll. For the purpose of answering our question, the most relevant one is:
...
The allowed choice are “Joe Biden,” “Donald Trump,” “Other,” “Not Sure,” and “I would not vote.” 42% of those polled indicated Joe Biden. Although rounding makes it impossible to know for sure, we will assume that 655 of the 1,559 “US adult citizens” would vote for Biden. Run the following code to see what our data looks like in a data table.
...
1. Our Polling Data includes exactly 1,559 rows. We don’t know how many rows are in our Preceptor Table because we don’t know, today, how many people will vote in November 2024.

2. The ID column is labeled “Voter ID” in the Preceptor Table and “Poll ID” in the Polling Data. The person labeled “2” in the first table, for example, has no necessary connection to the person labeled “2” in the second table. The numbers don’t mean anything. But, conceptually, having different labels helps to highlight the issue of sampling and concerns about representativeness, concerns which we will address under Justice.

3. The variable labels differ. “Voted for Biden” is not the same thing as “Would Vote for Biden.” Indeed, the two columns represent very different things.
...
Paste this sentence into your `models.qmd` file.
...
Start by piping `ndata` to `select()`, selecting `.epred`.
...
We do this so it only selects the `.epred` column, instead of displaying the entire dataset.
