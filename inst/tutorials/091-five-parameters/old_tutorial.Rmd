...
<!--AP: MADE A NEW VERSION OF THE ENTIRE THING!!!!! -->
...
<!-- SJ: Tutorial 9 and 10 are so similar that pretty much everything I wrote in tutorial 10 applies to 9 as well so see that for the bulk of my comments. -->
...
<!-- DK: Not sure this works? Predictive or causal? Or a mix of both? AP: does this work?-->
...
To better understand how holding political office might affect longevity, we can start by asking a more focused question: How many years do candidates typically live after an election, and does this differ depending on whether they win or lose? While this won't fully capture the impact of a political career on health, answering it can give us a useful prediction about life expectancy tied to election outcomes—helping you make more informed decisions about your future.
...
<!-- Should the 'x' variable below be replaced with 'governors' to represent the data? AP: not sure, will change for everything--> 
...
With a predictive model, there is only one outcome for each individual unit. There are not two potential outcomes because we are not considering any of the covariates to be a treatment variable. We are assuming that the values of all covariates are "fixed." 
...
<!-- SJ: Potentially an example here?. AP: maybe not because the student should think of examples?-->

```{r wisdom-7}
question_text(NULL,
	message = "Longevity of someone's parents is a good predictor for her own lifespan. A polygenic risk score for longevity might also be useful.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 4)
...
<!-- Possibly use suppressPackageStartupMessages instead? -->
...
*The longer the time period covered by the Preceptor Table (and the data), the more suspect the assumption of stability becomes.* This is why removing the data from before 1945 might actually make our model better than it would have been if we had included it.
...
5a) Run the equivalent of pp_check on the last model, just to confirm that things look good.

6) Once we have our DGM, we can create the LaTeX math: we have the mathematical formula which specifies exactly which variables to include and how to transform them. All the parameters were replaced by our estimates and with the error term dropped. The dependent variable wears a hat. This is our data generating mechanism.
...
Add `library(tidymodels)` and `library(broom)` to the `setup` code chunk in your QMD. `Cmd/Ctrl + Shift + K`.
...
Create a new code chunk in your QMD. Add a code chunk option: `cache: true`. Copy/paste the code from above, for estimating the model into the code chunk, assigning the result to `fit_years` and changing the data variable to `governors`. 
...
<!-- DK: Should this be a separate question? AP: not sure, nothing to run or sumbit-->
...
`Cmd/Ctrl + Shift + K` to make sure it works. Fit it if not. 
...
<!-- SJ: We should give our own AI example here. AP: not in template?-->
...
Again, this model is obviously flawed. I don't believe that a female political candidate who is 50 years old should expect to live fewer years after the election than a male candidate. I don't think that the relationship between `election_age` and `lived_after` is, for female candidates, nearly as flat as it appears.
