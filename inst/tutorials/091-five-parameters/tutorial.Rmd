...
```{r the-question-5}
question_text(NULL,
	message = "As a treatment variable, `sex` would make a lot of sense as we can manipulate the value of `sex`, at least in theory, by changing it. Changing the sex could have a profound effect on `lived_after`.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
You might have figured out that we are creating a Causal model. Causal models aim to understand the relationships between variables and how changing one variable (the treatment) causes changes in another variable (the outcome). They care about the difference in outcome between the two variables.

Write a sentence which speculates as to the number of different potential outcomes we might observe in `lived_after` (the outcome) when we change the value of `sex` (the treatment variable). 
...
In a predictive model, you get to change the "ingredients" in the recipe, but cannot change the taste of the ingredients.
