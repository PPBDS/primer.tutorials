...
question_text(NULL,
	...
	Is it interested in all candidates who are running in the next election? All those running in the future?",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
I also wonder how it would be relevant to include a candidate running from the future in our data, as we don't know how long they will live after they get elected. What if don't get elected at all! What kind of effects would this have on their longevity.
...
```{r wisdom-10}
question_text(NULL,
	message = "Our Preceptor Table would contain a list of candidates on the right-hand side of the table, and on the left-hand side, would contain variables concerning their sex, the year that they got elected, the age at which they got elected, and the number of yeyars that they lived after getting elected.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Let's create a new `.qmd` file. Name it "Five Parameters" and save it as `five-parameters.qmd`. Render the document with `Command/Ctrl+Shift+K`. 
...
3) At first glance, lot of the data is in the past century (although it may not all be in the past century)
...
Here, I immediately notice how they maximum year in 2011. Note that I was still right though, in that most of the material is still from the previous century (as we can tell by the `Mean` row in the `year` column).
...
The `Intercept` shows us that the number of years that the average governor will lived after being elected is 19.77. When converted, the Standard of Error is 66%, indicating that we are very uncertain with this number. 

However, we care about the `sexMale` row, which states that on average, male candidates live ~53 longer than female candidates! Obviously, something isn't right. However, we are 95% sure that this value falls between ~9.91 and ~97.33 (both positive numbers), indicating that male candidates still generally live longer than female candidates do.

Interestingly, the `election_age` row shows us that for each additional year of election age, the number of years lived after being elected is estimated to decrease by 0.06 years. Although this is unrelated to our questrion at hand, it's still useful for other questions that we may draw from the data.
...
Here we see something that we may have mised previously, which is the effect of `election_age` on `sexMale`. Previosuly this was presented in a way that made it hard to understand. However, now we see that for each additional year of election age, the effects on candidate longevity are worse on males than they are on females. Specifically, male candidates age 0.79 years more per year than female candidates do.
