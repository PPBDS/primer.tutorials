...
> How does sex impact the number of years that a governor lives after being elected? 
...
When starting with a new project, it's always a good idea to load up **tidyverse**. Load the package in the following box.
...
Next we should analyze our data. Our data is located in the **primer.data** package, so load this in as well. Also load this into the Console.
...
It's interesting to note that the paper that this data is based off studies the effects of winning a close election vs losing, and how it affects the longevity of the candidates. 

However, that's not a bad thing. Just because a dataset is measuring different outcomes doesn't mean that it's completely irrelevant. In our case, it contains valuable data that we can use to answer our question.
...
<!-- DK: Split up into three questions. - DONE -->
...
Create a new code chunk in `five-parameters.qmd`. Use `#| label: formula`.

Write the mathematical formula for your model, using $\LaTeX$ math notation. When it looks good, `Command/Ctrl + Shift + K` to ensure that it works.

```{r}
#| code-fold: true
#| code-summary: "Spoiler: Code for Formula"
