...
<!-- SD: FIX - Check with Preceptor on if this is correct -->
...
Realistically speaking, besides the columns in our data asking for names, all of the other columns would be valid answers to this problem. This just proves that in a question like this, there is really no right answer, and your response was probably fine. But, going forward, you should use the variable we have selected.

<!-- SD: a Causal model would need to have a tratment that we could manipulate. Expand on this -->

<!-- First, any data set can be used to construct a causal model as long as there is at least one covariate that we can, at least in theory, manipulate. It does not matter whether or not anyone did, in fact, manipulate it. 

Second, any data set can be used to construct a predictive model. 

Third, the same data set can be used to create, separately, lots and lots of different models, both causal and predictive. We can just use different outcome variables and/or specify different treatment variables. All of this stuff is a *conceptual* framework we apply to the data. It is never inherent in the data itself.-->

### Exercise 6

Causal models aim to understand how changing one variable (the treatment) causes changes in another variable (the outcome). Ech dfferent value of the treatment geenrates a different potential outcome.

Write a sentence which speculates as to the number of different potential outcomes we might observe in `lived_after` (the outcome) when we change the value of `sex` (the treatment variable) for one candidate.
<!-- SD: what would it be if it were different, and how much would it be different from our thing? -->
<!-- SD: FIX - Again, check with Preceptor on this. -->
```{r the-question-6}
question_text(NULL,
	message = "The value of `sex` would demonstrate 2 different potential outcomes for each unit as we vary the treatment variable.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- SD: FIX - finish then show Preceptor to fix any errors -->


<!-- SD: REAL EXAMPLES!!! -->
```{r the-question-7}
question_text(NULL,
	message = "For a given canddiate that the value of the treatment variables might be 'sex_1' or 'sex_2'. If the unitXX gets 'sex_1', then the outcome would be sex_out_1. If the unitXX gets 'sex_2', then the outcome would be XX_out_2. The causal effect on the outcome of a treatment of 'sex_1' versus 'sex_2' is sex_out_1 - sex_out_2, which equals sex_ce.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Again, we are trying to reinforce that the definition of a causal effect is the difference between two potential outcomes. Of course, you can't just say that the causal effect is 10. The exact value depends on which potential outcome comes first in the subtraction and which second.
...
In our case, you can change the variables that you use to form predictions, but you can't change the value of the variables.


<!-- SD: comparisons across and between rows-->
