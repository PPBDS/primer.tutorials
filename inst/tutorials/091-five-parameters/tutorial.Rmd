...
# fit_years <- lm(lived_after ~ election_age*sex, data = x)
...
Interpret 19.3, the value of the `(Intercept)`.
...
Interpret -0.06, the value coefficient for `election_age`.
...
Interpret [-1.5, -0.07], the confidence interval for the coefficient of `election_age:sexMale`.
...
<!-- DK: Is this interpretation correct? For example, is this graphic conditional on using the mean age for the whole data set or the mean age for each sex? I think it is the former . . . -->

This graphic shows our estimates for the number of years lived after the election for male and female candidates separately. There is a lot going on here!

First, there is much more uncertainty for female candidates than for male candidates, probably because there are so few females in the data.

Second, male candidates live longer than female candidates. That is surprising since, in general, women live longer than men.
