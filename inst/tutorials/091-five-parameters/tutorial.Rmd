...
Imagine that you are someone who is considering running for governor. One thing you might be interested in is how long candidates for governor live. 
...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"5 Paramters"` -- and an author (you). Render the document and save it as `tutorial-9.qmd`.
...
The first sentence is a general statement about the overall topic, mentioning both general class of the outcome variable and of at least one of the covariates. It is **not** connected to the initial "Imagine that you are someone considering runnign for governor" which set the stage for this project. That sentence can be rhetorical. It can be trite, or even a platitude. The purpose of the sentence to let the reader know, gently, about our topic.
...
<!-- DK: sentence not that good, fix later -->
...
Edit the summary paragraph in `tutorial-9.qmd` as you see fit, but do not copy/paste our answer exactly. `Command/Ctrl + Shift + K`, and then commit/push.
...
```
tutorial.helpers::show_file("tutorial-9.qmd", pattern = "tidymodels|gtsummary|equatiomatic")
```
...
```
tutorial.helpers::show_file("tutorial-9.qmd", start = -8)
```
...
```
tutorial.helpers::show_file("tutorial-9.qmd", pattern = "extract")
```
...
```
tutorial.helpers::show_file("tutorial-9.qmd", pattern = "tbl_regression")
```
...
```{r courage-14}
question_text( NULL,
  "We model ages governors lived after being elected as a linear function of election_age, seeking to predict the variable `lived_after`.",
  answer(NULL, correct = TRUE),
  allow_retry = FALSE, 
  incorrect = NULL,
  rows = 6 
...
```{r temperance-9}
question_text(NULL,
	message = "The quantity of interest in our problem is `lived_after`, as a function of `election_age`. The estimate of -0.86 represents the effect size, indicating how much the outcome is expected to change for a one-unit change in the predictor. The uncertainty around this estimate is captured by the 95% confidence interval, which ranges from -0.93 to -0.78. This range tells us that, with 95% confidence, the true effect lies within this interval. The fact that the confidence interval does not include zero indicates that the effect is statistically significant. Additionally, the p-value of less than 0.001 further supports this, suggesting that there is a very low probability that the observed effect is due to chance. Despite the significance, the confidence interval provides a valuable measure of uncertainty, showing that while the effect is negative, there is some variability in its exact magnitude.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r temperance-10}
question_text(NULL,
	message = "The estimates for the quantities of interest and their associated uncertainty might be wrong due to issues like model misspecification, such as failing to include important predictors or incorrect assumptions about linearity. Additionally, multicollinearity between variables could inflate standard errors, leading to wider confidence intervals, and small or unrepresentative sample sizes can exacerbate these problems. To improve the accuracy of both the estimates and their uncertainty, a more robust approach like generalized least squares (GLS) or bootstrapping methods for confidence intervals could be considered. These methods would account for potential violations of assumptions, providing more reliable estimates and more realistic uncertainty intervals.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```
tutorial.helpers::show_file("tutorial-9.qmd")
```
