...
A few things we noticed about this data:

1) The data is sorted alphabetically by state, year and population
2) The first & last names are in separate columns
3) At first glance, a lot of the data is in the past century (although it may not all be in the past century)
...
Run `tbl_regression()` on `fit_all` with the argument `intercept = TRUE`. 
...
Although we already saw much of this in the previous exercises, the regression table makes it nice and easy to read the values. 

Here we see something that we may have missed previously, which is the effect of `election_age` on `sexMale`. Previously this was presented in a way that made it hard to understand. However, now we see that for each additional year of election age, the effects on candidate longevity are worse on males than they are on females. Specifically, male candidates age 0.79 years more per year than female candidates do.
