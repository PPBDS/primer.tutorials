...
ndata <- expand_grid(tuition = c(2.63, 3.73), selectivity = unique(colleges_2$selectivity))
...
Update `analysis.qmd`. First, add `library(gtsummary)` to the `setup` code chunk,. Then, add a new code chunk which creates the table of model parameters. `Command/Ctrl + Shift + K` to ensure that everything works.
...
These must be the right hand side variables in `fit_colleges`. The type must match, since `selectivity` is a factor in the original data used to create `fit_colleges` then it must be a factor in `ndata`.
...
expand_grid(tuition = c(2.63, 3.73), selectivity = unique(colleges_2$selectivity))
```

###

We use `expand_grid()` to get all possible combinations of `tuition` and `selectivity` into our tibble.
