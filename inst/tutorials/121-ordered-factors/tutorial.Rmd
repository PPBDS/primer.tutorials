...
author: Tanay Janmanchi and David Kane
tutorial:
  id: ordered-factors
output:
  learnr::tutorial:
...
```{r the-question-5}
question_text(NULL,
	message = "We could create a new variable called `impv_food` which sees whether or not a college improved their food. We could manipulate this by giving funds to schools to improve their food and check the graduation rate in 5 years.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r the-question-6}
question_text(NULL,
	message = "There are XX potential outcomes because the treatment variable `impv_food` takes on XX posible values: list-the-values-here, i.e., exposure to Spanish-speakers on a train paltform versus no such exposure.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- TJ: Is this question necessary since it has been established that the treatment variable must have 2 potential outcomes? -->
...
Words like "effect" and "predict" signal to use that this is a predicative model.
...
If our question asked "What is the probability that a student will graduate Harvard University given they are rich?" Our outcome variable would be whether or not the student graduated Harvard in the end. The outcome is the main column which we are measuring.
...
```{r wisdom-9}
question_text(NULL,
	message = "The Preceptor Table contains a row for each college. There are four columns: one is tuition, which is our main covariate; another one is selectivity, which is another covariate; next we have ID and finally the last one is graduation rate, which is our outcome variable,  .",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- TJ: I know we talked about selectivity, but is it in the Preceptor Table? -->
...
# Creating the tibble with the specified values
data <- tibble(
  ID = c("1", "2", "...", "10", "11", "...", "950"),
  graduation_rate = c(0.95, 0.85, "...", 0.75, 0.65, "...", 0.55),
  selectivity = c("Elite", "Highly Selective", "...", "Moderately Selective", "Lowly Selective", "...", "Non-selective"),
  tuition = c(4.5, 4.0, "...", 3.0, 2.0, "...", 1.0)
)
...
data |>
  ...
  tab_spanner(label = "Covariate", columns = c(selectivity, tuition))
