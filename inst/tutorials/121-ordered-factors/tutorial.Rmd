...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"Ordered"` -- and an author (you). Render the document and save it as `XX.qmd`.
...
Add `library(tidymodels)`, `library(gtsummary)`, `library(equatiomatic)`, and `library(marginaleffects)` to the `setup` code chunk in your QMD. `Cmd/Ctrl + Shift + K`.
