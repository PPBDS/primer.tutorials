...
<!-- GK: Have to rewrite the operatorname for selectivity, cause it's hard to read when pop out -->
### 

<!-- $$ -->
<!-- \begin{aligned} -->
<!-- \operatorname{grad\_rate} &= \beta_{0} + \beta_{1}(\operatorname{tuition}) + \beta_{2}(\operatorname{selectivity}_{\operatorname{Highly Selective}}) + \beta_{3}(\operatorname{selectivity}_{\operatorname{Moderately Selective}})\ + \\ -->
<!-- &\quad \beta_{4}(\operatorname{selectivity}_{\operatorname{Lowly Selective}}) + \beta_{5}(\operatorname{selectivity}_{\operatorname{Non-selective}}) + \epsilon -->
<!-- \end{aligned} -->
<!-- $$ -->
### Exercise 6
...
The same approach applies to a categorical covariate with $N$ values. Such cases produce $N-1$ dummy $0/1$ variables. The presence of an intercept in most models means that we can't have $N$ categories. The "missing" category is incorporated into the intercept. If `selectivity` has five values --- "Elite", "Highly Selective", and "Moderately Selective", "Lowly Selective", "Non-selective" --- then the model creates 4 0/1 dummy variables, giving them names like $selectivity_{Highly Selective}$ and $selectivity_{Moderately Selective}$, and so on. The results for the *first* category are included in the intercept, which becomes the reference case, relative to which the other coefficients are applied.
...
<!-- GK: I think this is wrong. I think it is because `selectivity` is a factor with 5 levels. The first level is the reference level. The other 4 levels are compared to the reference level. -->
...
<!-- GK: Need to align first on what the coefficients mean -->
...
Enter this code into the Console, copy and paste your command into the answer. 
