...
The intercept is the predicted graduation rate for the baseline category of region (e.g., "East") when tuition is zero. The tuition coefficient ($\beta_1$) shows how graduation rate changes with each additional dollar of tuition. If $\beta_1 = -0.0031$, then a $1,000 increase in tuition is associated with a 3.1-point drop in predicted graduation rate.
...
```{r temperance-3}
question_text(NULL,
	message = "The estimate for tuition is -0.0003, meaning that for each additional dollar of tuition, graduation rate is predicted to decrease by 0.03 percentage points, adjusting for selectivity. The confidence interval (from -0.0005 to -0.0001) does not include zero, so the effect is statistically significant.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
