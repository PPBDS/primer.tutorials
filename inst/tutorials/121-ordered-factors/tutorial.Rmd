...
`college` contains data from over 900 colleges and universities in the United States. These draw primarily on data from the Department of
Education’s (DOE) IPEDS database in 2013 and the College Scorecard. 
...
```{r the-question-4}
question_text(NULL,
	message = "We should use `grad_rate`",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 2)
...
Recall from earlier when you ran `?college` you should've seen some information about `grad_rate`. `grad_rate` is the rate of graduation at that college/university from the year 2013.
...
```{r the-question-5}
question_text(NULL,
	message = "We can manipulate the value of `tuition`, at least in theory, by ... ",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
`tuition` could be very well be related to the overall graduation rate of a school. In theory, if you pay more for a school, you should be staying there and getting your money's worth! 

Again, `grad_rate` could be influenced by a plethora of things, but for the sake of this tutorial, we will stick with `tuition`
