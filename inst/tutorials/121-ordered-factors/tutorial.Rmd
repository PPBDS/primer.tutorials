...
ndata <- expand_grid(tuition = c(2.63, 3.73), 
                     selectivity = unique(colleges_2$selectivity))
...
<!-- DK: Update with new knowledge drop. Done -->
...
Because `grad_rate` is a continuous variable, we assume that an individual's attitude toward immigration is produced from a Normal distribution.
...
$$ \mu =  \beta_0 + \beta_1tuition + \beta_2selectivity $$

### Exercise 4

Add `library(tidymodels)` and `library(broom)` to the `setup` code chunk in `analysis.qmd`. Copy and paste the below code for the mathematical structure of the model to the body of `analysis.qmd`. `Command/Ctrl + Shift + K`. 


```
$$ grad\_rate_i \sim Normal(\mu)$$
$$ \mu =  \beta_0 + \beta_1tuition + \beta_2selectivity $$
```
...
```
tutorial.helpers::show_file("analysis.qmd", pattern = "tidymodels|broom|\\$\\$")
```
...
<!-- TJ: Is this a good enough knowledge drop -->

### Exercise 7

Continue the pipe to `fit(grad_rate ~ tuition + selectivity, data = colleges_2)`.
...
The first argument is the formula. Our left hand side variable `grad_rate` is the dependant variable, or what we are trying to measure. Our right hand side variables `tuition` and `selectivity` are the independant variables. The second argument is the data we will use, which is `colleges_2`.
...
The `Intercept` shows us that the predicted graduation rate for all the colleges in our data set is around 36.2%. 
...
```{r courage-13}
question_text(NULL,
	message = "We never know what the true value is for the `tuition` coefficient, but we are 95% certain that it lies between 0.07 and 0.1.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
We are never sure what the true value for a coefficient is, but, we can be pretty sure that it lies in this interval. This is especially helpful for stating any uncertainty with your value when writing your final paragraph.

### Exercise 14

Write a sentence explaining the `selectivity` coefficient.

```{r courage-14}
question_text(NULL,
	message = "As `selectivity` goes from 'Non-selective' to 'Elite', graduation rates seem to rise.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
The four coefficients for `selectivity` show the different relationships on `grad_rate` as `selectivity` goes from `"Non-selective"` to `"Elite"`. `selectivity.L"` show the linear relationship, `selectivity.Q` shows the quadratic relationship and so on. These coefficients are too difficult to interpret, so we will not try to decipher them. But, we can see by the mostly positive values that higher selectivity does seem to positively impact graduation rates.
...
```{r courage-15}
question_text(NULL,
	message = "Using data from a system of interrelated surveys on postsecondary eduation in 2013, we seek to understand the relationship between graduation rates tuition for colleges/universities in the United States. In particular, what is the difference in graduation rate for colleges in the 75th and 25th percentiles of tuiton? Tuition rates and graduation rates could've changed since 2013. We modeled graduation rate, a continuous variable, as a linear function of both tuition and selectivity. Graduation rates seem to rise as tuition increases.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Here is the R code which creates the `ndata` object: `expand_grid(tuition = c(2.63, 3.73), selectivity = unique(colleges_2$selectivity))`. Type it into the code exercise block and hit "Run Code."
...
It is best to ensure that the variable types in `ndata` match the variable types on `colleges_2`. The easiest way to ensure that is by using `colleges_2` itself in the creation `ndata`.
...
<!-- TJ: Not including #| echo: FALSE so that students can see the code -->
...
```{r temperance-10-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r temperance-10-hint, eval = FALSE}
fit_colleges |> 
  plot_predictions(newdata = ndata,
                   by = c(...))
```
...
Add the layer `scale_y_continuous()`. Set `labels` equal to `scales::percent_format(accuracy = 1)`

```{r temperance-11-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r temperance-11-hint, eval = FALSE}
...
We must make out plot understandable to someone who has absolutely 0 idea about data science. Adding percents helps the reader interpret out plot.

<!-- TJ: Trying to find a way to change the values in the fill legend to be 25th and 75th percentiles instead of the raw value. -->
...
<!-- TJ: Is the regression table the call to `tidy()`? -->
...
<!-- TJ: Are we going to make this a chapter in the primer? -->
