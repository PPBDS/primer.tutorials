...
In `analysis.qmd`, add `execute: echo: false` to the YAML header. Create a new code chunk (`Command/Control + Option/Alt + I`) and add `#| label: setup` and `#| message: FALSE`. Then, load the **tidyverse** and **primer.data** libraries. Run `tutorial.helpers::show_file("analysis.qmd", chunk = "first")`. CP/CR
...
```{r wisdom-21-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r wisdom-21-hint, eval = FALSE}
... |>
  select(tuition, grad_rate, ...) |>
  ... |>
...
```{r wisdom-22-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r wisdom-22-hint, eval = FALSE}
colleges_2 <- ...
```
