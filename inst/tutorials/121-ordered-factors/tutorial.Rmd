...
The first argument is the formula. Our left hand side variable `grad_rate` is the dependant variable, or what we are trying to measure. Our right hand side variables `tuition` and `selectivity` are the independant variables. The second argument is the data we will use, which is `x`.
...
<!-- TJ: Trying to find a way to change the values in the fill legend to be 25th and 75th percentiles instead of the raw value. Keep trying. -->
...
<!-- TJ: Is the regression table the call to `tidy()`? Yes, see template. -->
