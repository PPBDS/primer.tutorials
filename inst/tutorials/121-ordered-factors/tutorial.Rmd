...
We measure a causal effect by subtracting the control from the treatment. In our case, improving the food is the treatment and not improving the food is the control. We subtract these two to get a causal effect of 0.2, meaning that colleges that improve their dorm food on average have an extra 20% graduation rate!
...
```{r the-question-11}
question_text(NULL,
	message = "How does graduation rate differ for schools in the top 75th percentile of tuition and the lower 25th percentile?",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Your question may not have been the same as this, but again, that is completely OK! However, this will be the question we use for the rest of this tutorial.
...
Since our question is predicative, we will only need one covariate.
...
If our question asked "What is the probability that a student will graduate Harvard University given they are rich?" Then our units would be each individual student at the college. These are the rows of the Preceptor Table.
...
```{r wisdom-8}
question_text(NULL,
	message = "Since this is a predicative model, there are no treatments being observed.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
```{r wisdom-11}
question_text(NULL,
	message = "We are using data from the `colleges` database in R's `primer.data` package, which contains data from 950 US colleges and universities.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
It is necessary to understand the data you have before you start exploring it and making models.
...
<!-- TJ: Don't know what to put for a knowledge drop here -->
...
Now we have our quarto document set up, we can start exploring the data.
...
In `analysis.qmd`, add `execute: echo: false` to the YAML header. Create a new code chunk (`Command/Control + Option/Alt + I`) and add `#| label: setup` and `#| message: FALSE`. Then, load the **tidyverse** and **primer.data** libraries. Render the file and run `tutorial.helpers::show_file("analysis.qmd", chunk = "first")`. CP/CR

<!-- TJ: Is this the right code? -->
...
<!-- TJ: Haven't we already loaded up the packages and dropped enough knowledge in The Question? -->
...
Lets do some exploring of `colleges`

### Exercise 18

Use `select()` to select the `tuition` and `grad_rate` columns.
...
These are the two main variables we will use in out model later. Let's see if we can find any relationship between them.

### Exercise 19

Continue the plot to `ggplot()`. Map `tuition` to the x-axis and `grad_rate` to the y-axis using `aes()`. Add the `geom_point()` and `geom_smooth()` layers.
...
Copy your previous code but add `filter(tuition > 2)` before your call to `ggplot()`.
...
This confirms what we thought before. There isn't a relationship for schools that charge less than 20k per year. So we can exclude those schools when we make our model.

### Exercise 21

Copy your code from before and add `selectivity` to your call in `select()`. Also map `selectivity` to the x-axis instead of `tution` and remove the `geom_smooth()` layer.
...
colleges |>
  select(tuition, grad_rate, selectivity) |>
   filter(tuition > 2) |>
   ggplot(aes(x = selectivity, y = grad_rate)) +
   geom_point()
```

### 

Part of EDA is looking at what other covariates could influence your outcome variable. `selectivity` seems to be another variable that could potentially influence `grad_rate`. Elite schools have higher graduation rate as compared to moderately and lowly selective schools. This is another good covariate to measure.

### Exercise 22

Delete everything from `ggplot()` and onwards. Save this as a new object `colleges_2`
...
<!-- TJ: Do I change the question here? -->

### Exercise 23

Create a new code chunk in `analysis.qmd`. Add `#| label: eda`. Copy/Paste your code for the `colleges_2` object. Render and run `tutorial.helpers::show_file("analysis.qmd", chunk = "last")`. CP/CR.
...
Provide one reason why the assumption of validity might not hold for the outcome variable: `grad_rate`.

```{r wisdom-25}
question_text(NULL,
	message = "The graduation rate column in the Preceptor Table may be different from the graduation rate column in the Data since overall graduation rate may have changed over the last 11 years.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Recall our data is from 2013. A lot can happen in 11 years, such as the COVID-19 pandemic and other big events that may have affected average graduation rate. These changes in graduation rate make the columns from the Preceptor Table and the columns from the Data different.

<!-- TJ: Is this acceptable? -->
...
```{r wisdom-26}
question_text(NULL,
	message = "Inflation could make the tuition rate column in the Data different from the tuition rate column in the Preceptor Table.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Inflation has been a major issue to the United States economy over the last 10 years. Tuition rates now are a lot higher than they used to be.
...
<!-- TJ: I feel like both of my responses to validity are similar to what I want to say here: Data is from 2013 so data now could be different. -->
...
```{r justice-9}
question_text(NULL,
	message = "Using data from a system of interrelated surveys on postsecondary eduation in 2013, we seek to understand the relationship between graduation rates tuition for colleges/universities in the United States. In particular, what is the difference in average graduation rate between schools who are in the top 75th percentile of tuition and schools who are in the bottom 25th percentile, all charging more than $20,000 a year. Tuition rates and graduation rates could've changed tremendously since 2013.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
The command `brm()` which we will use to create our model is 
...
<!-- TJ: Are my knowledge drops acceptable? I struggled to drop knowledge -->
...
<!-- Search and replace "fit_colleges" as appropriate. As a convention, the name of a fitted model object should always start with `fit_`. -->
...
This code should've taken a while to run. 
...
We can never be fully certain what the answer to our question is. However, we are 95% sure that our answer lies between this interval.
...
This is especially helpful for stating any uncertainty with your value when writing your paragraph.

### Exercise 17

Write a sentence or two about the `selectivity` coefficient.
...
```{r courage-20-hint-1, eval = FALSE}
fit_colleges |> 
  tbl_...()
```

```{r courage-20-test, include = FALSE}
fit_colleges |>
  tbl_regression()
```
...
<!-- XX: Drop some knowledge about what you have learned by looking at the resulting table. Of course, you could have learned the same thing when you first took a look at fit_colleges. But the table makes it easier to see the relationships between the variables and the outcome. With luck, students will take the hint when they answer the next question. -->
...
```{r courage-21}
question_text(NULL,
	message = "Using data from a system of interrelated surveys on postsecondary eduation in 2013, we seek to understand the relationship between graduation rates tuition for colleges/universities in the United States. In particular, what is the difference in average graduation rate between schools who are in the top 75th percentile of tuition and schools who are in the bottom 25th percentile, all charging more than $20,000 a year. Tuition rates and graduation rates could've changed tremendously since 2013. We modeled tuition and selectivity both as ... . Graduation rates seem to rise as tuition increases.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
