...
There is one more important step. The folder "rsconnect" contains files specific to your computer that you do not want to push to GitHub. Therefore, as with `.Rproj` files before, we want to add the `rsconnect` folder to the `.gitignore` file. Click on `.gitignore`, add `rsconnect` there and hit "Save." You will see it disappear from your Git tab in the top right window once you hit refresh.
