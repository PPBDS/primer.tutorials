...
description: Making a website using the Distill package.
---
...
We will go over how to make a [Distill](https://rstudio.github.io/distill/) project in RStudio. We will also configure it for hosting on GitHub pages. 

### Exercise 1

In your browser, log in to your Github account. Navigate to your Repositories section. Create a new repository named `distill-tutorial` by pressing the green button. Select the option to add a README file. 
...
Back in the RStudio session from which you are running this tutorial, click File, then Open Project in New Session. Navigate to your current R project folder and open any project there other than the one you are currently in by selecting on the Rproj file. This is a bit of a hack, but there does not seem to be a better approach because you can not create a New Project while a tutorial is running.


We are doing this so that we can make the new project for your Github repo in a new window. If we tried to make the project in this Window, you would have to close the Tutorial. You will have to navigate back and forth between these windows for the remainder of the tutorial.

### 

In the distill-tutorial R Studio instance that you just opened. Go to File and the New Project. Select Version Control and then Git. Paste the URL to clone the Github repo into the repository URL box. The name should be automatically filled in based on the name of the repository. Do not select the option to open this project in a new session. 

###

You now have two R Studio sessions running. The first contains this tutorial. The second will be named "distill-tutorial" and is the location in which you will do all the work.

### 

In the Terminal of the distill-tutorial R Studio instance, run `pwd` to confirm that you are in the correct directory, i.e. a directory named `distill_tutorial` in the place you normally keep R projects. Copy and paste the command and result below.

<!-- DK: This seems too complex a point to mention here. > Since you are going to be running a large amount of terminal commands, it may be a good idea to open the shell and do the commands in there. Just go to the Tools menu in RStudio and select the Shell option to open the shell. This is basically the Terminal tab but in another window so that you can easily hide it or organize it. -->

```{r creating-a-website-1}
question_text(NULL,
...
In the Terminal of the distill-tutorial R Studio instance, run `ls -a` to view a list of all the files in the directory. You should see a `README.md` which we made when we created the github repo. You should also see two hidden files, `.gitignore` and `.git`, along with some other files/directories.

Copy/paste the command/results below.
...
In the Console of the distill-tutorial R Studio instance, load the `usethis` package. Then run the command `git_sitrep()`. Copy and paste the output below. 
...
`git_sitrep()` gives information on the status of Git on your computer and current project. In the "Git repo for current project" section, make sure that the active `usethis` project is set to the correct directory on your computer, i.e., the "distill-tutorial" directory.
...
To set up and manage the files needed to create a website with R Markdown files, we use the package `distill`. In the Console of the distill-tutorial R Studio instance, use the `library()` function to load the package. 

### 

To confirm that you have successfully installed and loaded the package run the function `search()` in the console of the distill-tutorial R Studio instance. Copy/paste the output below. 
...
You should also see a `docs` directory. This is where all the files necessary for the deployed version of your website, including the knitted HTMLs, live. [Github Pages](https://pages.github.com/) looks in the `docs` directory for the components necessary to build the site. 
...
If you do not, close and then re-open the distill-tutorial RStudio instance. (Do not close the R Studio instance in which you are completing the tutorial.)

### 

Open the project again by using the File Tab in the Tutorial R Instance. You should now see a Build tab.

### Exercise 7

In `index.Rmd` (in the distill-tutorial R Studio instance), replace the example description with the string "This is where I will describe my final project." Knit this file. This opens a preview of what this page of the website will look like.

### 

In the Terminal of the distill-tutorial R Studio instance, run `head index.Rmd`. This command prints the first ten lines which should include the text of your new description. Copy and paste the command below. 
...
Building the website rebuilds all the documents necessary to deploy the site, which live in your `docs` directory. In the Terminal of the distill-tutorial R Studio instance, run `ls -l docs`.  Copy and paste the output below.
...
<!-- DK: This comment does not work since exact times are not shown, at least on my Mac. -->

<!-- All these files should have very recent and identical times in the date last modified column. -->

### Exercise 9

Open the `.gitignore` file. In the file, on a new line, add `*.Rproj` to prevent your .Rproj file from being uploaded to Github. Save the `.gitignore` file. After it is saved, the file ending in `.Rproj` should disappear from the Git tab. If it has not, hit the refresh button in the top right corner of the panel. 

### 

In the Terminal of the distill-tutorial R Studio instance, run `cat .gitignore`. This should return the contents of your edited `.gitignore` file. Copy and paste the output below.

```{r creating-a-website-9}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
    try_again_button = "Edit Answer",
...
When working with text files like `.gitignore` it is wise to ensure that the last line of the file is blank. This generally does not matter but, when it does, chaos can ensue in the absence of a blank last line.
...
In the Terminal of the distill-tutorial R Studio instance, run `git log`. Copy and paste the output below. 

```{r creating-a-website-10}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
    try_again_button = "Edit Answer",
...
The output is a list of your commits. You should see two, the one at the top being the one you just made. Check to see that the time listed is that of your most recent commit. 

### Exercise 11

In your browser, go to Github. Navigate to the page for your repo, `distill_tutorial` Check to see that the new files we created are now on Github. 
...
Press Save. When you do so, Github will begin to build the website. *This takes a few minutes!* When it is finished, you will be able to access the site at the link which the page provides. The building process should take fewer than 3 minutes. 
...
In the distill-tutorial R Studio instance, open index.Rmd, which should be in the same directory as the .Rproj for your website. In the setup chunk, load the library `tidyverse` and the library `ggthemes`. Read (and then delete) the comments which are added by default in that code chunk.
