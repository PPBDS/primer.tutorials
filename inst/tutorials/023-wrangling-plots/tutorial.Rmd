...
<!-- Fix gender equity plot. Maybe get rid of legend. Use fct_reorder (or fct_reorder2) in place of reorder. parse_number() instead of as.numeric(). See https://r4ds.had.co.nz/data-import.html#parsing-a-vector for discussion. str_to_lower() instead of tolower(). Instead of head use slice(1:25). -->
...
If we had more categories of data, we also could have made use of   **[RColorBrewer](https://www.datanovia.com/en/blog/the-a-z-of-rcolorbrewer-palette/)** package. This package provides a function named `scale_fill_brewer()`. With this function, you can specify a color palette of your choice. 
...
Alternatively, we could have used `as.double()`.
...
Because we didn't specify the `geom`, we only see the empty figure. 
...
Alternatively, we could have used `geom_col()`. But we like `geom_point()` better for this plot.
