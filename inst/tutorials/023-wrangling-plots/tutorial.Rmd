...
income <- read_csv("data/inc_occ_gender.csv")
```
...
graph <- income %>%
  filter(M_weekly != "Na" & 
         F_weekly != 'Na' & 
         All_weekly != "Na") %>%
  mutate(All_weekly = as.numeric(All_weekly)) %>%
  mutate(Occupation = tolower(Occupation)) %>%
  drop_na() %>%
  arrange(desc(All_weekly)) %>%
  head(25) %>%
  select(Occupation, M_weekly, F_weekly, M_workers, F_workers) %>%
  mutate(difference_in_earning = as.numeric(M_weekly) - as.numeric(F_weekly)) %>%
  mutate(workers = M_workers - F_workers) %>%
  ggplot(aes(y=reorder(Occupation,difference_in_earning) , 
             x = difference_in_earning, 
             color = workers)) +
  geom_point() + 
  ...
  labs(y=NULL,
       color = 'Difference in workers (in thousands)',
       x='Difference in earning', 
       subtitle='There is significant gender inequality in many areas.',
       title='Gender Inequality in 25 most paying jobs',
...
Using `mutate`, overwrite the `All_weekly` column `as.numeric()`. 
...
... %>%
  mutate(All_weekly = as.numeric(...))
```

###

Alternatively, we coul have used `as.double()`.

### Exercise 4

    
For interpretability of results, it would be helpful if we converted `Occupation`s to lowercase. We can do this by overwriting `Occupation` column with `tolower` function.
...
Alternatively, we could have used `order()` function to sort rows. 

### Exercise 7

Now that we have ordered the tibble, we have to get 25 most-paying jobs. We can do this with `head()` function. 
...
Instead of `head()` function, we can make data frame manipulation like the following: `income[1:25,]`

### Exercise 8

Select `Occupation`, `M_weekly`, `F_weekly`, `M_workers`, and `F_workers` columns. 
...
...  %>%
   select(..., M_weekly, ..., ..., F_workers)
```

### 

`F_workers` and `M_workers` columns are the number of female workers, in thousands and the number of male workers, in thousands, respectively. 

### Exercise 9

One of the variable we will use in this graph is the difference between median incomes of female workers and male workers in an occupation. 

###

Create a variable `difference_in_earning` that is  `M_weekly - F_weekly`. Don't forget to convert these columns to numeric first. 
...
... %>% 
  mutate(difference_in_earning = as.numeric(M_weekly) - as.numeric(...)) 
```

### 

The reason we subtracted `F_weekly` from `M_weekly` and no the other way around is because gender inequality is a common problem and may cause, on average, men having higher wages than women. 

### Exercise 10

Create a new variable `workers` that is `M_workers - F_workers`.
...
The difference in the quantity of female and male workers may also be a useful indicator of gender inequality. 

### Exercise 11

Create the `ggplot()` object. Within `aes`argument, set`y` equal to`Occupation` that is decreasing-ordered based on `difference_in_training` column, set `x` equal to `difference_in_earning`, and `color` to `workers`.
...
... %>%
  ggplot(aes(y = reorder(..., difference_in_earning), 
             x = difference_in_earning, 
             color = ...)) 
```
...
Because we didn't speficy the `geom`, we only see the empty figure. 
...
Alternatively, we could have used `geom_col()`. However, with many categorical values, `geom_point()` provides a more insightful graph.

### Exercise 13

Because the wages are in USD, within `scale_x_continuous`, add dollar sign to x-axis `labels`.
...
... + 
  scale_x_continuous(... = dollar_format()) 
```
...
Using packge `ggthemes`, we can access many more themes such as `theme_tufte` and `theme_calc`. 
...
Within `theme()`, set `panel.grid.major.x` to `element_blank()`, `panel.grid.minor.x` to `element_blank()`, and `panel.grid.major.y`to `element_line(colour = "grey85", linetype = "dashed)`. 
