...
The `kenya` contains data from a study in which poll stations in Kenya were assigned to one of the two groups. Either the control group or a group in which one or more methods (an SMS reminder, canvassing, etc.) were used to encourage voter registration. 
...
kenya_p <- kenya %>%
  ...
  ggplot(mapping = aes(x = fct_reorder(treatment, mean_turnout), 
    ...
    labs(title = "Turnout Changes in a Kenyan Election",
         ...
         caption= "Source: A study entitled 'Electoral Administration in Fledgling Democracies:Experimental Evidence from Kenya ' ")
...
```{r kenya-voting-1, exercise = TRUE}

```

```{r kenya-voting-1-hint-1, eval = FALSE}
glimpse(...)
```

### Exercise 2

`filter()` `kenya` to keep only the rows that have a `treatment` of "control," "local," "SMS," or "canvass." To accelerate the process, use the `%in% `operator with `c()`.

```{r kenya-voting-2, exercise = TRUE}

```

```{r kenya-voting-2-hint-1, eval = FALSE}
kenya %>%
...
Although we have filtered the data, because `treatment` is a factor, unused factor levels still exist. Implement  `droplevels()` as a new layer of the pipe. This will avoid future complications. 

```{r kenya-voting-3, exercise = TRUE}

```

```{r kenya-voting-3-hint-1, eval = FALSE}
... %>% 
  droplevels()
```
...
If we did not `droplevels()` then the `treatment` variable would "remember" all the levels we began with, even though we deleted all the rows with data from those levels. These "phantom" levels can interfere with plotting. 

### Exercise 4

Now `select()` the columns `treatment`, `mean_age`, and `reg_byrv13`. 

```{r kenya-voting-4, exercise = TRUE}

```

```{r kenya-voting-4-hint-1, eval = FALSE}
... %>%
  select(...)
```
...
```{r kenya-voting-5, exercise = TRUE}

```

```{r kenya-voting-5-hint-1, eval = FALSE}
kenya %>%
...
In cases like this, where there is data belonging to a certain number of categories, Performing on group level is  advantageous. To render this possible and group the data by `treatment`, use `group_by()` function.

```{r kenya-voting-6, exercise = TRUE}

```

```{r kenya-voting-6-hint-1, eval = FALSE}
... %>%
  group_by(...)
```
...
`reg_byrv13` is a variable for the number of registered voters at polling location during intervention period divided by registered voters at polling location during 2013. We will use this to compare the impacts of different treatments. Higher this variable, greater the impact. 
...
```{r kenya-voting-7, exercise = TRUE}

```

```{r kenya-voting-7-hint-1, eval = FALSE}
... %>% 
  summarize(... = mean(..., na.rm = ...),
            .groups = "...")
```

```{r kenya-voting-7-hint-2, eval = FALSE}
...
`.groups` is an argument that tells `summarize()` what to do with the grouping of the data after it has used the grouping to produce its summary. `.groups` does not affect the data in the summary tibble. You can either `"keep"` the grouping, `"drop"` the grouping, or `"drop_last"`, i.e. to drop the last of the specified grouping variables (this is because retaining all the same groups after summary creates groups which only contain single rows). `"drop_last"` is the default in most cases. If you exclude `.groups`, you will receive a warning message. 

### Exercise 8

Continue your pipe and add  `ggplot()`.  Use `fct_reorder()` to reorder the `treatment` variable by `mean_turnout` and map this to the x-axis. Moreover, map `mean_turnout` to the y-axis. Then, add the layer `geom_col()` and specify its `fill` argument as `'cornflowerblue'`. 

```{r kenya-voting-8, exercise = TRUE}

```

```{r kenya-voting-8-hint-1, eval = FALSE}
... %>% 
  ggplot(mapping = aes(x = fct_reorder(treatment, mean_turnout), 
                       y = ...) +
  geom_col()
...
`geom_col()` is similar to `geom_bar()`, but `geom_col()` allows the heights of the bars to represent values in the data rather than count. 

### Exercise 9

Add the theme `theme_grey()` to the plot.

```{r kenya-voting-9, exercise = TRUE}

```

```{r kenya-voting-9-hint-1, eval = FALSE}
... +
  theme_bw()
```
