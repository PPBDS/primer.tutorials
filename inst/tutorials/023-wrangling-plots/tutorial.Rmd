...
graph <- data %>%
  ...
  scale_x_continuous(guide = guide_axis(angle = 70),breaks = round(seq(min(data$year), max(data$year), by = 10),1)) +
  theme_minimal() + 
  labs(y='percentage within all male baby names',
       subtitle = 'After 1950, names ending with n increased rapidly',
       title='Percentage of Last Letter of Baby Names',
       caption= 'Resource: R babynames library')
