...
```{r sps_plot}
educationlevels <- c("college", "high school", "secondary", "preschool")

sps_p <- sps %>%
  filter(education %in% educationlevels) %>%
  ...
  ggplot(mapping = aes(x = education, 
    ...
    scale_x_discrete(limits = educationlevels) +
    theme_minimal() +
    labs(x = "Education",
         y = "Average Change in Expenses",
         title = "Spending Changes and Seguro Popular",
         subtitle = "Treatment effect at college level reduced expenses the most",
         fill = "Treatment",
...
To use it when filtering and ordering bars, create a variable educationlevel and set it equal to vector of four education levels:
`college`, `high school`, `secondary`, and `preschool`. 
...
educationlevels <- c("...", "high school", "...", "preschool")
```
...
`filter()` `sps` to extract only the rows with an `education` level of the ones in `educationlevels`. 
...
```{r seguro-popular-4-hint-2, eval = FALSE}
... %>% 
  select(-c(..., ...))
```
...
<!-- Drop knowledge about if_else -->

### Exercise 10

Use `%>%` to add `ggplot()`. Map `education` to x-axis. Map `mean_change_in_expenses` to the y-axis, and `treatment` to the `fill`. Also add the layer `geom_col()`.
...
To set make the bar charts color of your choice, you can use `scale_fill_manual` function. This function takes on values as a vector. Within this argument, set `values` equal to a vector of two colors: `cornflowerblue` and `darkturquoise`.
...
We can achieve this by setting `limits` argument of `scale_x_discrete()` equal to order of factors you want, given in the `educationlevel` vector we created in the second exercise. 
...
```{r baby-lastnames-1, exercise = TRUE}
...
```{r baby-lastnames-2, exercise = TRUE}

```

```{r baby-lastnames-2-hint-1, eval = FALSE}
any(is.na(...))
```
...
```{r baby-lastnames-3, exercise = TRUE}

```

```{r baby-lastnames-3-hint-1, eval = FALSE}
babynames %>%
  filter(... == 'M')
...
Our graph is associated with names of the baby names and their popularity over the years, which are their proportions. Therefore, extract only `name`, `year`, and `prop` columns. 

```{r baby-lastnames-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-4-hint-1, eval = FALSE}
...  %>%
  select(..., ..., ...)
```
...
```{r baby-lastnames-5, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-5-hint-1, eval = FALSE}
... %>%
...
```{r baby-lastnames-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-6-hint-1, eval = FALSE}
... %>% 
   summarize(frequency = sum(prop), .groups = 'drop') 
```
...
Our data processing steps are done. Great job! 
...
```{r baby-lastnames-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-7-hint-1, eval = FALSE}
...%>%
...
```{r baby-lastnames-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-8-hint-1, eval = FALSE}
...  +
  geom_line()
```
###

Alternatively, we could have used other chart types such as scatter chart. However, for our goal, line plot is a agreat option. 
...
```{r baby-lastnames-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-9-hint-1, eval = FALSE}
...+ 
  gghighlight(max(...) > ...,
                label_key = ...) 
```
...
We don't think current x-axis ticks are informative enough. Therefore, let's set custom x-axis ticks.
...
```{r baby-lastnames-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-10-hint-1, eval = FALSE}
... +
...
A rule of thumb, however,  isto include ending and beginning years so that whoever sees your plot could understand the range of years that data is collected.
...
```{r baby-lastnames-11, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-11-hint-1, eval = FALSE}
... +
...
```{r baby-lastnames-12, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r baby-lastnames-12-hint-1, eval = FALSE}
... +
  theme_minimal()
```
...
We also could have used themes such as `theme_gray()`, `theme_classic()`, and`theme_bw()`.
...
```{r baby-lastnames-13-hint-1, eval = FALSE}
... +
  labs(...)
```
