...
## Introduction
In this tutorial, you will learn more about data visualization and wrangling with R and create various graphs. Each section follows a structure so that you will built pipes by adding new layers with each exercise to first wrangle a dataset and then create an appropriate graph for the data. This tutorial is 77-exercises long and is expected to 75 minutes to complete. 
...
<!-- DK: This example is very sloppy! -->

The `covid` tibble contains data about Covid-19 and related variables. The source of this data set is Johns Hopkins University, and we downloaded it from [ourworldindata.org](https://ourworldindata.org/coronavirus-source-data).  
...
covid_graph <- covid |>
  ...
  # median_age is the median_age of the country's population and new_cases is
  # the new DAILY cases. Location is the country.
  
  select(continent, median_age, new_cases, location, population) |>
  drop_na() |> 
  
  # Besides real continents, there were rows that showed the data of globe, and
  # the continent of those columns were world. I excluded that. case_by_pop is
  # the total case divided by population. This number is a better indicator than
  # only the total cases.
...
There are many NA variables. Does that look reasonable? Probably! Lots of countries don't have the public health infrastructure to tabulate cases of covid.

### Exercise 2

Start a pipe with `covid`, then `select()` the `continent`, `location`, `new_cases`,  `median_age` and `population` variables.
...
Although this code would be inefficient for these variables, it would be helpful if we were to select all `cases` related columns, all of which would end with `_cases`. 

### Exercise 3

Because we do not want to include rows where there is missing data, use `drop_na()`.
...
If we did not `drop_na()`, errors would have been raised in later steps. 


### Exercise 4

`filter` in a way that we extract all rows but the ones where the `continent` is `World`. Use `!=` to not include those rows.
...
<!-- DK: This question should be split into several separate ones. -->
...
Using the `first()` function, create a `"agemed"` and `"continent"` variables that should be equal to `median_age` and `continent` of the country, respectively.

### 

Use the `.by` argument with a value of `location` to perform these calculations for each country.
...
```{r covid19-6, exercise = TRUE, warning = FALSE}
...
```{r covid19-7, exercise = TRUE, warning = FALSE}
...
```{r covid-10, exercise = TRUE, warning = FALSE}
...
```{r covid19-9, exercise = TRUE, warning = FALSE}
...
Copy and paste everything you have above and run `ggplotly(covid_graph)`.

```{r covid19-10, exercise = TRUE, warning = FALSE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r covid19-10-hint-1, eval = FALSE}
...
...
Use `summarize()` to create the variable `frequency` that is equal to `sum()` of proportions of each group. 
...
In this tutorial, we learned how to create graphs using complex pipes. 
Takeaways from this tutorial are, 
1. "[selection helper](https://tidyselect.r-lib.org/reference/select_helpers.html)" can be used to select columns based on one or more common feature. 
2. When creating a bar plot, `fct_reorder()` can be used within the ggplot object to set bars in decreasing or increasing order. 
3. `if_else()` can be used to deal with rows and change values based on a condition. 
4. `annotate()`function can be used add a text or shape to the graph. 
5.  We can use `scale_x_continuous()` and `scale_y_continuous()` to change the frequencies and the labels of axis ticks. 
6.  We can only label `color` if it is used in the `ggplot()` object. If `fill` argument is specified in the `ggplot()`, we can label `fill` and not `color`.
