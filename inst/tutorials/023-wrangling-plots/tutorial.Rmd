...
nhanes_p <- nhanes %>%
  ...
  # BMI (body mass index) is a good measure to show the association of weight and height. Moreover, I chose to analyse by gender, using facet_wrap. Therefore, selected these variables. 
...
sps_p <- sps %>%
  ...
  # health_exp_3m is the health-related spendings in the last 3 month. t2 prefix states the data collected from July 2006 to August 2006, and variables without t2 are the data collected from August 2005 to September 2005. 
  # Because mexico saw inflation at 2005-2006, the  difference between t2_health_exp_3m and health_exp_3m measures how did the spending habits of participants change. Small difference indicate that health-related spendings decreased. 
  mutate(change_in_expenses = t2_health_exp_3m - health_exp_3m) %>%
  # To take into account that each education level would be influenced differently by the experiment, we group by treatment groups and education levels. 
  group_by(treatment, education) %>%
  summarize(mean_change_in_expenses = mean(change_in_expenses),
            .groups = "drop") %>%
  # treatment and control groups were given the numbers 1 and 0, respectively. More convenient to have them with actual group names. 
