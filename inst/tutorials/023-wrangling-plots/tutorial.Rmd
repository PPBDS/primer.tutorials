...
```{r covid}
covid_graph <- covid |>
  # median_age is the median_age of the country's population and new_cases is the new DAILY cases. Location is the country. 
  select(continent, median_age, new_cases, location, population) |>
  drop_na(new_cases) |> 
  group_by(location) |> 
  # Besides real continents, there were rows that showed the data of globe, and the continent of those columns were world. I excluded that. 
  # case_by_pop is the total case divided by population. This number is a better indicator than only the total cases. 
...
`filter` in a way that we extract all rows but the ones where the `continent` is `world`. Remember to use `!=` to not include specific rows.
...
Within `summarize` function from previous exercise, set `.groups` argument equal to `"drop"`. 
...
```{r covid19-8, exercise = TRUE, warning=FALSE}
...
```{r covid19-11, exercise = TRUE, warning = FALSE}
...
```{r covid19-12, exercise = TRUE, warning = FALSE}
...
income_p <- income |>
  # these columns were character columns, and there were 'Na's as characters rather than `Na` type. Exclude them below. 
  filter(f_weekly != "Na" & 
         m_weekly != "Na" & 
         all_weekly != "Na") |>
  # convert the character columns that contain numeric values to double. "_weekly" suffices are columns that show median weekly income for each job. 
...
boys_p <- babynames |>
  ...
  # group by year and last letter of baby names. 
...
Pass `name`, `-1` and `-1` arguments to `str_sub()`. Since we will use this variable only for grouping, you can create it within the `group_by()` function. Moreover, because we will also calculate popularity of last letters *by year*, pass `year` into the `group_by()` function as well.
